...
module.exports = async ({ cwd, redisUrl, uploadDir, verbose, env }) => {
  ...
  return await new Promise((resolve, reject) => {
    const arcangelProcess = spawn("node", [path.resolve(serverPath)], {
      cwd,
      env: {
        ...process.env,
    ...
    arcangelProcess.on("error", (err) => {
      console.log(err)
      reject(err)
      process.exit()
    })

    arcangelProcess.on("uncaughtException", (err) => {
      console.log(err)
      reject(err)
      arcangelProcess.kill()
    })

    arcangelProcess.on("message", (port) => {
      setPort(port)
      resolve(arcangelProcess)
    })

    arcangelProcess.on("exit", () => {
