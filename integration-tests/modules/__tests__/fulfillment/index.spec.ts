...
import { IFulfillmentModuleService, StockLocationDTO } from "@arcangel/types"
...
arcangelIntegrationTestRunner({
  ...
  testSuite: ({ getContainer, api, dbConnection, dbUtils }) => {
    ...
    /**
     * The test runner run both the arcangel migrations as well as the modules
    ...
    describe("Fulfillment module migrations backward compatibility", () => {
      it("should allow to create a full data structure after the backward compatible migration have run on top of the arcangel v1 database", async () => {
