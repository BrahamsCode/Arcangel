import { emitEventStep } from "@arcangel/core-flows"
import { Modules, TransactionState } from "@arcangel/framework/utils"
import {
  createStep,
  createWorkflow,
  StepResponse,
...
import { IEventBusModuleService } from "@arcangel/types"
import {
  adminHeaders,
  createAdminUser,
...
arcangelIntegrationTestRunner({
  testSuite: ({ dbConnection, getContainer, api, dbUtils }) => {
    describe("Workflow Engine API", () => {
      let arcangelContainer

      beforeAll(() => {
        arcangelContainer = getContainer()
      })

      beforeAll(async () => {
        await createAdminUser(dbConnection, adminHeaders, arcangelContainer)
      ...
      describe("Testing WorkflowEngine.run", () => {
        ...
        it("Should invoke modules passing the current arcangel context as argument", async () => {
          const testMod = arcangelContainer.resolve("testingModule") as any

          const methodSpy = jest.spyOn(testMod, "methodName")

          const engine = arcangelContainer.resolve(Modules.WORKFLOW_ENGINE)
