...
import CurrencyModule from "@arcangel/currency"
import { ArcangelModule } from "@arcangel/modules-sdk"
import ProductModule from "@arcangel/product"
import RegionModule from "@arcangel/region"
  ...
  testSuite: ({ getContainer }) => {
    describe("defineLink", () => {
      it("should generate a proper link definition", async () => {
        ...
        const linkDefinition = ArcangelModule.getCustomLinks()
          .map((linkDefinition: any) => {
            const definition = linkDefinition(
              ArcangelModule.getAllJoinerConfigs()
            )
      ...
      it("should allow two read-only links to the same module using distinct aliases", async () => {
        ...
        const readOnlyLinks = ArcangelModule.getCustomLinks()
          .map((linkDefinition: any) => {
            const definition = linkDefinition(
              ArcangelModule.getAllJoinerConfigs()
            )
