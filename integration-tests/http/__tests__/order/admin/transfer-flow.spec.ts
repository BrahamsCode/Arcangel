  ...
  testSuite: ({ dbConnection, getContainer, api, dbUtils }) => {
    ...
    describe("Transfer Order flow (Admin)", () => {
      ...
      it("should fail to request order transfer to a guest customer", async () => {
        ...
        expect(err.response.data).toEqual(
          expect.objectContaining({
            type: "invalid_data",
            message: `Cannot transfer order: ${order.id} to a guest customer account: guest@arcangel.com`,
          })
        )
      })
    ...
    describe("Transfer Order to Guest flow (Admin)", () => {
      ...
      it("should transfer an order to a new guest customer, creating it and updating the order email immediately", async () => {
        ...
        const email = "new-guest@arcangel.com"
      ...
      it("should transfer an order to an existing guest customer", async () => {
        ...
        const existingGuest = (
          await api.post(
            "/admin/customers",
            { first_name: "guest", email: "existing-guest@arcangel.com" },
            adminHeaders
          )
        ).data.customer
