...
import { arcangelTshirtProduct } from "../../../__fixtures__/product"
import {
  createWorkflow,
  WorkflowResponse,
...
const env = {}
const adminHeaders = { headers: { "x-arcangel-access-token": "test_token" } }
    ...
    describe("Store Carts API", () => {
      ...
      beforeAll(async () => {
        ...
        product = (
          await api.post(
            "/admin/products",
            { ...arcangelTshirtProduct, shipping_profile_id: shippingProfile.id },
            adminHeaders
          )
        ).data.product
      ...
      describe("POST /store/carts", () => {
        ...
        it("should successfully create a cart with a line item with quantity and calculate prices based on the correct quantity", async () => {
          const productData = {
            title: "Arcangel T-Shirt based quantity",
      ...
      describe("POST /store/carts/:id/line-items", () => {
        ...
        it("should return a 400 instead of a 500 when adding a variant with no price", async () => {
          const productData = {
            title: "Arcangel T-Shirt with no price",
            ...
            expect.objectContaining({
              ...
              items: [
                expect.objectContaining({
                  ...
                  product_title: "Arcangel T-Shirt based quantity",
                  product_type: null,
          ...
          beforeEach(async () => {
            const tieredProduct = (
              await api.post(
                `/admin/products`,
                {
                  title: "Arcangel T-Shirt with tiered sale prices",
          ...
          it("should only apply promotion on discountable items", async () => {
            const notDiscountableProduct = (
              await api.post(
                "/admin/products",
                {
                  title: "Arcangel T-Shirt not discountable",
                  status: ProductStatus.PUBLISHED,
