import { createUsersWorkflow } from "@arcangel/core-flows"
  ...
  testSuite: ({ dbConnection, getContainer, api, dbUtils }) => {
    ...
    describe("User creation with roles", () => {
      ...
      it("should create user with valid role", async () => {
        ...
        const { result: users } = await workflowService.run(
          "create-users-workflow",
          {
            input: {
              users: [
                {
                  email: "test-with-role@arcangel.js",
        ...
        expect(users[0].email).toEqual("test-with-role@arcangel.js")
      })
    })
