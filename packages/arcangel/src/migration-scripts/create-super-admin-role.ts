 ...
 * to all existing admin users in your Arcangel instance.
 *
 * Usage: npx tsx packages/arcangel/src/migration-scripts/create-super-admin-role.ts
 */
...
export default async function createSuperAdminRole({ container }: ExecArgs) {
  if (
    !ArcangelModule.isInstalled(Modules.USER) ||
    !ArcangelModule.isInstalled(Modules.AUTH) ||
    !ArcangelModule.isInstalled(Modules.RBAC)
  ) {
    const logger = container.resolve(ContainerRegistrationKeys.LOGGER)
    logger.info(
