...
import { Store, track } from "@arcangel/telemetry"
...
const PLUGIN_ID = "arcangel-dev@arcangel"
...
describe("promptClaudeCodePlugin", () => {
  ...
  describe("user says yes", () => {
    ...
    it("tracks telemetry with install=true", async () => {
      ...
      expect(mockTrack).toHaveBeenCalledWith("ARCANGEL_CLAUDE_CODE_PLUGIN_PROMPT", {
        install: true,
      })
    })
    ...
    it("shows fallback message when marketplace command fails", async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining("/plugin marketplace add arcangel/arcangel-agent-skills")
      )
    })
