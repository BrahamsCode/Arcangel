...
import { ContainerRegistrationKeys, generateJwtToken } from "@arcangel/utils"
...
import { configManager } from "@arcangel/framework/config"
import { logger as defaultLogger } from "@arcangel/framework/logger"
import { ConfigModule, ArcangelContainer } from "@arcangel/types"
...
export const createServer = async (rootDir) => {
  ...
  Object.entries(ModulesDefinition).forEach(([moduleKey, module]) => {
    moduleResolutions[moduleKey] = registerArcangelModule({
      moduleKey,
  ...
  container.registerAdd = function (this: ArcangelContainer, name, registration) {
  ...
  app.use((req, res, next) => {
    ;(req as ArcangelRequest).scope = container.createScope() as ArcangelContainer
    next()
  })
