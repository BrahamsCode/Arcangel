...
import { SpanStatusCode } from "@arcangel/framework/opentelemetry/api"
import {
  InMemorySpanExporter,
  NodeTracerProvider,
  SimpleSpanProcessor,
} from "@arcangel/framework/opentelemetry/sdk-trace-node"
  ...
  describe("traceMiddleware", () => {
    ...
    it("should not report a handled 4xx error as an error on the span", async () => {
      const span = await runMiddleware(
        new ArcangelError(ArcangelError.Types.INVALID_DATA, "Invalid promo code")
      )
