import { ConfigModule } from "@arcangel/framework/config"
import { ApiLoader } from "@arcangel/framework/http"
import { ArcangelContainer, PluginDetails } from "@arcangel/framework/types"
import { FeatureFlag } from "@arcangel/framework/utils"
...
export default async ({ app, container, plugins }: Options) => {
  ...
  if (FeatureFlag.isFeatureEnabled("backend_hmr")) {
    ...
    ;(global as any).__ARCANGEL_HMR_INITIAL_STACK_LENGTH__ = initialStackLength
  }

  const sourcePaths: string[] = []

  /**
   * Always load plugin routes before the Arcangel core routes, since it
   * will allow the plugin to define routes with higher priority
   * than Arcangel. Here are couple of examples.
   *
   * - Plugin registers a route called "/products/active"
   * - Arcangel registers a route called "/products/:id"
   *
   * Now, if Arcangel routes gets registered first, then the "/products/active"
  ...
  // Test: (packages/arcangel/src/api/routes/admin/currencies/update-currency.ts)
  try {
    await new ApiLoader({
      app: app,
