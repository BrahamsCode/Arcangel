...
jest.mock("@arcangel/framework/search", () => ({
...
// itself that is `<project>/.arcangel/server/src` — and index definitions are
...
describe("loadSearchIndexes", () => {
  ...
  it("loads the 'search' folder of every plugin when the module is configured", async () => {
    const { SearchIndexLoader } = require("@arcangel/framework/search")
