import { container, ArcangelAppLoader, policiesLoader } from "@arcangel/framework"
import { asValue } from "@arcangel/framework/awilix"
import { configLoader } from "@arcangel/framework/config"
import { pgConnectionLoader } from "@arcangel/framework/database"
import { featureFlagsLoader } from "@arcangel/framework/feature-flags"
import { expressLoader } from "@arcangel/framework/http"
import { JobLoader } from "@arcangel/framework/jobs"
import { LinkLoader } from "@arcangel/framework/links"
import { logger as defaultLogger } from "@arcangel/framework/logger"
import { SubscriberLoader } from "@arcangel/framework/subscribers"
import {
  ConfigModule,
  LoadedModule,
  ArcangelContainer,
...
async function subscribersLoader(
  ...
  const pluginSubscribersSourcePaths = [
    /**
     * Load subscribers from the arcangel/arcangel package. Remove once the arcangel core is converted to a plugin
     */
...
async function jobsLoader(
  ...
  const pluginJobSourcePaths = [
    /**
     * Load jobs from the arcangel/arcangel package. Remove once the arcangel core is converted to a plugin
     */
    join(__dirname, "../jobs"),
...
async function loadEntrypoints(
  ...
  expressApp.use((req: Request, res: Response, next: NextFunction) => {
    req.scope = container.createScope() as ArcangelContainer
...
): Promise<ArcangelContainer> {
  await featureFlagsLoader(rootDirectory)
  const configDir = await configLoader(rootDirectory, "arcangel-config", {
    throwOnValidationError: options?.throwOnValidationError,
  })
  await featureFlagsLoader(join(__dirname, ".."))

  // Load policies from core arcangel package and project root
...
export default async ({
  ...
  const {
    ...
    modules,
    gqlSchema,
  } = await new ArcangelAppLoader().load()
  ...
  const { createDefaultsWorkflow } = await import("@arcangel/core-flows")
