...
import { Logger, ArcangelContainer } from "@arcangel/framework/types"
...
function handleHMRReload(logger: Logger) {
  ...
  if (process.env.ARCANGEL_HMR_ENABLED === "true" && process.send) {
    ;(global as any).__ARCANGEL_HMR_ROUTE_REGISTRY__ = true
...
async function start(args: {
  ...
  if ("cluster" in args) {
    ...
    if (cluster.isPrimary) {
      ...
      process.on("message", async (msg: any) => {
        if (msg.workerMode) {
          process.env.ARCANGEL_WORKER_MODE = msg.workerMode
        }
