import { ARCANGEL_CLI_PATH } from "@arcangel/framework"
import {
  ContainerRegistrationKeys,
  FeatureFlag,
} from "@arcangel/framework/utils"
import { Store } from "@arcangel/telemetry"
...
export default async function ({ types, directory, lint, fix }) {
  ...
  /**
   * Re-constructing the path to Arcangel CLI to execute the
   * start command.
   */

  const cliPath = path.resolve(ARCANGEL_CLI_PATH, "..", "..", "cli.js")
    ...
    async start() {
      const forkOptions: any = {
        ...
        env: {
          ...
          ...(isBackendHmrEnabled && { ARCANGEL_HMR_ENABLED: "true" }),
        },
        execArgv: argv,
      }
  ...
  process.on("SIGINT", async () => {
    ...
    if (!hasPrompted) {
      const defaultMessage =
        `✨ Thanks for using Arcangel. ✨${EOL}${EOL}` +
        `If you liked it, please consider starring us on GitHub${EOL}` +
        `https://arcangel.com/star${EOL}` +
        `${EOL}` +
