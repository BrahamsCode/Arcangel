import { ArcangelAppLoader } from "@arcangel/framework"
import { LinkLoader } from "@arcangel/framework/links"
import { MigrationScriptsMigrator } from "@arcangel/framework/migrations"
import { ArcangelModule } from "@arcangel/framework/modules-sdk"
import {
  ContainerRegistrationKeys,
  getResolvedPlugins,
} from "@arcangel/framework/utils"
import { WorkflowLoader } from "@arcangel/framework/workflows"
import { ArcangelContainer } from "@arcangel/types"
import { runMigrationScripts } from "../db/run-scripts"

// ─── Mocks ────────────────────────────────────────────────────────────────────

jest.mock("@arcangel/framework", () => ({
  ArcangelAppLoader: jest.fn(),
}))

jest.mock("@arcangel/framework/links", () => ({
  LinkLoader: jest.fn(),
}))

jest.mock("@arcangel/framework/workflows", () => ({
  WorkflowLoader: jest.fn(),
}))

jest.mock("@arcangel/framework/migrations", () => ({
  MigrationScriptsMigrator: jest.fn(),
}))

jest.mock("@arcangel/framework/modules-sdk", () => ({
  ArcangelModule: {
    clearInstances: jest.fn(),
  },
}))

jest.mock("@arcangel/framework/utils", () => ({
  ContainerRegistrationKeys: {
...
function buildContainer(
  ...
  return {
    ...
    register: jest.fn(),
  } as unknown as ArcangelContainer
}
...
describe("runMigrationScripts", () => {
  ...
  beforeEach(() => {
    ...
    // ArcangelAppLoader mock — captures the options it was called with
    ;(ArcangelAppLoader as jest.Mock).mockImplementation((opts) => ({
      _opts: opts,
  ...
  describe("container forwarding to ArcangelAppLoader", () => {
    it("passes the caller's container to ArcangelAppLoader so QUERY is resolved from the same DI scope", async () => {
      ...
      // ArcangelAppLoader must have been constructed with the same container
      expect(ArcangelAppLoader).toHaveBeenCalledTimes(1)
      const [ctorOptions] = (ArcangelAppLoader as jest.Mock).mock.calls[0]
      expect(ctorOptions).toHaveProperty("container", container)
    })

    it("does NOT create ArcangelAppLoader with an undefined container", async () => {
      const container = buildContainer()
  ...
  describe("happy path", () => {
    ...
    it("calls onApplicationPrepareShutdown and onApplicationShutdown in finally block", async () => {
      ...
      ;(ArcangelAppLoader as jest.Mock).mockImplementation(() => ({
        load: jest.fn().mockResolvedValue({
  ...
  describe("ArcangelModule cleanup", () => {
    it("clears module instances before loading resources", async () => {
      ...
      expect(ArcangelModule.clearInstances).toHaveBeenCalledTimes(1)
    })
  })
  ...
  describe("WorkflowLoader", () => {
    ...
    it("loads WorkflowLoader after ArcangelAppLoader so module registrations are available", async () => {
      const callOrder: string[] = []

      ;(ArcangelAppLoader as jest.Mock).mockImplementation(() => ({
        load: jest.fn().mockImplementation(async () => {
          callOrder.push("ArcangelAppLoader.load")
          return {
            onApplicationPrepareShutdown: jest.fn().mockResolvedValue(undefined),
            onApplicationShutdown: jest.fn().mockResolvedValue(undefined),
