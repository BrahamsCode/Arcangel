 ...
 * `@arcangel/eslint-plugin`, so it is installed in the user's project and is
 * not bundled with Arcangel. Returns `null` when it cannot be found.
 */
...
export async function lintProject(opts: {
  ...
  const cacheLocation = path.join(opts.cwd, ".arcangel/cache/.eslintcache")
...
/**
 * Shared lint gate for `arcangel build` and `arcangel develop`. Encapsulates the
 * common flow; the only difference between the two callers is `failOnError`:
 *
 * - `--lint false`            → skipped silently (debug log). On `build` and
 *   `develop`, linting is off by default unless `@arcangel/eslint-plugin` is
  ...
  if (result.errorCount > 0) {
    ...
    opts.logger.warn(
      `Lint found ${result.errorCount} error(s)${warningNote}. ` +
        "Continuing anyway, but consider fixing the errors with the `--fix` flag or by running `arcangel lint --fix`."
    )
    return
  }
