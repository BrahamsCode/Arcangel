...
import { ILinkModule, ModuleJoinerConfig } from "@arcangel/framework/types"
import { defineLink, isObject, Modules } from "@arcangel/framework/utils"
...
ArcangelModule.setJoinerConfig(userJoinerConfig.serviceName, userJoinerConfig)
ArcangelModule.setJoinerConfig(carJoinerConfig.serviceName, carJoinerConfig)
ArcangelModule.setJoinerConfig(
  ...
  testSuite: ({ dbConfig }) => {
    describe("MigrationsExecutionPlanner", () => {
      test("should generate an execution plan", async () => {
        ...
        ArcangelModule.getCustomLinks().forEach((linkDefinition: any) => {
          ArcangelModule.setCustomLink(
            linkDefinition(ArcangelModule.getAllJoinerConfigs())
          )
        })
        ...
        let joinerConfigs = ArcangelModule.getCustomLinks().filter(
        ...
        joinerConfigs = ArcangelModule.getCustomLinks().filter(
          (link): link is ModuleJoinerConfig => isObject(link)
        )
