...
export function generateGraphQLSchema(
  ...
  for (const extend of joinerConfig.extends ?? []) {
    const extendedModule = ArcangelModule.getModuleInstance(extend.serviceName)
    if (!extendedModule) {
      throw new Error(
    ...
    if (joinerConfig?.isReadOnlyLink) {
      ...
      const extendedService = ArcangelModule.getModuleInstance(rel.serviceName)
    ...
    const fieldsAliasesField = Object.entries(extend.fieldAlias || {})
      .map(([field, config]) => {
        ...
        const targetEntityName = ArcangelModule.getJoinerConfig(
  ...
  const doesPrimaryExportSchema = !!ArcangelModule.getJoinerConfig(
    primary.serviceName
  )?.schema
  const doesForeignExportSchema = !!ArcangelModule.getJoinerConfig(
    foreign.serviceName
  )?.schema
