import { ArcangelModule, MODULE_SCOPE } from "@arcangel/framework/modules-sdk"
import {
...
export const initialize = async (
  ...
  const allLinks = {}
  const modulesLoadedKeys = ArcangelModule.getLoadedModules().map(
  ...
  await promiseAll(
    allLinksToLoad.map(async (linkDefinition) => {
      ...
      const loaded = await ArcangelModule.bootstrapLink({
