import { Lifetime, asFunction, asValue } from "@arcangel/framework/awilix"
import { moduleProviderLoader } from "@arcangel/framework/modules-sdk"
import { LoaderOptions, ModulesSdkTypes } from "@arcangel/framework/types"
import {
...
import { ArcangelCloudEmailNotificationProvider } from "../providers/arcangel-cloud-email"
...
>): Promise<void> => {
  ...
  // We add the Arcangel Cloud Email provider if there is no other email provider configured
  const hasEmailProvider = options?.providers?.some((provider) =>
    provider.options?.channels?.some((channel) => channel === "email")
  )
  if (!hasEmailProvider) {
    const shouldRegisterArcangelCloudEmailProvider = validateCloudOptions(
      options?.cloud
    )

    if (shouldRegisterArcangelCloudEmailProvider) {
      await registrationFn(ArcangelCloudEmailNotificationProvider, container, {
...
async function syncDatabaseProviders({
  ...
  const providerService: ModulesSdkTypes.IArcangelInternalService<
