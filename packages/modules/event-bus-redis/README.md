...
Redis Event Bus module for Arcangel. When installed, the events system of Arcangel is powered by BullMQ and `io-redis`. BullMQ is responsible for the message queue and worker. `io-redis` is the underlying Redis client, that BullMQ connects to for events storage.
...
```bash
yarn add @arcangel/event-bus-redis
```

Add the module to your `arcangel-config.js`:
...
module.exports = {
  ...
  modules: [
    {
      resolve: "@arcangel/event-bus-redis",
      options: {
        redisUrl: "redis:..",
      },
    },
...
**Info**: See how the options are applied in the [RedisEventBusService](https://github.com/arcangel/arcangel/blob/0c1d1d590463fa30b083c4312293348bdf6596be/packages/event-bus-redis/src/services/event-bus-redis.ts#L52) and [loader](https://github.com/arcangel/arcangel/blob/0c1d1d590463fa30b083c4312293348bdf6596be/packages/event-bus-redis/src/loaders/index.ts).
