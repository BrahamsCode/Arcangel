...
const workflowExecutionServiceMock = {
  ...
  upsert: jest.fn(),
  delete: jest.fn(),
} as unknown as ModulesSdkTypes.IArcangelInternalService<any>
...
const baseModuleDeps = {
  ...
  redisQueueName: "arcangel-workflows",
  redisJobQueueName: "arcangel-workflows-jobs",
  logger: loggerMock,
  isWorkerMode: true,
}
...
describe("RedisDistributedTransactionStorage", () => {
  ...
  describe("constructor - Queue configuration", () => {
    it("should create queues with default empty options when no options provided", () => {
      ...
      expect(Queue).toHaveBeenCalledWith("arcangel-workflows", {
        connection: redisMock,
      })

      expect(Queue).toHaveBeenCalledWith("arcangel-workflows-jobs", {
        connection: redisMock,
      })
    ...
    it("should create each queue with different options", () => {
      ...
      expect(Queue).toHaveBeenNthCalledWith(1, "arcangel-workflows", {
        ...mainQueueOptions,
        connection: redisMock,
      })

      expect(Queue).toHaveBeenNthCalledWith(2, "arcangel-workflows-jobs", {
        ...jobQueueOptions,
        connection: redisMock,
      })
