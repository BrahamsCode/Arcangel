...
moduleIntegrationTestRunner<IWorkflowEngineService>({
  ...
  moduleOptions: {
    redis: {
      ...
      queueName: `arcangel-workflows-${
        process.env.JEST_WORKER_ID ?? "1"
      }-${testRunId}`,
      jobQueueName: `arcangel-workflows-jobs-${
        process.env.JEST_WORKER_ID ?? "1"
      }-${testRunId}`,
    },
  },
  testSuite: ({ service: workflowOrcModule, arcangelApp }) => {
    describe("Workflow Orchestrator module", function () {
      beforeEach(async () => {
        await TestDatabase.clearTables()
        jest.clearAllMocks()

        query = arcangelApp.query
        sharedContainer_ = arcangelApp.sharedContainer
      })
      ...
      let sharedContainer_: ArcangelContainer
