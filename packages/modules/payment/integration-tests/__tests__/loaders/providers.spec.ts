import { IPaymentModuleService } from "@arcangel/framework/types"
...
moduleIntegrationTestRunner<IPaymentModuleService>({
  ...
  testSuite: ({ service }) => {
    describe("Payment Module Service", () => {
      describe("providers", () => {
        it("should load the system and arcangel payments providers by default", async () => {
          ...
          expect(paymentProviders).toEqual(
            expect.arrayContaining([
              ...
              expect.objectContaining({
                id: "pp_arcangel-payments_default",
              }),
            ])
          )
        })
...
moduleIntegrationTestRunner<IPaymentModuleService>({
  ...
  testSuite: ({ service }) =>
    describe("providers", () => {
      it("should not load the arcangel payments provider if the cloud options are not provided", async () => {
        const paymentProviders = await service.listPaymentProviders()
