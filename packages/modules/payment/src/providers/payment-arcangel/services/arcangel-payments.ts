...
import { ArcangelPaymentsOptions } from "../types"
import {
...
import {
  ...
  ArcangelAccountHolder,
  ArcangelPayment,
  ArcangelPaymentMethod,
  ArcangelPaymentMethodSession,
  ArcangelRefund,
  RefundPaymentRequest,
  UpdateAccountHolderRequest,
} from "../types/arcangel-payments"
...
export class ArcangelPaymentsProvider extends AbstractPaymentProvider<ArcangelPaymentsOptions> {
  static identifier = "arcangel-payments"
  protected readonly options_: ArcangelPaymentsOptions
  ...
  static validateOptions(options: ArcangelPaymentsOptions): void {
    return validateOptions(options)
  }

  constructor(cradle: Record<string, unknown>, options: ArcangelPaymentsOptions) {
  ...
  handleStripeError(error: CloudServiceError): HandledErrorType {
    let arcangelPayment: ArcangelPayment | undefined

    switch (error.type) {
      case "ArcangelCardError":
        // Arcangel has created a payment but it failed
        ...
        arcangelPayment = error.data as ArcangelPayment | undefined
        if (arcangelPayment) {
          return {
            retry: false,
            data: arcangelPayment,
          }
        } else {
          throw error
        }
      case "ArcangelPaymentUnexpectedStateError":
        arcangelPayment = error.data as ArcangelPayment | undefined
        if (arcangelPayment) {
          return {
            retry: false,
  ...
  async getPaymentStatus(
    ...
    const statusResponse = this.getStatus(
      payment.data as unknown as ArcangelPayment
    )
  ...
  async updatePayment({
    ...
    if (isPresent(amount) && data?.amount === amountNumeric) {
      return this.getStatus(
        data as unknown as ArcangelPayment
  ...
  async savePaymentMethod({
    ...
    })) as ArcangelPaymentMethodSession
  ...
  private getStatus(payment: ArcangelPayment) {
  ...
  /**
   * Constructs Arcangel Payments Webhook event
   * @param {object} data - the data of the webhook request: req.body
   *    ensures integrity of the webhook event
   * @return {object} Arcangel Payments Webhook event
   */
  constructWebhookEvent(data: ProviderWebhookPayload["payload"]) {
    const signature = data.headers["arcangel-payments-signature"] as string
...
const validateOptions = (options: ArcangelPaymentsOptions): void => {
  if (!isDefined(options.endpoint)) {
    throw new Error(
      "Required option `endpoint` is missing in Arcangel payments plugin"
    )
  }
  if (!isDefined(options.webhook_secret)) {
    throw new Error(
      "Required option `webhook_secret` is missing in Arcangel payments plugin"
    )
  }
  if (!isDefined(options.api_key)) {
    throw new Error(
      "Required option `api_key` is missing in Arcangel payments plugin"
    )
  }
  ...
  if (
    ...
    throw new Error(
      "Required option `environment_handle` or `sandbox_handle` is missing in Arcangel payments plugin"
    )
  }
}
