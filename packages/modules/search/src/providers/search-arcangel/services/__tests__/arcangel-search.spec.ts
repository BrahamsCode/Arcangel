...
import { ArcangelSearchService } from "../arcangel-search"
...
const createService = () =>
  new ArcangelSearchService(
    {},
    {
      api_key: "arcangel_test",
      endpoint: "https://search.arcangel.example",
      environment_handle: "test-env",
    }
  )

describe("ArcangelSearchService", () => {
  it("requires Cloud credentials from provider options", () => {
    expect(() => new ArcangelSearchService({}, {} as any)).toThrow(
      /explicit "api_key"/
    )
    expect(
      () => new ArcangelSearchService({}, { api_key: "arcangel_test" } as any)
  ...
  it("passes Arcangel index names through unchanged", async () => {
    const service = createService()
