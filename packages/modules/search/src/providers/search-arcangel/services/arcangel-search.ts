import { Logger, SearchTypes } from "@arcangel/framework/types"
import {
...
import {
  ...
  buildQueryPlan,
  fromSearchDocument,
  ArcangelSearchClient,
  CloudServiceError,
  ArcangelSearchProviderOptions,
  parseFacetResults,
  parseHighlights,
  toSearchDocument,
  toSearchFilter,
  validateArcangelSearchOptions,
  type FacetQuery,
  type IndexQuery,
...
export class ArcangelSearchService extends AbstractSearchProviderService {
  static identifier = "search-arcangel"
  ...
  constructor(
    ...
    validateArcangelSearchOptions(options)

    this.logger_ = logger
    this.options_ = options
    this.client_ = new ArcangelSearchClient(options)
  }

  /**
   * Cloud scopes storage from the environment handle. The provider
   * passes Arcangel physical index names unchanged.
   */
  ...
  async upsertIndex({
    ...
    // created with this same, unchanging schema. Arcangel can't apply a
  ...
  async listIndexes(): Promise<SearchTypes.SearchIndexInfo[]> {
    ...
    return await Promise.all(
      entries.map(async (entry) => {
        ...
        return {
          name: entry.id,
          provider: ArcangelSearchService.identifier,
  ...
  async deleteDocuments({
    ...
    if (!compiled) {
      throw new ArcangelError(
        ...
        "Deleting Arcangel search documents requires filters"
      )
    }
  ...
  protected buildMultiQuery(
    ...
    if (queries.length > MAX_MULTI_QUERIES) {
      throw new ArcangelError(
        ...
        `Arcangel search accepts at most ${MAX_MULTI_QUERIES} queries per multi-query; this search needs ${queries.length}`
      )
    }
    ...
    for (const row of rows) {
      ...
      if (rowBytes > MAX_WRITE_BYTES) {
        throw new ArcangelError(
          ...
          `A Arcangel search document exceeds the ${MAX_WRITE_BYTES}-byte provider limit`
        )
      }
