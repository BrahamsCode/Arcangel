...
import type { ArcangelSearchQueryOptions } from "./options"
...
export type QueryPlan = {
  ...
  skip: number
  take: number
  options: ArcangelSearchQueryOptions
...
): ArcangelSearchQueryOptions {
  return (input.search_options?.provider_options?.["search-arcangel"] ??
    {}) as ArcangelSearchQueryOptions
}
...
function buildHighlightPlan(
  ...
  if (!highlight.fields.length) {
    fail("Arcangel search highlighting requires at least one field")
  }
  ...
  const fields = highlight.fields.map((path, index) => {
    ...
    const key = `__arcangel_highlight_${index}__`
...
export function buildQueryPlan(
  ...
  if (input.pagination?.cursor !== undefined) {
    fail("The Arcangel search provider does not support cursor pagination")
  }
  if (skip + take > 10000) {
    fail("Arcangel search can return at most 10,000 results per query")
  }
  if (options.locales?.length) {
    fail(
      "Configure Arcangel search full-text language on index fields, not per query"
    )
  }
  if (options.min_score !== undefined) {
    fail("Arcangel search min_score is not implemented by this provider yet")
  }
