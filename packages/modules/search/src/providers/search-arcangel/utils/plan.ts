...
import type {
  ArcangelSearchFieldOptions,
  ArcangelSearchIndexOptions,
} from "./options"
...
export type IndexPlan = {
  ...
  searchable: string[]
  primary_key: string
  options: ArcangelSearchIndexOptions
  typo_tolerance: TypoToleranceSettings
  vectors: string[]
}

function fail(message: string): never {
  throw new ArcangelError(ArcangelError.Types.NOT_ALLOWED, message)
}
...
function providerOptions<T>(value: {
  ...
  return (value.provider_options?.["search-arcangel"] ?? {}) as T
}
...
function fieldType(
  ...
  switch (field.type) {
    ...
    case "vector":
      if (isArray) {
        fail("Arcangel search vector fields cannot be arrays")
      }
      if (!field.dimensions || field.dimensions < 1) {
        fail("Arcangel search vector fields must declare positive dimensions")
      }
      return `[${field.dimensions}]f32`
    case "geo":
      return fail("The Arcangel search provider does not support geo fields")
    default:
      return fail(
        `The Arcangel search provider does not support fields of type "${field.type}"`
      )
  }
}
...
export function assertIndexSupported(
  ...
  if (definition.settings.synonyms) {
    fail("The Arcangel search provider does not support synonyms")
  }
  if (definition.settings.stop_words?.length) {
    fail("The Arcangel search provider does not support custom stop-word lists")
  }
...
export function buildIndexPlan(
  ...
  const walk = (
    ...
    for (const [name, field] of Object.entries(group)) {
      ...
      const configured = providerOptions<ArcangelSearchFieldOptions>(field)
  ...
  return {
    ...
    options: providerOptions<ArcangelSearchIndexOptions>(definition.settings),
...
export function toSearchDocument(
  ...
  if (primaryKey === undefined || primaryKey === null || primaryKey === "") {
    fail(
      `A document written to Arcangel search is missing primary key "${plan.primary_key}"`
    )
  }

  const id = String(primaryKey)
  if (Buffer.byteLength(id, "utf8") > 64) {
    fail(`Arcangel search document IDs cannot exceed 64 bytes ("${id}")`)
  }
