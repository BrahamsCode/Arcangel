...
import {
  productIndex,
  products,
} from "../__fixtures__/arcangel-cloud-product-index"

type SearchService = SearchTypes.ISearchModuleService

const apiKey = process.env.ARCANGEL_CLOUD_API_KEY
const endpoint = process.env.ARCANGEL_CLOUD_SEARCH_ENDPOINT
const environmentHandle = process.env.ARCANGEL_CLOUD_ENVIRONMENT_HANDLE
...
if (apiKey && endpoint && environmentHandle) {
  ...
  moduleIntegrationTestRunner<SearchService>({
    ...
    testSuite: ({ service }) =>
      describe("Arcangel Cloud Search Provider", () => {
        afterAll(async () => {
        ...
        it("matches text case-insensitively", async () => {
          ...
          expect(result.hits.map((hit) => hit.id)).toContain("prod_arcangel_1")
        })
        ...
        it("returns highlighted fragments around the matched terms", async () => {
          ...
          const hit = result.hits.find((h) => h.id === "prod_arcangel_1")
  ...
  describe.skip("Arcangel Cloud Search Provider", () => {
    it("requires Arcangel Cloud search credentials", () => {})
  })
}
