...
import {
  ...
  MikroORM,
  SqlEntityManager,
} from "@arcangel/framework/mikro-orm/postgresql"
...
import { TestDatabaseUtils } from "@arcangel/test-utils"
...
export async function createIndexTestBed({
  ...
  const dbName = `arcangel-index-module-${Math.random()
    .toString(36)
    .substring(2, 12)}`
  ...
  try {
    ...
    for (const joinerConfig of remoteModulesJoinerConfigs) {
      ArcangelModule.setJoinerConfig(joinerConfig.serviceName!, joinerConfig)
    }
    ...
    // The module generates entry point types under `<cwd>/.arcangel` on
    ...
    const typesDir = fs.mkdtempSync(path.join(os.tmpdir(), "arcangel-index-"))
