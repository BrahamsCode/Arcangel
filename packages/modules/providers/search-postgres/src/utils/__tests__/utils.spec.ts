...
describe("postgres search utils", () => {
  ...
  describe("helpers", () => {
    ...
    it("builds a prefix tsquery for match_strategy last", () => {
      expect(keywordTsQuerySql("arcangel_search_english")).toBe(
        `plainto_tsquery('arcangel_search_english', ?)`
      )
      expect(keywordTsQuerySql("arcangel_search_english", "any")).toBe(
        `replace(plainto_tsquery('arcangel_search_english', ?)::text, ' & ', ' | ')::tsquery`
      )
      expect(keywordTsQuerySql("arcangel_search_english", "last")).toBe(
        `regexp_replace(plainto_tsquery('arcangel_search_english', ?)::text, '''([^'']+)''$', '''\\1'':*')::tsquery`
      )
    })
  })
})
