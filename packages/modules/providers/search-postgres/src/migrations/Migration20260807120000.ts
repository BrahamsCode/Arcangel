 ...
 * - `arcangel_search_english` — unaccent-backed text search config
 *
      ...
      BEGIN
        ...
        BEGIN
          IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'unaccent')
             AND NOT EXISTS (
               SELECT 1 FROM pg_ts_config WHERE cfgname = 'arcangel_search_english'
             ) THEN
            CREATE TEXT SEARCH CONFIGURATION arcangel_search_english (COPY = english);
            ALTER TEXT SEARCH CONFIGURATION arcangel_search_english
              ALTER MAPPING FOR hword, hword_part, word
              WITH unaccent, english_stem;
          END IF;
        EXCEPTION WHEN OTHERS THEN
          RAISE NOTICE 'search-postgres: could not create arcangel_search_english (%).', SQLERRM;
        END;
      END
      $$;
    `)
