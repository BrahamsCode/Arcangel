...
Search provider for Arcangel backed by PostgreSQL, with two engines:

| Engine             | When to use                        | Keyword                     | Vector               |
| ------------------ | ---------------------------------- | --------------------------- | -------------------- |
| `native` (default) | Local / self-hosted / any Postgres | GIN + `ts_rank` + `pg_trgm` | Not supported        |
| `lakebase`         | Arcangel Cloud / Lakebase Search     | `lakebase_bm25` (BM25)      | `lakebase_ann` (ANN) |

## Enable it

### Native (default, works locally and on Arcangel Cloud)
...
### Lakebase (works only on Arcangel Cloud)
...
npx arcangel db:migrate-search
```

The migration enables `pg_trgm` + `unaccent` and creates the catalog. On Arcangel Cloud it also enables `lakebase_vector` and `lakebase_text` (`CREATE EXTENSION ... CASCADE`). Those statements soft-fail on engines that do not ship Lakebase Search.
...
| `language`        | `"english"` | Text search config. With `unaccent`, creates `arcangel_search_<language>`.               |
