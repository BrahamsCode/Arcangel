...
describe("OidcEngine", () => {
  ...
  describe("exchangeCode", () => {
    ...
    it("throws UNAUTHORIZED when the token response contains no ID token", async () => {
      ...
      expect(error.type).toBe(ArcangelError.Types.UNAUTHORIZED)
  ...
  describe("mapClaims", () => {
    ...
    it("throws when the entity_id claim is missing", () => {
      ...
      expect(error.type).toBe(ArcangelError.Types.INVALID_DATA)
...
function getError(fn: () => unknown): ArcangelError {
  try {
    fn()
  } catch (e) {
    return e as ArcangelError
  }
