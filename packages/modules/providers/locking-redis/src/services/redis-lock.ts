import { ArcangelError, promiseAll } from "@arcangel/framework/utils"
import { ILockingProvider } from "@arcangel/types"
...
export class RedisLockingProvider implements ILockingProvider {
  ...
  constructor({ redisClient, prefix }, options: RedisCacheModuleOptions) {
    ...
    this.keyNamePrefix = prefix ?? "arcangel_lock:"
      ...
      const acquireLock = async () => {
        while (true) {
          if (cancellationToken?.cancelled) {
            throw new ArcangelError(ArcangelError.Types.CONFLICT, errMessage)
          }
  ...
  private async getTimeout(
    ...
    return new Promise(async (_, reject) => {
      ...
      reject(new ArcangelError(ArcangelError.Types.CONFLICT, "Timed-out acquiring lock."))
    })
  }
}
