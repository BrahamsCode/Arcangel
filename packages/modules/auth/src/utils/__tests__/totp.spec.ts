...
describe("TOTP utilities", () => {
  ...
  it("percent-encodes the otpauth issuer and label", () => {
    const uri = generateTotpUri({
      issuer: "Arcangel Cloud",
      accountName: "test@example.com",
      secret,
    })

    expect(uri).toContain("otpauth://totp/Arcangel%20Cloud:test%40example.com")
    expect(uri).toContain("issuer=Arcangel%20Cloud")
    expect(uri).not.toContain("Arcangel+Cloud")
  })
})
