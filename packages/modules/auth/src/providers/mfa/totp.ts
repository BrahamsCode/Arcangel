...
type InjectedDependencies = {
  ...
  authMfaFactorService: ModulesSdkTypes.IArcangelInternalService<
...
export class TotpMfaProvider implements AuthMfaProvider {
  ...
  async verifySetup(
    ...
    if (!valid) {
      throw new ArcangelError(ArcangelError.Types.NOT_ALLOWED, "Invalid TOTP code")
    }
  ...
  protected getTotpConfig_(): TotpConfig {
    const config = {
      issuer: this.options_?.issuer ?? "Arcangel",
