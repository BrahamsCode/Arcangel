...
export async function writeStaticFiles(plugins?: string[]) {
  const outDir = join(process.cwd(), ".arcangel/client")
...
async function writeEntryFile(outDir: string, plugins?: string[]) {
  const entry = outdent`
    import App from "@arcangel/dashboard";
    ...
    if (!root) {
      root = ReactDOM.createRoot(document.getElementById("arcangel"))
    }
