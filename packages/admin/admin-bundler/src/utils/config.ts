import { VIRTUAL_MODULES } from "@arcangel/admin-shared"
...
export async function getViteConfig(
  ...
  const { default: arcangel } = await import("@arcangel/admin-vite-plugin")
  ...
  const root = path.resolve(process.cwd(), ".arcangel/client")
...
function getAllowedHosts(): string[] | undefined {
  const hosts = process.env.__ARCANGEL_ADMIN_ADDITIONAL_ALLOWED_HOSTS
  if (!hosts) {
    return undefined
  }
