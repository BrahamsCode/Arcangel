import { vi } from "vitest"

import fs from "fs/promises"
import * as utils from "../../utils"
import { generateWidgets } from "../generate-widgets"

vi.mock("../../utils", async () => {
  const actual = await vi.importActual("../../utils")
  return {
    ...actual,
    crawl: vi.fn(),
  }
})

vi.mock("fs/promises", () => ({
  default: {
    readFile: vi.fn(),
  },
}))

const mockFileContents = [
  `
    import { defineWidgetConfig } from "@arcangel/admin-sdk"

    const Widget = () => {
        return <div>Widget 1</div>
    }

    export const config = defineWidgetConfig({
        zone: "product.details.after",
    })

    export default Widget
`,
]

// `<component-name>-<short hash>`. The hash is of the admin-relative path
// "widgets/widget.tsx" — independent of the absolute/OS path, so both the Unix
// and Windows cases share this id.
const expectedWidgets = `
    widgets: [
        {
            Component: WidgetComponent0,
            zone: ["product.details.after"],
            widgetId: "Widget-e4f7"
        }
    ]
`

describe("generateWidgets", () => {
  it("should generate widgets", async () => {
    const mockFiles = ["Users/user/arcangel/src/admin/widgets/widget.tsx"]
    vi.mocked(utils.crawl).mockResolvedValue(mockFiles)

    vi.mocked(fs.readFile).mockImplementation(async (file) =>
      Promise.resolve(mockFileContents[mockFiles.indexOf(file as string)])
    )

    const result = await generateWidgets(
      new Set(["Users/user/arcangel/src/admin"])
    )

    expect(result.imports).toEqual([
      `import WidgetComponent0, { config as WidgetConfig0 } from "Users/user/arcangel/src/admin/widgets/widget.tsx"`,
    ])
    expect(utils.normalizeString(result.code)).toEqual(
      utils.normalizeString(expectedWidgets)
    )
  })
  it("should handle windows paths", async () => {
    const mockFiles = ["C:\\arcangel\\src\\admin\\widgets\\widget.tsx"]
    vi.mocked(utils.crawl).mockResolvedValue(mockFiles)

    vi.mocked(fs.readFile).mockImplementation(async (file) =>
      Promise.resolve(mockFileContents[mockFiles.indexOf(file as string)])
    )

    const result = await generateWidgets(new Set(["C:\\arcangel\\src\\admin"]))

    expect(result.imports).toEqual([
      `import WidgetComponent0, { config as WidgetConfig0 } from "C:/arcangel/src/admin/widgets/widget.tsx"`,
    ])
    expect(utils.normalizeString(result.code)).toEqual(
      utils.normalizeString(expectedWidgets)
    )
  })
  it("should use an explicit id from the widget config", async () => {
    const mockFiles = ["Users/user/arcangel/src/admin/widgets/widget.tsx"]
    vi.mocked(utils.crawl).mockResolvedValue(mockFiles)

    vi.mocked(fs.readFile).mockImplementation(async () =>
      Promise.resolve(`
        import { defineWidgetConfig } from "@arcangel/admin-sdk"

        const Widget = () => <div>Widget</div>

        export const config = defineWidgetConfig({
            zone: "product.details.after",
            id: "my-plugin:product-summary",
        })

        export default Widget
      `)
    )

    const result = await generateWidgets(
      new Set(["Users/user/arcangel/src/admin"])
    )

    expect(utils.normalizeString(result.code)).toEqual(
      utils.normalizeString(`
        widgets: [
            {
                Component: WidgetComponent0,
                zone: ["product.details.after"],
                widgetId: "my-plugin:product-summary"
            }
        ]
      `)
    )
  })
})
