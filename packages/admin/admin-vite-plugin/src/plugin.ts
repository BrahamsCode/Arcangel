...
import { ArcangelVitePlugin } from "./types"
...
export const arcangelVitePlugin: ArcangelVitePlugin = (options) => {
  ...
  async function generatePluginEntryModule(
    ...
    return `
      // Auto-generated index file for Arcangel Admin UI extensions
    ${widgetModule.code}
    ${routeModule.code}
    ${menuItemModule.code}
    ${formModule.code}
  ...
  return {
    name: "@arcangel/admin-vite-plugin",
    enforce: "pre",
    async buildStart() {
      switch (mode) {
        case Mode.PLUGIN: {
