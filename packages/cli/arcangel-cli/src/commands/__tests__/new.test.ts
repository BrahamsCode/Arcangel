...
jest.mock("@arcangel/deps/pg", () => ({
  Pool: jest.fn(() => ({ query: mockQuery, end: mockEnd })),
}))

jest.mock("@arcangel/telemetry", () => ({ track: jest.fn() }))

jest.mock("@arcangel/utils", () => ({
...
describe("arcangel new - database setup", () => {
  beforeEach(() => {
    jest.clearAllMocks()
  })
  ...
  describe("createDatabase", () => {
    it("connects with the given credentials and issues CREATE DATABASE", async () => {
      ...
      await createDatabase("arcangel-db-abc12", credentials)

      expect(Pool).toHaveBeenCalledWith(credentials)
      expect(mockQuery).toHaveBeenCalledWith(`CREATE DATABASE "arcangel-db-abc12"`)
    ...
    it("closes the pool even when the query fails", async () => {
      ...
      await expect(
        createDatabase("arcangel-db-abc12", credentials)
  ...
  describe("setupDB", () => {
    it("reports success when the database is created", async () => {
      ...
      await setupDB("arcangel-db-abc12", credentials)

      expect(reporter.success).toHaveBeenCalledWith(
        "db-activity",
        `Created database "arcangel-db-abc12"`
      )
    ...
    it("treats Postgres error 42P04 as 'already exists' and still succeeds", async () => {
      ...
      expect(reporter.success).toHaveBeenCalledWith(
        "db-activity",
        `Database arcangel-db-abc12 already exists; skipping setup`
      )
      expect(reporter.failure).not.toHaveBeenCalled()
    })
