...
import { Pool } from "@arcangel/deps/pg"
import url from "url"
import { track } from "@arcangel/telemetry"
// @ts-ignore
import inquirer from "inquirer"

import { getNodeVersion, MIN_SUPPORTED_NODE_VERSION } from "@arcangel/utils"
import reporter from "../reporter"
import { PanicId } from "../reporter/panic-handler"
import { getPackageManager } from "../util/package-manager"

const STARTER_PATH = `arcangel/dtc-starter`
...
const successMessage = (path) => {
  reporter.info(`Your new Arcangel project is ready for you! To start developing run:
...
export const createDatabase = async (dbName, credentials) => {
  ...
  try {
    // `dbName` is always internally generated (`arcangel-db-<base36>`) and is never
...
export const newStarter = async (args: NewCommandArgs) => {
  ...
  if (nodeVersion < MIN_SUPPORTED_NODE_VERSION) {
    reporter.error(
      `Arcangel requires at least v20 of Node.js. You're using v${nodeVersion}. Please install at least v20 and try again: https://nodejs.org/en/download`
    )
    process.exit(1)
  }
  track("CLI_NEW")
  ...
  if (urlObject.protocol && urlObject.host) {
    if (/arcangel-starter/gi.test(rootPath)) {
      reporter.panic({
  ...
  const dbName = `arcangel-db-${Math.random().toString(36).substring(2, 7)}` // generate random 5 character string
  ...
  if (creds === null) {
    reporter.info(
      "Skipping automatic database setup. Please note that you need to create a database and run migrations before you can run your Arcangel backend"
    )
  } else {
    if (!skipDb) {
