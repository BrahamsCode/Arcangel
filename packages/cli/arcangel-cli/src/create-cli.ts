import { setTelemetryEnabled } from "@arcangel/telemetry"
...
/**
 * Whether `@arcangel/eslint-plugin` is installed in the consumer project.
 * Linting is enabled by default on `build` and `develop` only when this is true.
 */
function isArcangelEslintPluginInstalled(cwd: string): boolean {
  try {
    require.resolve("@arcangel/eslint-plugin", { paths: [cwd] })
    return true
  } catch {
    return false
  }
...
function addLintOptions(builder, commandLabel: string, directory: string) {
  const lintByDefault = isArcangelEslintPluginInstalled(directory)

  return builder
    .option("lint", {
      type: "boolean",
      describe: `Run the Arcangel linter before ${commandLabel}. Enabled by default when @arcangel/eslint-plugin is installed. Use --no-lint to skip linting.`,
...
function buildLocalCommands(cli, isLocalProject) {
  ...
  function resolveLocalCommand(command) {
    if (!isLocalProject && command !== "new") {
      console.error(
        `The "${command}" command must be run inside a Arcangel project. Make sure you are in the root directory of a Arcangel project and try again.`
      )
      process.exit(1)
    }

    try {
      const cmdPath = resolveCwd.silent(`@arcangel/arcangel/commands/${command}`)!
  ...
  cli
    .command({
      ...
      builder: (_) =>
        _.option(`seed`, {
          ...
          .option(`v2`, {
            type: `boolean`,
            describe: `Install Arcangel with the V2 feature flag enabled. WARNING: Arcangel V2 is still in development and shouldn't be used in production.`,
            default: false,
          })
          .option(`branch`, {
            type: `string`,
      ...
      desc: `Create a new Arcangel project.`,
    ...
    .command({
      ...
      desc: "Sync database schema with the links defined by your application and Arcangel core",
      builder: (builder) => {
      ...
      builder: (_) =>
        addLintOptions(
          _.option("types", {
            ...
            default: true,
            describe:
              "Generate automated types for modules inside the .arcangel directory",
          })
            .option(`H`, {
              alias: `host`,
              type: `string`,
      ...
      builder: (_) =>
        addLintOptions(
          _.option("admin-only", {
            ...
            type: "boolean",
            describe:
              "Only build the admin to serve it separately (outDir .arcangel/admin)",
          }),
          "building",
          directory
        ).option("quiet", {
    ...
    .command({
      command: `mcloud [args..]`,
      desc: `Run mcloud CLI commands through Arcangel CLI.`,
      builder: (builder) =>
...
function isLocalArcangelProject() {
  let inArcangelProject = false

  try {
    const { dependencies, devDependencies } = require(path.resolve(
      `./package.json`
    ))
    // Draft order plugin can't have @arcangel/arcangel as dependency,
    // so we also check for @arcangel/cli
    inArcangelProject = !!(
      (dependencies &&
        (dependencies["@arcangel/arcangel"] || dependencies["@arcangel/cli"])) ||
      (devDependencies &&
        (devDependencies["@arcangel/arcangel"] ||
          devDependencies["@arcangel/cli"]))
    )
  } catch (err) {
    // ignore
  }

  return inArcangelProject
}

function getVersionInfo() {
  const { version } = require(`../package.json`)
  const isArcangelProject = isLocalArcangelProject()
  if (isArcangelProject) {
    let arcangelVersion = ""
    try {
      arcangelVersion = require(path.join(
        process.cwd(),
        `node_modules`,
        `@arcangel/arcangel`,
        `package.json`
    ...
    if (!arcangelVersion) {
      arcangelVersion = `unknown`
    }

    return `Arcangel CLI version: ${version}
Arcangel version: ${arcangelVersion}
  Note: this is the Arcangel version for the site at: ${process.cwd()}`
  } else {
    return `Arcangel CLI version: ${version}`
  }
}
...
export default (argv) => {
  const cli = yargs()
  const isLocalProject = isLocalArcangelProject()
  ...
  try {
    cli.version(
      `version`,
      `Show the version of the Arcangel CLI and the Arcangel package in the current project`,
      getVersionInfo()
    )
  } catch (e) {
    // ignore
