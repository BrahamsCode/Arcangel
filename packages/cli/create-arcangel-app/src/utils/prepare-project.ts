...
async function prepareProject({
  ...
  // Update arcangel dependencies versions
  if (version) {
  ...
  let env = `ARCANGEL_ADMIN_ONBOARDING_TYPE=${onboardingType}${EOL}STORE_CORS=${STORE_CORS}${EOL}ADMIN_CORS=${ADMIN_CORS}${EOL}AUTH_CORS=${AUTH_CORS}${EOL}REDIS_URL=${DEFAULT_REDIS_URL}${EOL}JWT_SECRET=supersecret${EOL}COOKIE_SECRET=supersecret${EOL}${AUTH_MFA_ENCRYPTION_KEY}=${randomBytes(
    32
  ).toString("hex")}`
  ...
  if (nextjsDirectory) {
    env += `${EOL}ARCANGEL_ADMIN_ONBOARDING_NEXTJS_DIRECTORY=${nextjsDirectory}`
  }
  ...
  if (!skipDb && migrations) {
    ...
    // run migrations
    const migrationExecResult = await packageManager.runArcangelCommand(
      "db:migrate",
      npxOptions,
      {
        verbose,
    ...
    const userExecResult = (await packageManager.runArcangelCommand(
    ...
    if (apiKeys.rowCount) {
      ...
      fs.writeFileSync(
        ...
        originalContent.replace(
          "NEXT_PUBLIC_ARCANGEL_PUBLISHABLE_KEY=",
          `NEXT_PUBLIC_ARCANGEL_PUBLISHABLE_KEY=${apiKeys.rows[0].token}`
        )
      )
    }
  }
