...
describe("PackageManager", () => {
  ...
  describe("runArcangelCommand", () => {
    it("should set package manager before running if not set", async () => {
      process.env.npm_config_user_agent = "yarn/1.22.0"

      const pm = new PackageManager(processManager)
      await pm.runArcangelCommand("migrate", { cwd: "/test/path" })

      expect(pm.getPackageManager()).toBe("yarn")
    })

    it("should execute yarn arcangel command for yarn", async () => {
      mockExecute.mockResolvedValue({ stdout: "", stderr: "" })
      ...
      expect(mockExecute).toHaveBeenCalledWith(
        ["yarn arcangel migrate", { cwd: "/test/path" }],
        { verbose: false }
      )
    })

    it("should execute pnpm arcangel command for pnpm", async () => {
      mockExecute.mockResolvedValue({ stdout: "", stderr: "" })

      const pm = new PackageManager(processManager)
      pm["packageManager"] = "pnpm"

      await pm.runArcangelCommand("seed", { cwd: "/test/path" })

      expect(mockExecute).toHaveBeenCalledWith(
        ["pnpm arcangel seed", { cwd: "/test/path" }],
        { verbose: false }
      )
    })

    it("should execute npx arcangel command for npm", async () => {
      mockExecute.mockResolvedValue({ stdout: "", stderr: "" })

      const pm = new PackageManager(processManager)
      pm["packageManager"] = "npm"

      await pm.runArcangelCommand("start", { cwd: "/test/path" })

      expect(mockExecute).toHaveBeenCalledWith(
        ["npx arcangel start", { cwd: "/test/path" }],
        { verbose: false }
      )
    })
    ...
    it("should pass through verbose options", async () => {
      ...
      await pm.runArcangelCommand("migrate", { cwd: "/test/path" }, { needOutput: true })
    ...
    it("should return execution result", async () => {
      ...
      const output = await pm.runArcangelCommand("migrate", {})
