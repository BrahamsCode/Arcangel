...
import startArcangel from "../start-arcangel.js"
import {
  getBackendDirectory,
...
// Arcangel Project Creator
export class ArcangelProjectCreator
  ...
  implements ProjectCreator
{
  ...
  private async setupDatabase(): Promise<void> {
    let dbName = `arcangel-${slugify(this.projectName)}`
    ...
    logMessage({
      message: boxen(
        chalk.green(
          `Change to the \`${
            this.projectName
          }\` directory to explore your Arcangel project.${EOL}${EOL}Start your Arcangel application again with the following command:${EOL}${EOL}${commandStr}${EOL}${EOL}${
            this.inviteToken
              ? `After you start the Arcangel application, you can create an admin user with the URL http://localhost:9000/app/invite?token=${this.inviteToken}&first_run=true${EOL}${EOL}`
              : ""
          }${
            this.nextjsDirectory?.length
              ...
              : ""
          }Check out the Arcangel ${terminalLink(
            "documentation",
            "https://docs.arcangel.com/"
          )} to start your development:${EOL}${EOL}Star us on ${terminalLink(
            "GitHub",
            "https://github.com/arcangel/arcangel/stargazers"
