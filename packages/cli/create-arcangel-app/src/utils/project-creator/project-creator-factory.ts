...
import { PluginProjectCreator } from "./arcangel-plugin-creator.js"
import { ArcangelProjectCreator } from "./arcangel-project-creator.js"
...
export class ProjectCreatorFactory {
  static async create(
    ...
    return options.plugin
      ...
      : new ArcangelProjectCreator(projectName, options, args)
  }
  ...
  private static validateNodeVersion(): void {
    ...
    if (nodeVersion < MIN_SUPPORTED_NODE_VERSION) {
      logMessage({
        message: `Arcangel requires at least v20 of Node.js. You're using v${nodeVersion}. Please ${terminalLink(
          "install Node.js",
  ...
  const { projectName } = await inquirer.prompt([
    {
      ...
      default: isPlugin ? "my-arcangel-plugin" : "my-arcangel-store",
      filter: (input) => {
