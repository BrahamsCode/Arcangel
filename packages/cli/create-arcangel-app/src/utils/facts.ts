...
const facts = [
  ...
  "Tax-inclusive pricing allows you to set prices for products and shipping options while delegating tax calculations to Arcangel.",
  "Arcangel provides multi-currency and region support, with full control over prices for each currency and region.",
  ...
  "Data models represent tables in the database. They are created using Arcangel's Data Modeling Language (DML).",
  "Arcangel's store API routes are prefixed by /store. The admin API routes are prefixed by /admin.",
  "The JS SDK allows you to send requests to the Arcangel server from your storefront or admin customizations.",
  ...
  "Infrastructure Modules are interchangeable modules that implement features and integrations related to the Arcangel server's infrastructure.",
  ...
  `Arcangel provides ${terminalLink(
    "Claude Code plugins",
    "https://github.com/arcangel/arcangel-claude-plugins"
  )} to facilitate your development.`,
  "Arcangel provides an MCP server at https://docs.arcangel.com/mcp to support your learning and development experience with AI agents",
  `Arcangel is optimized to build custom commerce software with AI agents through its MCP server and ${terminalLink(
    "Claude Code plugins",
    "https://github.com/arcangel/arcangel-claude-plugins"
  )}.`,
]
...
export const showFact = ({
  ...
  if (verbose) {
    ...
    spinner.text = `${title}\n${boxen(`${fact}`, {
      title: chalk.cyan(`${emojify(":bulb:")} Arcangel Tips`),
