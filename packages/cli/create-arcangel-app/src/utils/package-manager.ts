...
export default class PackageManager {
  ...
  private applyNpmWorkspaceChanges(packageJson: Record<string, unknown>): void {
    ...
    // root, but Arcangel's stack needs two: the backend's migration tooling
    ...
    // (`arcangel db:migrate`/`build`) and the storefront (`next lint`) resolve a
      ...
      process: async () => {
        try {
          ...
          if (this.packageManager === "pnpm" && packageManagerConflictMatch) {
            ...
            throw new Error(
              `Cannot install with pnpm because ${conflictingFile} has a "packageManager" field that specifies a different package manager. Consider removing that field from ${conflictingFile}, or create your Arcangel project in a different directory.`
            )
          }
          throw error
        }
  ...
  async runArcangelCommand(
    ...
    const formats: Record<PackageManagerType, string> = {
      yarn: `yarn arcangel ${command}`,
      pnpm: `pnpm arcangel ${command}`,
      npm: `npx arcangel ${command}`,
      nub: `nubx arcangel ${command}`,
    }
