...
describe("Config", () => {
  ...
  describe("load() with override (bypasses file reading)", () => {
    it("returns package defaults when called with an empty override", () => {
      ...
      expect(config.importSources.commonRequest).toBe("@arcangel/framework/types")
      expect(config.importSources.dal).toBe("@arcangel/framework/types")
    ...
    it("deep-merges validatorGlobs over defaults", () => {
      const config = Config.load({
        validatorGlobs: {
          admin: "packages/arcangel/src/api/admin/*/validators.ts",
          store: "packages/arcangel/src/api/store/*/validators.ts",
        },
      })

      expect(config.validatorGlobs.admin).toBe(
        "packages/arcangel/src/api/admin/*/validators.ts"
      )
      expect(config.validatorGlobs.store).toBe(
        "packages/arcangel/src/api/store/*/validators.ts"
      )
    })
