...
## Usage in the Arcangel monorepo
...
The repo root's `http-types.config.json` configures the Arcangel-specific paths automatically — no additional setup is needed.
...
```bash
npm install --save-dev @arcangel/http-types-generator
# or run without installing:
npx @arcangel/http-types-generator generate
```
...
```json
{
  ...
  "importSources": {
    "commonRequest": "@arcangel/framework/types",
    "dal": "@arcangel/framework/types"
  },
...
| `importSources.commonRequest` | Module that exports `FindParams` and `SelectParams`. | `"@arcangel/framework/types"` |
| `importSources.dal` | Module that exports `BaseFilterable` and `OperatorMap`. | `"@arcangel/framework/types"` |
...
```bash
# Generate
npx @arcangel/http-types-generator generate --dry-run
npx @arcangel/http-types-generator generate --domain products

# Validate
npx @arcangel/http-types-generator validate
npx @arcangel/http-types-generator validate --ci
```
