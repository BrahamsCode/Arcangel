 ...
 * Constants
 */
// Arcangel core package directory
const arcangelPackagePath = path.dirname(require.resolve("@arcangel/arcangel"))
...
export const commandOptions: Option[] = [
  ...
  new Option(
    "--local",
    "Generate OAS from local files rather than public OAS. This is useful for generating references in the Arcangel monorepo."
  ),
]
...
async function getOASFromCodebase(apiType: ApiType): Promise<OpenAPIObject> {
  ...
  const gen = await swaggerInline(
    [
      ...
      path.resolve(arcangelPackagePath, "dist", "utils/middlewares"),
    ],
    {
...
async function getPublicOas(apiType: ApiType) {
  const url = `https://docs.arcangel.com/api/download/${apiType}`
...
/**
 * Sets `info.version` to the version of the installed `@arcangel/arcangel`
 * package, so that consumers of the OAS can tell which Arcangel version the
 * document describes. The version in the base OAS files is a static
 * placeholder, which is why it's overridden here.
 */
function setArcangelVersion(oas: OpenAPIObject): void {
  let arcangelVersion: string | undefined

  try {
    arcangelVersion = require("@arcangel/arcangel/package.json").version
  } catch {
  ...
  oas.info = {
    ...oas.info,
    version: arcangelVersion,
  }
  console.log(`🔵 Set OAS version to ${arcangelVersion}`)
}
