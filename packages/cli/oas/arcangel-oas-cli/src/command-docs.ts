...
const arcangelPluginRelativePath = "./plugins/arcangel/index.js"
const arcangelPluginAbsolutePath = path.resolve(
  basePath,
  "redocly/plugins/arcangel/index.js"
)
...
const createTmpConfig = async (
  ...
  config.plugins = (config.plugins ?? []).filter(
    (plugin) => plugin !== arcangelPluginRelativePath
  )
  config.plugins.push(arcangelPluginAbsolutePath)
...
`
      ...
      Object.keys(recommendation).forEach((recKey) => {
        originalContent.decorators["arcangel/circular-patch"].schemas[recKey] = [
          ...(originalContent.decorators["arcangel/circular-patch"].schemas[
            recKey
          ] || []),
