...
export abstract class AbstractAuthModuleProvider implements IAuthProvider {
   ...
   *
   * Related Read: [Learn about the different authentication flows in Arcangel](https://docs.arcangel.com/resources/commerce-modules/auth/authentication-route).
   *
   ...
   *       }
   *     } catch (error) {
   *       if (error.type === ArcangelError.Types.NOT_FOUND) {
   ...
   * The frontend should then send a request to the Arcangel application's validate callback API route, passing it the code.
   ...
   *
   * Related Guide: [Learn about the different authentication flows in Arcangel](https://docs.arcangel.com/resources/commerce-modules/auth/authentication-route).
   *
   * @param {AuthenticationInput} data - The details of the authentication request.
   * @param {AuthIdentityProviderService} authIdentityProviderService - The service used to retrieve or
   * create an auth identity. It has two methods: `create` to create an auth identity,
