import { BigNumberInput } from "@arcangel/types"
import {
  ApplicationMethodAllocation,
  ApplicationMethodType,
} from "../../promotion"
import { MathBN } from "../math"
import { ARCANGEL_EPSILON } from "../big-number"
...
export function calculateAdjustmentAmountFromPromotion(
  ...
  */
  if (promotion.allocation === ApplicationMethodAllocation.ACROSS) {
    ...
    if (MathBN.lte(applicableAmount, ARCANGEL_EPSILON)) {
    ...
    if (MathBN.lte(returnValue, ARCANGEL_EPSILON)) {
      return MathBN.convert(0)
    }
