import { BigNumberInput, BigNumberRawValue, IBigNumber } from "@arcangel/types"
...
export class BigNumber implements IBigNumber {
  ...
  get numeric(): number {
    ...
    if (Math.abs(value) <= ARCANGEL_EPSILON.numeric_) {
      return 0
    }
...
export const ARCANGEL_EPSILON = new BigNumber(
  process.env.ARCANGEL_EPSILON || "0.0001"
)

export const ARCANGEL_DEFAULT_CURRENCY_EPSILON = new BigNumber(
  process.env.ARCANGEL_DEFAULT_CURRENCY_EPSILON || "0.01"
)

export const getEpsilonFromDecimalPrecision = (decimalDigits?: number) => {
  if (!isDefined(decimalDigits)) {
    return ARCANGEL_DEFAULT_CURRENCY_EPSILON
  }
