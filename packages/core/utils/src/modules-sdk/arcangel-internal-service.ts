...
import {
  ...
  type EntityManager,
  type EntitySchema,
} from "@arcangel/deps/mikro-orm/core"
import {
  isDefined,
  isObject,
  isPresent,
...
import { ArcangelMikroOrmEventSubscriber } from "./create-arcangel-mikro-orm-event-subscriber"

type InternalService = {
  new <TContainer extends object = object, TEntity extends object = any>(
    container: TContainer
  ): ModulesSdkTypes.IArcangelInternalService<TEntity, TContainer>

  setEventSubscriber(subscriber: ArcangelMikroOrmEventSubscriber): void
}
...
export function registerInternalServiceEventSubscriber(
  context: Context,
  subscriber?: ArcangelMikroOrmEventSubscriber
) {
...
export const ArcangelInternalServiceSymbol = Symbol.for(
  "ArcangelInternalServiceSymbol"
)

/**
 * Check if a value is a Arcangel internal service
 * @param value
 */
export function isArcangelInternalService(value: any): value is InternalService {
  return (
    !!value?.[ArcangelInternalServiceSymbol] ||
    !!value?.prototype?.[ArcangelInternalServiceSymbol]
  )
}

export function ArcangelInternalService<
...
>(rawModel: any): InternalService {
  ...
  class AbstractService_
    implements ModulesSdkTypes.IArcangelInternalService<TEntity, TContainer>
  {
    [ArcangelInternalServiceSymbol] = true

    #eventSubscriber?: ArcangelMikroOrmEventSubscriber
    ...
    setEventSubscriber(subscriber: ArcangelMikroOrmEventSubscriber) {
