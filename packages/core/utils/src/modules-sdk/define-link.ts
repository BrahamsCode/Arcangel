import { LinkModulesExtraFields, ModuleJoinerConfig } from "@arcangel/types"
import {
  camelToSnakeCase,
  getCallerFilePath,
  isFileDisabled,
...
export function defineLink(
  ...
  if (isFileDisabled(callerFilePath ?? "")) {
    return { [ARCANGEL_SKIP_FILE]: true } as any
  }
  ...
  const register = function (
    ...
    if (!serviceAInfo) {
      throw new Error(`Service ${serviceAObj.module} was not found. If this is your module, make sure you set isQueryable to true in arcangel-config.js:
...
${serviceAObj.module}: {
  ...
  definition: {
    ...
    if (!serviceBInfo) {
      throw new Error(`Service ${serviceBObj.module} was not found. If this is your module, make sure you set isQueryable to true in arcangel-config.js:
...
${serviceBObj.module}: {
  ...
  global.ArcangelModule.setCustomLink(register)
