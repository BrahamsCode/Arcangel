import { ArcangelInternalService } from "../arcangel-internal-service"
import { lowerCaseFirst } from "../../common"

const defaultContext = { __type: "ArcangelContext" }
...
describe("Internal Module Service Factory", () => {
  ...
  const IArcangelInternalService = ArcangelInternalService<any>(Model)

  describe("Internal Module Service Methods", () => {
    let instance

    beforeEach(() => {
      instance = new IArcangelInternalService(containerMock)
    })
    ...
    it("should throw an error on retrieve if composite key values are not defined", async () => {
      ...
      const compositeIArcangelInternalService =
        ArcangelInternalService<any>(CompositeModel)

      const instance = new compositeIArcangelInternalService(containerMock)
