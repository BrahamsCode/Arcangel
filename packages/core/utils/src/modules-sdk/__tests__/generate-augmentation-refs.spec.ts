import { PluginDetails } from "@arcangel/types"
...
function makePlugin(
  ...
  return {
    name,
    id: name,
    resolve: path.join(BASE_DIR, "node_modules", name, ".arcangel/server/src"),
    options: {},
    version: "1.0.0",
    ...overrides,
  }
...
describe("generateAugmentationRefs", () => {
  it("should emit a reference for plugin that exposes root types via exports map", async () => {
    ...
    const content = await fs.contents(".arcangel/types/augmentation-refs.d.ts")
  ...
  it("normalizes a core module registered through its bare package name to the canonical arcangel subpath", async () => {
    await fs.create(
      `node_modules/@arcangel/arcangel/rbac.js`,
      "module.exports = {}"
    )
    await fs.create(`node_modules/@arcangel/arcangel/rbac.d.ts`, "export {}")
    await fs.createJson(`node_modules/@arcangel/arcangel/package.json`, {
      name: "@arcangel/arcangel",
      exports: {
        "./rbac": "./rbac.js",
    ...
    const content = await fs.contents(".arcangel/types/augmentation-refs.d.ts")
    expect(content).toContain('/// <reference types="@arcangel/arcangel/rbac" />')
    expect(content).not.toContain('"@arcangel/rbac"')
  })

  it("keeps a core module's @arcangel/arcangel subpath resolve path as is", async () => {
    await fs.create(
      `node_modules/@arcangel/arcangel/event-bus-redis.js`,
      "module.exports = {}"
    )
    await fs.create(
      `node_modules/@arcangel/arcangel/event-bus-redis.d.ts`,
      "export {}"
    )
    await fs.createJson(`node_modules/@arcangel/arcangel/package.json`, {
      name: "@arcangel/arcangel",
    ...
    await generateAugmentationRefs({
      ...
      modules: {
        event_bus: {
          __definition: {
            key: "event_bus",
            resolvePath: "@arcangel/arcangel/event-bus-redis",
          },
        } as any,
      },
    })
    ...
    const content = await fs.contents(".arcangel/types/augmentation-refs.d.ts")
    expect(content).toContain(
      '/// <reference types="@arcangel/arcangel/event-bus-redis" />'
    )
  })
