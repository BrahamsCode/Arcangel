import type { LoadedModule } from "@arcangel/types"
 ...
 * when the output file is inside the ".arcangel/types" directory.
 * For example:
 *
...
export async function generateContainerTypes(
    ...
    (result, key) => {
      ...
      services.forEach((service) => {
        ...
        if (SERVICES_INTERFACES[key]) {
          result.imports.push(
            `import type { ${SERVICES_INTERFACES[key]} } from '@arcangel/framework/types'`
          )
  ...
  const fileContents = `${imports.join(
    "\n"
  )}\n\ndeclare module '@arcangel/framework/types' {
