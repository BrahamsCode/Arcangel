...
export function InjectManager(managerProperty?: string): MethodDecorator {
  return function (
    ...
    if (!target.ArcangelContextIndex_) {
      throw new Error(
        `To apply @InjectManager you have to flag a parameter using @ArcangelContext`
      )
    }
    ...
    descriptor.value = function (...args: any[]) {
      ...
      if (
        ...
        throw new Error(
          `Could not find a manager in the context. Ensure that "${managerProperty}" is set on your service and points to a repository — e.g. extend ArcangelService(...), which provides "baseRepository_" automatically.`
        )
      }
      ...
      copiedContext.__type = ArcangelContextType
