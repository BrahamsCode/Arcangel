...
import { createArcangelMikroOrmEventSubscriber } from "./create-arcangel-mikro-orm-event-subscriber"
import { EmitEvents, InjectManager, ArcangelContext } from "./decorators"
...
import { isArcangelInternalService } from "./arcangel-internal-service"
import {
  BaseMethods,
  ExtractKeysFromConfig,
  ArcangelServiceReturnType,
  ModelConfigurationsToConfigTemplate,
  ModelEntries,
  ModelsConfigTemplate,
} from "./types/arcangel-service"
...
/**
 * Accessible from the ArcangelService, holds the model objects when provided
 */
export const ArcangelServiceModelObjectsSymbol = Symbol.for(
  "ArcangelServiceModelObjectsSymbol"
)

/**
 * Symbol to mark a class as a Arcangel service
 */
export const ArcangelServiceSymbol = Symbol.for("ArcangelServiceSymbol")

/**
 * Accessible from the ArcangelService, holds the model name to linkable keys map
 * to be used for softDelete and restore methods
 */
export const ArcangelServiceModelNameToLinkableKeysMapSymbol = Symbol.for(
  "ArcangelServiceModelNameToLinkableKeysMapSymbol"
)

/**
 * Check if a value is a Arcangel service
 * @param value
 */
export function isArcangelService(
  value: any
): value is ArcangelServiceReturnType<any> {
  return value && value?.prototype[ArcangelServiceSymbol]
}
 ...
 * }
 *
 * class MyService extends ModulesSdkUtils.ArcangelService(models) {}
 *
 * @param models
 */
export function ArcangelService<
...
> {
  const buildAndAssignMethodImpl = function (
    ...
    const applyMethod = function (impl: Function, contextIndex) {
      ...
      ArcangelContext()(klassPrototype, methodName, contextIndex)
      ...
      case "softDelete":
        methodImplementation = async function <T extends { id: string }>(
          ...
          const mappedCascadedModelsMap = mapObjectTo(
            cascadedModelsMap,
            this[ArcangelServiceModelNameToLinkableKeysMapSymbol],
            {
  ...
  class AbstractModuleService_ {
    [ArcangelServiceSymbol] = true

    static [ArcangelServiceModelObjectsSymbol] =
      models as unknown as ArcangelServiceReturnType<
        ModelsConfig extends { __empty: any }
          ? ModelConfigurationsToConfigTemplate<TModels>
          : ModelsConfig
      >["$modelObjects"];

    [ArcangelServiceModelNameToLinkableKeysMapSymbol]: MapToConfig
    ...
    constructor(container: Record<any, any>) {
      ...
      if (joinerConfig?.serviceName !== "index") {
        let globalSubscriber!: ReturnType<
          typeof createArcangelMikroOrmEventSubscriber
        >
        ...
        Object.keys(container)
          ...
          .forEach((key: string) => {
            globalSubscriber ??= createArcangelMikroOrmEventSubscriber(
              ["__arcangel_entities_subscriber__"],
              this
            )
            try {
              const service = container[key]
              if (isArcangelInternalService(service)) {
