import { Filter as MikroORMFilter } from "@arcangel/deps/mikro-orm/core"
import { TSMigrationGenerator } from "@arcangel/deps/mikro-orm/migrations"
import { ModuleServiceInitializeOptions } from "@arcangel/types"
...
export async function mikroOrmCreateConnection(
  ...
  const { MikroORM, defineConfig } = await import(
    "@arcangel/deps/mikro-orm/postgresql"
  )
  const mikroOrmConfig = defineConfig({
    ...
    // soft-deleted. Arcangel was designed around MikroORM 6.4.x where this didn't exist, so
  ...
  const maxRetries = process.env.__ARCANGEL_DB_CONNECTION_MAX_RETRIES
    ? parseInt(process.env.__ARCANGEL_DB_CONNECTION_MAX_RETRIES)
    : 5

  const retryDelay = process.env.__ARCANGEL_DB_CONNECTION_RETRY_DELAY
    ? parseInt(process.env.__ARCANGEL_DB_CONNECTION_RETRY_DELAY)
    : 1000
