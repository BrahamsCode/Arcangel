import { MikroORM, MikroORMOptions } from "@arcangel/deps/mikro-orm/core"
import {
  MigrateOptions,
  MigrationResult,
  UmzugMigration,
} from "@arcangel/deps/mikro-orm/migrations"
import {
  defineConfig,
  PostgreSqlDriver,
} from "@arcangel/deps/mikro-orm/postgresql"
...
const replacements = [
  // MikroORM imports - replace mikro-orm/{subpath} with @arcangel/framework/mikro-orm/{subpath}
  {
    pattern: /from\s+['"]@?mikro-orm\/([^'"]+)['"]/g,
    // eslint-disable-next-line quotes
    replacement: 'from "@arcangel/framework/mikro-orm/$1"',
  },
  // PG imports - replace pg with @arcangel/framework/pg
  {
    pattern: /from\s+['"]pg['"]/g,
    // eslint-disable-next-line quotes
    replacement: 'from "@arcangel/framework/pg"',
  },
]
...
export class Migrations extends EventEmitter<MigrationsEvents> {
  ...
  protected async migrateSnapshotFile(snapshotPath: string): Promise<void> {
    ...
    // renamed to `.snapshot-arcangel-<module>.json` for plugin:db:generate users).
    // Matching any `.json` file risks picking up full-DB snapshots.
    const expectedName = basename(snapshotPath)
    const legacyName = expectedName.replace(/^\.snapshot-arcangel-/, ".snapshot-")
