import { MetadataStorage } from "@arcangel/deps/mikro-orm/core"
...
describe("Generate migrations", () => {
  ...
  test("rename existing snapshot file to the new filename (plugins)", async () => {
    // Legacy snapshot name: no arcangel- prefix, matching the module name
    ...
    expect(
      await fs.exists(".snapshot-arcangel-my-test-generate.json")
    ).toBeTruthy()
...
describe("migrateSnapshotFile", () => {
  ...
  test("preserves correct module snapshot when a DB-name snapshot also exists", async () => {
    ...
    const dbSnapshotName = ".snapshot-arcangel-my-module.json"
  ...
  test("renames legacy plugin snapshot (no arcangel- prefix) to the new plugin snapshot name (with arcangel- prefix)", async () => {
    const legacySnapshotName = ".snapshot-my-module.json"
    const newSnapshotName = ".snapshot-arcangel-my-module.json"
