...
/**
 * Resolves a module identifier relative to the Arcangel project directory
 * instead of relative to the directory `@arcangel/utils` itself happens to
 * be installed in.
 *
 * @param id - a bare specifier, subpath specifier, or absolute path
 * @param projectDir - the root directory of the Arcangel application
...
export function resolveFromProject(
  ...
  try {
     ...
     * `@arcangel/utils` itself (for example when Arcangel is executed from
