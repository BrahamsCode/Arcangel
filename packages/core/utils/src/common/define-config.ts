...
const ARCANGEL_CLOUD_EXECUTION_CONTEXT = "arcangel-cloud"
...
const DEFAULT_DATABASE_URL = "postgres://localhost/arcangel-starter-default"
...
export function defineConfig(config: InputConfig = {}): ConfigModule {
  const options = {
    isCloud: process.env.EXECUTION_CONTEXT === ARCANGEL_CLOUD_EXECUTION_CONTEXT,
  }
  ...
  const remappedModules = modules.reduce((acc, moduleConfig) => {
    ...
    if (!serviceName && "resolve" in moduleConfig) {
      if (!moduleConfig.resolve) {
        throw new Error(
          `Module must have a 'resolve' property in arcangel-config.ts and it can't be empty. Please provide a module name or path for the module.`
        )
      }
       ...
       * ("<plugin>/.arcangel/server/src/modules/<name>"), so they must be
       * resolved from the project directory. Otherwise the lookup starts in
       * whichever "node_modules" directory `@arcangel/utils` was hoisted into,
...
function resolveModules(
  ...
  const sharedModules = [
    ...
    // Cloud with ARCANGEL_CLOUD_SEARCH_ENDPOINT: search-arcangel is the default
    // (registered from options.cloud, same pattern as notification cloud email
    // and Arcangel Payments). Postgres stays registered so a provider switch can
    // drop the previous engine's indexes during `db:migrate`.
    {
      ...
      options: {
        ...
        ...(process.env.ARCANGEL_CLOUD_SEARCH_ENDPOINT
...
function normalizeProjectConfig(
  ...
  const mergedCloudOptions: ArcangelCloudOptions = {
    environmentHandle: process.env.ARCANGEL_CLOUD_ENVIRONMENT_HANDLE,
    sandboxHandle: process.env.ARCANGEL_CLOUD_SANDBOX_HANDLE,
    apiKey: process.env.ARCANGEL_CLOUD_API_KEY,
    webhookSecret: process.env.ARCANGEL_CLOUD_WEBHOOK_SECRET,
    emailsEndpoint: process.env.ARCANGEL_CLOUD_EMAILS_ENDPOINT,
    paymentsEndpoint: process.env.ARCANGEL_CLOUD_PAYMENTS_ENDPOINT,
    searchEndpoint: process.env.ARCANGEL_CLOUD_SEARCH_ENDPOINT,
    oauthAuthorizeEndpoint: process.env.ARCANGEL_CLOUD_OAUTH_AUTHORIZE_ENDPOINT,
    oauthTokenEndpoint: process.env.ARCANGEL_CLOUD_OAUTH_TOKEN_ENDPOINT,
    oauthCallbackUrl: process.env.ARCANGEL_CLOUD_OAUTH_CALLBACK_URL,
    oauthDisabled:
      process.env.ARCANGEL_CLOUD_OAUTH_DISABLED === "true" ? true : undefined,
    oauthJwksUri: process.env.ARCANGEL_CLOUD_OAUTH_JWKS_ENDPOINT,
    oauthAudience: process.env.ARCANGEL_CLOUD_OAUTH_AUDIENCE,
    ...cloud,
  }
        ...
        ? {
            dynamodbOptions: {
              ...
              initialized: process.env.DYNAMO_DB_SESSIONS_CREATE_TABLE
                ? false
                : true,
              table: process.env.DYNAMO_DB_SESSIONS_TABLE ?? "arcangel-sessions",
...
function normalizeAdminConfig(
   ...
   */
  return {
    backendUrl: process.env.ARCANGEL_BACKEND_URL || DEFAULT_ADMIN_URL,
    path: "/app",
