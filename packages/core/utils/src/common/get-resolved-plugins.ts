import { ConfigModule, PluginDetails } from "@arcangel/types"
import fs from "fs/promises"
import path from "path"
import { isString } from "./is-string"
import { readDir } from "./read-dir-recursive"

const ARCANGEL_APP_SOURCE_PATH = "src"
const ARCANGEL_PLUGIN_SOURCE_PATH = ".arcangel/server/src"
const ARCANGEL_PLUGIN_OPTIONS_FILE_PATH =
  ".arcangel/server/arcangel-plugin-options.json"
export const ARCANGEL_PROJECT_NAME = "project-plugin"
...
/**
 * Reads the "arcangel-plugin-options.json" file from the plugin root
...
async function resolvePluginOptions(
  ...
  try {
    const contents = await fs.readFile(
      path.join(pluginRootDir, ARCANGEL_PLUGIN_OPTIONS_FILE_PATH),
      "utf-8"
    )
...
async function resolvePlugin(
  ...
  const resolve = path.join(resolvedPath, ARCANGEL_PLUGIN_SOURCE_PATH)
  ...
  return {
    ...
    modules: modules.map((mod) => {
      return {
        resolve: `${pluginPath}/${ARCANGEL_PLUGIN_SOURCE_PATH}/modules/${mod.name}`,
        options: pluginOptions,
      }
    }),
  }
...
export async function getResolvedPlugins(
  ...
  if (isArcangelProject) {
    const extensionDirectory = path.join(rootDirectory, ARCANGEL_APP_SOURCE_PATH)
    resolved.push({
      resolve: extensionDirectory,
      name: ARCANGEL_PROJECT_NAME,
      id: createPluginId(ARCANGEL_PROJECT_NAME),
      admin: {
        type: "local",
