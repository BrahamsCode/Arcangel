...
describe("getResolvedPlugins | relative paths", () => {
  test("resolve configured plugins", async () => {
    await fs.createJson("node_modules/@arcangel/draft-order/package.json", {
      name: "@arcangel/draft-order",
      version: "1.0.0",
    })
    ...
    expect(plugins).toEqual(
      expect.arrayContaining([
        {
          id: "@arcangel/draft-order",
          modules: [],
          name: "@arcangel/draft-order",
          options: {},
          resolve: path.join(
            fs.basePath,
            "node_modules/@arcangel/draft-order/.arcangel/server/src"
          ),
          version: "1.0.0",
        },
        {
          resolve: path.join(fs.basePath, "./plugins/dummy/.arcangel/server/src"),
          admin: undefined,
          name: "my-dummy-plugin",
          id: "my-dummy-plugin",
  ...
  test("scan plugin modules", async () => {
    ...
    await fs.create(
      "plugins/dummy/.arcangel/server/src/modules/blog/index.js",
      ``
    )
        ...
        modules: [
          {
            ...
            resolve: "./plugins/dummy/.arcangel/server/src/modules/blog",
          },
        ],
      },
      {
  ...
  test("resolve admin source from arcangel-plugin-options file", async () => {
    await fs.createJson("node_modules/@arcangel/draft-order/package.json", {
      name: "@arcangel/draft-order",
      version: "1.0.0",
    })
    ...
    await fs.createJson(
      "plugins/dummy/.arcangel/server/arcangel-plugin-options.json",
      {
...
describe("getResolvedPlugins | package reference", () => {
    ...
    expect(plugins).toEqual(expect.arrayContaining([
      {
        resolve: path.join(
          fs.basePath,
          "node_modules/@plugins/dummy/.arcangel/server/src"
        ),
        admin: undefined,
        name: "my-dummy-plugin",
        id: "my-dummy-plugin",
  ...
  test("scan plugin modules", async () => {
    ...
    await fs.create(
      "node_modules/@plugins/dummy/.arcangel/server/src/modules/blog/index.js",
      ``
    )
