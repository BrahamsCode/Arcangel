 ...
 */
export const LOCALE_STORAGE_KEY = "arcangel_locale"
...
/**
 * The main HTTP client for the Arcangel JS SDK.
 */
export class Client {
  ...
  private DEFAULT_JWT_STORAGE_KEY = "arcangel_auth_token"
  private token = ""
  ...
  setLocale(locale: string) {
    if (!window) {
      this.logger.warn(
        "setLocale is not available in the server environment. Please set the locale directly through the 'x-arcangel-locale' header."
      )
      return
    }
  ...
  protected initClient(): ClientFetch {
    ...
    this.logger.debug(
      "Initiating Arcangel client with default headers:\n",
    ...
    return async (input: FetchInput, init?: FetchArgs) => {
      ...
      if (this.locale) {
        headers.set("x-arcangel-locale", this.locale)
      }

      const customHeaders = {
        "x-arcangel-locale": this.locale,
