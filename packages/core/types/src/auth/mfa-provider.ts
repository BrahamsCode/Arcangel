 ...
 * import { Logger, ModulesSdkTypes } from "@arcangel/framework/types"
 *
 ...
 *   protected authMfaFactorService_: ModulesSdkTypes.IArcangelInternalService<any>
...
export interface AuthMfaProvider extends IAuthMfaProvider {
   ...
   * the provider.
   * 
   * To create the factor in a pending state within Arcangel, use the injected `authMfaFactorService` and
   ...
   * in Arcangel once the setup is verified in `verifySetup`.
   *
   ...
   * to `"enabled"` in Arcangel once the setup is verified.
   *
 ...
 *
 *   protected logger_: Logger
 *   protected authMfaRecoveryCodeService_: ModulesSdkTypes.IArcangelInternalService<any>
 *
 *   constructor ({
 *     logger,
