 ...
 * @example
 * ```ts
 * // In @arcangel/translation module:
...
export interface AdminOptions {
  ...
  /**
   * The URL of your Arcangel application. Defaults to the browser origin. This is useful to set when running the admin on a separate domain.
   *
   * @example
   * ```js title="arcangel-config.ts"
   * module.exports = defineConfig({
  ...
  /**
   * The URL of your Arcangel storefront application. This will help generate links from the admin
   ...
   * module.exports = defineConfig({
   *   admin: {
   *     storefrontUrl: process.env.ARCANGEL_STOREFRONT_URL ||
   *       "http://localhost:9000"
   *   },
   *   // ...
   * })
 ...
 * @interface
 *
 * Arcangel Cloud configurations.
 */
export type ArcangelCloudOptions = {
  /**
   * The environment handle of the Arcangel Cloud environment.
   */
  environmentHandle?: string
  /**
   * The sandbox handle of the Arcangel Cloud sandbox.
   */
  sandboxHandle?: string
  /**
   * The API key used to access Arcangel Cloud services.
   */
  apiKey?: string
  /**
  ...
  /**
   * The endpoint of the Arcangel Cloud payment service.
   */
  paymentsEndpoint?: string
  /**
   * The endpoint of the Arcangel Cloud email service.
   */
  emailsEndpoint?: string
  /**
   * The endpoint of the Arcangel Cloud search service.
   */
  searchEndpoint?: string
  /**
   * The authorization endpoint of the Arcangel Cloud OAuth service.
   */
  oauthAuthorizeEndpoint?: string
  /**
   * The token endpoint of the Arcangel Cloud OAuth token service.
   */
  oauthTokenEndpoint?: string
  /**
   * The callback URL for the Arcangel Cloud OAuth service. If not provided, it will be set to `${AdminOptions.backendUrl}/auth/user/cloud/callback`.
   */
  oauthCallbackUrl?: string
  /**
   * Whether the Arcangel Cloud OAuth service is disabled.
   */
  oauthDisabled?: boolean
  /**
   * The JWKS URI for the Arcangel Cloud OAuth service. This is used to verify the JWT tokens issued by the Arcangel Cloud OAuth service.
   * If not provided, an error is thrown when trying to use the Arcangel Cloud OAuth service.
   *
   * @since 2.16.0
   */
  oauthJwksUri?: string
  /**
   * The expected `aud` claim of id_tokens issued by the Arcangel Cloud OAuth service.
   * If not provided, an error is thrown when trying to use the Arcangel Cloud OAuth service.
   *
   * @since 2.16.0
   */
 ...
 * @interface
 *
 * Essential configurations related to the Arcangel application, such as database and CORS configurations.
 */
export type ProjectConfigOptions = {
   ...
   *
   * ```bash
   * DATABASE_URL=postgres://postgres@localhost/arcangel-store
   * ```
   *
   * Then, use the value in `arcangel-config.ts`:
   *
   * ```js title="arcangel-config.ts"
   * module.exports = defineConfig({
   *   projectConfig: {
  ...
  /**
   * This configuration specifies the connection URL to Redis to store the Arcangel server's session.
   *
   * :::note
   *
  ...
  /**
   * This configuration defines a prefix on all keys stored in Redis for the Arcangel server's session. The default value is `sess:`.
   *
   ...
   * module.exports = defineConfig({
   *   projectConfig: {
   *     redisPrefix: process.env.REDIS_URL || "arcangel:",
   *     // ...
   *   },
   *   // ...
   * })
  ...
  /**
   * This configuration defines options to pass ioredis for the Redis connection used to store the Arcangel server's session. Refer to [ioredis’s RedisOptions documentation](https://redis.github.io/ioredis/index.html#RedisOptions)
  ...
  /**
   * This configuration defines additional options to pass to [express-session](https://www.npmjs.com/package/express-session), which is used to store the Arcangel server's session.
   *
   * @example
   * ```js title="arcangel-config.ts"
   * module.exports = defineConfig({
   ...
   *
   * ![Diagram showcasing how the server and worker work together](https://res.cloudinary.com/dza7lstvk/image/upload/fl_lossy/f_auto/r_16/ar_16:9,c_pad/v1/Arcangel%20Book/arcangel-worker_klkbch.jpg?_a=BATFJtAA0)
   *
   * Arcangel has three runtime modes:
   *
  ...
  http: {
    ...
    cookieSecret?: string
    /**
     * The Arcangel application's API Routes are protected by Cross-Origin Resource Sharing (CORS). So, only allowed URLs or URLs matching a specified pattern can send requests to the backend’s API Routes.
     *
     ...
     * 2. Or a regular expression pattern that can match more than one origin. For example, `.example.com`. The regex pattern that Arcangel tests for is `^([\/~@;%#'])(.*?)\1([gimsuy]*)$`.
     *
     * @example
     ...
     * ```
     *
     * Then, set the configuration in `arcangel-config.ts`:
     *
     * ```js title="arcangel-config.ts"
     * module.exports = defineConfig({
     *   projectConfig: {
     ...
     * ```
     *
     * If you’re adding the value directly within `arcangel-config.ts`, make sure to add an extra escaping `/` for every backslash in the pattern. For example:
     *
     * ```js title="arcangel-config.ts"
     * module.exports = defineConfig({
     *   projectConfig: {
     ...
     * Learn more in the [API Reference](https://docs.arcangel.com/api/store/http-compression).
     *
     * @example
     * ```js title="arcangel-config.ts"
     * module.exports = defineConfig({
  ...
  /**
   * This property holds configurations for running in Arcangel Cloud.
   * It gets automatically populated in the cloud, and is not needed outside of it.
   */
  cloud?: ArcangelCloudOptions
}

/**
 * @interface
 *
 * The configurations for your Arcangel application are set in `arcangel-config.ts` located in the root of your Arcangel project. The configurations include configurations for database, modules, and more.
 *
 * :::note
 *
 * Some Arcangel configurations are set through environment variables, which you can find in [this documentation](https://docs.arcangel.com/learn/fundamentals/environment-variables#predefined-arcangel-environment-variables).
 *
 * :::
 *
 * `arcangel-config.ts` exports the value returned by the `defineConfig` utility function imported from `@arcangel/framework/utils`.
 *
 * `defineConfig` accepts as a parameter an object with the following properties:
 *
 * - {@link ConfigModule.projectConfig | projectConfig} (required): An object that holds general configurations related to the Arcangel application, such as database or CORS configurations.
 * - {@link ConfigModule.plugins | plugins}: An array of strings or objects that hold the configurations of the plugins installed in the Arcangel application.
 * - {@link ConfigModule.admin | admin}: An object that holds admin-related configurations.
 * - {@link ConfigModule.modules | modules}: An object that configures the Arcangel application's modules.
 ...
 *
 * It's highly recommended to store the values of configurations in environment variables, then reference them within `arcangel-config.ts`.
 *
 * During development, you can set your environment variables in the `.env` file at the root of your Arcangel application project. In production,
...
export type ConfigModule = {
  /**
   * This property holds essential configurations related to the Arcangel application, such as database, CORS configurations and Logger.
   */
  projectConfig: ProjectConfigOptions

  /**
   * This property holds configurations for the Arcangel Admin dashboard.
   *
   * @example
   * ```ts title="arcangel-config.ts"
   * module.exports = defineConfig({
  ...
  /**
   * On your Arcangel server, you can use [Plugins](https://docs.arcangel.com/learn/fundamentals/plugins) to add re-usable Arcangel customizations. Plugins
   * can include modules, workflows, API Routes, and other customizations. Plugins are available starting from [Arcangel v2.3.0](https://github.com/arcangel/arcangel/releases/tag/v2.3.0).
   *
   * Aside from installing the plugin with NPM, you need to pass the plugin you installed into the `plugins` array defined in `arcangel-config.ts`.
   *
   ...
   *     - `options`: An object that includes options to be passed to the modules within the plugin. Learn more in [this documentation](https://docs.arcangel.com/learn/fundamentals/modules/options).
   *
   * Learn how to create a plugin in [this documentation](https://docs.arcangel.com/learn/fundamentals/plugins/create).
   *
   * @example
   * ```ts title="arcangel-config.ts"
   * module.exports = {
   *   plugins: [
   *     `arcangel-my-plugin-1`,
   *     {
   *       resolve: `arcangel-my-plugin`,
   *       options: {
  ...
  plugins: (
    | {
         ...
         * Learn more in [this documentation](https://docs.arcangel.com/learn/fundamentals/modules/options).
         */
  ...
  /**
   * This property holds all custom modules installed in your Arcangel application.
   *
   * :::note
   *
   * Arcangel's Commerce Modules are configured by default, so only
  ...
  /**
   * Some features in the Arcangel application are guarded by a feature flag. This ensures constant shipping of new features while maintaining the engine’s stability.
   *
   ...
   * variables or through this configuration property exported in `arcangel-config.ts`.
   *
   ...
   *
   * You can find available feature flags and their key name [here](https://github.com/arcangel/arcangel/tree/develop/packages/arcangel/src/feature-flags).
   *
   * @example
   * ```ts title="arcangel-config.ts"
   * module.exports = defineConfig({
 ...
 * @interface
 *
 * Resolved details of a plugin loaded by the Arcangel application.
 */
