import { Context, LoadedModule, ArcangelContainer } from "@arcangel/types"
import {
...
export class LocalWorkflow {
  protected container_: ArcangelContainer
  ...
  protected arcangelContext?: Context

  get container(): ArcangelContainer {
    return this.container_
  }

  set container(modulesLoaded: LoadedModule[] | ArcangelContainer) {
    this.resolveContainer(modulesLoaded)
  }

  constructor(
    workflowId: string,
    modulesLoaded?: LoadedModule[] | ArcangelContainer
  ) {
  ...
  private resolveContainer(modulesLoaded?: LoadedModule[] | ArcangelContainer) {
    let container

    if (!Array.isArray(modulesLoaded) && modulesLoaded) {
      if (!("cradle" in modulesLoaded)) {
        container = createArcangelContainer(modulesLoaded)
      } else {
        container = createArcangelContainer({}, modulesLoaded) // copy container
      }
    ...
    this.container_ = this.contextualizedArcangelModules(container)
  }

  private contextualizedArcangelModules(container) {
    if (!container) {
      return createArcangelContainer()
    }
    ...
    container.resolve = function (keyName, opts) {
      ...
      if (resolved?.constructor?.__type !== ArcangelModuleType) {
        return resolved
      }
      ...
      return new Proxy(resolved, {
        get: function (target, prop) {
          ...
          return (...args) => {
            const ctxIndex = ArcangelContext.getIndex(target, prop as string)

            const hasContext = args[ctxIndex!]?.__type === ArcangelContextType
            if (!hasContext && isDefined(ctxIndex)) {
              const context = this_.arcangelContext
              if (context?.__type === ArcangelContextType) {
  ...
  async cancel(
    ...
    if (this.arcangelContext) {
      this.arcangelContext.eventGroupId =
        transaction.getFlow().metadata!.eventGroupId
      transaction.getFlow().metadata!.cancelingFromParentStep ??=
        this.arcangelContext.cancelingFromParentStep
    }
  ...
  private onLoad(transaction: DistributedTransactionType) {
    if (this.arcangelContext) {
      ...
      this.arcangelContext.eventGroupId = metadata.eventGroupId
      this.arcangelContext.parentStepIdempotencyKey =
        metadata.parentStepIdempotencyKey
      this.arcangelContext.preventReleaseEvents = metadata?.preventReleaseEvents
      this.arcangelContext.cancelingFromParentStep =
