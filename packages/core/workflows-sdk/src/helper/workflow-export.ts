...
import { ArcangelWorkflow } from "../arcangel-workflow"
...
function getCachedLoadedModules(): LoadedModule[] {
  if (!cachedLoadedModules) {
    cachedLoadedModules = ArcangelModule.getLoadedModules().map(
...
> &
  ...
  const originalExecution = async (
    ...
    if (!executionContainer) {
      const container_ = flow.container as ArcangelContainer
  ...
  const newRun = async ({
    ...
    const context = {
      ...outerContext,
      __type: ArcangelContextType as Context["__type"],
    }
...
export const exportWorkflow = <TData = unknown, TResult = unknown>(
  ...
  ArcangelWorkflow.registerWorkflow(workflowId, exportedWorkflow)
...
function attachOnFinishReleaseEvents(
  ...
  const wrappedOnFinish = async (args: {
    ...
    const logger =
      (flow.container as ArcangelContainer).resolve<Logger>(
    ...
    const eventBusService = (
      flow.container as ArcangelContainer
