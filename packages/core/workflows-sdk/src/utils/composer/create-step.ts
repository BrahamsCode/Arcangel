...
>): StepFunctionResult<TInvokeResultOutput> {
  return function (this: CreateWorkflowComposerContext) {
    ...
    refRet.config = (
      ...
      if (global[OrchestrationUtils.SymbolArcangelWorkflowComposerCondition]) {
        const flagSteps =
    ...
    if (global[OrchestrationUtils.SymbolArcangelWorkflowComposerCondition]) {
      global[
        OrchestrationUtils.SymbolArcangelWorkflowComposerCondition
      ].steps.push(refRet)
    }
...
>(
  ...
  const returnFn = function (
    ...
    const context = global[
      OrchestrationUtils.SymbolArcangelWorkflowComposerContext
