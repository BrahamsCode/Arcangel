import { isDefined, isObject, OrchestrationUtils } from "@arcangel/utils"
 ...
 *
 * Learn more about why you can't use if conditions and `when-then` in [this documentation](https://docs.arcangel.com/learn/fundamentals/workflows/conditions).
 *
...
export function when(...args) {
  ...
  global[OrchestrationUtils.SymbolArcangelWorkflowComposerCondition] = {
    input,
    condition,
    steps: [],
  }
  ...
  return {
    then: (fn) => {
      ...
      const applyCondition =
        global[OrchestrationUtils.SymbolArcangelWorkflowComposerCondition].steps
      ...
      if (
        ...
        if (!isDefined(name)) {
          name = "when-then-" + ulid()
          const context =
            global[OrchestrationUtils.SymbolArcangelWorkflowComposerContext]
      ...
      delete global[OrchestrationUtils.SymbolArcangelWorkflowComposerCondition]
