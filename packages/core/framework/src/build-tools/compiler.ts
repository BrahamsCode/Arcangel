import type { AdminOptions, ConfigModule, Logger } from "@arcangel/types"
import { FileSystem, getConfigFile, getResolvedPlugins } from "@arcangel/utils"
...
/**
 * The compiler exposes the opinionated APIs for compiling Arcangel
 ...
 *
 * - buildAppBackend: Compile the Arcangel application backend source code to the
 *   ".arcangel/server" directory. The admin source and integration-tests are
 *   skipped.
 *
...
export class Compiler {
  ...
  constructor(projectRoot: string, logger: Logger) {
    ...
    this.#adminOnlyDistFolder = path.join(this.#projectRoot, ".arcangel/admin")
    this.#pluginsDistFolder = path.join(this.#projectRoot, ".arcangel/server")
   ...
   * or uses the ".arcangel/server" folder
   */
  #computeDist(tsConfig: { options: { outDir?: string } }): string {
    const distFolder = tsConfig.options.outDir ?? ".arcangel/server"
  ...
  /**
   * Loads the arcangel config file and prints the error to
   * the console (in case of any errors). Otherwise, the
   * file path and the parsed config is returned
   */
  async #loadArcangelConfig() {
    const { configModule, configFilePath, error } =
      await getConfigFile<ConfigModule>(this.#projectRoot, "arcangel-config")
    if (error) {
      this.#logger.error(`Failed to load arcangel-config.(js|ts) file`)
  ...
  /**
   * Builds the frontend source code of a Arcangel application
   * using the "@arcangel/admin-bundler" package.
   */
  async buildAppFrontend(
    ...
    /**
     * Step 1: Load the arcangel config file to read
     * admin options
     */
    const configFile = await this.#loadArcangelConfig()
    if (!configFile) {
      return false
    }
    ...
    if (configFile.configModule.admin.disable && !adminOnly) {
      this.#logger.info(
        "Skipping admin build, since its disabled inside the arcangel-config file"
      )
      return true
    }
    ...
    if (!configFile.configModule.admin.disable && adminOnly) {
      this.#logger.warn(
        `You are building using the flag --admin-only but the admin is enabled in your arcangel-config, If you intend to host the dashboard separately you should disable the admin in your arcangel config`
      )
    }
  ...
  async developPluginBackend(
    ...
    await fs.createJson("arcangel-plugin-options.json", {
