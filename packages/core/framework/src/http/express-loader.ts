...
import { dynamicImport } from "@arcangel/utils"
...
const isHealthCheck = (req: ArcangelRequest) => req.path === "/health"
...
export async function expressLoader({
  ...
  if (httpCompressionOptions.enabled) {
    ...
    // `req.scope` off the Arcangel request, which only exists at runtime, hence
   ...
   */
  function shouldSkipHttpLog(req: ArcangelRequest, res: ArcangelResponse) {
    return (
      isTest ||
      isHealthCheck(req) ||
