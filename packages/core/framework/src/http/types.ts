...
import { ArcangelContainer } from "../container"
...
type SyncRouteHandler = (req: ArcangelRequest, res: ArcangelResponse) => void
...
export type MiddlewareFunction =
  | ArcangelRequestHandler
  | ((...args: any[]) => any)

export type ArcangelErrorHandlerFunction = (
  error: any,
  req: ArcangelRequest,
  res: ArcangelResponse,
...
export type MiddlewaresConfig = {
  errorHandler?: false | ArcangelErrorHandlerFunction
...
export interface ArcangelRequest<
  Body = unknown,
...
> extends Request<{ [key: string]: string }, any, Body> {
  ...
  /**
   * An object containing the select, relation, skip, take and order to be used with arcangel internal services
   */
  listConfig: FindConfig<unknown>
  /**
   * An object containing the select, relation to be used with arcangel internal services
   */
   ...
   */
  remoteQueryConfig: ArcangelRequest["queryConfig"]
...
export interface AuthenticatedArcangelRequest<
  Body = unknown,
  QueryFields = Record<string, unknown>
> extends ArcangelRequest<Body, QueryFields> {
...
export interface ArcangelStoreRequest<
  Body = unknown,
  QueryFields = Record<string, unknown>
> extends ArcangelRequest<Body, QueryFields> {
  auth_context?: AuthContext
...
export type ArcangelResponse<Body = unknown> = Response<Body>

export type ArcangelNextFunction = NextFunction

export type ArcangelRequestHandler<Body = unknown, Res = unknown> = (
  req: ArcangelRequest<Body>,
  res: ArcangelResponse<Res>,
  next: ArcangelNextFunction
) => Promise<void> | void
