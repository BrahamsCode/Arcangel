...
describe("applyLocale", () => {
  let mockRequest: Partial<ArcangelRequest>
  let mockResponse: ArcangelResponse
  ...
  beforeEach(() => {
    ...
    mockResponse = {} as ArcangelResponse
  ...
  it("should set locale from query parameter", async () => {
    ...
    await applyLocale(mockRequest as ArcangelRequest, mockResponse, nextFunction)

    expect(mockRequest.locale).toBe("en-US")
    expect(nextFunction).toHaveBeenCalledTimes(1)
  })

  it("should set locale from x-arcangel-locale header when query param is not present", async () => {
    mockRequest.query = {}
    ;(mockRequest.get as jest.Mock).mockImplementation((header: string) => {
      if (header === "x-arcangel-locale") {
        return "fr-FR"
      }
      return undefined
    })
  ...
  it("should prioritize query parameter over x-arcangel-locale header", async () => {
  ...
  it("should handle various locale formats", async () => {
    ...
    for (const locale of locales) {
      ...
      await applyLocale(
        mockRequest as ArcangelRequest,
        mockResponse,
        nextFunction
      )
