...
describe("getHttpResponseFromError", () => {
  test.each([
    [ArcangelError.Types.INVALID_DATA, 400],
    [ArcangelError.Types.NOT_ALLOWED, 400],
    [ArcangelError.Types.UNAUTHORIZED, 401],
    [ArcangelError.Types.FORBIDDEN, 403],
    [ArcangelError.Types.NOT_FOUND, 404],
    [ArcangelError.Types.CONFLICT, 409],
    [ArcangelError.Types.PAYMENT_AUTHORIZATION_ERROR, 422],
    [ArcangelError.Types.DUPLICATE_ERROR, 422],
    [ArcangelError.Types.DB_ERROR, 500],
    [ArcangelError.Types.UNEXPECTED_STATE, 500],
    [ArcangelError.Types.INVALID_ARGUMENT, 500],
  ])("should map %s to %i", (type, statusCode) => {
    expect(getHttpResponseFromError(new ArcangelError(type, "Failed"))).toEqual(
...
describe("errorHandler", () => {
  ...
  it("should respond with the mapped status code and body", () => {
    ...
    errorHandler()(
      new ArcangelError(ArcangelError.Types.INVALID_DATA, "Promotion not found"),
  ...
  it("should respond with the zod issues when present", () => {
    const res = createResponse()
    const err: any = new ArcangelError(
