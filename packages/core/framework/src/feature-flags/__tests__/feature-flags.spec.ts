import { FileSystem } from "@arcangel/utils"
...
const buildFeatureFlag = (
  ...
  return `
    export default {
      ...
      key: "${snakeCaseKey}",
      env_key: "ARCANGEL_FF_${snakeCaseKey.toUpperCase()}",
...
describe("feature flags", () => {
  ...
  it("should load the flag from env", async () => {
    process.env.ARCANGEL_FF_FLAG_1 = "false"
  ...
  it("should load mix of flags", async () => {
    ...
    process.env.ARCANGEL_FF_FLAG_3 = "true"
    await filesystem.create(
      "feature-flags/flag-1.js",
      buildFeatureFlag("flag-1", false)
    )
