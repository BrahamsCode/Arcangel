...
describe("configLoader", () => {
  ...
  it("should load the config properly", async () => {
    ...
    await configLoader(entryDirectory, "arcangel-config")
    ...
    await configLoader(entryDirectory, "arcangel-config-2")
    ...
    process.env.ARCANGEL_WORKER_MODE = "worker"
  ...
  it("should fail on config file errors", async () => {
    ...
    await expect(
      configLoader(entryDirectory, "arcangel-config-throwing")
    ...
    expect(logged).toContain("Uncaught error in arcangel-config-throwing.js")
