...
jest.mock("@arcangel/utils", () => {
  const actual = jest.requireActual("@arcangel/utils")
  return {
    ...actual,
    ModulesSdkUtils: {
...
jest.mock("../../config", () => ({
  configManager: {
    config: {
      projectConfig: {
        databaseUrl: "postgres://user:pass@127.0.0.1:1/arcangel",
...
import { ModulesSdkUtils } from "@arcangel/utils"
...
describe("pgConnectionLoader", () => {
  ...
  beforeEach(() => {
    ...
    process.env.__ARCANGEL_DB_CONNECTION_MAX_RETRIES = "2"
    process.env.__ARCANGEL_DB_CONNECTION_RETRY_DELAY = "0"
  })
  ...
  it("rethrows the real connection error (not the masked KnexTimeoutError) once retries are exhausted", async () => {
    ...
    expect(error.message).toContain(
      "https://docs.arcangel.com/resources/troubleshooting/database-errors"
    )
  ...
  it("fails fast on a fatal connection error without retrying", async () => {
    const realError = Object.assign(
      new Error(`database "arcangel" does not exist`),
      { code: "3D000" }
    )
    ...
    expect(error.message).toContain(
      `Failed to connect to the database: database "arcangel" does not exist`
    )
    expect(error.message).toContain(
      "https://docs.arcangel.com/resources/troubleshooting/database-errors"
    )
