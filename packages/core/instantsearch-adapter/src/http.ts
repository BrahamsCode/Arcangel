...
export function resolveSearchUrl(
  ...
  if (!baseUrl) {
    throw new Error(
      "@arcangel/instantsearch-adapter: `baseUrl` is required when `path` is relative"
    )
  }
...
function extractResults(payload: unknown): SearchResult[] {
  ...
  throw new Error(
    "@arcangel/instantsearch-adapter: unexpected search response shape. Expected `{ results: SearchResult[] }`, an array of results, or a single SearchResult."
  )
}
...
export function unwrapSearchResults(
  ...
  if (results.length !== expected) {
    throw new Error(
      `@arcangel/instantsearch-adapter: expected ${expected} search result(s), received ${results.length}`
    )
  }
  return results
}
...
export function assertAdapterTransport(
  ...
  if (!options.path) {
    throw new Error("@arcangel/instantsearch-adapter: provide `path`")
  }

  if (!options.sdk && !options.baseUrl && !isAbsoluteUrl(options.path)) {
    throw new Error(
      "@arcangel/instantsearch-adapter: provide `sdk`, `requester`, or `baseUrl`"
    )
  }
}
