/**
 * Wire types for Arcangel Search HTTP requests and InstantSearch's search client.
 * Kept local so the adapter has no runtime dependency on `@arcangel/types`.
 */
...
/**
 * Minimal JS SDK surface the adapter needs. Pass a `Arcangel` instance, or any
 * object exposing `client.fetch`.
 */
export interface ArcangelSdkLike {
  client: {
    fetch: (
      path: string,
      init?: {
...
export type InstantSearchAdapterOptions = {
  /**
   * Arcangel JS SDK instance. Preferred: it sends the publishable API key and
  ...
  /**
   * Arcangel backend origin, used with native `fetch` when `sdk` and `requester`
   ...
   * `search-arcangel` providers support stats facets.
   */
