# `@arcangel/instantsearch-adapter`

An [InstantSearch](https://www.algolia.com/doc/guides/building-search-ui/what-is-instantsearch/js/) search client for Arcangel Search. It translates InstantSearch requests into Arcangel `SearchQuery` payloads and maps `SearchResult` responses back into the shape InstantSearch widgets expect.
...
```bash
yarn add @arcangel/instantsearch-adapter instantsearch.js
```

Use a Arcangel JS SDK instance so store requests include the publishable API key:

```ts
import { createInstantSearchAdapter } from "@arcangel/instantsearch-adapter"
...
`indexName` is the Search index `name` (the `entity` sent to Arcangel). For `sortBy`, encode the sort in the index name:
...
The adapter POSTs Arcangel `SearchQuery` objects, not Algolia-format params.
...
- **Range widgets** (`rangeSlider`, `rangeInput`, `numericMenu`, `ratingMenu`) need stats facets. List those fields in `numericAttributes`, and mark them `facetable({ types: ["stats"] })` (or include `"stats"` alongside `"range"`) on the index — stats are opt-in. Both the Postgres and `search-arcangel` providers support stats.
- **Hierarchical menu / breadcrumb** work if the index stores Algolia-style `categories.lvl0`, `categories.lvl1`, … fields.
- **Highlighting** is provider-dependent (available on `search-arcangel`, not on Postgres).
