...
async function fetchRegionById(regionId: string, container: ArcangelContainer) {
...
async function fetchDefaultStore(container: ArcangelContainer) {
  const query = container.resolve(ContainerRegistrationKeys.QUERY)
...
export const findOneOrAnyRegionStep = createStep(
  ...
  async (data: FindOneOrAnyRegionStepInput, { container }) => {
    ...
    if (!store) {
      throw new ArcangelError(ArcangelError.Types.NOT_FOUND, "Store not found")
    }
