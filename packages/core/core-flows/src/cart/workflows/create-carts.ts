...
export const prepareCartToCreateStep = createStep(
  ...
  async (data: PrepareCartToCreateStepInput) => {
    if (!data.region) {
      throw new ArcangelError(ArcangelError.Types.NOT_FOUND, "No regions found")
    }
 ...
 * [Create Cart Store API Route](https://docs.arcangel.com/api/store/carts/create-cart).
 *
 * This workflow has a hook that allows you to perform custom actions on the created cart. You can see an example in [this guide](https://docs.arcangel.com/resources/commerce-modules/cart/extend#step-4-consume-cartcreated-workflow-hook).
 *
 ...
 *
 * ```ts
 * import { createCartWorkflow } from "@arcangel/arcangel/core-flows";
