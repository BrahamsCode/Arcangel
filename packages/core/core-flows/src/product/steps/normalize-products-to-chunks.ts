...
import type { HttpTypes, IFileModuleService } from "@arcangel/framework/types"
import {
  CSVNormalizer,
  ArcangelError,
  Modules,
...
import { ArcangelErrorTypes } from "@arcangel/framework/utils"
...
export const normalizeCsvToChunksStep = createStep(
  ...
  async (fileKey: NormalizeProductCsvV1StepInput, { container }) => {
    // eslint-disable-next-line @arcangel/step-must-return-step-response
    return new Promise<
      StepResponse<{
        chunks: Chunk[]
    ...
    >(async (resolve, reject) => {
      try {
        ...
        if (error instanceof CsvError) {
          return reject(
            new ArcangelError(ArcangelErrorTypes.INVALID_DATA, error.message)
          )
        }
        reject(error)
      }
