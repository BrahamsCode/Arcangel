...
import { asFunction, createContainer } from "@arcangel/framework/awilix"
import { Modules } from "@arcangel/framework/utils"
import { createWorkflow, WorkflowResponse } from "@arcangel/workflows-sdk"
...
): { container: ArcangelContainer; calls: Record<string, any[]> } => {
  const calls: Record<string, any[]> = {
    createProductOptions: [],
    deleteProductOptions: [],
  }
  const container = createContainer() as unknown as ArcangelContainer
  container.register(
    Modules.PRODUCT,
    asFunction(() => ({
