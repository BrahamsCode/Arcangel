...
export type CreateProductVariantsWorkflowInput = {
  ...
  product_variants: (ProductTypes.CreateProductVariantDTO & {
    ...
    inventory_items?: {
       ...
       * Arcangel checks the availability of the quantity multiplied by the value set for `required_quantity`.
       * When the customer orders the quantity, Arcangel reserves the ordered quantity multiplied by the value
...
/**
 * This workflow creates one or more product variants. It's used by the [Create Product Variant Admin API Route](https://docs.arcangel.com/api/admin/products/create-variant).
 *
