 ...
 * :::note
 *
 * By default, Arcangel uses a single store. This is useful
 * if you're building a multi-tenant application or a marketplace where each tenant has its own store.
 * If you delete the only store in your application, the Arcangel application will re-create it on application start-up.
 *
 * :::
 *
