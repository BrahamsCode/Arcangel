...
import type { OrderPreviewDTO } from "@arcangel/framework/types"
import {
  ChangeActionType,
  ArcangelError,
  OrderChangeStatus,
...
export const acceptOrderTransferValidationStep = createStep(
  ...
  async function ({
    ...
    if (!token.length || token !== transferCustomerAction?.details!.token) {
      throw new ArcangelError(ArcangelError.Types.NOT_ALLOWED, "Invalid token.")
    }
  }
)
 ...
 * [Accept Order Transfer Store API Route](https://docs.arcangel.com/api/store/orders/accept-transfer).
 *
