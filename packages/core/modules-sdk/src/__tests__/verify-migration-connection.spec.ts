...
import { verifyMigrationConnection } from "../arcangel-app"

describe("verifyMigrationConnection", () => {
  afterEach(() => {
    delete process.env.ARCANGEL_DB_MIGRATION_CONNECTION_TIMEOUT
  })
  ...
  it("throws an actionable error with a troubleshooting link when the connection hangs", async () => {
    process.env.ARCANGEL_DB_MIGRATION_CONNECTION_TIMEOUT = "50"
    ...
    expect(error).toBeInstanceOf(ArcangelError)
    expect(error.type).toEqual(ArcangelError.Types.DB_ERROR)
