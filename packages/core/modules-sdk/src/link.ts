...
export class Link {
  // To not lose the context chain, we need to set the type to ArcangelModuleType
  static __type = ArcangelModuleType
  ...
  constructor(modulesLoaded?: LoadedModule[]) {
    if (!modulesLoaded?.length) {
      modulesLoaded = ArcangelModule.getLoadedModules().map(
