import { ArcangelContainer, ModuleProvider } from "@arcangel/types"
import {
  dynamicImport,
  isFileSkipped,
  isString,
...
import { asFunction, Lifetime } from "@arcangel/deps/awilix"
...
export async function loadModuleProvider(
  ...
  try {
    ...
    if (isString(provider.resolve)) {
       ...
       * project directory rather than from wherever `@arcangel/modules-sdk`
       * was hoisted to.
       */
