...
import { asFunction, asValue } from "@arcangel/deps/awilix"
...
export async function loadInternalModule(args: {
  ...
  const localContainer = createArcangelContainer()
  ...
  if (loadingProviders) {
    ...
    for (const moduleProviderService of moduleProviderServices) {
      ...
      modProvider_.__type = ArcangelModuleProviderType
      ...
      container.register({
        [registrationKey]: asFunction(() => {
          ;(moduleProviderService as any).__type = ArcangelModuleType
    ...
    container.register({
      [keyName]: asFunction((cradle) => {
        ;(moduleService as any).__type = ArcangelModuleType
