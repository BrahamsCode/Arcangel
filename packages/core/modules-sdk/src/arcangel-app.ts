...
function getMigrationConnectionTimeout(): number {
  return process.env.ARCANGEL_DB_MIGRATION_CONNECTION_TIMEOUT
    ? parseInt(process.env.ARCANGEL_DB_MIGRATION_CONNECTION_TIMEOUT)
    : 10000
}
...
export async function verifyMigrationConnection(
  ...
  try {
    ...
    if (error instanceof ArcangelError) {
      throw error
    }
...
export type ArcangelModuleConfig = {
  [key: string | Modules]:
    | string
    | boolean
...
export async function loadModules(args: {
  modulesConfig: ArcangelModuleConfig
  ...
  for (const moduleName of Object.keys(modulesConfig)) {
    ...
    if (moduleName === Modules.SEARCH) {
      ...
      for (const definition of configured) {
        ArcangelModule.setSearchIndex(definition)
      }

      const registered = ArcangelModule.getSearchIndexes()
  ...
  const loaded = (await ArcangelModule.bootstrapAll(modulesToLoad, {
    migrationOnly,
    schemaOnly,
    loaderOnly,
    workerMode,
...
function getLoadedSchema(): string {
  return ArcangelModule.getAllJoinerConfigs()
    .map((joinerConfig) => joinerConfig?.schema ?? "")
    .join("\n")
}

export type ArcangelAppOutput = {
...
export type ArcangelAppOptions = {
  workerMode?: "shared" | "worker" | "server"
  sharedContainer?: ArcangelContainer
  sharedResourcesConfig?: SharedResources
...
async function ArcangelApp_({
  ...
  workerMode = "shared",
  cwd = process.cwd(),
}: ArcangelAppOptions = {}): Promise<ArcangelAppOutput> {
  const sharedContainer_ = createArcangelContainer({}, sharedContainer)
  ...
  const onApplicationShutdown = async () => {
    await promiseAll([
      ArcangelModule.onApplicationShutdown(),
      sharedContainer_.dispose(),
    ])
  }

  const onApplicationPrepareShutdown = async () => {
    await promiseAll([ArcangelModule.onApplicationPrepareShutdown()])
  }

  const onApplicationStart = async () => {
    await ArcangelModule.onApplicationStart()
  }

  const modules: ArcangelModuleConfig =
    modulesConfig ??
    (
      await dynamicImport(
        arcangelConfigPath ?? cwd + (modulesConfigFileName ?? "/modules-config")
      )
    ).default
  ...
  linkModules.push(...ArcangelModule.getCustomLinks())

  const allLoadedJoinerConfigs = ArcangelModule.getAllJoinerConfigs()
  ...
  const { remoteLink, getMigrationPlanner } = await initializeLinks({
    ...
    linkModules,
    injectedDependencies,
    moduleExports: isArcangelModule(linkModule) ? linkModule : undefined,
    migrationOnly,
    schemaOnly,
  })
  ...
  const modulesLoaded = ArcangelModule.getLoadedModules().map(
  ...
  const applyMigration = async ({
    ...
    const moduleResolutions = Array.from(new Set(modulesNames)).map(
      (moduleName) => {
        return {
          moduleName,
          resolution: ArcangelModule.getModuleResolutions(moduleName),
        }
      }
    )
    ...
    if (missingModules.length) {
      const error = new ArcangelError(
        ArcangelError.Types.UNKNOWN_MODULES,
        `Cannot ${action} migrations for unknown module(s) ${missingModules.join(
          ","
        )}`,
        ArcangelError.Codes.UNKNOWN_MODULES
      )
    ...
    const run = async (
      ...
      await lockKnex.transaction(async (trx) => {
        ...
        if (action === "revert") {
          await ArcangelModule.migrateDown(migrationOptions, migrationNames)
        } else if (action === "run") {
          const ranMigrationsResult = await ArcangelModule.migrateUp(
            migrationOptions
          )
          ...
          await ArcangelModule.migrateGenerate(migrationOptions)
        }
      })
    }
...
export async function ArcangelApp(
  options: ArcangelAppOptions = {}
): Promise<ArcangelAppOutput> {
  return await ArcangelApp_(options)
}

export async function ArcangelAppMigrateUp(
  options: ArcangelAppOptions & { allOrNothing?: boolean } = {}
): Promise<void> {
  const migrationOnly = true

  const { runMigrations } = await ArcangelApp_({
    ...options,
    migrationOnly,
  })
...
export async function ArcangelAppMigrateDown(
  moduleNames: string[],
  options: ArcangelAppOptions = {}
): Promise<void> {
  const migrationOnly = true

  const { revertMigrations } = await ArcangelApp_({
    ...options,
    migrationOnly,
  })

  await revertMigrations(moduleNames).finally(ArcangelModule.clearInstances)
}

export async function ArcangelAppMigrateGenerate(
  moduleNames: string[],
  options: ArcangelAppOptions = {}
): Promise<void> {
  const migrationOnly = true

  const { generateMigrations } = await ArcangelApp_({
    ...options,
    migrationOnly,
  })

  await generateMigrations(moduleNames).finally(ArcangelModule.clearInstances)
}

export async function ArcangelAppGetLinksExecutionPlanner(
  options: ArcangelAppOptions = {}
): Promise<ILinkMigrationsPlanner> {
  const migrationOnly = true

  const { linkMigrationExecutionPlanner } = await ArcangelApp_({
    ...options,
    migrationOnly,
  })
