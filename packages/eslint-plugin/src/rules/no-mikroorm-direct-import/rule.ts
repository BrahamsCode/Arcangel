...
/**
 * The `@mikro-orm/*` subpackages that `@arcangel/framework` re-exports as of
 * v2.11.0 (`@arcangel/framework/mikro-orm/<sub>`). Imports of these can be
...
/** `awilix` is re-exported wholesale as `@arcangel/framework/awilix`. */
const AWILIX_SOURCE = "awilix"
const AWILIX_REEXPORT = "@arcangel/framework/awilix"
...
function rewriteSource(value: string): string | null {
  ...
  if (value.startsWith(MIKRO_ORM_PREFIX)) {
    ...
    if (MIKRO_ORM_REEXPORTED_SUBPATHS.has(rest)) {
      return `@arcangel/framework/mikro-orm/${rest}`
    }
  }
...
export const rule = createRule<[], MessageIds>({
  ...
  meta: {
    ...
    docs: {
      description:
        "Don't import MikroORM or awilix directly. As of Arcangel v2.11, they're bundled into `@arcangel/framework`; use the `@arcangel/framework/mikro-orm/*` and `@arcangel/framework/awilix` re-exports.",
    },
    messages: {
      useFrameworkReexport:
        "Import from `{{ replacement }}` instead of `{{ source }}`. MikroORM and awilix are re-exported by `@arcangel/framework` as of Arcangel v2.11.",
      noDirectMikroOrmImport:
        "Don't import directly from `{{ source }}`. As of Arcangel v2.11, MikroORM is bundled into `@arcangel/framework` — use a `@arcangel/framework/mikro-orm/*` re-export.",
    },
    fixable: "code",
    schema: [],
  },
