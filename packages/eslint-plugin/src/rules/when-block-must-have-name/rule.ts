...
export const rule = createRule<[], MessageIds>({
  ...
  meta: {
    ...
    docs: {
      description:
        "Require a `name` on `when(...)` calls whose `.then()` callback returns a value that isn't a step invocation. Arcangel core assigns a random name at runtime in this case and logs a warning — pass `when(name, values, condition)` instead.",
    },
    messages: {
      whenBlockMissingName:
        "This `when` block's `.then()` returns a value that will be wrapped in a synthetic step. Without an explicit name, Arcangel assigns a random one and logs a warning in production. Pass a name as the first argument: `when('<unique-name>', values, condition)`.",
    },
    schema: [],
  },
  defaultOptions: [],
