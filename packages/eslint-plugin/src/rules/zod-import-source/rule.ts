...
const BARE_ZOD = "zod"
const ARCANGEL_ZOD = "@arcangel/framework/zod"
...
function check(
  ...
  context.report({
    ...
    fix(fixer) {
      ...
      return fixer.replaceText(source, `${quote}${ARCANGEL_ZOD}${quote}`)
    },
  })
}
...
export const rule = createRule<[], MessageIds>({
  ...
  meta: {
    ...
    docs: {
      description:
        "Zod should be imported from `@arcangel/framework/zod` (Arcangel v2.13+), not the bare `zod` package.",
    },
    messages: {
      zodImportSource:
        "Import Zod from `@arcangel/framework/zod` instead of `zod`.",
    },
    fixable: "code",
    schema: [],
  },
