...
ruleTester.run("zod-import-source", rule, {
  valid: [
    // Canonical Arcangel import.
    {
      code: `import { z } from "@arcangel/framework/zod"`,
    },
    // Unrelated import.
    {
    ...
    // Re-export from the Arcangel source.
    {
      code: `export { z } from "@arcangel/framework/zod"`,
    },
  ...
  invalid: [
    // Named import.
    {
      ...
      output: `import { z } from "@arcangel/framework/zod"`,
      errors: [{ messageId: "zodImportSource" }],
    },
    // Default-ish namespace import.
    {
      code: `import * as z from "zod"`,
      output: `import * as z from "@arcangel/framework/zod"`,
      errors: [{ messageId: "zodImportSource" }],
    },
    ...
    {
      code: `import "zod"`,
      output: `import "@arcangel/framework/zod"`,
      errors: [{ messageId: "zodImportSource" }],
    },
    // Named re-export.
    {
      code: `export { z } from "zod"`,
      output: `export { z } from "@arcangel/framework/zod"`,
      errors: [{ messageId: "zodImportSource" }],
    },
    // Wildcard re-export.
    {
      code: `export * from "zod"`,
      output: `export * from "@arcangel/framework/zod"`,
      errors: [{ messageId: "zodImportSource" }],
    },
  ],
})
