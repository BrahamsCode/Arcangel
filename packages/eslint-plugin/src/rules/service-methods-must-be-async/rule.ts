...
export const rule = createRule<[], MessageIds>({
  ...
  meta: {
    ...
    docs: {
      description:
        "All public methods on a service class that extends `ArcangelService(...)` must be `async` (or return a `Promise`). Arcangel always awaits service method calls.",
    },
    fixable: "code",
    messages: {
      methodMustBeAsync:
  ...
  defaultOptions: [],
  create(context) {
    const bindings = createArcangelServiceBindings()

    function checkClass(
      node: TSESTree.ClassDeclaration | TSESTree.ClassExpression
    ) {
      // Only check classes that extend `ArcangelService(...)`. Avoids false
      // positives on unrelated classes (including `*Service`-named helpers)
      // that don't expose awaitable service methods.
      if (!isArcangelServiceSuper(node.superClass, bindings)) {
        return
      }
    ...
    return {
      ImportDeclaration(node) {
        trackArcangelServiceImports(node, bindings)
      },
