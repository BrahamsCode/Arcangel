...
ruleTester.run("prefer-link-over-remote-link", rule, {
  valid: [
    ...
    {
      code: `import type { Link } from "@arcangel/framework/types"`,
    },
  ...
  invalid: [
    ...
    // Type import: RemoteLink from @arcangel/framework/types.
    {
      code: `import type { RemoteLink } from "@arcangel/framework/types"`,
      errors: [{ messageId: "remoteLinkTypeImport" }],
      output: `import type { Link } from "@arcangel/framework/types"`,
    },
    // Type import: IRemoteLink from @arcangel/types.
    {
      code: `import { IRemoteLink } from "@arcangel/types"`,
      errors: [{ messageId: "remoteLinkTypeImport" }],
      output: `import { ILink } from "@arcangel/types"`,
    },
    // Mixed: rename only the deprecated specifier.
    {
      code: `import type { Link, RemoteLink } from "@arcangel/framework/types"`,
      errors: [{ messageId: "remoteLinkTypeImport" }],
      output: `import type { Link, Link } from "@arcangel/framework/types"`,
    },
    // Aliased import — rename only the `imported` part, keep alias.
    {
      code: `import type { RemoteLink as MyLink } from "@arcangel/framework/types"`,
      errors: [{ messageId: "remoteLinkTypeImport" }],
      output: `import type { Link as MyLink } from "@arcangel/framework/types"`,
    },
    // Multiple violations in one file.
    {
      code: `
        import { RemoteLink } from "@arcangel/framework/types"
      ...
      output: `
        import { Link } from "@arcangel/framework/types"
