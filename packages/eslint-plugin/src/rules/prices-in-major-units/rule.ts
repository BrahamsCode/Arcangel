...
export const rule = createRule<[], MessageIds>({
  ...
  meta: {
    ...
    docs: {
      description:
        "Prices in Arcangel are stored in major units (decimals). Flag values that look like they're in the smallest currency unit (cents).",
    },
    messages: {
      multipliedByHundred:
        "`{{key}}` looks like it's multiplied to cents. Arcangel expects prices in major units (decimals).",
      centsIdentifier:
        "`{{key}}` is set from a `{{name}}` value that looks like it's in the smallest currency unit. Arcangel expects prices in major units (decimals).",
      largeIntegerLikelyCents:
        "`{{key}}: {{value}}` looks like it's in the smallest currency unit. Arcangel expects prices in major units (decimals).",
    },
    schema: [],
  },
  defaultOptions: [],
