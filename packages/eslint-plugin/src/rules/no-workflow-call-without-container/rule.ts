 ...
 * looks like a Arcangel workflow — either the canonical `ReturnWorkflow<...>`
...
export const rule = createRule<[], MessageIds>({
  ...
  meta: {
    ...
    docs: {
      description:
        "Workflow invocations must pass the Arcangel container — `workflow(container).run({ input })`. Calling the workflow with no arguments or accessing `.run` directly bypasses container resolution.",
