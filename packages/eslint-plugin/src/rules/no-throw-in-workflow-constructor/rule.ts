  ...
  meta: {
    ...
    messages: {
      throwInWorkflowConstructor:
        "Do not throw inside a workflow constructor or a `when().then()` callback — both run at definition time, not execution time, so a throw here bypasses compensation for any steps that already ran. Extract the check into a validation step (as `ArcangelError`) so a failure triggers compensation.",
    },
    schema: [],
  },
  defaultOptions: [],
