...
export const rule = createRule<[], MessageIds>({
  ...
  meta: {
    ...
    docs: {
      description:
        "Arcangel middlewares must be defined in `src/api/middlewares.{ts,js}` with a default export of `defineMiddlewares(...)`. Other filenames or locations are ignored by the framework.",
    },
    messages: {
      singularFilename:
