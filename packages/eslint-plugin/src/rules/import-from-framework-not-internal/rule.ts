 ...
 * `@arcangel/framework/*` subpaths. Importing them still works today but is
 * discouraged — the framework entry points are the supported surface.
 */
const OLD_PACKAGE_REWRITES: Record<string, string> = {
  "@arcangel/utils": "@arcangel/framework/utils",
  "@arcangel/types": "@arcangel/framework/types",
  "@arcangel/workflows-sdk": "@arcangel/framework/workflows-sdk",
  "@arcangel/modules-sdk": "@arcangel/framework/modules-sdk",
  "@arcangel/orchestration": "@arcangel/framework/orchestration",
}

/**
 * Deep import into a `@arcangel/*` package's compiled build output, e.g.
 * `@arcangel/arcangel/dist/...` or `@arcangel/framework/dist/...`. Requires a
 * path segment that is exactly `dist` so package names like
 * `@arcangel/some-dist-thing` aren't matched.
 */
const ARCANGEL_DIST_IMPORT = /^@arcangel\/[^/]+\/(?:[^/]+\/)*dist(?:\/|$)/
...
function check(
  ...
  if (ARCANGEL_DIST_IMPORT.test(value)) {
    context.report({
      node: source,
...
export const rule = createRule<[], MessageIds>({
  ...
  meta: {
    ...
    docs: {
      description:
        "Use the public `@arcangel/framework/*` entry points; don't import from a package's internal `dist/` build output or from deprecated standalone packages.",
    },
    messages: {
      useFrameworkEntrypoint:
        "Import from `{{ replacement }}` instead of the deprecated `{{ source }}` package.",
      noInternalImport:
        "Don't import from internal build output `{{ source }}`. Use a public `@arcangel/framework/*` entry point instead.",
    },
    fixable: "code",
    schema: [],
  },
