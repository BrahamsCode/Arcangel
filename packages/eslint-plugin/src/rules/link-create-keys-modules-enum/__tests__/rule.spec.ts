...
ruleTester.run("link-create-keys-modules-enum", rule, {
  ...
  invalid: [
    // Identifier key for a known module → autofix + add import.
    {
      ...
      output: `import { Modules } from "@arcangel/framework/utils"
...
await link.create({ [Modules.PRODUCT]: { product_id: "1" } })`,
    ...
    // Existing framework/utils import → append `Modules` to specifier list.
    {
      ...
      errors: [{ messageId: "preferEnumKey" }],
      output: `
        import { defineLink, Modules } from "@arcangel/framework/utils"
        await link.create({ [Modules.PRODUCT]: { product_id: "1" } })
      `,
    },
    ...
    {
      code: `
        import { Modules as M } from "@arcangel/framework/utils"
    ...
    // Workflow step: createRemoteLinkStep.
    {
      code: `
        import { Modules } from "@arcangel/framework/utils"
        import { createRemoteLinkStep } from "@arcangel/arcangel/core-flows"
    ...
    // Workflow step alias.
    {
      code: `
        import { Modules } from "@arcangel/framework/utils"
        import { dismissRemoteLinkStep as drls } from "@arcangel/arcangel/core-flows"
        drls({ order: { id: "1" } })
      `,
      errors: [{ messageId: "preferEnumKey" }],
      output: `
