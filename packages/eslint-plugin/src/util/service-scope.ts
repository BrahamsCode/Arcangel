...
export const ARCANGEL_SERVICE = "ArcangelService"
export const CONTEXT_TYPE = "Context"

export type ArcangelServiceBindings = {
  arcangelService: Set<string>
}

export function createArcangelServiceBindings(): ArcangelServiceBindings {
  return {
    arcangelService: new Set(),
  }
}

/**
 * Records local names bound to `ArcangelService` from
 * `@arcangel/framework/utils` (honors `import { ArcangelService as MS }`).
 *
 * Call from an `ImportDeclaration` visitor.
 */
export function trackArcangelServiceImports(
  node: TSESTree.ImportDeclaration,
  bindings: ArcangelServiceBindings
): void {
  ...
  for (const specifier of node.specifiers) {
    ...
    if (specifier.imported.name !== ARCANGEL_SERVICE) {
      continue
    }
    bindings.arcangelService.add(specifier.local.name)
  }
}

/**
 * True when `superClass` is a `CallExpression` whose callee resolves to a
 * tracked `ArcangelService` binding (e.g. `class X extends ArcangelService({...})`).
 */
export function isArcangelServiceSuper(
  ...
  return bindings.arcangelService.has(callee.name)
}

/**
 * Generic import tracker for named specifiers from
 * `@arcangel/framework/utils`. Given a map of canonical imported names to
 ...
 * this gate are scoped to Arcangel service classes where `Context` is the
 * convention.
 */
