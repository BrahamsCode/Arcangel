...
describe("@arcangel/eslint-plugin", () => {
  it("exposes a meta block with the plugin name", () => {
    expect(plugin.meta).toEqual({ name: "@arcangel/eslint-plugin" })
  })
  ...
  it("modules preset registers the plugin and applies module rules", () => {
    ...
    expect((pluginBlock!.plugins as Record<string, unknown>)["@arcangel"]).toBe(
      plugin
    )
    ...
    expect(allRules).toHaveProperty("@arcangel/service-methods-must-be-async")
    expect(allRules).toHaveProperty("@arcangel/module-name-snake-case")
    expect(allRules).toHaveProperty(
      "@arcangel/data-model-table-name-snake-case"
    )
    ...
    const moduleDefBlock = modules.find(
      (block) =>
        ...
        (block.rules as Record<string, unknown>)[
          "@arcangel/module-name-snake-case"
        ]
    )
  ...
  it("recommended preset includes a global-ignores block, a global plugin registration, and a TS parser block", () => {
    ...
    // `@arcangel/*` rule references — it is intentionally not scoped to TS.
