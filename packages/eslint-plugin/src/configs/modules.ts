...
/**
 * Modules preset — the rules that govern a Arcangel **module** (or provider)
 ...
 * path (matching how modules live inside a Arcangel application), this preset
 * targets the layout of a standalone module/provider package — Arcangel's own
 ...
 *   classes extending `ArcangelService`, so a broad scope is safe and needs no
...
export function buildModules(plugin: unknown): Linter.Config[] {
  return [
    ...
    tsParserBlock(),
    {
      ...
      files: ["**/*.{ts,tsx}"],
      rules: {
        [ruleId("arcangel-context-on-context-param")]: "warn",
