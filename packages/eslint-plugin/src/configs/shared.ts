 ...
 * Arcangel source — linting them produces false positives (e.g. treating a test
 * `arcangel-config.js` fixture as a project config), so they're excluded here.
 */
 ...
 * processes can resolve `@arcangel/*` rule references — including `.js`/`.mjs`/
 ...
 * `Could not find plugin "@arcangel" in configuration`.
 */
