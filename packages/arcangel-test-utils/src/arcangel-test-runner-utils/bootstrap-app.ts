...
import { GracefulShutdownServer, promiseAll } from "@arcangel/framework/utils"
...
async function bootstrapApp({
  ...
  const loaders = require("@arcangel/arcangel/loaders/index").default
...
export async function startApp({
  ...
  let arcangelShutdown: () => Promise<void> = async () => void 0
  ...
  try {
    ...
    if (arcangelShutdown) {
      try {
        ...
        logger.error("Error cleaning up arcangel:", cleanupError)
      }
    }
    throw error
  }
