import { asValue } from "@arcangel/framework/awilix"
import { logger } from "@arcangel/framework/logger"
import { Migrator } from "@arcangel/framework/migrations"
import { ArcangelAppOutput } from "@arcangel/framework/modules-sdk"
...
import {
  ...
  startApp,
  syncLinks,
} from "./arcangel-test-runner-utils"
import { waitWorkflowExecutions } from "./arcangel-test-runner-utils/wait-workflow-executions"
import { ulid } from "ulid"

export interface ArcangelSuiteOptions {
  dbConnection: any // knex instance
  getContainer: () => ArcangelContainer
  api: any
  dbUtils: {
  ...
  getArcangelApp: () => ArcangelAppOutput
  utils: {
...
interface TestRunnerConfig {
  ...
  dbName?: string
  arcangelConfigFile?: string
  schema?: string
  debug?: boolean
  inApp?: boolean
  hooks?: {
    beforeServerStart?: (container: ArcangelContainer) => Promise<void>
  }
  cwd?: string
}

class ArcangelTestRunner {
  ...
  private globalContainer: ArcangelContainer | null = null
  ...
  constructor(config: TestRunnerConfig) {
    ...
    this.dbName =
      config.dbName ??
      `arcangel-${moduleName.toLowerCase()}-integration-${tempName}`
    this.dbTemplateName = `${this.dbName}-template`
    this.schema = config.schema ?? "public"
    this.cwd = config.cwd ?? config.arcangelConfigFile ?? process.cwd()
    this.modulesConfigPath = config.arcangelConfigFile ?? this.cwd
    this.env = config.env ?? {}
  ...
  private async setupApplication(): Promise<void> {
    const { container, ArcangelAppLoader } = await import("@arcangel/framework")
    const appLoader = new ArcangelAppLoader({
      arcangelConfigPath: this.modulesConfigPath,
      cwd: this.cwd,
    })
    ...
    const { loadSearchIndexes } = require("@arcangel/arcangel/loaders/search")
    ...
    try {
      ...
      await waitWorkflowExecutions(this.globalContainer as ArcangelContainer)
    } catch (error) {
  ...
  public getOptions(): ArcangelSuiteOptions {
    return {
      api: this.createApiProxy(),
      dbConnection: this.createDbConnectionProxy(),
      getArcangelApp: () => this.loadedApplication,
      getContainer: () => this.globalContainer as ArcangelContainer,
      dbConfig: {
        dbName: this.dbName,
        schema: this.schema,
      ...
      utils: {
        waitWorkflowExecutions: () =>
          waitWorkflowExecutions(this.globalContainer as ArcangelContainer),
      },
    }
  }
}

export function arcangelIntegrationTestRunner({
  moduleName,
  dbName,
  arcangelConfigFile,
  schema = "public",
  ...
  debug?: boolean
  inApp?: boolean
  testSuite: (options: ArcangelSuiteOptions) => void
  hooks?: TestRunnerConfig["hooks"]
  cwd?: string
}) {
  const runner = new ArcangelTestRunner({
    moduleName,
    dbName,
    arcangelConfigFile,
    schema,
  ...
  return describe("", () => {
    let testOptions: ArcangelSuiteOptions
