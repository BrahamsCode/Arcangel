...
interface ModuleTestRunnerConfig<TService = any> {
  ...
  hooks?: {
    ...
    afterModuleInit?: (arcangelApp: any, service: TService) => Promise<void>
  }
}
...
class ModuleTestRunner<TService = any> {
  ...
  private createArcangelAppProxy(): any {
    return new Proxy(
      {},
      {
        get: (target, prop) => {
          return this.arcangelApp?.[prop]
        },
      }
    )
  }
  ...
  public async beforeEach(): Promise<void> {
    try {
      ...
      this.arcangelApp = output.arcangelApp
      this.moduleService = output.arcangelApp.modules[this.moduleName]

      if (this.hooks?.afterModuleInit) {
        await this.hooks.afterModuleInit(this.arcangelApp, this.moduleService)
      }
    } catch (error) {
  ...
  public getOptions(): SuiteOptions<TService> {
    return {
      ...
      arcangelApp: this.createArcangelAppProxy(),
