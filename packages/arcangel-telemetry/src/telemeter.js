...
const ARCANGEL_TELEMETRY_VERBOSE = process.env.ARCANGEL_TELEMETRY_VERBOSE || false
...
class Telemeter {
  ...
  getArcangelVersion() {
    try {
      const packageJson = require.resolve(`@arcangel/arcangel/package.json`)
      const { version } = JSON.parse(fs.readFileSync(packageJson, `utf-8`))
      return version
    } catch (e) {
      if (isTruthy(ARCANGEL_TELEMETRY_VERBOSE)) {
        console.error("failed to get arcangel version", e)
      }
    }
    return `-0.0.0`
  }
  ...
  getCliVersion() {
    try {
      const jsonfile = join(
        require.resolve(`@arcangel/cli`).split(`${sep}dist`).shift(),
        "package.json"
      )
  ...
  enqueue_(type, data) {
    const event = {
      ...
      arcangel_version: this.getArcangelVersion(),
