...
const nextConfig = {
  ...
  async redirects() {
    return catchBadRedirects([
      ...
      {
        source: "/references/arcangel-config",
        destination: `${process.env.NEXT_PUBLIC_BASE_URL}/learn/configurations/arcangel-config`,
        permanent: true,
      },
      {
      ...
      {
        source: "/arcangel-cli/commands/start-cluster",
        destination: "/arcangel-cli/commands/start",
        permanent: true,
      },
      {
      ...
      {
        source: "/deployment/arcangel-application/railway",
  ...
  experimental: {
    optimizePackageImports: ["@arcangel/icons", "@arcangel/ui", "elkjs"],
  },
  rewrites: async () => {
    return {
      beforeFiles: [
