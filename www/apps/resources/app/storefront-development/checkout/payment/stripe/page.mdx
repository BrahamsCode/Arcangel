...
<Prerequisites items={[
  {
    text: "Stripe Module Provider installed and configured in your Arcangel application.",
...
export const highlights = [
  ...
  ["86", "complete", "Send a request to the Arcangel application\nto complete the cart and place the order."],
...
1. Create a `StripePayment` component that wraps the actual form with Stripe's `Elements` component.
    - In the `StripePayment` component, you obtain the client secret from the payment session's `data` field. This is set in the Arcangel application after you initialize the payment session using the [Initialize Payment Sessions API route](!api!/store/payment-collections/initialize-payment-session).
