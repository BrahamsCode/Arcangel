...
Then, to update the cart's address, use the [Update Cart API route](!api!/store/carts/update-a-cart).
...
To retrieve the logged-in customer's addresses, use the [List Customer Addresses API route](!api!/store/customers/list-customers-addresses). Then, once the customer selects an address, use the [Update Cart API route](!api!/store/carts/update-a-cart) to update the cart's addresses.

<Note title="Good to Know">

A customer's address and a cart's address are represented by different data models in the Arcangel application, as they're managed by the [Customer Module](../../../commerce-modules/customer/page.mdx) and the [Cart Module](../../../commerce-modules/cart/page.mdx), respectively. So, addresses that the customer used previously during checkout aren't automatically saved to their account. You need to save the customer's address using the [Create Customer Address API route](!api!/store/customers/create-address).
...
export const fetch2Highlights = [
  ...
  ["3", "retrieveCustomerAddresses", "Retrieve the customer's addresses from Arcangel."],
...
When the customer selects an address, you send a request to [Update Cart API route](!api!/store/carts/update-a-cart) passing the selected address as a shipping and billing address.
