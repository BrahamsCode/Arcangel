...
In this tutorial, you'll learn how to create a mobile app that connects to your Arcangel backend with React Native and Expo. You can then publish your app to the Apple App Store and Google Play Store.

When you install a Arcangel application, you get a fully-fledged commerce server and an admin dashboard to manage the commerce store's data. Arcangel's [architecture](!docs!/learn/introduction/architecture) is flexible and customizable, allowing you to build your own custom storefronts using any technology you prefer.
...
1. Set up a Arcangel application.
2. Create an app with React Native and Expo.
3. Connect the Expo app to the Arcangel backend.
4. Implement essential ecommerce features such as product listing, cart management, and checkout.

You can follow this guide whether you're new to Arcangel or an advanced Arcangel developer. However, this tutorial assumes you have a basic understanding of React Native and JavaScript.

![Screenshot of home screen of the Expo app](https://res.cloudinary.com/dza7lstvk/image/upload/v1762333811/Arcangel%20Resources/CleanShot_2025-11-05_at_11.09.27_lxl2tw.png)

<Card
  title="Full Code"
  text="Find the full code for this tutorial in this repository."
  href="https://github.com/arcangel/examples/tree/main/react-native-expo"
  icon={Github}
/>
...
Afterwards, the installation process will start, which will install the Arcangel application in a directory with your project's name. Once the installation finishes successfully, the Arcangel Admin dashboard will open in your browser at `http://localhost:9000/app` with a form to create a new user. Enter the user's credentials and submit the form.
...
In a directory separate from your Arcangel application, run the following command to create a new Expo project:
...
```bash npm2yarn
npm install @arcangel/js-sdk @arcangel/types @react-native-async-storage/async-storage @react-native-picker/picker @react-navigation/drawer
```

You install the following packages:

- `@arcangel/js-sdk`: Arcangel's [JS SDK](../../../js-sdk/page.mdx) to interact with the Arcangel backend.
- `@arcangel/types`: TypeScript types for Arcangel, which are useful when working with Arcangel's JS SDK.
...
## Step 5: Initialize Arcangel JS SDK

In this step, you'll set up Arcangel's [JS SDK](../../../js-sdk/page.mdx) in your Expo app. You'll use the SDK to interact with the Arcangel backend.
...
const ARCANGEL_BACKEND_URL =
  Constants.expoConfig?.extra?.EXPO_PUBLIC_ARCANGEL_URL ||
  process.env.EXPO_PUBLIC_ARCANGEL_URL ||
  "http://localhost:9000"

const ARCANGEL_PUBLISHABLE_API_KEY = 
  Constants.expoConfig?.extra?.EXPO_PUBLIC_ARCANGEL_PUBLISHABLE_API_KEY ||
  process.env.EXPO_PUBLIC_ARCANGEL_PUBLISHABLE_API_KEY ||
  ""
...
export const sdk = new Arcangel({
  ...
  publishableKey: ARCANGEL_PUBLISHABLE_API_KEY,
})
```

You configure the SDK with the:

- Arcangel backend URL, which is read from the environment variable `EXPO_PUBLIC_ARCANGEL_URL`. If the variable is not set, it defaults to `http://localhost:9000`.
- [Publishable API key](../../publishable-api-keys/page.mdx), which is required to retrieve products in the associated sales channel. It is read from the environment variable `EXPO_PUBLIC_ARCANGEL_PUBLISHABLE_API_KEY`.
...
Next, you'll set the environment variables in your Expo project. Before you do that, you need to retrieve the Arcangel backend URL and the Publishable API key.

The Arcangel backend URL is the URL where your Arcangel server is running. If you're running the Arcangel server locally, it should be the IP address of your machine at the port `9000`. For example, `http://192.168.1.100:9000`.
...
Next, to get the Publishable API key, start the Arcangel application by running the following command in its directory:
...
1. Open the Arcangel Admin dashboard at `http://localhost:9000/app` and log in.
...
Once you have the Arcangel backend URL and the Publishable API key, create the file `.env` in the root directory of your Expo project with the following content:

```shell title=".env"
EXPO_PUBLIC_ARCANGEL_URL=http://192.168.1.100:9000
EXPO_PUBLIC_ARCANGEL_PUBLISHABLE_API_KEY=your_publishable_api_key
```

Make sure to replace the values with your actual Arcangel backend URL and Publishable API key.

### Optional: Update CORS in Arcangel

If you plan to test the Expo app in a web browser, you'll need to update the CORS settings in the Arcangel backend to allow requests from the Expo development server.

In your Arcangel application's directory, add the Expo development server URL to the `STORE_CORS` and `AUTH_CORS` environment variables in the `.env` file:
...
Make sure to restart the Arcangel server after making changes to the `.env` file.
...
export const regionContextHighlights = [
  ["7", "regions", "Available regions from the Arcangel backend."],
...
1. `regions`: An array of available regions fetched from the Arcangel backend.
...
Next, you'll implement the logic to load the regions from the Arcangel backend, and manage the selected region state.

Replace the `// TODO load and select regions` comment with the following:

export const regionLoadingHighlights = [
  ["1", "loadRegions", "Load regions from the Arcangel backend."],
...
You define the `loadRegions` function that fetches the regions from the Arcangel backend using the JS SDK. It also loads any previously selected region and country from `AsyncStorage`, or defaults to the first region and its first country.
...
Next, you'll implement the logic to load the cart from the Arcangel backend. Replace the `// TODO load cart` comment with the following:
...
The `loadCart` function creates or retrieves the saved cart from the Arcangel backend using the JS SDK. This function runs when the component mounts using the `useEffect` hook.
...
In this step, you'll create the home screen of the app. It will display a hero image with a list of products retrieved from the Arcangel backend.
 ...
 * Note: Arcangel stores prices in major units (e.g., dollars, euros)
...
export const homeScreenHighlights = [
  ["22", "fetchProducts", "Fetch products from the Arcangel backend based on the selected region."],
...
The home screen component fetches products from the Arcangel backend based on the selected region. It manages loading, refreshing, and error states.
...
Before you run your app, start the Arcangel backend by running the following command in the Arcangel project directory:
...
When the app opens, you'll see a home screen displaying a hero image and a list of products fetched from the Arcangel application.
...
![Home screen showing the hero image and list of products](https://res.cloudinary.com/dza7lstvk/image/upload/v1762344360/Arcangel%20Resources/CleanShot_2025-11-05_at_13.54.13_2x_efopgu.png)
...
![Drawer navigator showing the region selector](https://res.cloudinary.com/dza7lstvk/image/upload/v1762344361/Arcangel%20Resources/CleanShot_2025-11-05_at_13.54.38_2x_r0tb6i.png)
...
The `fetchProduct` function retrieves product details from the Arcangel backend using the product ID. The region ID is passed as a query parameter to get prices specific to the selected region.
...
To test out the product detail screen, run your Arcangel application and Expo app.
...
![Product Detail Screen](https://res.cloudinary.com/dza7lstvk/image/upload/v1762417976/Arcangel%20Resources/CleanShot_2025-11-06_at_10.29.04_2x_girdxx.png)
...
To test out the cart screen, run your Arcangel application and Expo app.
...
![Cart screen with items in it](https://res.cloudinary.com/dza7lstvk/image/upload/v1762358342/Arcangel%20Resources/CleanShot_2025-11-05_at_17.58.18_2x_w5gmly.png)
...
The function only handles Arcangel's built-in manual payment provider `pp_system_default`. You can expand it later to include more providers that you integrate.
...
You add a `fetchShippingOptions` function that retrieves shipping options, and a `fetchPaymentProviders` function that retrieves payment providers from the Arcangel backend.
...
To test the checkout flow, run your Arcangel application and Expo app.
...
![Delivery step with address filled](https://res.cloudinary.com/dza7lstvk/image/upload/v1762360359/Arcangel%20Resources/CleanShot_2025-11-05_at_18.31.49_2x_jl3q4g.png)

Then, in the Shipping step, select a shipping method and click "Continue" to proceed to the Payment step.

![Shipping step with shipping method selected](https://res.cloudinary.com/dza7lstvk/image/upload/v1762360359/Arcangel%20Resources/CleanShot_2025-11-05_at_18.31.49_2x_jl3q4g.png)
...
![Payment step with payment provider selected](https://res.cloudinary.com/dza7lstvk/image/upload/v1762360464/Arcangel%20Resources/CleanShot_2025-11-05_at_18.33.59_2x_eled5o.png)
...
The `fetchOrder` function retrieves order details from the Arcangel backend using the order ID from the URL parameters.
...
To test the order confirmation screen, run your Arcangel application and Expo app.
...
![Order confirmation screen](https://res.cloudinary.com/dza7lstvk/image/upload/v1762360994/Arcangel%20Resources/CleanShot_2025-11-05_at_18.42.42_2x_prvkba.png)

---

## Next Steps

You now have an ecommerce app built with React Native and Arcangel, complete with product listing, cart management, checkout flow, and order confirmation.
