...
To retrieve a product category by its ID, send a request to the [Get a Product Category API route](!api!/store/product-categories/get-a-product-category):
...
In this example, you send a request to the [Get a Product Category API route](!api!/store/product-categories/get-a-product-category) with the category's ID.
...
To retrieve a product by its handle, send a request to the [List Product Categories API route](!api!/store/product-categories/list-product-categories) passing it the `handle` query parameter:
