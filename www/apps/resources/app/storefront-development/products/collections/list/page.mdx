...
To retrieve the list of product collections, send a request to the [List Product Collections API route](!api!/store/collections/list-collections):
...
In this example, you retrieve the list of product collections by sending a request to the [List Product Collections API route](!api!/store/collections/list-collections).

The response has a `collections` field, which is an array of [product collections](!api!/store/collections/list-collections).

---

## Paginate Product Collections

To paginate product collections, pass the following query parameters to the [List Product Collections API route](!api!/store/collections/list-collections):
...
export const paginateHighlights = [
  ...
  ["27", "count", "The total number of product collections in the Arcangel application."],
...
In this example, you paginate the product collections by passing the `limit` and `offset` query parameters to the [List Product Collections API route](!api!/store/collections/list-collections).
...
The [List Product Collections API route](!api!/store/collections/list-collections) accepts query parameters to filter the collections by title, handle, external ID, and more.

Refer to the [API reference](!api!/store/collections/list-collections) for the full list of accepted query parameters.
...
In these examples, only the collections matching the specified criteria are returned.
