...
Arcangel provides localization features through the [Translation Module](../../commerce-modules/translation/page.mdx). The Translation Module allows you to manage locales and translations for various resources in your store, such as products and categories.

Admin users can [specify supported locales](!user-guide!/settings/store#manage-store-locales), and [add translations for different resources](!user-guide!/settings/translations) from the Arcangel Admin. Then, you can allow customers to choose their locale and show them translated content in your storefront.
...
The response has a `locales` field, which is an array of [locales](!api!/store/locales/schema).
...
By default, Arcangel serves the original content of a resource. You can override the locale for a request to get the translated content if available.

### Set Locale in JS SDK

The JS SDK has a `setLocale` method that you can use to set the locale for subsequent requests.

The JS SDK will automatically include the `x-arcangel-locale` header in API requests. Also, if your application supports `localStorage`, the method sets the locale in the `arcangel_locale` key of the `localStorage`.
...
If you're sending requests to the Store API without using the JS SDK, you can set the locale using either the `locale` query parameter or the `x-arcangel-locale` header.
...
When a customer selects a locale in your storefront, set a cookie (for example, `_arcangel_locale`) with the selected locale value:
...
export const setLocale = async (locale: string) => {
  ...
  cookies_.set("_arcangel_locale", locale, {
    maxAge: 60 * 60 * 24 * 7,
  })
}
```

Then, create a utility that returns the locale from the cookie as a `x-arcangel-locale` header:
...
export const getLocaleHeader = async () => {
  try {
    const cookies_ = await cookies()
    const locale = cookies_.get("_arcangel_locale")?.value

    return { "x-arcangel-locale": locale }
  } catch {
    return {}
  }
}
...
Finally, use the utility to set the `x-arcangel-locale` header in your Store API requests:
