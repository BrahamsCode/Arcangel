...
To create a cart, send a request to the [Create Cart API route](!api!/store/carts/create-cart).
...
In this example, you create a cart by sending a request to the [Create Cart API route](!api!/store/carts/create-cart).

The response of the Create Cart API route has a `cart` field, which is a [cart object](!api!/store/carts/schema).

Refer to the [Create Cart API reference](!api!/store/carts/create-cart) for details on other available request parameters.

### Cart's Sales Channel Scope

As mentioned before, you must always pass the publishable API key in the header of the request (which is done automatically by the JS SDK, as explained in the [Publishable API Keys](../../publishable-api-keys/page.mdx) guide). So, Arcangel will associate the cart with the sales channel(s) of the publishable API key.

This is necessary, as the cart's sales channel scopes the products you retrieve for the storefront, the available inventory, and the sales channel of the order created from the cart. If you want to associate the cart with a different sales channel, or if the publishable API key is associated with multiple sales channels and you want to specify which one to use, you can pass the `sales_channel_id` parameter to the [Create Cart API route](!api!/store/carts/create-cart) with the desired sales channel's ID.

<Note>

Arcangel doesn't reject a product variant added to the cart when its product isn't available in the cart's sales channel. Sales channel availability filters the products you retrieve, but Arcangel doesn't enforce it when adding items to the cart. Refer to the [Enforce Sales Channel Availability in Carts](../../../commerce-modules/cart/sales-channel-availability/page.mdx) guide to learn how to add that validation.
...
You can set the cart's locale by passing the `locale` request body parameter to the [Create Cart API route](!api!/store/carts/create-cart). For example:
