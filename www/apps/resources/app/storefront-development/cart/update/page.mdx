...
The [Update Cart API route](!api!/store/carts/update-a-cart) accepts a `region_id` request body parameter, whose value is the new region to associate with the cart.
...
This API route is only available after [Arcangel v2.0.5](https://github.com/arcangel/arcangel/releases/tag/v2.0.5).
...
You can set the cart's locale by passing the `locale` request body parameter to the [Update Cart API route](!api!/store/carts/create-cart). For example:
