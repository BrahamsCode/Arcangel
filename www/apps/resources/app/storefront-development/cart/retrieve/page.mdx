...
Assuming you stored the cart's ID in the `localStorage` as explained in the [Create Cart guide](../create/page.mdx), you can retrieve a cart by sending a request to the [Get a Cart API route](!api!/store/carts/get-a-cart).
...
In this example, you retrieve a cart by sending a request to the [Get a Cart API route](!api!/store/carts/get-a-cart).

The response of the [Get a Cart API route](!api!/store/carts/get-a-cart) has a `cart` field, which is a [cart object](!api!/store/carts/schema).
