...
If you don't have email verification enabled in the Arcangel application, follow the [Register Customer in Storefront](../register/page.mdx) guide instead.

</Note>

<Note>

This guide has been updated since Arcangel [v2.16.0](https://github.com/arcangel/arcangel/releases/tag/v2.16.0), which made changes to the overall verification flow.
...
<Prerequisites
  items={[
    ...
    {
      text: "Email verification email sending implemented in the Arcangel application",
...
By default, Arcangel doesn't require email verification for customers. When [email verification is enabled](../../../commerce-modules/auth/email-verification/page.mdx#enable-email-verification), the registration flow is as follows:

```mermaid
sequenceDiagram
    actor C as Customer
    participant S as Storefront
    participant M as Arcangel
    C->>S: Submit form
    S->>M: Register
...
2. Send a `POST` request to the `/auth/customer/emailpass/register` [Get Registration Token](!api!/store/auth/retrieve-registration-jwt-token) API route to obtain a registration JWT token.
3. Request returns a registration JWT token if the registration is successful.
4. Redirect customer to login.
5. Send a `POST` request to the `/auth/customer/emailpass` [Login Customer](!api!/store/auth/authenticate-customer) API route, passing the registration JWT token in the header.
6. Request returns an object indicating that verification is required.
7. Send a `POST` request to the `/auth/verification/request` [Request Verification](!api!/store/auth/request-verification) API route to send the verification email to the customer.
8. Customer receives the email and clicks the verification link, which is a page in the storefront.
9. The verification page sends a `POST` request to the `/auth/verification/confirm` [Confirm Verification](!api!/store/auth/confirm-verification) API route to verify the customer's email using the `token` query parameter in the URL.
10. Once the email is verified, the customer can log in.
11. During login, if the email is verified but the customer's account is not active, send a request to the [Register Customer API route](!api!/store/customers/register-customer) passing the login JWT token in the header.
...
After the customer verifies their email and logs in again, if the email is verified but the customer's account is not active, send a request to the [Register Customer API route](!api!/store/customers/register-customer) passing the login JWT token in the header, then log in the customer again to obtain an active JWT token.
...
Next, create a page in your storefront at the path that the verification link points to (for example, `/verify-account`). The page reads the `token` from the query string and sends it to the [Confirm Verification API route](!api!/store/auth/confirm-verification) to verify the customer's email.
...
- `verifying`: The page is verifying the token by sending a request to the Arcangel application.
