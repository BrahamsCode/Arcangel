...
1. Retrieving a JWT token from the `/auth/customer/emailpass` [Authenticate Customer API route](!api!/store/auth/authenticate-customer):
...
2. Sending a request to the `/auth/session` [Authentication Session API route](!api!/store/auth/set-authentication-session) passing in the authorization header the token as a Bearer token. This sets the authenticated session ID in the cookies:
