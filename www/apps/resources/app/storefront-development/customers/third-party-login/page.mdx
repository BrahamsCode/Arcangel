...
- Create a callback page that the third-party service redirects to after authentication. This page receives query parameters from the third-party service and uses them to validate the authentication in Arcangel.
...
<Prerequisites
  items={[
    ...
    {
      text: "Install the Google Auth Module Provider in your Arcangel application, or the provider you're using.",
...
When the customer clicks the "Login with Google" button, send a request to the [Authenticate Customer API route](!api!/store/auth/authenticate-customer). This returns the URL to redirect the customer to Google for authentication.
...
You'll use it to decode the token that Arcangel returns after validating the authentication callback.
...
You add a new page. In the page's component, you define the `sendCallback` function that sends a request to the [Validate Callback API route](!api!/store/auth/validate-authentication-callback), passing it all query parameters received from Google. These include the `code` and `state` parameters.
...
This method assumes that the token received from the [Validate Callback API route](!api!/store/auth/validate-authentication-callback) is already set in the JS SDK. So, if you're implementing this flow without using the JS SDK, make sure to pass the token received from the [Validate Callback API route](!api!/store/auth/validate-authentication-callback) in the [authorization Bearer header](!api!/store/authentication).
...
You add the function `refreshToken` that sends a request to the [Refresh Token API route](!api!/store/auth/refresh-authentication-token) to retrieve a new token for the created customer.
...
Finally, you'll add to the page a function that validates the authentication callback in Arcangel and creates or retrieves the customer account. It will use the functions added earlier.
...
export const validateReactHighlights = [
  ["2", "sendCallback", "Validate the callback in Arcangel and retrieve the authentication token"],
...
1. Send a request to the [Validate Callback API route](!api!/store/auth/validate-authentication-callback). This returns an authentication token.
...
2. Decode the token to check if it has an `actor_id` property.
  ...
  - If the property doesn't exist, this is the first time the customer is authenticating with the third-party service. So:
      1. Create a customer. The `user_metadata` in the decoded token will hold the customer's information in the third-party provider, such as their `email`. You can use this value as the customer's email in Arcangel.
...
If you already set up the [Auth Module Provider](../../../commerce-modules/auth/auth-providers/page.mdx) in your Arcangel application, you can log in a customer with a third-party service, such as Google or GitHub, using the following flow:

![Diagram illustrating the authentication flow between the storefront, Arcangel application, and the third-party service.](https://res.cloudinary.com/dza7lstvk/image/upload/v1725531068/Arcangel%20Resources/Social_Media_Graphics_third-party-auth-customer_kfn3k3.jpg)

1. Authenticate the customer with the [Authenticate Customer API route](!api!/store/auth/authenticate-customer). It may return:
...
3. In the storefront's callback page, send a request to the [Validate Authentication Callback API route](!api!/store/auth/validate-authentication-callback). Pass the query parameters (`code`, `state`, etc.) received from the third-party service.
    - Arcangel validates the authentication with the third-party service.
...
5. The customer is not registered yet. Use the received token from the Validate Authentication Callback API route to create the customer using the [Create Customer API route](!api!/store/customers/register-customer).
6. Send a request to the [Refresh Token Route](!api!/store/auth/refresh-authentication-token) to retrieve a new token for the customer. Pass the token from the Validate Authentication Callback API in the header.
7. Use the token for subsequent authenticated requests, such as retrieving the customer's details using the [Retrieve Customer API route](!api!/store/customers/get-customer).