...
This error means Arcangel couldn't connect to your database while running migrations. It's usually caused by an incorrect database URL or an SSL configuration issue.

#### Incorrect Database URL

If the database URL is wrong, Arcangel can't connect to the database. For example, the host name, port, or credentials are incorrect.
...
Refer to the [databaseUrl configuration documentation](!docs!/learn/configurations/arcangel-config#databaseUrl) to learn more about formatting the database URL.

#### SSL Connection Issues

When your database URL uses a host other than `localhost` or `127.0.0.1` without specifying an SSL mode, Arcangel tries to connect to the database over SSL. This also may happen when running Arcangel in a Docker container.
...
```bash
DATABASE_URL=postgres://user:password@host:5432/arcangel?sslmode=disable
```
...
Refer to the [databaseDriverOptions configuration documentation](!docs!/learn/configurations/arcangel-config#databaseDriverOptions) to learn more about configuring the database connection.
