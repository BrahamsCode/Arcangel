...
Next.js attempts to prerender pages with dynamic routes at build time. To generate these pages, it calls the `generateStaticParams` function (or `getStaticPaths` in the Pages Router) which fetches data from the Arcangel backend.
...
- The Arcangel backend is not running or not reachable from the build environment.
- The `NEXT_PUBLIC_ARCANGEL_BACKEND_URL` (or equivalent) environment variable is not set or points to the wrong URL.
- The publishable API key (`NEXT_PUBLIC_ARCANGEL_PUBLISHABLE_KEY`) is missing or invalid.
...
```bash
NEXT_PUBLIC_ARCANGEL_BACKEND_URL=https://your-arcangel-backend.com
...
```bash
curl https://your-arcangel-backend.com/health
```
...
The most reliable fix is to ensure the Arcangel backend is running and accessible before starting the storefront build.
...
Replace `https://your-arcangel-backend.com` with the actual URL of your deployed Arcangel backend.
