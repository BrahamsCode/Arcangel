...
export const metadata = {
  title: `Arcangel Examples`,
}

# {metadata.title}

This documentation page has examples of customizations useful for your custom development in the Arcangel application.
...
To resolve resources from the Arcangel container in an API route:

```ts highlights={[["8", "resolve", "Resolve the Product Module's\nmain service from the Arcangel container."]]}
...
Learn more in [this documentation](!docs!/learn/fundamentals/arcangel-container).
...
export const POST = async (
  req: ArcangelRequest<HelloWorldReq>,
  res: ArcangelResponse
) => {
  res.json({
...
To change it to another content type, use the `writeHead` method of `ArcangelResponse`:
...
export const POST = async (
  req: ArcangelRequest<PostStoreCustomSchemaType>,
  res: ArcangelResponse
) => {
  res.json({
...
To throw errors in an API route, use `ArcangelError` from the Arcangel Framework:

```ts highlights={[["9", "ArcangelError"]]}
import { ArcangelRequest, ArcangelResponse } from "@arcangel/framework/http"
...
By default, Arcangel configures CORS for all routes starting with `/admin`, `/store`, and `/auth`.
...
import { parseCorsOrigins } from "@arcangel/framework/utils"
...
By default, the Arcangel application parses a request's body using JSON.
...
5. Add the module to the configurations in `arcangel-config.ts`:
...
1. Pass options to the module in `arcangel-config.ts`:

```ts title="arcangel-config.ts" highlights={[["6", "options"]]}
module.exports = defineConfig({
  // ...
  modules: [
    {
...
export default class BlogModuleService extends ArcangelService({
  Post,
}){
...
A data model represents a table in the database. Arcangel provides a data model language to intuitively create data models.
...
Learn more in [this documentation](!docs!/learn/fundamentals/data-models/indexes).
...
The service factory `ArcangelService` generates data-management methods for your data models.

To extend the service factory in your module's service:

```ts highlights={[["4", "ArcangelService"]]}
import { ArcangelService } from "@arcangel/framework/utils"
import Post from "./models/post"
...
Query fetches data across modules. It’s a set of methods registered in the Arcangel container under the `query` key.
...
To resolve resources from the Arcangel container in a subscriber, use the `container` property of its parameter:
...
To limit the scheduled job's execution to a number of times during the Arcangel application's runtime, use the `numberOfExecutions` configuration:
...
A loader is a function defined in a module that's executed when the Arcangel application starts.
...
export default async function helloWorldLoader() {
  console.log(
    "[HELLO MODULE] Just started the Arcangel application!"
  )
}
```
...
You can customize the Arcangel Admin to inject widgets in existing pages, or create new pages using UI routes.
...
Widgets created in a details page, such as widgets in the `product.details` injection zone, receive a prop of the data of the details page (for example, the product):
...
Arcangel provides a `@arcangel/test-utils` package with utility tools to create integration tests for your custom API routes, modules, or other Arcangel customizations.
...
Arcangel provides all its commerce features as separate Commerce Modules, such as the Product or Order modules.
...
To create an actor type that can authenticate to the Arcangel application, such as a `manager`:
