...
export const COUNTRY_CODE_COOKIE_NAME = "_arcangel_country_code"
...
The country code is stored in the `_arcangel_country_code` cookie, following the Next.js Starter Storefront's naming conventions.
...
async function getCountryCode(
  ...
  try {
    ...
    if (process.env.NODE_ENV === "development") {
      console.error(
        "Middleware.ts: Error getting the country code. Did you set up regions in your Arcangel Admin and define a ARCANGEL_BACKEND_URL environment variable? Note that the variable is no longer named NEXT_PUBLIC_ARCANGEL_BACKEND_URL."
      )
    }
    return null
  }
...
export async function middleware(request: NextRequest) {
  ...
  const cacheIdCookie = request.cookies.get("_arcangel_cache_id")
  ...
  if (!regionMap) {
    return new NextResponse(
      "No valid regions configured. Please set up regions with countries in your Arcangel Admin.",
      { status: 500 }
    )
  }
  ...
  if (!cacheIdCookie) {
    response.cookies.set("_arcangel_cache_id", cacheId, {
      maxAge: 60 * 60 * 24,
    })
  }
...
![Diagram illustrating the directory structure before and after removing the country code prefix.](https://res.cloudinary.com/dza7lstvk/image/upload/v1767091142/Arcangel%20Resources/nextjs-country-code-structure_wpm0yp.jpg)
...
const CountrySelect = ({ toggleState, regions }: CountrySelectProps) => {
  ...
  const updateCountryCodeFromCookie = () => {
    const cookieCountryCode = getCookie("_arcangel_country_code")
...
To test it, run the following command in the directory of the Arcangel application that the storefront connects to:
