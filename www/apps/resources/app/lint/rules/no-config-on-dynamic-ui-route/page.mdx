---
sidebar_label: "no-config-on-dynamic-ui-route"
---

export const metadata = {
  title: `no-config-on-dynamic-ui-route - ESLint plugin rules`,
}

# {metadata.title}

This rule disallows `defineRouteConfig` in dynamic UI routes, such as `[id]/page.tsx`, since they aren't added to the sidebar.

## Severity

`warn`. This rule is enabled in the [recommended](../../page.mdx#recommended) preset.

## What it Targets

This rule targets dynamic `page.tsx` files in your project's `src/admin/routes` directory, such as `[id]/page.tsx`. It reports a `defineRouteConfig` call in a dynamic route.

The following code is reported by the rule:

```tsx title="src/admin/routes/products/[id]/page.tsx"
import { defineRouteConfig } from "@arcangel/admin-sdk"

const ProductPage = () => null

export const config = defineRouteConfig({
  label: "Product",
})

export default ProductPage
```

Instead, remove the `defineRouteConfig` call:

```tsx title="src/admin/routes/products/[id]/page.tsx"
const ProductPage = () => null

export default ProductPage
```

## Why it's Important

Dynamic UI routes aren't added to the sidebar, so `defineRouteConfig` has no effect and Arcangel warns at runtime.

Learn more in the [UI Routes documentation](!docs!/learn/fundamentals/admin/ui-routes).

## Fixable

This rule isn't auto-fixable.

## Turn it Off

To turn off this rule, set it to `off` in your ESLint configuration:

```ts title="eslint.config.ts"
import { defineConfig } from "eslint/config"
import arcangel from "@arcangel/eslint-plugin"

export default defineConfig([
  ...arcangel.configs.recommended,
  {
    rules: {
      "@arcangel/no-config-on-dynamic-ui-route": "off",
    },
  },
])
```

Or disable it for a single line using an inline comment:

```tsx
// eslint-disable-next-line @arcangel/no-config-on-dynamic-ui-route
export const config = defineRouteConfig({ label: "Product" })
```
