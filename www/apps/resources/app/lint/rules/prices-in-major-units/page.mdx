...
This rule disallows price values that look like they're in the smallest currency unit, since Arcangel stores prices in major units.
...
Arcangel expects prices in major units, such as `19.99` rather than `1999`. Passing a value in the smallest currency unit results in prices that are inflated by a factor of the currency's minor units.
...
export default defineConfig([
  ...
  {
    rules: {
      "@arcangel/prices-in-major-units": "off",
    },
  },
])
```
...
```ts
// eslint-disable-next-line @arcangel/prices-in-major-units
const item = { amount: price * 100 }
```
