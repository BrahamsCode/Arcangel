...
This rule is available since [Arcangel v2.19.0](https://github.com/arcangel/arcangel/releases/tag/v2.19.0).
...
Internal module events (enums ending in `Events`, with values in the form `{module}.{data-model}.{action}`) are implementation details of Arcangel's service layer. They can change between releases without a deprecation notice.
...
export default defineConfig([
  ...
  {
    rules: {
      "@arcangel/prefer-workflow-event-over-module-event": "off",
    },
  },
])
```
...
```ts
// eslint-disable-next-line @arcangel/prefer-workflow-event-over-module-event
