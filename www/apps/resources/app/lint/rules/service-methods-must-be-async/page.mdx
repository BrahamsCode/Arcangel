...
Arcangel always awaits service method calls. A method that isn't `async` and doesn't return a `Promise` can lead to unexpected behavior when its result is awaited.
