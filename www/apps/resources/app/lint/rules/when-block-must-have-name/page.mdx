...
When the `.then()` callback returns a `transform(...)` call, a plain object literal, or any other non-step result, Arcangel wraps the result in a synthetic step. Without an explicit name, Arcangel assigns a random name to that step at runtime and logs a warning in production.
...
export default defineConfig([
  ...
  {
    rules: {
      "@arcangel/when-block-must-have-name": "off",
    },
  },
])
```
...
```ts
// eslint-disable-next-line @arcangel/when-block-must-have-name
