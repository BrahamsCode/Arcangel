...
Refer to the [Arcangel Cache Cloud](!cloud!/cache) guide for setting up the Caching Module in Cloud.

</Note>

<Note>

The Caching Module is available starting [Arcangel v2.11.0](https://github.com/arcangel/arcangel/releases/tag/v2.11.0). It replaces the deprecated [Cache Module](../cache/page.mdx).

</Note>

## What is the Caching Module?

The Caching Module provides functionality to cache data in your Arcangel application, improving performance and reducing latency for frequently accessed data.

For example, Arcangel uses the Caching Module to cache product information, and you can cache custom data such as brand information.

The Caching Module stores and retrieves cached data using the caching service you integrate, such as [Redis](./providers/redis/page.mdx) or [Memcached](./guides/memcached/page.mdx). This provides flexibility in customizing your Arcangel application's infrastructure to meet your performance and scalability requirements.

![Diagram illustrating the Caching Module architecture](https://res.cloudinary.com/dza7lstvk/image/upload/v1759846791/Arcangel%20Resources/caching-overview_tz91tw.jpg)

### Caching Module vs Cache Module

Before Arcangel v2.11.0, you used the [Cache Module](../cache/page.mdx) to cache data. The Cache Module is now deprecated and has been replaced by the Caching Module.
...
The Caching Module is installed by default in your application. To use it, enable the caching feature flag and register the module in your `arcangel-config.ts` file.

### 1. Enable Caching Feature Flag

The caching feature is currently behind a feature flag. To enable it, set the `ARCANGEL_FF_CACHING` environment variable to `true` in your `.env` file:

```bash
ARCANGEL_FF_CACHING=true
```

This enables you to use the Caching Module and activates caching features in Arcangel's core.

### 2. Register the Caching Module

Next, add the Caching Module to the `modules` property of the exported object in `arcangel-config.ts`:
...
Refer to the [Caching Module Providers guide](./providers/page.mdx) to learn more about Caching Module Providers in Arcangel, and how to configure the default provider.
...
After you enable the Caching Module, Arcangel automatically caches data for several business-critical APIs to boost performance and throughput. This includes all cart-related operations, where the following data is cached:
...
You can also use the Caching Module's service directly to cache custom data in your Arcangel application.
...
In the example above, you create a step that resolves the Caching Module's service from the [Arcangel container](!docs!/learn/fundamentals/arcangel-container).
...
You can pass the following options to the Caching Module when registering it in your `arcangel-config.ts` file:
