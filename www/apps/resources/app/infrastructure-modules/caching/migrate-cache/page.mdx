...
The Caching Module is available starting [Arcangel v2.11.0](https://github.com/arcangel/arcangel/releases/tag/v2.11.0).

</Note>

<Note title="This guide is for developers who have">

1. Set up a Cache Module in `arcangel-config.ts`.
...
![Diagram illustrating the change in architecture from Cache Module to Caching Module](https://res.cloudinary.com/dza7lstvk/image/upload/v1759845565/Arcangel%20Resources/cache-to-caching_yxaped.jpg)
...
<Prerequisites
  items={[
    {
      text: "Updated your Arcangel application to v2.11.0 or later",
      link: "https://github.com/arcangel/arcangel/releases/tag/v2.11.0"
    }
  ]}
/>
...
For example, if you set up the Redis Cache Module, remove the following code from `arcangel-config.ts`:

```ts title="arcangel-config.ts" highlights={[["5"], ["6"], ["7"], ["8"], ["9"], ["10"]]}
