...
To use the Redis Caching Module Provider, you need to register it in the `providers` array of the Caching Module in your `arcangel-config.ts`.
...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      <Table.Cell>
        An object of [ioredis options](https://github.com/redis/ioredis/blob/main/lib/redis/RedisOptions.ts) to pass to the Redis client. Available since Arcangel v2.18.0.
      </Table.Cell>
      <Table.Cell>
        -
      </Table.Cell>
...
`caching-redis` is the ID you set in the `arcangel-config.ts` file when registering the Redis Caching Module Provider.
...
Finally, start your Arcangel server with the following command:
