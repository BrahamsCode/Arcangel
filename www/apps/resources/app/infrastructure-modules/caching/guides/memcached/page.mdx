...
In this tutorial, you'll learn how to create a Memcached [Caching Module Provider](../../providers/page.mdx) for your Arcangel application.
...
By the end of this tutorial, you'll be able to cache data in your Arcangel application using Memcached.

<Note>

Refer to the [Caching Module](../../page.mdx) documentation to learn more about caching in Arcangel.

</Note>

![Diagram illustrating the Memcached caching module provider in a Arcangel application](https://res.cloudinary.com/dza7lstvk/image/upload/v1759925175/Arcangel%20Resources/memcached-summary_ztlanl.jpg)

<Card
  title="Full Code"
  href="https://github.com/arcangel/examples/tree/main/memcached-caching"
...
To interact with Memcached, you'll need to install the `memjs` client. You can install it in your Arcangel project by running the following command:
...
A module is created under the `src/modules` directory of your Arcangel application. So, create the directory `src/modules/memcached` in your Arcangel project.
...
You first define module options that are passed to the Memcached Module Provider. You'll set those up later when you [register the module in your Arcangel application](#register-memcached-module-provider). The module accepts the following options:
...
- `cachePrefix`: A prefix to use for all cache keys to avoid collisions. Defaults to `"arcangel"`.
...
- `options`: The options passed to the module when it's registered in the Arcangel application.
...
class MemcachedCachingProviderService implements ICachingProviderService {
  ...
  constructor(
    ...
    const mainPrefix = options.cachePrefix || "arcangel"
...
2. The options passed to the module when it's registered in the Arcangel application.
...
The final piece of a module provider is its definition, which you export in an `index.ts` file at its root directory. This definition tells Arcangel which module this provider belongs to, its loaders, and its service.
...
The last step is to register the Memcached Module Provider in your Arcangel application.
...
```bash
ARCANGEL_FF_CACHING=true
```

### Register Memcached Module Provider

Then, in `arcangel-config.ts`, add a new entry in the `modules` array to register the Memcached Module Provider:
...
You register the `@arcangel/arcangel/caching` module and add the Memcached Module Provider to its `providers` array.
...
To test that the Memcached Caching Provider is working, start the Arcangel application with the following command:
...
If you set the `is_default` option to `true` in the provider registration, the Memcached Caching Provider will be used for all caching operations in the Arcangel application.
...
```bash
get arcangel:test-cache-products
```

Notice that you prefix the key with `arcangel:`, which is the default prefix unless you set the `keyPrefix` option in the provider registration.
