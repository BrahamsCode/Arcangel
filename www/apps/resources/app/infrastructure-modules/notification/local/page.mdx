...
The Local Notification Module Provider can also be used to send notifications to the [Arcangel Admin's notification panel](!user-guide!#check-notifications). 
...
- Resolve the Notification Module's main service from the [Arcangel container](!docs!/learn/fundamentals/arcangel-container).
...
Then, place an order. The subscriber will run, sending a notification to the Arcangel Admin's notification panel.
