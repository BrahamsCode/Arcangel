...
This feature is available since Arcangel [v2.14.0](https://github.com/arcangel/arcangel/releases/tag/v2.14.0).
...
- Resolve the Notification Module's main service and [Query](!docs!/learn/fundamentals/module-links/query) from the [Arcangel container](!docs!/learn/fundamentals/arcangel-container).
...
And create a product either using the [API route](!api!/admin/products/create-product) or the [Arcangel Admin](!user-guide!/products/create). This runs the subscriber and sends an email using SendGrid.

### Other Events to Handle

Arcangel emits other events that you can handle to send notifications using the SendGrid Notification Module Provider, such as `order.placed` when an order is placed.

Refer to the [Events Reference](/references/events) for a complete list of events emitted by Arcangel.
...
You can use the [sendNotificationsStep](/references/arcangel-workflows/steps/sendNotificationsStep) in your workflow to send an email using SendGrid.
