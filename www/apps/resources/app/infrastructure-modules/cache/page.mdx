...
In this document, you'll learn what a Cache Module is and how to use it in your Arcangel application.

<Note title="Deprecation Notice">

The Cache Module is deprecated starting from [Arcangel v2.11.0](https://github.com/arcangel/arcangel/releases/tag/v2.11.0). Use the [Caching Module](../caching/page.mdx) instead.
...
The underlying database, third-party service, or caching logic is flexible since it's implemented in a module. You can choose from Arcangel’s cache modules or create your own to support something more suitable for your architecture.

### Default Cache Module

By default, Arcangel uses the [In-Memory Cache Module](./in-memory/page.mdx). This module uses a plain JavaScript Map object to store the cache data. While this is suitable for development, it's recommended to use other Cache Modules, such as the [Redis Cache Module](./redis/page.mdx), for production. You can also [Create a Cache Module](./create/page.mdx).
...
In the example above, you create a workflow that has a step. In the step, you resolve the service of the Cache Module from the [Arcangel container](!docs!/learn/fundamentals/arcangel-container).
...
Arcangel provides the following Cache Modules. You can use one of them, or [Create a Cache Module](./create/page.mdx).
