...
The PostgreSQL Locking Module Provider uses PostgreSQL's advisory locks to control and manage locks across multiple instances of Arcangel. Advisory locks are lightweight locks that do not interfere with other database transactions. By using PostgreSQL's advisory locks, Arcangel can create distributed locks directly through the database.
...
To register the PostgreSQL Locking Module Provider, add it to the list of providers of the Locking Module in `arcangel-config.ts`:
  ...
  modules: [
    {
      ...
      options: {
        providers: [
          {
            resolve: "@arcangel/arcangel/locking-postgres",
            id: "locking-postgres",
