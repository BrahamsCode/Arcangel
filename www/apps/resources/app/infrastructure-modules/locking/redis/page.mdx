...
The Redis Locking Module Provider uses Redis to manage locks across multiple instances of Arcangel. Redis ensures that locks are globally available, which is ideal for distributed environments.

This provider is recommended for production environments where Arcangel is running in a multi-instance setup.
...
To register the Redis Locking Module Provider, add it to the list of providers of the Locking Module in `arcangel-config.ts`:
...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      `arcangel_lock:`. So, all locked keys are prefixed with `arcangel_lock:`.
...
To test out the Redis Locking Module Provider, start the Arcangel application:
...
```ts title="arcangel-config.ts" highlights={defaultHighlights}
module.exports = defineConfig({
  // ...
  modules: [
    {
