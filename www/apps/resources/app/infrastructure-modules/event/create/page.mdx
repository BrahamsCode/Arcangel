...
The Event Module's main service must extend the `AbstractEventBusModuleService` class from the Arcangel Framework:

```ts title="src/modules/my-event/service.ts"
import { AbstractEventBusModuleService } from "@arcangel/framework/utils"
import { Message } from "@arcangel/types"
...
The `emit` method is used to push an event from the Arcangel application into your messaging system. The subscribers to that event would then pick up the message and execute their asynchronous tasks.
...
To use your Event Module, add it to the `modules` object exported as part of the configurations in `arcangel-config.ts`. An Event Module is added under the `eventBus` key.
