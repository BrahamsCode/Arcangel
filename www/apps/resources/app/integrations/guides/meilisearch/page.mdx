...
export const metadata = {
  title: `Integrate Meilisearch with Arcangel`,
}

# {metadata.title}

In this tutorial, you'll learn how to integrate Meilisearch with Arcangel to enable advanced search capabilities in your storefront.

When you install a Arcangel application, you get a fully-fledged commerce platform with a Framework for customization. Arcangel's architecture supports integrating third-party services, such as search engines, allowing you to build custom features around core commerce flows.

[Meilisearch](https://www.meilisearch.com/) is an open-source, fast, and relevant search engine that you can integrate with Arcangel to enhance your storefront's search functionality.
...
- Integrate Meilisearch into Arcangel.
...
![Diagram illustrating the integration of Meilisearch with Arcangel](https://res.cloudinary.com/dza7lstvk/image/upload/v1758091098/Arcangel%20Resources/meilisearch-summary_bdig6e.jpg)

<Card
  title="Example Repository"
  text="Find the full code of the guide in this repository."
  href="https://github.com/arcangel/examples/tree/main/meilisearch-integration"
  icon={Github}
/>
...
To integrate third-party services into Arcangel, you create a custom module. A module is a reusable package with functionalities related to a single feature or domain. Arcangel integrates the module into your application without side effects on your setup.

In this step, you'll create a custom module that provides the necessary functionalities to integrate Meilisearch with Arcangel.
...
Before building the module, you need to install Meilisearch's JavaScript client. Run the following command in your Arcangel application's root directory:
...
A module is created under the `src/modules` directory of your Arcangel application. So, create the directory `src/modules/meilisearch`.
...
The next methods you'll add to the service are methods to retrieve and delete data from the Meilisearch index. You'll use these later to keep the Meilisearch index in sync with Arcangel.
...
The last method you'll implement is a method to search through the Meilisearch index. This method lets you expose search functionality to clients through Arcangel's API routes.
...
To keep the Meilisearch index in sync with Arcangel, you need to trigger indexing when products are created, updated, or deleted in Arcangel. You can also allow admins to manually trigger a reindex.
...
Arcangel provides the `useQueryGraphStep` in its `@arcangel/arcangel/core-flows` package. You only need to implement the other steps.
...
If you get a type error on resolving the Meilisearch Module, run the Arcangel application once with the `npm run dev` or `yarn dev` command to generate the necessary type definitions, as explained in the [Automatically Generated Types guide](!docs!/learn/fundamentals/generated-types).
...
In the step function, you resolve the Meilisearch Module's service from the Arcangel container using the name you exported in the module definition's file.
...
The `deleteProductsFromMeilisearchStep` step deletes products from Meilisearch when they are unpublished or deleted in Arcangel.
...
1. Retrieve products from Arcangel's database using `useQueryGraphStep`. This step uses Arcangel's [Query](!docs!/learn/fundamentals/module-links/query) tool to retrieve data across modules. You pass it the pagination and filter parameters you received in the input.
...
2. Creating an API route that the Arcangel Admin dashboard can call to emit the `meilisearch.sync` event, which triggers the subscriber.
3. Adding a new page or UI route to the Arcangel Admin dashboard to allow admins to trigger the reindex.
...
To allow the Arcangel Admin dashboard to trigger the `meilisearch.sync` event, you need to create an API route that emits the event.
...
In the route handler, you use the Arcangel container that is available in the request object. You resolve the [Event Module](../../../infrastructure-modules/event/page.mdx). This module manages events and their subscribers.
...
Before creating the UI route, you'll configure Arcangel's [JS SDK](../../../js-sdk/page.mdx). You can use it to send requests to the Arcangel server from any client application, including your Arcangel Admin customizations.
...
So, to create a new page under the Settings section of the Arcangel Admin, create the file `src/admin/routes/settings/meilisearch/page.tsx` with the following content:
...
You'll now test out the entire flow. Start by triggering the reindex manually from the Arcangel Admin dashboard, then check the Meilisearch dashboard for the indexed products.
...
![The Meilisearch Sync page in the Arcangel Admin dashboard with a button to sync products to Meilisearch](https://res.cloudinary.com/dza7lstvk/image/upload/v1758094934/Arcangel%20Resources/CleanShot_2025-09-17_at_10.41.51_2x_wph4pp.png)
...
Finally, you can check the Meilisearch dashboard to see the indexed products. Open the Meilisearch dashboard, either Cloud or local, and choose the index you specified for products in the environment variable `MEILISEARCH_PRODUCT_INDEX_NAME`. You'll find your Arcangel products indexed there.

![The Meilisearch dashboard showing the indexed products](https://res.cloudinary.com/dza7lstvk/image/upload/v1758095145/Arcangel%20Resources/CleanShot_2025-09-17_at_10.45.29_2x_y8ytzf.png)
...
Then, either create a product or update an existing one using the Arcangel Admin dashboard. If you check the Meilisearch dashboard, you'll find that the product's index was created or updated.
...
Then, delete a product from the Arcangel Admin dashboard. If you check the Meilisearch dashboard, you'll find that the product index was deleted there as well.

---

## Step 6: Search Products in Next.js Starter Storefront

The last step is to provide search functionalities to customers on your storefront. In the first step, you installed the [Next.js Starter Storefront](../../../nextjs-starter/page.mdx) along with the Arcangel application.
...
![The Next.js Starter Storefront showing the search modal with search results](https://res.cloudinary.com/dza7lstvk/image/upload/v1758096281/Arcangel%20Resources/CleanShot_2025-09-17_at_11.04.26_2x_bhb9k7.png)

---

## Next Steps

You've now integrated Meilisearch with Arcangel and added search functionality to your storefront. You can expand on these features to:
