...
export const metadata = {
  title: `Integrate Slack (Notification) with Arcangel`,
}

# {metadata.title}

In this tutorial, you'll learn how to integrate Slack with Arcangel to receive notifications about created orders.
...
Arcangel's [Notification Module](../../../infrastructure-modules/notification/page.mdx) allows you to customize Arcangel's infrastructure to send notifications using the third-party provider that fits your business needs, such as [Slack](https://slack.com/).

In this tutorial, you'll integrate Slack with Arcangel to receive notifications about created orders.
...
- Integrate Slack with Arcangel.
- Handle Arcangel's `order.placed` event to send notifications to Slack.
...
<Card
  ...
  href="https://github.com/arcangel/examples/tree/main/slack-integration"
  icon={Github}
/>
...
Arcangel's [Notification Module](../../../infrastructure-modules/notification/page.mdx) provides an interface to send notifications in your Arcangel application. It delegates the actual sending of notifications to the underlying provider, such as Slack.
...
To send requests to Slack, you'll use the [Axios](https://axios-http.com/) library. So, run the following command to install it in your Arcangel application:
...
A module is created under the `src/modules` directory of your Arcangel application. So, create the directory `src/modules/slack`.
...
export const serviceHighlights = [
  ...
  ["7", "admin_url", "The URL of the Arcangel Admin dashboard, used to add links in the notifications."],
...
The service must also have an `identifier` static property, which is a unique identifier for the module. This identifier is used when registering the module in the [Arcangel container](!docs!/learn/fundamentals/arcangel-container).
...
- `options`: Options that are passed to the module provider when it's registered in Arcangel's configurations. You define the following options:
    ...
    - `admin_url`: The URL of the Arcangel Admin dashboard, which you'll use to add links in the notifications.
...
When the Arcangel application needs to send a notification through a channel (such as Slack), it calls the `send` method of the channel's module provider.
...
In the method, you format the Slack message to show the order's ID, customer information, shipping address, order items, and total amounts. You also add a link to the order's details page in the Arcangel Admin dashboard.

Finally, you send the formatted message to Slack using the `axios.post` method with the configured webhook URL.

#### send Method

You can now implement the `send` method, which is the method that Arcangel calls to send notifications using the provider. Add the `send` method to the `SlackNotificationProviderService` class:
...
export const configHighlights = [
  ...
  ["14", "admin_url", "The URL of the Arcangel Admin dashboard, used to add links in the notifications."]
]

```ts title="arcangel-config.ts" highlights={configHighlights}
...
To pass a Notification Module Provider to the Notification Module, you add the `modules` property to the Arcangel configuration and pass the Notification Module in its value.
...
![Slack Apps page interface showing the prominent green "Create New App" button in the top navigation area, which initiates the process of creating a new Slack application for integrating with external services like Arcangel](https://res.cloudinary.com/dza7lstvk/image/upload/v1748940543/Arcangel%20Resources/CleanShot_2025-06-03_at_10.43.01_2x_zadmwn.png)
...
![Slack webhook URL generation interface showing the newly created webhook URL with a copy button, which provides the unique endpoint URL needed to configure Arcangel for sending automated notifications to the selected Slack channel](https://res.cloudinary.com/dza7lstvk/image/upload/v1748940718/Arcangel%20Resources/CleanShot_2025-06-03_at_10.45.51_2x_xet5u5.png)

Then, in the `.env` file of your Arcangel application, set the `SLACK_WEBHOOK_URL` and `SLACK_ADMIN_URL` environment variables:
...
In development, the Arcangel Admin dashboard is running on `http://localhost:9000/app`, so you can use that URL for the `SLACK_ADMIN_URL` environment variable.
...
Now that you've integrated Slack with Arcangel, you can send notifications to Slack at any point, including when an order is created.
  ...
  workflow={{
    ...
    steps: [
      ...
      },
      {
        ...
        link: "/references/arcangel-workflows/steps/sendNotificationsStep",
        depth: 1
      }
    ]
  }}
...
Arcangel provides both steps in its `@arcangel/arcangel/core-flows` package.
...
import { sendNotificationsStep, useQueryGraphStep } from "@arcangel/arcangel/core-flows"
...
- Retrieve the order's details using the `useQueryGraphStep`. This step uses Arcangel's [Query](!docs!/learn/fundamentals/module-links/query) tool to retrieve data across modules. You pass it the order ID to retrieve.
...
- `container`: The [Arcangel container](!docs!/learn/fundamentals/arcangel-container) to access the Framework and commerce tools.

In the subscriber function, you execute the `orderPlacedNotificationWorkflow` by invoking it, passing the Arcangel container as a parameter. Then, you chain a `run` method, passing it the order ID from the event's data payload as input.
...
You can also click on the order's ID in the notification to open the order's details page in the Arcangel Admin dashboard.

![Slack Notification Example](https://res.cloudinary.com/dza7lstvk/image/upload/v1748942706/Arcangel%20Resources/CleanShot_2025-06-03_at_11.24.16_2x_umecps.png)

---

## Next Steps

You've now integrated Slack with Arcangel to receive notifications about created orders. You can expand on this integration to send other types of notifications, such as order updates. Refer to the [Events Reference](/references/events) for a list of events you can listen to in your subscribers.
