...
export const metadata = {
  title: `Integrate Segment (Analytics) with Arcangel`,
}

# {metadata.title}

In this tutorial, you'll learn how to integrate Segment with Arcangel to track events and analytics.
...
To track analytics in your Arcangel application, you can integrate [Segment](https://segment.com/), a service that collects analytics from multiple sources and sends them to various destinations. This tutorial will help you set up Segment in your Arcangel application and track common events.
...
- Integrate Segment with your Arcangel application.
- Handle Arcangel's `order.placed` event to track order placements.
- Track custom events in your Arcangel application with Segment.

You can follow this tutorial whether you're new to Arcangel or an advanced Arcangel developer.

![Diagram showcasing the integration of Segment with Arcangel](https://res.cloudinary.com/dza7lstvk/image/upload/v1748264333/Arcangel%20Book/segment-overview_apkrtp.jpg)

<Card
  title="Example Repository"
  text="Find the full code of the guide in this repository."
  href="https://github.com/arcangel/examples/tree/main/segment-integration"
  icon={Github}
/>
...
In this step, you'll integrate Segment as an Analytics Module Provider. Later, you'll use it to track events in your Arcangel application.
...
A module is created under the `src/modules` directory of your Arcangel application. So, create the directory `src/modules/segment`.
...
- `options`: Options that are passed to the module provider when it's registered in Arcangel's configurations. You define the following option:
    - `writeKey`: The Segment write key. You'll learn how to retrieve and set this option in the [Add Module Provider to Arcangel's Configurations](#h-add-module-provider-to-arcangels-configurations) section.
...
// other imports...
import { ProviderIdentifyAnalyticsEventDTO } from "@arcangel/types"
...
```ts title="src/modules/segment/service.ts"
// other imports...
import { ProviderTrackAnalyticsEventDTO } from "@arcangel/types"
...
The `shutdown` method is used to gracefully shut down the Segment client when the Arcangel application is stopped. It allows you to send all pending events to Segment before the application exits.
...
To pass an Analytics Module Provider to the Analytics Module, you add the `modules` property to the Arcangel configuration and pass the Analytics Module in its value.

The Analytics Module accepts a `providers` option, which is an array of Analytics Module Providers to register. However, you can only register one analytics provider in your Arcangel application.
...
![Add more button in the Connections page](https://res.cloudinary.com/dza7lstvk/image/upload/v1748254988/Arcangel%20Book/CleanShot_2025-05-26_at_10.47.24_2x_qqlkwk.png)

3. In the "Choose a Source" step, select "Node.js" and click the "Next" button.

![Select Node.js as the Source](https://res.cloudinary.com/dza7lstvk/image/upload/v1748255028/Arcangel%20Book/CleanShot_2025-05-26_at_10.47.57_2x_zy9g8k.png)
...
![Copy the write key](https://res.cloudinary.com/dza7lstvk/image/upload/v1748255065/Arcangel%20Book/CleanShot_2025-05-26_at_10.48.41_2x_lpgsbb.png)
...
You'll first track the order-placement event, which is triggered natively in the Arcangel application.

Arcangel's events system allows you to listen to events triggered by the Arcangel application and execute custom logic asynchronously in a [subscriber](!docs!/learn/fundamentals/events-and-subscribers).
...
export const stepHighlights = [
  ...
  ["13", "analyticsModuleService", "Resolve the Analytics Module's service from the Arcangel container."],
...
In the step function, you resolve the Analytics Module's service from the Arcangel container. This service is the interface to track events with the configured Analytics Module Provider, which is Segment in this case.
...
1. Retrieve the Arcangel order using the `useQueryGraphStep` helper step. This step uses Arcangel's [Query](!docs!/learn/fundamentals/module-links/query) tool to retrieve data across modules. You pass it the order ID to retrieve.
...
In the subscriber function, you execute the `trackOrderPlacedWorkflow` by invoking it, passing the Arcangel container as a parameter. Then, you chain a `run` method, passing it the order ID from the event's data payload as input.
...
First, run the following command in your `apps/backend` directory to start the Arcangel server:
...
![Order Placed Event in Segment Debugger](https://res.cloudinary.com/dza7lstvk/image/upload/v1748259137/Arcangel%20Book/CleanShot_2025-05-26_at_14.31.25_2x_xi5p9h.png)

---

## Track Custom Event

In your Arcangel application, you often need to track custom events that are relevant to your business use case. For example, a B2B business may want to track whenever a user requests a quote.

In Arcangel, you can emit custom events in your workflows when an action occurs. Then, you can create a subscriber that listens to the custom event and executes a workflow to track it in Segment.

For example, if you have a `createQuoteWorkflow`, you can use Arcangel's [emitEventStep](!docs!/learn/fundamentals/events-and-subscribers/emit-event#emit-event-in-a-workflow) to emit a custom event after the quote is created:
...
You've now integrated Segment with your Arcangel application and tracked common events like order placement. You can expand on the features in this tutorial to:

- Track more events in your Arcangel application, such as user sign-ups, cart additions, and more. You can refer to the [Events Reference](/references/events) for a full list of events emitted by Arcangel.
