...
In this tutorial, you'll learn how to build a [plugin](!docs!/learn/fundamentals/plugins) that migrates data, such as products, from Magento to Arcangel.
...
Arcangel's modular architecture allows you to build a custom digital commerce platform that meets your business requirements without the limitations of a monolithic system. By migrating from Magento to Arcangel, you can take advantage of Arcangel's modern technology stack to build a scalable and flexible commerce platform that grows with your business.

By following this tutorial, you'll create a Arcangel plugin that migrates data from a Magento server to a Arcangel application in minimal time. You can re-use this plugin across multiple Arcangel applications, allowing you to adopt Arcangel across your projects.
...
- Implement a Magento Module in the plugin to connect to Magento's APIs and retrieve products.
    - This guide will only focus on migrating product data from Magento to Arcangel. You can extend the implementation to migrate other data, such as customers, orders, and more.
- Trigger data migration from Magento to Arcangel in a scheduled job.

You can follow this tutorial whether you're new to Arcangel or an advanced Arcangel developer.

![Diagram showcasing the flow of migrating data from Magento to Arcangel](https://res.cloudinary.com/dza7lstvk/image/upload/v1739360550/Arcangel%20Resources/magento-summary_hsewci.jpg)

<Card
  title="Example Repository"
  text="Find the full code of the guide in this repository. The repository also includes additional features, such as triggering migrations from the Arcangel Admin dashboard."
  href="https://github.com/arcangel/examples/tree/main/migrate-from-magento"
  icon={Github}
/>
...
You'll first install a Arcangel application that exposes core commerce features through REST APIs. You'll later install the Magento plugin in this application to test it out.
...
```bash npm2yarn
npx create-arcangel-app@latest arcangel-plugin-magento --plugin
```

Where `arcangel-plugin-magento` is the name of the plugin's directory and the name set in the plugin's `package.json`. So, if you wish to publish it to NPM later under a different name, you can change it here in the command or later in `package.json`.

Once the installation process is done, a new directory named `arcangel-plugin-magento` will be created with the plugin project files.
...
npx arcangel plugin:add arcangel-plugin-magento
```
...
module.exports = defineConfig({
  ...
  plugins: [
    {
      resolve: "arcangel-plugin-magento",
      options: {
        // TODO add options
      },
    },
...
To connect to external applications in Arcangel, you create a custom module. A module is a reusable package with functionalities related to a single feature or domain. Arcangel integrates the module into your application without implications or side effects on your setup.
...
export default class MagentoModuleService {
  ...
  async authenticate() {
    ...
    if (!response.ok) {
      throw new ArcangelError(ArcangelError.Types.UNAUTHORIZED, `Failed to authenticate with Magento: ${token}`)
    }
...
If the request is successful, you store the access token and its expiration date in the `accessToken` property. If the request fails, you throw a `ArcangelError` with the error message returned by Magento.
...
![Diagram showcasing the types file to create](https://res.cloudinary.com/dza7lstvk/image/upload/v1739346287/Arcangel%20Resources/magento-3_fpghog.jpg)
...
![Diagram showcasing the module definition file to create](https://res.cloudinary.com/dza7lstvk/image/upload/v1739348316/Arcangel%20Resources/magento-4_bmepvh.jpg)
...
As mentioned earlier when you registered the plugin in the Arcangel Application's `arcangel-config.ts` file, you can pass options to the plugin. These options are then passed to the modules in the plugin.

So, add the following options to the plugin's registration in the `arcangel-config.ts` file of the Arcangel application:
...
- `MAGENTO_IMAGE_BASE_URL`: The base URL to use for product images. Magento stores product images in the `pub/media/catalog/product` directory, so you can reference them directly or use a CDN URL. If the URLs of product images in the Arcangel server already have a different base URL, you can omit this option.

<Note title="Tip">

Arcangel supports integrating third-party services, such as [S3](../../../infrastructure-modules/file/s3/page.mdx), in a File Module Provider. Refer to the [File Module](../../../infrastructure-modules/file/page.mdx) documentation to find other module providers and how to create a custom provider.
...
In this section, you'll add the feature to migrate products from Magento to Arcangel. To implement this feature, you'll use a workflow.
  ...
  workflow={{
    ...
    steps: [
      ...
      },
      {
        ...
        description: "Retrieve Arcangel store details, which you'll need when creating the products.",
        depth: 1,
      ...
      },
      {
        type: "step",
        name: "useQueryGraphStep",
        description: "Retrieve Magento products that are already in Arcangel to update them, instead of creating them.",
        depth: 1,
        link: "/references/helper-steps/useQueryGraphStep"
      },
      {
        type: "step",
        name: "useQueryGraphStep",
        description: "Retrieve existing product options from Arcangel.",
        depth: 1,
        link: "/references/helper-steps/useQueryGraphStep"
      },
      {
        ...
        description: "Create missing product options in Arcangel.",
        depth: 1,
        link: "/references/arcangel-workflows/createProductOptionsWorkflow"
      },
      {
        type: "workflow",
        name: "createProductsWorkflow",
        description: "Create products in the Arcangel application.",
        depth: 1,
      ...
      },
      {
        ...
        description: "Update existing products in the Arcangel application.",
        depth: 1,
...
You only need to implement the `getMagentoProductsStep` step, which retrieves the products from Magento. The other steps and workflows are provided by Arcangel's `@arcangel/arcangel/core-flows` package.
...
If you get a type error on resolving the Magento Module, run the Arcangel application once with the `npm run dev` or `yarn dev` command to generate the necessary type definitions, as explained in the [Automatically Generated Types guide](!docs!/learn/fundamentals/generated-types).

</Note>

![Diagram showcasing the get-magento-products file to create](https://res.cloudinary.com/dza7lstvk/image/upload/v1739349590/Arcangel%20Resources/magento-5_ueb4wn.jpg)
...
You'll now create the workflow that migrates products from Magento using the step you created and steps from Arcangel's `@arcangel/arcangel/core-flows` package.

In your plugin, create the file `src/workflows/migrate-products-from-magento.ts` with the following content:

![Diagram showcasing the migrate-products-from-magento file to create](https://res.cloudinary.com/dza7lstvk/image/upload/v1739349820/Arcangel%20Resources/magento-6_jjdaxj.jpg)
...
1. An object with the workflow's configuration, including the name and whether to store the workflow's executions. You enable storing the workflow execution so that you can view it later in the Arcangel Admin dashboard.
...
Next, you'll retrieve the Arcangel store details and shipping profiles. These are necessary to prepare the data of the products to create or update.
...
You use the `useQueryGraphStep` step to retrieve the store details and shipping profiles. `useQueryGraphStep` is a Arcangel step that wraps [Query](!docs!/learn/fundamentals/module-links/query), allowing you to use it in a workflow. Query is a tool that retrieves data across modules.
...
Since the Arcangel application creates an internal representation of the workflow's constructor function, you can't manipulate data directly, as variables have no value while creating the internal representation.
...
2. Retrieve existing options from the Arcangel application using `useQueryGraphStep`.
3. Check which attributes don't exist in Arcangel and prepare to create them as options using `transform`.
4. Create the missing options in Arcangel using `createProductOptionsWorkflow`.
...
export const prepareHighlights = [
  ...
  ["12", "productsToCreate", "Create a map to store the products to create."],
  ["13", "productsToUpdate", "Create a map to store the products to update."],
  ["18", "description", "Try to retrieve description from the Magento product's custom attributes."],
  ["22", "handle", "Try to retrieve the product's handle from the Magento product's custom attributes."],
  ["25", "external_id", "Set the Magento product's ID in the Arcangel product's `external_id` property."],
  ["26", "thumbnail", "Try to retrieve the product's thumbnail from the Magento product's media gallery entries."],
  ["29", "sales_channels", "Associate the product with the default sales channel."],
  ["32", "shipping_profile_id", "Associate the product with a shipping profile."],
  ["34", "existingProduct", "Find the existing product in Arcangel that matches the Magento product's ID."],
  ["40", "options", "Map the Magento product's configurable product options to Arcangel product options."],
  ["77", "variants", "Map the Magento product's children to Arcangel product variants."],
]
...
You use `transform` again to prepare the data to create and update the products in the Arcangel application. For each Magento product, you map its equivalent Arcangel product's data:
...
- You map the Magento product's configurable product options to Arcangel product options by referencing the options created earlier (or existing ones). You map the option values to their IDs using the `attribute.options` and `existingOption.values` properties.
- You map the Magento product's children to Arcangel product variants. For the variant options, you pass an object whose keys is the option's label, such as "Color", and values is the option's value, such as "Red". For the prices, you set the variant's price based on the Magento child's price for every supported currency in the Arcangel store. Also, you set the Magento child product's ID in the Arcangel variant's `metadata.external_id` property.
- You map the Magento product's media gallery entries to Arcangel product images. You filter out the thumbnail image and set the URL and the Magento image's ID in the Arcangel image's `metadata.external_id` property.
...
You use the `createProductsWorkflow` and `updateProductsWorkflow` workflows from Arcangel's `@arcangel/arcangel/core-flows` package to create and update products in the Arcangel application.
...
You can now use this workflow to migrate products from Magento to Arcangel. You'll learn how to use it in the next steps.

---

## Step 6: Schedule Product Migration

There are many ways to execute tasks asynchronously in Arcangel, such as [scheduling a job](!docs!/learn/fundamentals/scheduled-jobs) or [handling emitted events](!docs!/learn/fundamentals/events-and-subscribers). 

In this guide, you'll learn how to schedule the product migration at a specified interval using a scheduled job. A scheduled job is an asynchronous function that the Arcangel application runs at the interval you specify during the Arcangel application's runtime.
...
![Diagram showcasing the migrate-magento file to create](https://res.cloudinary.com/dza7lstvk/image/upload/v1739358924/Arcangel%20Resources/magento-7_rqoodo.jpg)
...
- An asynchronous function that executes the job's logic. The function receives the [Arcangel container](!docs!/learn/fundamentals/arcangel-container) as a parameter.
...
Based on the job's configurations, the Arcangel application will run the job at midnight every day.
...
This ensures that the plugin's latest changes are reflected in the Arcangel application.

Finally, start the Arcangel application that the plugin is installed in:
...
To confirm that the products were migrated, open the Arcangel Admin dashboard at `http://localhost:9000/app` and log in. Then, click on Products in the sidebar. You'll see your magento products in the list of products.

![Click on products at the sidebar on the right, then view the products in the table in the middle.](https://res.cloudinary.com/dza7lstvk/image/upload/v1739359394/Arcangel%20Resources/Screenshot_2025-02-12_at_1.22.44_PM_uva98i.png)

---

## Next Steps

You've now implemented the logic to migrate products from Magento to Arcangel. You can re-use the plugin across Arcangel applications. You can also expand on the plugin to:
...
- Allow triggering migrations from the Arcangel Admin dashboard using [Admin Customizations](!docs!/learn/fundamentals/admin). This feature is available in the [Example Repository](https://github.com/arcangel/example-repository/tree/main/src/admin).
