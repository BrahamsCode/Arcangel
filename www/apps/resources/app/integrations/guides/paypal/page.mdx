...
import { Github, EllipsisHorizontal } from "@arcangel/icons"
import { Card, Prerequisites, WorkflowDiagram, InlineIcon } from "docs-ui"

export const metadata = {
  title: `Integrate PayPal (Payment) with Arcangel`,
}

# {metadata.title}

In this tutorial, you'll learn how to integrate PayPal with Arcangel for payment processing.

When you install a Arcangel application, you get a fully-fledged commerce platform with a Framework for customization. Arcangel's architecture facilitates integrating third-party services to handle various functionalities, including payment processing.

[PayPal](https://www.paypal.com/) is a widely used payment gateway that allows businesses to accept payments online securely. By integrating PayPal with Arcangel, you can offer your customers a convenient and trusted payment option.
...
- Integrate PayPal as a Payment Module Provider in Arcangel.
...
![Diagram showing the checkout flow between customer, Arcangel, and PayPal](https://res.cloudinary.com/dza7lstvk/image/upload/v1765268705/Arcangel%20Resources/paypal-overview_rsqisb.jpg)

<Card
  href="https://github.com/arcangel/examples/tree/main/paypal-integration"
  title="Full Code"
...
To integrate third-party services into Arcangel, you create a custom module. A module is a reusable package with functionalities related to a single feature or domain.
...
In this step, you'll integrate PayPal as a Payment Module Provider and configure it in your Arcangel application. Later, you'll use it to process payments.
...
To interact with PayPal's APIs, run the following command in your Arcangel application to install the PayPal server SDK:
...
A module is created under the `src/modules` directory of your Arcangel application. So, create the directory `src/modules/paypal`.
...
- `options`: Options that are passed to the module provider when it's registered in Arcangel's configurations. You define the following option:
...
You return an object with the PayPal order ID and a `data` object with additional information, such as the intent and approval URL. Arcangel stores the `data` object in the payment session's `data` field, allowing you to access it for later processing.
...
You return an object with a `data` field containing additional information, such as the authorization or capture ID. Arcangel will store the `data` object in the newly created payment record for later processing.
...
- An admin user captures the payment from the Arcangel Admin dashboard.
- PayPal webhook notifies Arcangel that the payment has been captured.
...
You return an object with a `data` field containing additional information, such as the capture ID. Arcangel updates the payment record's `data` field with the returned `data` object.
...
The [refundPayment](/references/payment/provider#refundpayment) method refunds a previously captured payment with the third-party service. It's called when an admin user issues a refund from the Arcangel Admin dashboard.
...
import { BigNumber } from "@arcangel/framework/utils"
...
You return an object with a `data` field containing additional information, such as the refund ID. Arcangel updates the payment record's `data` field with the returned `data` object.
...
You return an object with a `data` field containing the updated payment information. Arcangel updates the payment session's `data` field with the returned `data` object.
...
The [cancelPayment](/references/payment/provider#cancelpayment) method cancels a previously authorized payment with the third-party service. It's called when an admin user cancels an order from the Arcangel Admin dashboard.
...
You return an object with the existing `data` field without making any changes. Arcangel updates the payment record's `data` field with the returned `data` object.
...
1. Extract the PayPal order ID from the `data` field. This is the same `data.order_id` you returned in the `initiatePayment` method.
2. Retrieve the PayPal order details.
3. Map the PayPal order status to Arcangel's `PaymentSessionStatus`.
...
The [getWebhookActionAndData](/references/payment/provider#getwebhookactionanddata) method processes incoming webhook events from the third-party service. Arcangel provides a webhook endpoint at `/hooks/payment/{provider_id}` that you can use to receive PayPal webhooks. This endpoint calls the `getWebhookActionAndData` method to process the webhook event.
...
class PayPalPaymentProviderService extends AbstractPaymentProvider<Options> {
  ...
  async getWebhookActionAndData(
    ...
    try {
      ...
      // Map PayPal webhook events to Arcangel actions
      switch (eventType) {
...
4. Map the PayPal webhook event types to Arcangel actions.

You return an object containing the action Arcangel should take (such as `authorized`), along with the payment session ID and amount. Based on the returned action, Arcangel uses the methods you implemented earlier to perform the necessary operations.
...
### e. Add Module Provider to Arcangel's Configuration

Once you finish building the module, add it to Arcangel's configurations to start using it.

In `arcangel-config.ts`, add a `modules` property:
...
To pass Payment Module Providers to the Payment Module, add the `modules` property to the Arcangel configuration and pass the Payment Module in its value.
...
![PayPal environment toggle highlighted with Sandbox selected](https://res.cloudinary.com/dza7lstvk/image/upload/v1765207135/Arcangel%20Resources/CleanShot_2025-12-08_at_17.18.06_2x_sbm8bv.png)
...
![PayPal app details showing Client ID and Secret](https://res.cloudinary.com/dza7lstvk/image/upload/v1765207279/Arcangel%20Resources/CleanShot_2025-12-08_at_17.20.49_2x_zu9muk.png)
...
Deploy your Arcangel application with [Cloud](!cloud!/sign-up) in minutes. Benefit from features like zero-configuration deployments, automatic scaling, GitHub integration, and more.
...
This will create a public URL that tunnels to your local Arcangel server running on port `9000`. Copy the generated URL (for example, `https://abcd1234.ngrok.io`).
...
In this step, you'll enable the PayPal Payment Module Provider in a region of your Arcangel store. A region is a geographical area where you sell products, and each region has its own settings, such as currency and payment providers.

You must enable the PayPal Payment Module Provider in at least one region. To do this:

1. Run the following command to start your Arcangel application:

```bash npm2yarn
npm run dev
```

2. Open the Arcangel Admin dashboard in your browser at `http://localhost:9000/app` and log in.
...
![Arcangel Admin dashboard showing region edit screen with PayPal selected as payment provider](https://res.cloudinary.com/dza7lstvk/image/upload/v1765265899/Arcangel%20Resources/CleanShot_2025-12-09_at_09.36.54_2x_g4guz6.png)

---

## Step 4: Add PayPal to Storefront

In this step, you'll customize the [Next.js Starter Storefront](../../../nextjs-starter/page.mdx) that you set up with the Arcangel application to support paying with PayPal. You'll add a PayPal button to the checkout page that allows customers to pay using PayPal.
...
Copy the same PayPal Client ID you used in the Arcangel application, then add it to the `.env.local` file in the storefront directory:
...
You create a `PayPalWrapper` component that accepts a Arcangel payment session and children components as props.
...
Next, you'll add the function handlers for creating PayPal orders and placing the Arcangel order.

Replace the `// TODO: add function handlers` comment with the following:

export const functionHandlerHighlights = [
  ["1", "onPaymentCompleted", "Place the Arcangel order after PayPal payment approval."],
  ["14", "getPayPalOrderId", "Retrieve the PayPal order ID from the payment session data."],
  ["28", "createOrder", "Return the PayPal order ID created by the Arcangel server."],
  ["59", "onApprove", "Handle PayPal payment approval and place the Arcangel order."],
...
// The Arcangel PayPal provider should create a PayPal order during initialization
...
const createOrder = async () => {
  ...
  try {
    ...
    // Check if Arcangel server already created a PayPal order
    const existingOrderId = getPayPalOrderId()

    if (existingOrderId) {
      // Arcangel already created the order, use that order ID
...
const onApprove = async (data: { orderID: string }) => {
  try {
    ...
    // The Arcangel server will handle the payment authorization
...
- `onPaymentCompleted`: Places the Arcangel order by calling the `placeOrder` function. This function is called after the PayPal payment is approved.
- `getPayPalOrderId`: Retrieves the PayPal order ID from the payment session's `data` field.
- `createOrder`: Returns the PayPal order ID that was created by the Arcangel server during payment initialization. If no order ID exists, it throws an error.
- `onApprove`: Called when the customer approves the PayPal payment. It calls the `onPaymentCompleted` function to place the Arcangel order.
...
![PayPal Developer Dashboard showing sandbox accounts](https://res.cloudinary.com/dza7lstvk/image/upload/v1765267446/Arcangel%20Resources/CleanShot_2025-12-09_at_10.03.49_2x_phd4cj.png)
...
First, run the Arcangel application with the following command:
...
![Next.js Starter Storefront checkout page showing PayPal selected as payment method](https://res.cloudinary.com/dza7lstvk/image/upload/v1765267123/Arcangel%20Resources/CleanShot_2025-12-09_at_09.58.25_2x_b4ogqz.png)

This navigates you to the review step, where a PayPal button appears for completing your order.

![Next.js Starter Storefront checkout page showing PayPal button](https://res.cloudinary.com/dza7lstvk/image/upload/v1765267289/Arcangel%20Resources/CleanShot_2025-12-09_at_09.59.36_2x_r9sboh.png)
...
If you've [set up webhooks](#paypal-webhook-id) using ngrok or with your deployed Arcangel instance, PayPal sends webhook events to your Arcangel application after payment completion.

You'll see the following logged in your Arcangel application's terminal:
...
Arcangel uses the `getWebhookActionAndData` method you implemented earlier to process the webhook event and perform any necessary actions, such as authorizing the payment.

#### Capturing and Refunding Payments

In the Arcangel Admin dashboard, you can go to Orders and view the order you just placed. You can see the payment status and details.

From the order's details page, you can capture the authorized payment, and refund the captured payment from the **Payments** section. Arcangel will use your PayPal Payment Module Provider to perform these actions.

![Arcangel Admin dashboard showing order payment details with capture and refund options](https://res.cloudinary.com/dza7lstvk/image/upload/v1765267658/Arcangel%20Resources/CleanShot_2025-12-09_at_10.06.57_2x_qsazep.png)

---

## Next Steps

You've successfully integrated PayPal with Arcangel. You can now receive payments using PayPal in your Arcangel store.
