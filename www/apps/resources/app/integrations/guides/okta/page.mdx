...
[Okta](https://www.okta.com/) is an enterprise-grade identity management service that provides secure authentication. By integrating Okta with your Arcangel application, you allow users in your Okta organization to authenticate and access the Arcangel Admin without needing to create separate credentials.
...
<Card
  ...
  href="https://github.com/arcangel/examples/tree/main/okta-integration"
  icon={Github}
/>
...
Before you implement the Okta integration, this section provides a high-level overview of how the Okta authentication flow works in Arcangel.

![Diagram of the authentication flow between the Arcangel Admin, Arcangel server, and Okta](https://res.cloudinary.com/dza7lstvk/image/upload/v1764663019/Arcangel%20Resources/okta-auth-overview_ngbx80.jpg)

The authentication flow consists of the following steps:

1. An admin user clicks the "Login with Okta" button on the Arcangel Admin login page. This action triggers a request to the Arcangel server to initiate the authentication process.
2. The Arcangel server generates an authorization URL with the necessary parameters.
3. Arcangel Admin redirects the user to the Okta authorization URL.
4. The user authenticates with Okta using their Okta credentials.
5. After successful authentication, Okta redirects the user back to the Arcangel Admin with an authorization code.
6. The Arcangel Admin validates the callback by sending a request to the Arcangel server with the authorization code.
7. The Arcangel server exchanges the authorization code for tokens and retrieves the user's information from Okta.
8. The Arcangel server creates or updates the user's auth identity in Arcangel and returns the authentication response to the Arcangel Admin.
9. If this is a new user, the Arcangel Admin sends a request to create a new admin user in Arcangel.
10. The Arcangel Admin refreshes the user's token and logs in the user.

<Note>

Refer to the [Auth Route Flows](../../../commerce-modules/auth/authentication-route/page.mdx) guide to learn more about the authentication flows in Arcangel.

</Note>

Arcangel's [Auth Module](../../../commerce-modules/auth/page.mdx) provides the interface to authenticate users. It delegates the actual authentication logic to the underlying Auth Module Provider, which in this case is Okta.
...
2. An admin user creation flow in the Arcangel Admin that creates a new admin user in Arcangel after successful authentication with Okta.
3. An admin widget that adds the "Login with Okta" button to the Arcangel Admin login page and handles the authentication flow from the frontend.
...
A module is created under the `src/modules` directory of your Arcangel application. So, create the directory `src/modules/okta`.
...
export const serviceHighlights = [
  ...
  ["19", "identifier", "A unique identifier used to register the Okta Auth Module Provider in Arcangel."],
]
...
2. `identifier`: The unique identifier of the Auth Module Provider. This identifier is used to form the ID of the Auth Module Provider when registering it in Arcangel.
...
Next, you'll create or update the user's auth identity in Arcangel.
...
### e. Add Module Provider to Arcangel's Configurations
...
To pass a Module Provider to the Auth Module, you add the `modules` property to the Arcangel configuration and pass the Auth Module in its value.
...
5. In the creation form, enter the following details:
   - **App integration name**: A name for your application. For example, "Arcangel".
   - **Sign-in redirect URIs**: The redirect URI where Okta will redirect users after authentication. For example, set it to `http://localhost:9000/app/login`. You can replace the localhost URL with your Arcangel Admin's URL if it's different.
...
![Okta application creation form](https://res.cloudinary.com/dza7lstvk/image/upload/v1764672257/Arcangel%20Resources/CleanShot_2025-12-02_at_12.44.00_2x_nygahc.png)
...
![Okta application settings page](https://res.cloudinary.com/dza7lstvk/image/upload/v1764672504/Arcangel%20Resources/CleanShot_2025-12-02_at_12.48.03_2x_imaqs4.png)

#### Set Environment Variables

Next, set the following environment variables in your Arcangel application's `.env` file:
...
![Okta domain in Authorization Servers page](https://res.cloudinary.com/dza7lstvk/image/upload/v1764672662/Arcangel%20Resources/CleanShot_2025-12-02_at_12.50.40_2x_exdpgj.png)
...
The Okta integration is now ready. You'll test it out once you set up the authentication flow in the Arcangel Admin.
...
This API route is secured by Okta authentication since only authenticated users in your Okta organization can access it. However, for other authentication providers, proceed with caution if the authentication provider allows public sign-ups, such as social login with Google. In those scenarios, anyone with a Google account could create an admin user in your Arcangel application.
    ...
    steps: [
      {
        ...
        description: "Create an admin user in Arcangel.",
        link: "/references/arcangel-workflows/createUsersWorkflow",
        depth: 1
      },
      {
        type: "step",
        ...
        link: "/references/arcangel-workflows/steps/setAuthAppMetadataStep",
        depth: 2
      }
    ]
  }}
...
Arcangel provides the workflow and step out-of-the-box, so you can create the workflow.
...
import { createUsersWorkflow, setAuthAppMetadataStep } from "@arcangel/arcangel/core-flows"
...
1. Create the user using the [createUsersWorkflow](/references/arcangel-workflows/createUsersWorkflow).
...
3. Associate the created user with the auth identity (created by Okta) using the [setAuthAppMetadataStep](/references/arcangel-workflows/steps/setAuthAppMetadataStep).
...
export const POST = async (
  req: AuthenticatedArcangelRequest<CreateUserBody>, 
...
In the API route, you execute the `createUserWorkflow` by invoking it, passing it the [Arcangel container](!docs!/learn/fundamentals/arcangel-container), then calling its `run` method with the workflow's input.
...
import { authenticate, defineMiddlewares, validateAndTransformBody } from "@arcangel/framework/http"
...
1. [authenticate](!docs!/learn/fundamentals/api-routes/protected-routes): This middleware ensures only admin users with an authentication token can access the route. You set the `allowUnregistered` option to `true` to allow users who don't have an associated Arcangel user yet (new Okta users) to access the route.
...
- The user already has an associated Arcangel user. If so, you throw an error since the user shouldn't access the route.
...
You'll test out this API route after you implement the Okta authentication flow in the Arcangel Admin.

---

## Step 4: Add Login with Okta in Arcangel Admin

In this step, you'll customize the Arcangel Admin login form to add a "Login with Okta" button that initiates the Okta authentication flow.

The Arcangel Admin is customizable, allowing you to either inject custom components into existing pages or create new pages.
...
By default, the Arcangel Admin uses session-based authentication. However, to support third-party authentication providers like Okta, you need to switch to token-based authentication.

To set the admin authentication type, add the following environment variable to your Arcangel application's `.env` file:
...
Next, you'll configure Arcangel's [JS SDK](../../../js-sdk/page.mdx). It allows you to send requests to the Arcangel server from any client application, including your Arcangel Admin customizations.
...
A [widget](!docs!/learn/fundamentals/admin/widgets) is a React component that you can inject into predefined zones in the Arcangel Admin.
...
The `oktaLogin` function uses the JS SDK's `auth.login` method to initiate the Okta authentication flow. This method sends a request to Arcangel's `/auth/user/okta` API route, which executes the `authenticate` method of the Okta Auth Module Provider's service.
...
Next, you'll add a function that validates the Okta authentication callback in the Arcangel server.
...
The `sendCallback` function uses the JS SDK's `auth.callback` method to send the query parameters returned by Okta to Arcangel's `/auth/user/okta/callback` API route.
...
1. Call the `sendCallback` function to validate the Okta authentication in the Arcangel server.
...
First, run the following command to start the Arcangel server:

```bash npm2yarn
npm run dev
```

Then, open the Arcangel Admin in your browser at `http://localhost:9000/app`. You'll see a "Login with Okta" button on the login page.

![Arcangel Admin login page with a "Login with Okta" button](https://res.cloudinary.com/dza7lstvk/image/upload/v1764676152/Arcangel%20Resources/CleanShot_2025-12-02_at_13.48.58_2x_qjpsbk.png)

Click the button to initiate the Okta authentication flow. If you're already logged in to Okta, you'll be redirected back to the Arcangel Admin immediately where you'll be logged in. If not, you'll be prompted to log in to Okta first, then redirected back to the Arcangel Admin.

Once the authentication is successful, you can access the Arcangel Admin dashboard as an authenticated user.

---

## Next Steps

You've now set up Okta as an authentication provider for admin users in your Arcangel application. This allows your Okta organization's users to log in to the Arcangel Admin using their Okta credentials without needing to manage separate credentials for Arcangel.

You can also manage user access and permissions in Okta. For example, if you deny a user's access to the Arcangel application in Okta, that user will no longer be able to log in to the Arcangel Admin.
