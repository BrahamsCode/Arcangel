...
export const metadata = {
  title: `Integrate Strapi (CMS) with Arcangel`,
}

# {metadata.title}

In this tutorial, you'll learn how to integrate [Strapi](https://strapi.io/) with Arcangel.
...
This guide was built with Strapi v5.30.1. If you're using a different version and you run into issues, consider [opening an issue](https://github.com/arcangel/arcangel/issues/new?template=docs.yml).
...
- Integrate Strapi with Arcangel to interact with Strapi's API.
- Implement two-way synchronization of product data between Arcangel and Strapi:
    - Handle product events to sync data from Arcangel to Strapi.
    - Handle Strapi webhooks to sync data from Strapi to Arcangel.
- Display product data from Strapi in the Next.js Starter Storefront.

You can follow this tutorial whether you're new to Arcangel or an advanced Arcangel developer, but you're expected to have knowledge in Strapi, as its concepts are not explained in the tutorial.

![Diagram illustrating the flow of data between Arcangel, Strapi, admin, and customer (storefront)](https://res.cloudinary.com/dza7lstvk/image/upload/v1763375209/Arcangel%20Resources/strapi-summary_pioikw.jpg)

<Card
  title="Full Code"
  text="Find the full code of the guide in this repository."
  href="https://github.com/arcangel/examples/tree/main/strapi-integration"
  icon={Github}
/>
...
<Prerequisites items={[
  {
    text: "Node.js v20.19.0+ or v22.12.0+ (Versions supported by Strapi)",
...
In a separate directory from your Arcangel application, run the following command to create a new Strapi project:
...
In this section, you'll define a content type for products in Strapi. These products will be synced from Arcangel, allowing you to manage their content using Strapi's CMS features.
...
```json title="src/api/product/content-types/product/schema.json" badgeLabel="Strapi" badgeColor="orange"
{
  ...
  "info": {
    ...
    "pluralName": "products",
    "displayName": "Product",
    "description": "Products from Arcangel"
  },
  "options": {
...
1. `arcangelId`: A unique identifier that maps to the Arcangel product ID.
...
```json title="src/api/product-variant/content-types/product-variant/schema.json" badgeLabel="Strapi" badgeColor="orange"
{
  ...
  "info": {
    ...
    "description": "Product variants from Arcangel"
  },
  "options": {
    "draftAndPublish": false
  },
...
1. `arcangelId`: A unique identifier that maps to the Arcangel product variant ID.
...
```json title="src/api/product-option/content-types/product-option/schema.json" badgeLabel="Strapi" badgeColor="orange"
{
  ...
  "info": {
    ...
    "description": "Product options from Arcangel"
  },
  "options": {
    "draftAndPublish": false
  },
...
1. `arcangelId`: A unique identifier that maps to the Arcangel product option ID.
...
```json title="src/api/product-option-value/content-types/product-option-value/schema.json" badgeLabel="Strapi" badgeColor="orange"
{
  ...
  "info": {
    ...
    "description": "Product option values from Arcangel"
  },
  "options": {
    "draftAndPublish": false
  },
...
1. `arcangelId`: A unique identifier that maps to the Arcangel product option value ID.
...
You now have all the customizations in Strapi ready. You'll return to Strapi later after you set up the integration with Arcangel.

---

## Step 3: Integrate Strapi with Arcangel

In this step, you'll integrate Strapi with Arcangel by creating a Strapi Module.

A [module](!docs!/learn/fundamentals/modules) is a reusable package that provides functionalities related to a single feature or domain. Arcangel integrates the module into your application without implications or side effects on your setup.
...
First, you'll install the Strapi client in your Arcangel application to interact with Strapi's API.

In your `apps/backend` directory, run the following command to install the Strapi client:

```bash badgeLabel="Arcangel application" badgeColor="green"
npm install @strapi/client
```

### b. Create Module Directory

A module is created under the `src/modules` directory of your Arcangel application. So, create the directory `src/modules/strapi`.

### c. Create Strapi Client Loader

Next, you'll create the Strapi client when the Arcangel server starts by creating a loader.

A [loader](!docs!/learn/fundamentals/modules/loaders) is an asynchronous function that runs when the Arcangel server starts. Loaders are useful for setting up connections to third-party services and reusing those connections throughout your module.
...
```ts title="src/modules/strapi/loaders/init-client.ts" badgeLabel="Arcangel application" badgeColor="green"
...
```ts title="src/modules/strapi/service.ts" badgeLabel="Arcangel application" badgeColor="green"
...
This method takes an image ID as a parameter and deletes the corresponding image from Strapi using the Strapi client's files API. If the deletion fails, it throws a `ArcangelError` with a formatted error message.
...
Next, you'll add a method to update a document of a content type in Strapi. This will be useful to implement two-way synching between Arcangel and Strapi.
...
Next, you'll add a method to delete a document of a content type from Strapi. You'll use this method when a document is deleted in Arcangel, or when reverting document creation in case of failures.
...
#### Retrieve Document by Arcangel ID Method

Next, you'll add a method to retrieve a document of a content type from Strapi by its Arcangel ID. This will be useful to retrieve a document in case you need to revert changes.
...
export default class StrapiModuleService {
  // ...
  async findByArcangelId(
    collection: Collection, 
    arcangelId: string | string[], 
    populate?: string[]
  ) {
    try {
      const result = await this.client_.collection(collection).find({
        filters: {
          arcangelId: {
            $in: Array.isArray(arcangelId) ? arcangelId : [arcangelId],
          },
        },
        populate,
      })
...
1. `collection`: The collection (content type) in which the document exists. It uses the `Collection` enum.
2. `arcangelId`: A single Arcangel ID or an array of Arcangel IDs of the documents to be retrieved.
...
The final piece of a module is its definition, which you export in an `index.ts` file at the module's root directory. This definition tells Arcangel the name of the module, its service, and optionally its loaders.

To create the module's definition, create the file `src/modules/strapi/index.ts` with the following content:

```ts title="src/modules/strapi/index.ts" badgeLabel="Arcangel application" badgeColor="green"
...
### f. Add Module to Arcangel's Configurations
...
![Strapi dashboard with the API Token settings opened](https://res.cloudinary.com/dza7lstvk/image/upload/v1763136134/Arcangel%20Resources/CleanShot_2025-11-14_at_18.00.44_2x_ec8lds.png)

4. In the API token form:
    - Set a name for the token, such as "Arcangel".
...
![Strapi dashboard with the Create API Token form filled](https://res.cloudinary.com/dza7lstvk/image/upload/v1763136134/Arcangel%20Resources/CleanShot_2025-11-14_at_18.01.25_2x_hdex6b.png)

Then, copy the generated API token.

Finally, set the following environment variables in your Arcangel project's `.env` file:
...
In this step, you'll define a virtual read-only link between the product content type in Strapi and the `Product` model in Arcangel. Later, you'll be able to retrieve products from Strapi when retrieving products in Arcangel.

### a. Define the Link

To define a virtual read-only link, create the file `src/links/product-strapi.ts` with the following content:

```ts title="src/links/product-strapi.ts" badgeLabel="Arcangel application" badgeColor="green"
...
- An object of the second data model that is part of the link. In this case, it's the product content type from the Strapi Module. You set the following properties:
    ...
    - `primaryKey`: the primary key of the linked data model, which is `product_id`. Arcangel will look for this field in the retrieved `Products` from Strapi to match it with the `id` field of the `Product` model.
...
When you retrieve products from Arcangel with their `strapi_product` link, Arcangel will call the `list` method of the Strapi Module's service to retrieve the linked products from Strapi.
...
The `list` method receives a `filter` object with a `product_id` property, which contains the Arcangel product ID(s) to retrieve their corresponding data from Strapi.
...
You can now retrieve product data from Strapi when retrieving products in Arcangel. You'll learn how to do this in the upcoming steps.

---

## Step 5: Handle Product Creation

In this step, you'll implement the logic to listen to product creation events in Arcangel and create the corresponding product data in Strapi.
...
2. A [subscriber](!docs!/learn/fundamentals/events-and-subscribers) that listens to the product creation event in Arcangel and triggers the workflow.
...
The first step is available out-of-the-box in Arcangel. You need to create the rest of the steps.
...
```ts title="src/workflows/steps/create-options-in-strapi.ts" badgeLabel="Arcangel application" badgeColor="green"
...
In the step function, you resolve the Strapi Module's service from the Arcangel container. Then, you loop through the product options and create them in Strapi using the service's `create` method.
...
```ts title="src/workflows/steps/create-option-values-in-strapi.ts" badgeLabel="Arcangel application" badgeColor="green"
import { createStep, StepResponse } from "@arcangel/framework/workflows-sdk"
import { STRAPI_MODULE } from "../../modules/strapi"
import StrapiModuleService, { Collection } from "../../modules/strapi/service"
...
The `updateProductOptionValuesMetadataStep` stores the Strapi IDs of the created product option values in the `metadata` property of the corresponding product option values in Arcangel. This allows you to reference the Strapi option values later, such as when updating or deleting them.
...
```ts title="src/workflows/steps/update-product-option-values-metadata.ts" badgeLabel="Arcangel application" badgeColor="green"
import { createStep, StepResponse } from "@arcangel/framework/workflows-sdk"
import { Modules } from "@arcangel/framework/utils"
import { ProductOptionValueDTO } from "@arcangel/framework/types"
...
```ts title="src/workflows/create-options-in-strapi.ts" badgeLabel="Arcangel application" badgeColor="green" collapsibleLines="1-15" expandButtonLabel="Show Imports"
import { 
  createWorkflow, 
  WorkflowResponse,
  transform,
      ...
      return data.options.flatMap((option) => {
        return option.values.map((value) => {
          const strapiOption = data.strapiOptions.find(
            (strapiOption) => strapiOption.arcangelId === option.id
          )
          if (!strapiOption) {
            return null
          }
...
1. Retrieve the product options in Arcangel using the `useQueryGraphStep`.
    - This step uses [Query](!docs!/learn/fundamentals/module-links/query) to retrieve data in Arcangel across modules.
...
In a workflow, you can't manipulate data because Arcangel stores an internal representation of the workflow on application startup. Learn more in the [Data Manipulation](!docs!/learn/fundamentals/workflows/variable-manipulation) documentation.
    ...
    steps: [
      {
        ...
        link: "/references/arcangel-workflows/steps/acquireLockStep",
        depth: 1
      },
      {
        type: "step",
      ...
      },
      {
        ...
        link: "/references/arcangel-workflows/steps/releaseLockStep",
        depth: 4
      }
    ]
  }}
...
The first, second, and last steps are available out-of-the-box in Arcangel. You need to create the rest of the steps.
...
```ts title="src/workflows/steps/upload-images-to-strapi.ts" badgeLabel="Arcangel application" badgeColor="green"
import { createStep, StepResponse } from "@arcangel/framework/workflows-sdk"
import { STRAPI_MODULE } from "../../modules/strapi"
...
```ts title="src/workflows/steps/create-variants-in-strapi.ts" badgeLabel="Arcangel application" badgeColor="green"
import { createStep, StepResponse } from "@arcangel/framework/workflows-sdk"
import { STRAPI_MODULE } from "../../modules/strapi"
import StrapiModuleService, { Collection } from "../../modules/strapi/service"
...
The `updateProductVariantsMetadataStep` stores the Strapi IDs of the created product variants in the `metadata` property of the corresponding product variants in Arcangel. This allows you to reference the Strapi variants later, such as when updating or deleting them.

To create the step, create the file `src/workflows/steps/update-product-variants-metadata.ts` with the following content:

```ts title="src/workflows/steps/update-product-variants-metadata.ts" badgeLabel="Arcangel application" badgeColor="green"
import { createStep, StepResponse } from "@arcangel/framework/workflows-sdk"
import { Modules } from "@arcangel/framework/utils"
...
```ts title="src/workflows/create-variants-in-strapi.ts" badgeLabel="Arcangel application" badgeColor="green" collapsibleLines="1-12" expandButtonLabel="Show Imports"
import { 
  createWorkflow, 
  WorkflowResponse,
  transform,
...
import { acquireLockStep, releaseLockStep, useQueryGraphStep } from "@arcangel/arcangel/core-flows"
        ...
        const variantsMetadataUpdate = transform({ strapiVariants }, (data) => {
          return {
            updates: data.strapiVariants.map((strapiVariant) => ({
              variantId: strapiVariant.arcangelId,
...
The workflow receives the IDs of the product variants to create in Strapi and the Arcangel product ID they belong to.
...
2. Retrieve the product variants in Arcangel using the `useQueryGraphStep`.
...
In a workflow, you can't perform steps based on conditions because Arcangel stores an internal representation of the workflow on application startup. Learn more in the [Conditions in Workflows](!docs!/learn/fundamentals/workflows/conditions) documentation.
...
You need to create the `createProductInStrapiStep`, `retrieveFromStrapiStep`, and `updateProductInStrapiStep` steps. The rest of the steps and workflows are either available out-of-the-box in Arcangel or you have already created them.

#### retrieveFromStrapiStep

The `retrieveFromStrapiStep` retrieves items from Strapi by their Arcangel IDs.

To create the step, create the file `src/workflows/steps/retrieve-from-strapi.ts` with the following content:

```ts title="src/workflows/steps/retrieve-from-strapi.ts" badgeLabel="Arcangel application" badgeColor="green"
import { createStep, StepResponse } from "@arcangel/framework/workflows-sdk"
import { STRAPI_MODULE } from "../../modules/strapi"
import StrapiModuleService, { Collection } from "../../modules/strapi/service"
...
export const retrieveFromStrapiStep = createStep(
  ...
  async ({ collection, ids, populate }: RetrieveFromStrapiInput, { container }) => {
    ...
    const items = await strapiService.findByArcangelId(collection, ids, populate)
...
The step receives a collection name, an array of Arcangel IDs, and an optional array of fields to populate.

In the step, you use the Strapi Module's service `findByArcangelId` method to retrieve items from Strapi.
...
```ts title="src/workflows/steps/update-product-in-strapi.ts" badgeLabel="Arcangel application" badgeColor="green"
import { createStep, StepResponse } from "@arcangel/framework/workflows-sdk"
import { STRAPI_MODULE } from "../../modules/strapi"
import StrapiModuleService, { Collection } from "../../modules/strapi/service"
...
export const updateProductInStrapiStep = createStep(
  ...
  async ({ product }: UpdateProductInStrapiInput, { container }) => {
    ...
    const [originalProduct] = await strapiService.findByArcangelId(Collection.PRODUCTS, product.id, ["options"])
...
The step receives a product object with an ID and optional fields to update (title, subtitle, description, handle, and optionIds). It finds the Strapi product by its Arcangel ID and updates it with the provided details.
...
```ts title="src/workflows/steps/create-product-in-strapi.ts" badgeLabel="Arcangel application" badgeColor="green"
import { createStep, StepResponse } from "@arcangel/framework/workflows-sdk"
import { STRAPI_MODULE } from "../../modules/strapi"
import StrapiModuleService, { Collection } from "../../modules/strapi/service"
...
```ts title="src/workflows/create-product-in-strapi.ts" badgeLabel="Arcangel application" badgeColor="green"
import { 
  createWorkflow, 
  WorkflowResponse,
  transform,
...
export const createProductInStrapiWorkflow = createWorkflow(
  ...
  (input: CreateProductInStrapiWorkflowInput) => {
    ...
    const productMetadataUpdate = transform({ strapiProduct }, (data) => {
      return {
        selector: { id: data.strapiProduct.arcangelId },
        update: {
          metadata: {
    ...
    const optionsToCreate = transform({ existingStrapiOptions, optionIds }, (data) => {
      const existingArcangelIds = new Set(data.existingStrapiOptions.map((option) => option.arcangelId))
      return data.optionIds.filter((id: string) => !existingArcangelIds.has(id))
    })
...
2. Retrieve the product in Arcangel using the `useQueryGraphStep`.
...
Finally, you need to create a subscriber that listens to the product creation event in Arcangel and triggers the `createProductInStrapiWorkflow`.
...
```ts title="src/subscribers/product-created-strapi-sync.ts" badgeLabel="Arcangel application" badgeColor="green"
import {
...
Then, run the following command in the `apps/backend` directory to start the Arcangel server:

```bash npm2yarn
npm run dev
```

Next, open the Arcangel Admin dashboard and [create a new product](!user-guide!/products/create). Once you create the product, you'll see the following in the Arcangel server logs:
...
Next, you'll handle product updates in Strapi and synchronize the changes back to Arcangel. You'll create a workflow to update the relevant product data in Arcangel based on the data received from Strapi.

Then, you'll create an API route webhook that Strapi can call whenever product data is updated. With this setup, you'll have two-way synchronization between Arcangel and Strapi for product data.
        ...
        steps: [
          {
            type: "workflow",
            name: "updateProductsWorkflow",
            description: "If the data updated in Strapi is product data, update the product in Arcangel.",
            link: "/references/arcangel-workflows/updateProductsWorkflow",
            depth: 1
          },
          {
        ...
        steps: [
          {
            ...
            description: "If the data updated in Strapi is product variant data, update the product variant in Arcangel.",
            link: "/references/arcangel-workflows/updateProductVariantsWorkflow",
            depth: 1
          },
          {
            type: "step",
        ...
        steps: [
          {
            ...
            description: "If the data updated in Strapi is product option data, update the product option in Arcangel.",
            link: "/references/arcangel-workflows/updateProductOptionsWorkflow",
            depth: 1
          },
          {
            type: "step",
        ...
        steps: [
          {
            ...
            description: "If the data updated in Strapi is product option value data, update the product option value in Arcangel.",
            depth: 1
          },
          {
            type: "step",
...
You only need to create the `prepareStrapiUpdateDataStep`, `clearProductCacheStep`, and `updateProductOptionValueStep` steps. The rest of the steps and workflows are available out-of-the-box in Arcangel.
...
```ts title="src/workflows/steps/prepare-strapi-update-data.ts" badgeLabel="Arcangel application" badgeColor="green"
import { createStep, StepResponse } from "@arcangel/framework/workflows-sdk"
  ...
  async ({ entry }: { entry: any }) => {
    ...
    switch (model) {
      ...
      case "product-option-value":
        data = {
          optionValueId: entry.entry.arcangelId,
...
The `clearProductCacheStep` clears the product cache in Arcangel to ensure that updated data is served to clients. This is necessary as you'll enable [caching](../../../infrastructure-modules/caching/page.mdx) later, which may cause stale data to be served to the storefront.

To create the step, create the file `src/workflows/steps/clear-product-cache.ts` with the following content:

```ts title="src/workflows/steps/clear-product-cache.ts" badgeLabel="Arcangel application" badgeColor="green"
import { createStep, StepResponse } from "@arcangel/framework/workflows-sdk"
import { Modules } from "@arcangel/framework/utils"
...
The `updateProductOptionValueStep` updates product option values in Arcangel based on the data received from Strapi.

To create the step, create the file `src/workflows/steps/update-product-option-value.ts` with the following content:

```ts title="src/workflows/steps/update-product-option-value.ts" badgeLabel="Arcangel application" badgeColor="green"
import { createStep, StepResponse } from "@arcangel/framework/workflows-sdk"
import { Modules } from "@arcangel/framework/utils"
...
In the step, you resolve the Product Module's service and update the option value in Arcangel.
...
```ts title="src/workflows/handle-strapi-webhook.ts" badgeLabel="Arcangel application" badgeColor="green" collapsibleLines="1-19" expandButtonLabel="Show Imports"
import { 
  createWorkflow, 
  when,
  transform,
...
2. Check if the updated model is a product using `when`. If so, you:
    - Update the product in Arcangel using the `updateProductsWorkflow`.
...
3. Check if the updated model is a product variant using `when`. If so, you
    - Update the product variant in Arcangel using the `updateProductVariantsWorkflow`.
    - Clear the product cache using the `clearProductCacheStep`.
4. Check if the updated model is a product option using `when`. If so, you:
    - Update the product option in Arcangel using the `updateProductOptionsWorkflow`.
    - Clear the product cache using the `clearProductCacheStep`.
5. Check if the updated model is a product option value using `when`. If so, you:
    - Update the product option value in Arcangel using the `updateProductOptionValueStep`.
...
```ts title="src/api/webhooks/strapi/route.ts" badgeLabel="Arcangel application" badgeColor="green"
import { ArcangelRequest, ArcangelResponse } from "@arcangel/framework/http"
import { simpleHash, Modules } from "@arcangel/framework/utils"
import { 
...
export const POST = async (
  ...
  if (body.event === "entry.update") {
    ...
    const entityCacheKey = `strapi-update:${body.model}:${entry.arcangelId}`
...
3. Generate a hash of the webhook payload to detect duplicate webhook calls. This is necessary since you've implemented two-way synchronization between Arcangel and Strapi, which may lead to infinite loops of updates.
...
```ts title="src/api/middlewares.ts" badgeLabel="Arcangel application" badgeColor="green"
import { 
  defineMiddlewares, 
  ...
  routes: [
    {
      ...
      middlewares: [
        async (
          ...
          try {
            // Validate the API key using Arcangel's API Key Module
...
The [Caching Module](../../../infrastructure-modules/caching/page.mdx) is currently guarded by a feature flag. To enable it, add the feature flag and module in your `arcangel-config.ts` file:
...
You can now use the Caching Module's service in your workflows and API routes. Arcangel will also cache product and cart data automatically to improve performance.

### e. Webhook Handling Preparation

Before you test the webhook handling, you need to create a secret API key in Arcangel, then configure webhooks in Strapi.

Make sure to start both the Arcangel and Strapi servers if they are not already running.

#### Create Secret API Key in Arcangel

To create the secret API key in Arcangel:

1. Open the Arcangel Admin dashboard.
...
![Arcangel Admin dashboard with the Secret API Keys page showing the Strapi API key](https://res.cloudinary.com/dza7lstvk/image/upload/v1763368257/Arcangel%20Resources/CleanShot_2025-11-17_at_10.20.28_2x_b6cilm.png)

#### Configure Webhook in Strapi

Next, you need to configure a webhook in Strapi to call the Arcangel webhook API route whenever product data is updated.
...
4. In the webhook creation form:
   - **Name**: Enter a name for the webhook. For example, "Arcangel".
   - **URL**: Enter the URL of the Arcangel webhook API route. It should be `http://localhost:9000/webhooks/strapi` if you're running Arcangel locally.
   - **Headers**: Add a new header with the key `Authorization` and the value `Bearer YOUR_SECRET_API_KEY`. Replace `YOUR_SECRET_API_KEY` with the API key you created in Arcangel.
...
![Strapi Webhook Creation form](https://res.cloudinary.com/dza7lstvk/image/upload/v1763368257/Arcangel%20Resources/CleanShot_2025-11-17_at_10.30.22_2x_zohl1q.png)

### Test Strapi Webhook Handling

To test out the webhook handling:

1. Make sure both the Arcangel and Strapi servers are running.
...
Once you save the changes, Strapi will send a webhook to Arcangel. You should see the following in the Arcangel server logs:
...
You can also check the product in the Arcangel Admin dashboard to verify that the changes made in Strapi are reflected in Arcangel.

---

## Step 7: Show Strapi Data in Storefront

Now that you've integrated Strapi with Arcangel, you can customize the Next.js Starter Storefront to display product content from Strapi, allowing you to show product content and assets optimized for the storefront.
...
Since you've created a [virtual read-only link](#step-4-create-virtual-read-only-link-to-strapi-products) to Strapi products in Arcangel, you can retrieve Strapi product data when retrieving Arcangel products.
...
The Strapi product data will now be included in the `strapi_product` property of each Arcangel product.
...
/**
 * Get Strapi product data from a Arcangel product
 */
...
/**
 * Get product title from Strapi, fallback to Arcangel
 */
...
/**
 * Get product description from Strapi, fallback to Arcangel
 */
...
/**
 * Get product thumbnail from Strapi, fallback to Arcangel
 */
...
/**
 * Get product images from Strapi, fallback to Arcangel
 */
export function getProductImages(
  ...
  if (strapiProduct?.images && strapiProduct.images.length > 0) {
    // Convert Strapi media to Arcangel product image format
...
/**
 * Get variant title from Strapi, fallback to Arcangel
 */
export function getVariantTitle(
  ...
  const strapiVariant = strapiProduct?.variants?.find(
    (v) => v.arcangelId === variant.id
  )
  return strapiVariant?.title || variant.title || ""
}

/**
 * Get option title from Strapi, fallback to Arcangel
 */
export function getOptionTitle(
  ...
  const strapiOption = strapiProduct?.options?.find(
    (o) => o.arcangelId === option.id
  )
  return strapiOption?.title || option.title || ""
}

/**
 * Get option value text from Strapi, fallback to Arcangel
 */
export function getOptionValueText(
  ...
  const strapiProduct = getStrapiProduct(product)
  const strapiOption = strapiProduct?.options?.find(
    (o) => o.arcangelId === optionValue.option_id
  )
  const strapiOptionValue = strapiOption?.values?.find(
    (v) => v.arcangelId === optionValue.id
  )
...
export function getVariantImages(
  ...
  // Fall back to Arcangel variant images
...
- `getStrapiProduct`: Retrieves the Strapi product data from a Arcangel product.
- `getProductTitle`: Retrieves the product title from Strapi, falling back to Arcangel if not available.
- `getProductSubtitle`: Retrieves the product subtitle from Strapi.
- `getProductDescription`: Retrieves the product description from Strapi, falling back to Arcangel if not available.
- `getProductThumbnail`: Retrieves the product thumbnail from Strapi, falling back to Arcangel if not available.
- `getProductImages`: Retrieves the product images from Strapi, falling back to Arcangel if not available.
- `getVariantTitle`: Retrieves the variant title from Strapi, falling back to Arcangel if not available.
- `getOptionTitle`: Retrieves the option title from Strapi, falling back to Arcangel if not available.
- `getOptionValueText`: Retrieves the option value text from Strapi, falling back to Arcangel if not available.
- `getVariantOptionValues`: Retrieves all option values for a variant with Strapi labels.
- `getVariantImages`: Retrieves images for a specific variant from Strapi, falling back to Arcangel if not available.
...
function getImagesForVariant(
  ...
  // Get Strapi images or fallback to Arcangel images
  ...
  // Get variant images from Strapi or fallback to Arcangel
...
To test the storefront customizations, make sure both the Arcangel and Strapi servers are running.
...
Your setup now supports creating products in Strapi when they're created in Arcangel. However, you should also support updating and deleting products and their related models to keep data in sync between systems.
...
You can find all workflows and subscribers for product events in the [Strapi Integration Repository](https://github.com/arcangel/examples/tree/main/strapi-integration/arcangel).

---

## Next Steps

You've successfully integrated Arcangel with Strapi to manage content related to products, variants, and options. You can expand this integration by adding more features, such as:
...
2. Enable [internationalization](https://docs.strapi.io/cms/features/internationalization) in Strapi to support multiple languages:
    - You only need to manage the localized content in Strapi. Only the default locale will be synced with Arcangel.
