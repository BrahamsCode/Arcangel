...
Since [Arcangel v2.14.0](https://github.com/arcangel/arcangel/releases/tag/v2.14.0), adjustment data models like `OrderLineItemAdjustment` and `OrderShippingMethodAdjustment` also include a `version` property to track which version they belong to.
...
When the order is changed, such as when an item is exchanged, Arcangel updates the order's version and its related data:
