...
In this guide, you'll learn how to retrieve order totals in your Arcangel application using [Query](!docs!/learn/fundamentals/module-links/query).

You may need to retrieve order totals in your Arcangel customizations, such as workflows or custom API routes, to perform custom actions with them. The ideal way to retrieve totals is using Query.
...
To perform arithmetic on these totals without losing precision, use the `MathBN` utility from `@arcangel/framework/utils`. Don't use JavaScript's arithmetic operators, as they can introduce rounding errors:
...
To display an order total, convert it to a plain number and format it. Since Arcangel stores prices in major currency units, you can format the value directly:
