...
This feature is available since Arcangel [v2.18.0](https://github.com/arcangel/arcangel/releases/tag/v2.18.0).

</Note>

In this guide, you'll learn how Arcangel handles transferring an order to a guest customer.
...
If no customer exists with the given email address, Arcangel creates a new guest customer. The transfer can only be made to guest (non-registered) customers.

---

## Transfer Order to Guest Workflow

The transfer is handled by the `transferOrderToGuestWorkflow`, which is executed by the [Transfer Order to Guest API route](https://docs.arcangel.com/api/admin/orders/transfer-to-guest).

You can also use this workflow in your customizations:

```ts title="src/workflows/transfer-to-guest.ts"
import { transferOrderToGuestWorkflow } from "@arcangel/arcangel/core-flows"
