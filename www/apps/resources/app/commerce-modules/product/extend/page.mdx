...
- Set the `custom_name` property when a product is created or updated using Arcangel's API routes.
...
<Prerequisites
  items={[
    {
      text: "Module must be registered in arcangel-config.js",
...
To do that, you'll consume the [productsCreated](/references/arcangel-workflows/createProductsWorkflow#productscreated) hook of the [createProductsWorkflow](/references/arcangel-workflows/createProductsWorkflow). This workflow is executed in the [Create Product Admin API route](!api!/admin/products/create-product)
...
Also, make sure to replace `{shipping_profile_id}` with an existing shipping profile's ID. You can retrieve one either from the Arcangel Admin dashboard or using the [List Shipping Profiles API Route](!api!/admin/shipping-profiles/list-shipping-profiles).
...
Similar to the `productsCreated` hook, you'll consume the [productsUpdated](/references/arcangel-workflows/updateProductsWorkflow#productsUpdated) hook of the [updateProductsWorkflow](/references/arcangel-workflows/updateProductsWorkflow) to update `custom_name` when the product is updated.
...
import { updateProductsWorkflow } from "@arcangel/arcangel/core-flows"
import { 
