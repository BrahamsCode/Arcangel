...
In this guide, you'll learn how to retrieve the available inventory quantity of a product variant in your Arcangel application customizations. That includes API routes, workflows, subscribers, scheduled jobs, and any resource that can access the [Arcangel container](!docs!/learn/fundamentals/arcangel-container).
...
Product variants have a `manage_inventory` boolean field that indicates whether the Arcangel application manages the inventory of the product variant. 

When `manage_inventory` is disabled, the Arcangel application always considers the product variant to be in stock. So, you can't retrieve the inventory quantity for those products.
...
To retrieve the inventory quantity of a product variant, use the `getVariantAvailability` utility function imported from `@arcangel/framework/utils`. It returns the available quantity of the product variant.
...
import { getVariantAvailability } from "@arcangel/framework/utils"
...
import { ArcangelStoreRequest, ArcangelResponse } from "@arcangel/framework/http"
import { getVariantAvailability } from "@arcangel/framework/utils"
...
Notice that the request object's type is `ArcangelStoreRequest` instead of `ArcangelRequest` to ensure the availability of the `publishable_key_context` property.
