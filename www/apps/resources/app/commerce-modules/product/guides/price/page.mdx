...
In this document, you'll learn how to retrieve product variant prices in the Arcangel application using [Query](!docs!/learn/fundamentals/module-links/query).

<Note title="Why use Query?">

The Product Module doesn't provide pricing functionalities. The Arcangel application links the Product Module's `ProductVariant` data model to the Pricing Module's `PriceSet` data model.
