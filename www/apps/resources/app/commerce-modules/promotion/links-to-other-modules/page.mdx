...
A promotion can be applied on line items and shipping methods of a cart. Arcangel defines a link between the `Cart` and `Promotion` data models.
...
Arcangel also defines a read-only link between the [Cart Module](../../cart/page.mdx)'s `LineItemAdjustment` data model and the `Promotion` data model. Because the link is read-only from the `LineItemAdjustment`'s side, you can only retrieve the promotion applied on a line item, and not the other way around.
