...
Refer to this [Arcangel Admin User Guide](!user-guide!/promotions) to learn how to manage promotions using the dashboard.
...
The Arcangel Admin UI may not provide a way to create each of these promotion examples. However, they are supported by the Promotion Module and Arcangel's workflows and API routes.
...
The `limit` property is available since [Arcangel v2.12.0](https://github.com/arcangel/arcangel/releases/tag/v2.12.0).
...
![A diagram showcasing the relation between Promotion and PromotionRule](https://res.cloudinary.com/dza7lstvk/image/upload/v1709833196/Arcangel%20Resources/promotion-promotion-rule_msbx0w.jpg)
...
The `attribute` must be an exact key that Arcangel recognizes. Refer to [Rule Attributes](#rule-attributes) for the list of supported keys.
...
![A diagram showcasing the relation between PromotionRule and PromotionRuleValue when a rule has multiple values](https://res.cloudinary.com/dza7lstvk/image/upload/v1709897383/Arcangel%20Resources/promotion-promotion-rule-multiple_hctpmt.jpg)

In this case, a customer’s group must be in the `VIP` and `B2B` set of values to use the promotion.

### Rule Attributes

A rule's `attribute` must be an exact key that Arcangel recognizes for the resource the rule targets. The supported keys depend on where you set the rule:

- The promotion's `rules` property: Arcangel tests the rule against the cart.
- The application method's `target_rules` and `buy_rules` properties: Arcangel tests the rule against each cart line item or shipping method, based on the application method's `target_type`.
...
If you set an unrecognized attribute on an automatic promotion, Arcangel filters the promotion out and never applies it, so the discount stays `0` without an error.
...
These numeric conditions aren't available in the Arcangel Admin dashboard. Create them using workflows or the Admin API.
...
Arcangel evaluates the rule against the cart's total when the promotion is applied. Within a single computation, the total doesn't reflect discounts from other promotions applied in the same computation.
...
If you're managing promotions using [Arcangel's workflows](../../../arcangel-workflows-reference/page.mdx) or the API routes that use them, you can specify rules for the promotion or its [application method](../application-method/page.mdx).

For example, if you're creating a promotion using the [createPromotionsWorkflow](/references/arcangel-workflows/createPromotionsWorkflow):
