...
When you pass more than one promotion to `computeActions`, Arcangel applies them one after another and returns the combined actions.

Arcangel applies the promotions in the following order:

1. `buyget` promotions, then `standard` promotions.
2. Within each type, Arcangel sorts promotions by their application method's `value` in descending order.

Arcangel computes each promotion against the remaining amount of a line item or shipping method: its original amount minus the amount that promotions earlier in the same computation already discounted. Arcangel never discounts an item below `0`.
...
Arcangel tracks the remaining amount within a single `computeActions` call. Across separate calls, Arcangel reflects previously applied adjustments in the item's amount.

</Note>

### Order-Level Promotions

Arcangel always distributes a promotion whose application method's `target_type` is `order` across the cart's applicable items, whether its value is a percentage or a fixed amount. Arcangel distributes the discount proportionally based on each item's remaining amount.
...
If you apply a `$100` fixed discount on the order, Arcangel distributes it proportionally across the two items based on their share of the `$400` total:

- Item A: `$100 x (250 / 400) = $62.5`
- Item B: `$100 x (150 / 400) = $37.5`

If another order-level promotion applies afterward, Arcangel recalculates each item's share using its remaining amount (`$187.5` and `$112.5`).
...
When the Arcangel application receives this action type, it removes the `ShippingMethodAdjustment` (or `OrderShippingMethodAdjustment`) with the specified ID in the `adjustment_id` property.
