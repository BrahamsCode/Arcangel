...
Arcangel defines a link between the `Customer` and `AccountHolder` data models, allowing payment providers to save payment methods for a customer, if the payment provider supports it.

<Note>

This link is available starting from Arcangel `v2.5.0`.
...
A customer can have many account holders, but each account holder belongs to only one customer. If you try to link an account holder that already belongs to another customer, Arcangel throws a `Cannot create multiple links between 'customer' and 'payment'` error. To transfer an account holder to a different customer, remove the existing link first, then create the new one.
...
You can specify for each region which payment providers are available. The Arcangel application defines a link between the `PaymentProvider` and the `Region` data models.

![A diagram showcasing an example of how resources from the Payment and Region modules are linked](https://res.cloudinary.com/dza7lstvk/image/upload/v1711569520/Arcangel%20Resources/payment-region_jyo2dz.jpg)
