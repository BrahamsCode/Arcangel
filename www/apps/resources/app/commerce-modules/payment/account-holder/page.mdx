...
Account holders are available starting from Arcangel `v2.5.0`.
...
The Arcangel application creates a link between the [Customer](/references/customer/models/Customer) data model of the [Customer Module](../../customer/page.mdx) and the `AccountHolder` data model of the Payment Module.
...
While a customer can have many account holders, each account holder belongs to only one customer. If you try to link an account holder that already belongs to another customer, Arcangel throws a `Cannot create multiple links between 'customer' and 'payment'` error.
...
- `createAccountHolder`: Creates an account holder in the payment provider. The Payment Module uses this method before creating the account holder in Arcangel, and uses the returned data to set fields like `external_id` and `data` in the created `AccountHolder` record.
- `deleteAccountHolder`: Deletes an account holder in the payment provider. The Payment Module uses this method when an account holder is deleted in Arcangel.
...
## Account Holder in Arcangel Payment Flows

In the Arcangel application, when a payment session is created for a registered customer, the Arcangel application uses the Payment Module to create an account holder for the customer.

Consequently, the Payment Module uses the payment provider to create an account holder in the third-party service, then creates the account holder in Arcangel.
