...
This document showcases the module links that Arcangel defines between the Translation Module and other Commerce Modules.

## Summary

Arcangel defines the following links between the Translation Module and other Commerce Modules:
...
Arcangel defines the following read-only links between the Translation and Product Modules:
...
Instead, Arcangel defines a read-only link from the Store Module's `StoreLocale` data model to the Translation Module's `Locale` data model. This means you can retrieve the details of a store's supported locales, but you don't manage the links in a pivot table in the database.
