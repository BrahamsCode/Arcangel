...
Locales are automatically populated in your Arcangel application the first time the Translation Module is used.
...
## Retrieve Translations in Arcangel
...
You can retrieve translations for a resource on the Arcangel server using [Query](!docs!/learn/fundamentals/module-links/query). You can filter translations by `reference_id` and `locale_code` to get the specific translation you need.
...
If you've enabled the [Caching Module](../../../infrastructure-modules/caching/page.mdx) in your Arcangel application, you can also cache translation queries to improve performance:
