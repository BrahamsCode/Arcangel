...
<Prerequisites
  items={[
    {
      text: "Arcangel v2.12.4 or later",
      link: "https://github.com/arcangel/arcangel/releases/tag/v2.12.4"
    },
    {
...
The Translation Module allows you to extend translation capabilities to custom data models in your Arcangel application. Then, you can [manage translations from the Arcangel Admin](!user-guide!/settings/translations), and serve translated resources in your configured locales.

By following this guide, you'll learn how to:

- Configure the Translation Module to support translations for your custom data models.
- Manage translations for your custom data models from the Arcangel Admin.
...
The module must also be registered in `arcangel-config.ts`. For example:
...
The `translatable` method is available since [Arcangel v2.13.0](https://github.com/arcangel/arcangel/releases/tag/v2.13.0). Previous versions required [configuring translatable data models](#deprecated-manage-from-arcangel-configts) in `arcangel-config.ts`, which is now deprecated.

</Note>

The first step is to configure the properties of your custom data models that you want to support translations for. This will allow admin users to translate the data model's records from the Arcangel Admin, and manage whether the model and its properties are translatable.
...
When you add the `.translatable()` method to a property, the data model automatically appears in Settings → Translations in the Arcangel Admin.

Next, go to [Step 2: Manage Translations from Arcangel Admin](#step-2-manage-translations-from-arcangel-admin).

### Deprecated: Manage From arcangel-config.ts

<Details summaryContent="Before Arcangel v2.13.0">

If you're using a Arcangel version prior to v2.13.0, you need to configure the Translation Module in `arcangel-config.ts` to support translations for your custom data models.
...
Next, in `arcangel-config.ts`, add the `options.entities` property to the Translation Module configuration:
...
## Step 2: Manage Translations from Arcangel Admin

After configuring the Translation Module, you can manage translations for your custom data models from the Arcangel Admin.
...
Then, open the Arcangel Admin and go to Settings -> Translations. You should see your custom data model in the list of translatable resources.

![Post data model highlighted in the list of translatable resources in the Arcangel Admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1767687396/Arcangel%20Resources/CleanShot_2026-01-06_at_10.12.09_2x_w1hmua.png)
...
Arcangel supports passing the desired locale in API requests to `/store` routes using either:

1. The `locale` query parameter.
2. The `x-arcangel-locale` header.
...
import { applyLocale, defineMiddlewares } from "@arcangel/framework/http"
...
In your custom API routes, you can retrieve the request's locale from the `locale` property of the `ArcangelRequest` object. Pass that property to [Query](!docs!/learn/fundamentals/module-links/query) to retrieve your data models with translated fields.
...
In this example, the `locale` option is set to `req.locale`. Arcangel will set the `title` field of each post to its translated value if a translation is available for the requested locale. Otherwise, it returns the original value stored in the data model.

---

## Step 4: Test the Implementation

To test the implementation, start the Arcangel application if you haven't already:
