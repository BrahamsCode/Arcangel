...
Arcangel automatically configures MFA with sensible defaults, including setting the encryption key to the `AUTH_MFA_ENCRYPTION_KEY` environment variable. If you've created your application after v2.15.5 of Arcangel, this key is generated for you and stored in the `.env` file.
...
You only need to explicitly configure MFA options in your `arcangel-config.ts` if you want to override the defaults:
...
- [Create a Custom MFA Provider](/references/auth/mfa-module-provider)
- [Create a Custom Recovery Code Provider](/references/auth/recovery-code-mfa-module-provider)
