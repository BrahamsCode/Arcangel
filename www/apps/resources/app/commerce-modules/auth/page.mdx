...
Arcangel has auth related features available out-of-the-box through the Auth Module. A [module](!docs!/learn/fundamentals/modules) is a standalone package that provides features for a single domain. Each of Arcangel's commerce features are placed in Commerce Modules, such as this Auth Module.
...
import { Modules, ArcangelError } from "@arcangel/framework/utils"
import { ArcangelRequest } from "@arcangel/framework/http"
import { AuthenticationInput } from "@arcangel/framework/types"
...
Arcangel provides the following authentication providers out-of-the-box. You can use them to authenticate admin users, customers, or custom actor types.
