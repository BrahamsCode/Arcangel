...
In this guide, you'll learn how to enable email verification requirement for customers in your Arcangel application, and how to send email verification notifications.

<Note>

The steps of this guide have changed since Arcangel [v2.16.0](https://github.com/arcangel/arcangel/releases/tag/v2.16.0). It now uses the `http.authVerificationsPerActor` configuration instead of the `require_verification` option in the Emailpass Auth Module Provider.
...
<Prerequisites
  items={[
    {
      text: "Arcangel v2.16.0 or later",
...
By default, customers can register an account and log in to your Arcangel store without verifying their email address. You can enable email verification to require customers to verify their email address before they can log in.
...
To enable email verification, set the `http.authVerificationsPerActor` configuration in `arcangel-config.ts`:
...
Learn more in the [authVerificationsPerActor configuration reference](!docs!/learn/configurations/arcangel-config#httpauthVerificationsPerActor).
...
When the storefront requests to send an email verification notification, Arcangel emits the `auth.verification_requested` event. You can listen to this event in a [subscriber](!docs!/learn/fundamentals/events-and-subscribers) to send the email verification notification.

<Note>

The `auth.verification_requested` event payload changed in Arcangel v2.17.0. The `actor_type` and `provider_identity_id` fields were removed, `provider` was renamed to `code_provider`, and `entity_type` was added. If you have a subscriber written before v2.17.0, update it to use the payload described in this guide.
...
For development purposes, you can set the [Notification Local Module Provider](../../../infrastructure-modules/notification/local/page.mdx)'s channel to `email` in `arcangel-config.ts`:
...
To format the verification link in the email, you need to set the URL to your storefront in `arcangel-config.ts`. You can use the `admin.storefrontUrl` configuration for that:
...
- `code`: The verification code to include in the verification link. When the customer clicks the verification link, the storefront should send the code back to the Arcangel application to verify the customer's email address.
...
In the subscriber, you first check that the `entity_type` is `email` to only handle email verifications. Then, you format the storefront URL either using the value of `admin.storefrontUrl` set in `arcangel-config.ts` or a placeholder URL. You need this URL to format the verification link sent in the email.
...
When the customer clicks the verification link, they should be directed to a page in your storefront that sends the token back to the Arcangel application to verify the customer's email address.
