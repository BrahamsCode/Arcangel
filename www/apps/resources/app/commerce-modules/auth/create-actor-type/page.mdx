...
2. Set the `app_metadata` property of the associated auth identity using the `setAuthAppMetadataStep` from Arcangel's core workflows. You specify the actor type `manager` in the `actorType` property of the step’s input.
...
export const createRouteHighlights = [
  ["15", "AuthenticatedArcangelRequest", "The request must be authenticated to associate the manager with an auth identity."],
...
The `deleteAuthIdentityStep` is available since [Arcangel v2.19.0](https://github.com/arcangel/arcangel/releases/tag/v2.19.0)
