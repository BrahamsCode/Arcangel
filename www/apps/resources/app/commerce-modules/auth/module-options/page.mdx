...
When the Arcangel application starts, these providers are registered and can be used to handle authentication.
...
The Arcangel application's authentication API routes are defined under the `/auth` prefix that requires setting the `authCors` property of the `http` configuration.

By default, the Arcangel application you created will have an `AUTH_CORS` environment variable, which is used as the value of `authCors`.

Refer to [Arcangel's configuration guide](!docs!/learn/configurations/arcangel-config#httpauthCors) to learn more about the `authCors` configuration.
...
The Arcangel application's configuration accept an `authMethodsPerActor` configuration which restricts the allowed auth providers used with an actor type.
