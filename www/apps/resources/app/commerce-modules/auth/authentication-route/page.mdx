...
These routes are added by Arcangel's HTTP layer, not the Auth Module.
...
![Diagram showcasing the basic authentication flow between the frontend and the Arcangel application](https://res.cloudinary.com/dza7lstvk/image/upload/v1725539370/Arcangel%20Resources/basic-auth-routes_pgpjch.jpg)
...
2. Use the authentication token to create the user with their respective API route.
    - For example, for customers you would use the [Create Customer API route](!api!/store/customers/register-customer).
    - For admin users, you accept an invite using the [Accept Invite API route](!api!/admin/invites/accept-invite)
...
![Diagram showcasing the authentication flow between the frontend, Arcangel application, and third-party service](https://res.cloudinary.com/dza7lstvk/image/upload/v1725528159/Arcangel%20Resources/Third_Party_Auth_tvf4ng.jpg)
...
The Arcangel application defines an API route at `/auth/{actor_type}/{provider}/register` that creates an auth identity for an actor type, such as a `customer`. It returns a JWT token that you pass to an API route that creates the user.
...
2. Send a request to the [Create Customer API route](!api!/store/customers/register-customer) to create the customer, passing the [JWT token in the header](!api!/store/authentication).
...
An auth identity with the same email may already exist in Arcangel. This can happen if:
...
The Arcangel application defines an API route at `/auth/{actor_type}/{provider}` that authenticates a user of an actor type. It returns a JWT token that can be passed in [the header of subsequent requests](!api!/store/authentication) to send authenticated requests.
...
A custom auth provider can't change the shape of this route's response body. The provider's [authenticate method](/references/auth/provider#authenticate) returns an [AuthenticationResponse](/references/auth/interfaces/auth.AuthenticationResponse) object, but this route only uses its `success`, `authIdentity`, `location`, and `error` fields to build the response. Any other fields you return from the provider are ignored.

If you need to return a custom response body, create a [custom API route](!docs!/learn/fundamentals/api-routes) that calls the [Auth Module's authenticate method](/references/auth/authenticate), then shape the response as you need.
...
The Arcangel application defines an API route at `/auth/{actor_type}/{provider}/callback` that's useful for validating the authentication callback or redirect from third-party services like Google.
...
- If not, use the token in the header of a request that creates the user, such as the [Create Customer API route](!api!/store/customers/register-customer).
...
The Arcangel application defines an API route at `/auth/token/refresh` that's useful after authenticating a user with a third-party service to populate the user's token with their new information.
...
This route is available since [Arcangel v2.16.0](https://github.com/arcangel/arcangel/releases/tag/v2.16.0). On earlier versions, the route does not exist.

</Note>

The Arcangel application defines an API route at `/auth/{actor_type}/{auth_provider}/reset-password` that emits the `auth.password_reset` event, passing the token in the payload.
...
The Arcangel application defines an API route at `/auth/{actor_type}/{auth_provider}/update` that accepts a token and, if valid, updates the user's password.
...
Before [Arcangel v2.6](https://github.com/arcangel/arcangel/releases/tag/v2.6), you passed the token as a query parameter. Now, you must pass it in the `Authorization` header.
