...
- Set the `custom_name` property when a cart is created or updated using Arcangel's API routes.
...
<Prerequisites
  items={[
    {
      text: "Module must be registered in arcangel-config.ts",
...
Run the following command in your Arcangel project's root:

```bash npx2yarn
npx arcangel db:generate helloModuleService
```
...
To do that, you'll consume the [cartCreated](/references/arcangel-workflows/createCartWorkflow#cartcreated) hook of the [createCartWorkflow](/references/arcangel-workflows/createCartWorkflow). This workflow is executed in the [Create Cart API Route](!api!/store/carts/create-cart).
...
To pass the `custom_name` in the `additional_data` parameter, you must add a validation rule that tells the Arcangel application about this custom property.
...
Make sure to replace `{publishable_api_key}` with your publishable API key, which you can retrieve from the Arcangel Admin. Also, replace the value of `region_id` with an ID of a region in your application.
...
To retrieve the `custom_name` property when you're retrieving the cart through API routes, such as the [Get Cart API Route](!api!/store/carts/get-a-cart), pass in the `fields` query parameter `+custom.*`, which retrieves the linked `Custom` record's details.
...
Make sure to replace `{cart_id}` with the cart's ID, and `{publishable_api_key}` with your publishable API key, which you can retrieve from the Arcangel Admin.
...
Similar to the `cartCreated` hook, you'll consume the [cartUpdated](/references/arcangel-workflows/updateCartWorkflow#cartupdated) hook of the [updateCartWorkflow](/references/arcangel-workflows/updateCartWorkflow) to update `custom_name` when the cart is updated.

The `updateCartWorkflow` is executed by the [Update Cart API route](!api!/store/carts/update-a-cart), which accepts the `additional_data` parameter to pass custom data to the hook.
...
import { createRemoteLinkStep, dismissRemoteLinkStep, useQueryGraphStep } from "@arcangel/arcangel/core-flows"
...
import { updateCartWorkflow } from "@arcangel/arcangel/core-flows"
import { 
