...
You can manage prices and tiers using the Arcangel Admin:
...
For most use cases where you're building customizations in the Arcangel application, it's highly recommended to use [Arcangel's workflows](../../../arcangel-workflows-reference/page.mdx) rather than using the Pricing Module directly. Arcangel's workflows already implement extensive functionalities that you can re-use in your custom flows, with reliable roll-back mechanism.

</Note>

<CodeTabs group="price-example">
  <CodeTab label="Using Arcangel Workflows" value="workflows">
...
![A diagram showcasing the relation between the PriceRule and Price](https://res.cloudinary.com/dza7lstvk/image/upload/v1709648772/Arcangel%20Resources/price-rule-1_vy8bn9.jpg)

A price can have multiple price rules.

For example, a price can be restricted by a region and a zip code.

![A diagram showcasing the relation between the PriceRule and Price with multiple rules.](https://res.cloudinary.com/dza7lstvk/image/upload/v1709649296/Arcangel%20Resources/price-rule-3_pwpocz.jpg)
...
![A diagram showcasing the relation between the PriceSet, PriceList, Price, RuleType, and PriceListRuleValue](https://res.cloudinary.com/dza7lstvk/image/upload/v1709641999/Arcangel%20Resources/price-list_zd10yd.jpg)
...
Price rules only affect shipping options whose `price_type` is `flat`. For options with a `price_type` of `calculated`, the fulfillment provider determines the price during checkout, so Arcangel ignores any price rules you set. To offer free shipping above a threshold, use a flat-rate shipping option.
