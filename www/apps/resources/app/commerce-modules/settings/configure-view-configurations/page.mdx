...
<Prerequisites
  items={[
    {
      text: "Arcangel v2.18.0 or later",
      link: "https://github.com/arcangel/arcangel/releases/tag/v2.18.0"
    },
    {
...
In the Settings Module's configurations, you can override the generated columns of a core or custom data model. These overrides are applied to the configurable data table in the Arcangel Admin dashboard when the view configuration feature is enabled, and only to the system default view configuration.
...
To override the generated columns of an entity, pass the `entityOverrides` option to the Settings Module in `arcangel-config.ts`. Its value is an object whose keys are data model names as passed to the `ArcangelService` extended by the module's service, and whose values are the entity's override configuration.
...
import { ConfigurableDataTable } from "@arcangel/dashboard/components"
import { createTableAdapter } from "@arcangel/dashboard/lib"
...
If you go to `/admin/brands` in your Arcangel Admin dashboard, you'll see the configurable data table for the `Brand` data model. Based on the configurations you defined in `arcangel-config.ts`, it shows the `name` and `description` columns, and the `name` column appears first. You can customize the configurations, such as the visible columns and their order, and save your view configuration.

<Note title="Tip">

If you don't need saved views or dynamic columns, use the [DataTable component](../../../admin-components/components/data-table/page.mdx) from `@arcangel/ui` instead. It's a stable, fully-supported table primitive.
...
The Settings Module infers how to render each column's value, which is used to decide the column's render mode. The Arcangel Admin dashboard defines cell renderers for these render modes, which are used to render the column's value in the configurable data table.

The Settings Module and Arcangel Admin dashboard support the following built-in render modes:
...
Cell renderers are defined in the `src/admin/lib/cell-renderers.tsx` file of your Arcangel project. You can define multiple cell renderers in this file, each defined with the `defineCellRenderer` utility.
...
import { defineCellRenderer } from "@arcangel/dashboard/lib"
