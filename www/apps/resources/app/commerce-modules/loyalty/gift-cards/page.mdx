...
When a customer purchases a [gift card product](#gift-card-products) and the order is placed, Arcangel automatically:
...
When the gift card recipient enters the gift card code at checkout, Arcangel looks up the linked anonymous store credit account and creates a credit line on the cart. The credit line reduces the cart total by up to the gift card's balance.

### 3. Place Order

When the customer places the order, Arcangel debits the anonymous store credit account by the credit line amount, consuming the gift card's balance.
