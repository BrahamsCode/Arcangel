...
export const metadata = {
  title: `Arcangel and Module Container Resources`,
}

# {metadata.title}

This reference provides the list of resources registered in the Arcangel container and module containers.
...
Refer to the [Arcangel Container](!docs!/learn/fundamentals/arcangel-container) documentation for more information on how to use these resources.

</Note>

<Note title="Tip">

You can use the `ContainerRegistrationKeys` enum imported from `@arcangel/framework/utils` where specified.
...
<Table>
  ...
  <Table.Body>
    <Table.Row>
      ...
      [Arcangel Configurations](!docs!/learn/configurations/arcangel-config)

      </Table.Cell>
      <Table.Cell>

      The configurations defined in `arcangel-config.ts`.
    ...
    <Table.Row>
      ...
      The main service of every module added in `arcangel-config.ts`.

      </Table.Cell>
      <Table.Cell>

      - For custom modules, the registration name is the first parameter of `Module` exported in the `index.ts` file of the module.
      - For Arcangel's Commerce and Infrastructural Modules, use the `Modules` enum imported from `@arcangel/framework/utils`.
...
Use the `ContainerRegistrationKeys` and `Modules` enums imported from `@arcangel/framework/utils` where specified.
...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      The service of every data model passed to the `ArcangelService` function that your module's service extends. The service has the methods described in the [service factory reference](../service-factory-reference/page.mdx).
