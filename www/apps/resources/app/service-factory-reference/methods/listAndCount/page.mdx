...
The default limit of `50` records is enforced by the HTTP query-config middleware layer, not by the module service method itself. It only applies when the method is called through Arcangel's built-in HTTP routes.
