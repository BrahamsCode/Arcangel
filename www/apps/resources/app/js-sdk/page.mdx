...
export const metadata = {
  title: `Arcangel JS SDK`,
}

# {metadata.title}

In this documentation, you'll learn how to install and use Arcangel's JS SDK.

## What is Arcangel JS SDK?

Arcangel's JS SDK is a library to easily send requests to your Arcangel application. You can use it in your admin customizations or custom storefronts.

---

## How to Install Arcangel JS SDK?

The Arcangel JS SDK is available in your Arcangel application by default. So, you don't need to install it before using it in your admin customizations.

To install the Arcangel JS SDK in other projects, such as a custom storefront, run the following command:
...
- `@arcangel/js-sdk`: the Arcangel JS SDK.
- `@arcangel/types`: Arcangel's types library, which is useful if you're using TypeScript in your development.
...
<CodeTabs group="sdk-project">
  <CodeTab label="Admin (Arcangel project)" value="admin-arcangel-project">
...
```
  ...
  <CodeTab label="Admin (Arcangel Plugin)" value="admin-arcangel-plugin">
...
In Arcangel Admin customizations that are created in a Arcangel project, you use `import.meta.env` to access environment variables, whereas in customizations built in a Arcangel plugin, you use the global variable `__BACKEND_URL__` to access the backend URL. You can learn more in the [Admin Environment Variables](!docs!/learn/fundamentals/admin/environment-variables) chapter.

</Note>

### JS SDK Configurations

The `Arcangel` initializer accepts as a parameter an object with the following properties:
...
<Table>
  ...
  <Table.Body>
    <Table.Row>
      ...
      A required string indicating the URL to the Arcangel backend.
    ...
    <Table.Row>
      ...
      A string indicating the publishable API key to use in the storefront. You can retrieve it from the Arcangel Admin.
    ...
    <Table.Row>
      ...
      - `custom` to define custom storage using the `auth.storage` configuration. If `auth.storage` isn't defined, the SDK throws an error. This option is only available after Arcangel v2.5.1.
    ...
    <Table.Row>
      ...
      This option is only available after [Arcangel v2.1.1](https://github.com/arcangel/arcangel/releases/tag/v2.1.1).
...
The sidebar shows the different methods that you can use to send requests to Arcangel's API routes.
...
Prefer `sdk.client.fetch` over the raw [Fetch API](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API/Using_Fetch) when sending requests to your Arcangel backend.
...
import { Container, Heading, Button, Text } from "@arcangel/ui"
...
The `setLocale` method stores the locale in the JS SDK to pass the it as a `x-arcangel-locale` header in all subsequent requests. Also, if your application supports `localStorage`, the method sets the locale in the `arcangel_locale` key of the `localStorage`.
...
The `setLocale` method accepts the locale string in the [IETF BCP 47 standard](https://gist.github.com/typpo/b2b828a35e683b9bf8db91b5404f1bd1). All subsequent requests sent using the JS SDK will include the specified locale in the `x-arcangel-locale` header.
...
Tanstack Query is installed by default in your Arcangel application.
...
import { Button, Container } from "@arcangel/ui"
