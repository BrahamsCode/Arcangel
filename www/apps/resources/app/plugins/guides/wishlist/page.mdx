...
export const ogImage = "https://res.cloudinary.com/dza7lstvk/image/upload/v1737564127/Arcangel%20Resources/plugins-wishlist_ktut0d.jpg"
...
In this guide, you'll learn how to build a wishlist [plugin](!docs!/learn/fundamentals/plugins) in Arcangel.
...
- Install and set up a Arcangel application project.
- Install and set up a Arcangel plugin.
...
- Test and use the wishlist plugin in your Arcangel application.

You can follow this guide whether you're new to Arcangel or an advanced Arcangel developer.

<CardList items={[
  {
    href: "https://github.com/arcangel/examples/tree/main/wishlist-plugin",
...
You'll first install a Arcangel application that exposes core commerce features through REST APIs. You'll later install the wishlist plugin in this application to test it out.
...
Afterward, the installation process will start, which will install the Arcangel application as a monorepository in a directory with your project's name. The backend is installed in the `apps/backend` directory. If you chose to install the Next.js Starter Storefront, it'll be in the `apps/storefront` directory.
...
In this guide, all file paths are relative to the `apps/backend` directory of your Arcangel project.

</Note>

---

## Step 2: Install a Arcangel Plugin Project

A plugin is a package of reusable Arcangel customizations that you can install in any Arcangel application. You can add in the plugin [API Routes](!docs!/learn/fundamentals/api-routes), [Workflows](!docs!/learn/fundamentals/workflows), and other customizations, as you'll see in this guide. Afterward, you can test it out locally in a Arcangel application, then publish it to npm to install and use it in any Arcangel application.
...
A Arcangel plugin is set up in a different project, giving you the flexibility in building and publishing it, while providing you with the tools to test it out locally in a Arcangel application.

To create a new Arcangel plugin project, run the following command in a directory different than that of the Arcangel application:

```bash npx2yarnExec
npx create-arcangel-app@latest arcangel-plugin-wishlist --plugin
```

Where `arcangel-plugin-wishlist` is the name of the plugin's directory and the name set in the plugin's `package.json`. So, if you wish to publish it to NPM later under a different name, you can change it here in the command or later in `package.json`.

Once the installation process is done, a new directory named `arcangel-plugin-wishlist` will be created with the plugin project files.
...
## Step 3: Set up Plugin in Arcangel Application

Before you start your development, you'll set up the plugin in the Arcangel application you installed in the first step. This will allow you to test the plugin during your development process.
...
Next, you'll install the plugin in the Arcangel application from the local registry.

<Note>

If you've installed your Arcangel project before v2.3.1, you must install [yalc](https://github.com/wclr/yalc) as a development dependency first.
...
npx arcangel plugin:add arcangel-plugin-wishlist
```

This command installs the plugin in the Arcangel application from the local package registry.

Next, register the plugin in the `arcangel-config.ts` file of the Arcangel application:
...
module.exports = defineConfig({
  ...
  plugins: [
    {
      resolve: "arcangel-plugin-wishlist",
      options: {},
    },
  ],
})
...
To add custom tables to the database, which are called data models, you create a module. A module is a package with functionalities related to a single feature or domain. Arcangel integrates the module into your application without implications or side effects on your setup.

While you can create modules outside of a plugin and install them in the Arcangel application, plugins allow you to bundle modules with other customizations, such as API routes and workflows.
...
![Diagram showcasing the module directory to create](https://res.cloudinary.com/dza7lstvk/image/upload/v1737461182/Arcangel%20Resources/wishlist-1_z3kzfv.jpg)
...
![Directory structure after adding the Wishlist model](https://res.cloudinary.com/dza7lstvk/image/upload/v1737461304/Arcangel%20Resources/wishlist-2_co2lht.jpg)
...
- `sales_channel_id`: The ID of the sales channel where the wishlist is created. In Arcangel, product availability can differ between sales channels. This ensures only products available in the customer's sales channel are added to the wishlist.
...
![Directory structure after adding the WishlistItem model](https://res.cloudinary.com/dza7lstvk/image/upload/v1737461521/Arcangel%20Resources/wishlist-3_fxcjxy.jpg)
...
![Directory structure after adding the service file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737461698/Arcangel%20Resources/wishlist-4_j5ka26.jpg)
...
export default class WishlistModuleService extends ArcangelService({
  Wishlist,
  WishlistItem,
}) {}
```

The `WishlistModuleService` extends `ArcangelService` from the Modules SDK which generates a class with data-management methods for your module's data models. This saves you time on implementing Create, Read, Update, and Delete (CRUD) methods.
...
![Directory structure after adding the module definition file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737461829/Arcangel%20Resources/wishlist-5_mb4tjf.jpg)
...
Arcangel's CLI tool generates the migrations for you. To generate a migration for the Wishlist Module, run the following command in the plugin project:
...
Then, to reflect these migrations on the database of the Arcangel application using this module, run the following command:
...
The Wishlist Module's data models store IDs of records in data models implemented in Arcangel's core Commerce Modules, such as the ID of a customer or a product variant.

However, modules are [isolated](!docs!/learn/fundamentals/modules/isolation) to ensure they're re-usable and don't have side effects when integrated into the Arcangel application. So, to build associations between modules, you define [module links](!docs!/learn/fundamentals/module-links). A Module link associates two modules' data models while maintaining module isolation.
...
![Directory structure after adding the link file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737462649/Arcangel%20Resources/wishlist-6_xqytog.jpg)
...
3. An object of configurations for the module link. By default, Arcangel creates a table in the database to represent the link you define. However, in this guide, you only want this link to retrieve the customer associated with a wishlist. So, you enable `readOnly` telling Arcangel not to create a table for this link.
...
![Directory structure after adding the link file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737462829/Arcangel%20Resources/wishlist-7_ddiwxy.jpg)

```ts title="src/modules/wishlist/links/wishlist-sales-channel.ts"
import { defineLink } from "@arcangel/framework/utils"
import WishlistModule from "../modules/wishlist"
import SalesChannelModule from "@arcangel/arcangel/sales-channel"
...
![Directory structure after adding the link file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737467010/Arcangel%20Resources/wishlist-8_hgoaby.jpg)
...
![Directory structure after adding the step file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737467678/Arcangel%20Resources/wishlist-9_lzlifn.jpg)
...
1. An async function that executes the step's logic. The function receives two parameters:
   ...
   - An object holding the workflow's context, including the [Arcangel Container](!docs!/learn/fundamentals/arcangel-container) that allows you to resolve Framework and commerce tools.
...
![Directory structure after adding the step file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737467998/Arcangel%20Resources/wishlist-10_xex4d0.jpg)
...
![Directory structure after adding the workflow file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737468333/Arcangel%20Resources/wishlist-11_absftb.jpg)
...
![Directory structure after adding the route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737468859/Arcangel%20Resources/wishlist-12_gvvb9z.jpg)
...
You'll now test that this API route defined in the plugin is working as expected using the Arcangel application you installed in the first step.
...
Before sending the request, you need to obtain a publishable API key. So, open the Arcangel Admin at `http://localhost:9000/app` and log in with the user you created earlier.
...
![In the admin, click on Publishable API key in the sidebar. A table will show your API keys and allow you to create one.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733230421/Arcangel%20Resources/Screenshot_2024-12-03_at_2.53.07_PM_gau9jy.png)

### Retrieve Authenticated Customer Token

Then, you need an authentication token of a registered customer. To create a customer, first, send the following request to the Arcangel application:
...
To test the API route, start the Arcangel application.
...
The `useQueryGraphStep` is from Arcangel's workflows package. So, you'll only implement the other steps.
...
![Directory structure after adding the step file](https://res.cloudinary.com/dza7lstvk/image/upload/v1740071251/Arcangel%20Resources/wishlist-29_bq6kcn.jpg)
...
![Directory structure after adding the step file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737470093/Arcangel%20Resources/wishlist-13_nn924e.jpg)
...
![Directory structure after adding the step file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737470156/Arcangel%20Resources/wishlist-14_ckoesz.jpg)
...
![Directory structure after adding the step file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737470302/Arcangel%20Resources/wishlist-15_oc696x.jpg)
...
![Directory structure after adding the workflow file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737470660/Arcangel%20Resources/wishlist-16_ovujwp.jpg)
...
![Directory structure after adding the route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737470985/Arcangel%20Resources/wishlist-17_zmqk6c.jpg)
...
export async function POST(
  req: AuthenticatedArcangelRequest<PostStoreCreateWishlistItemType>,
  res: ArcangelResponse
) {
  if (!req.publishable_key_context?.sales_channel_ids.length) {
    throw new ArcangelError(
...
In Arcangel, you create validation schemas using [Zod](https://zod.dev/) in a TypeScript file under the `src/api` directory. So, create the file `src/api/store/customers/me/wishlists/items/validators.ts` with the following content:

![Directory structure after adding the validation schema file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737471383/Arcangel%20Resources/wishlist-18_hj9iom.jpg)
...
The `validateAndTransformBody` middleware is available out-of-the-box in Arcangel, allowing you to validate and transform the request body using a Zod schema.
...
![Directory structure after adding the middleware file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737471615/Arcangel%20Resources/wishlist-19_ryyzdk.jpg)
...
In this file, you export the middlewares definition using `defineMiddlewares` from the Arcangel Framework. This function accepts an object having a `routes` property, which is an array of middleware configurations to apply on routes.
...
Start the Arcangel application to test out the API route.
...
The `useQueryGraphStep` is from Arcangel's workflows package, and you implemented the `validateWishlistExistsStep` [previously](#validatewishlistexistsstep) . So, you'll only implement the other steps.
...
![Directory structure after adding the step file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737474621/Arcangel%20Resources/wishlist-20_jcwrtf.jpg)
...
![Directory structure after adding the step file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737474703/Arcangel%20Resources/wishlist-21_e50lrg.jpg)
...
![Directory structure after adding the workflow file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737474872/Arcangel%20Resources/wishlist-22_wt1g36.jpg)
...
![Directory structure after adding the route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737475074/Arcangel%20Resources/wishlist-23_qatcia.jpg)
...
![Directory structure after adding the route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737475331/Arcangel%20Resources/wishlist-24_tiwjpr.jpg)
...
You first retrieve the [http Arcangel configuration](!docs!/learn/configurations/arcangel-config#http) which holds configurations related to JWT secrets and expiration times. You then use the `jsonwebtoken` package to sign a token containing the wishlist ID. You return the token in the response.
...
![Directory structure after adding the route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737475795/Arcangel%20Resources/wishlist-25_sodzsr.jpg)

```ts title="src/api/store/wishlists/[token]/route.ts"
import { ArcangelResponse, ArcangelStoreRequest } from "@arcangel/framework"
import { ArcangelError } from "@arcangel/framework/utils"
...
## Step 13: Show Wishlist Count in Arcangel Admin

In this step, you'll customize the Arcangel Admin dashboard to show for each product the number of wishlists it's in.

The Arcangel Admin dashboard's pages are customizable to insert widgets of custom content in pre-defined injection zones. You create these widgets as React components that allow admin users to perform custom actions.
...
export default class WishlistModuleService extends ArcangelService({
  ...
  async getWishlistsOfVariants(
    variantIds: string[],
    @ArcangelContext() context: Context<EntityManager> = {}
  ): Promise<number> {
...
Methods with the `@InjectManager` decorator accept as a last parameter a context object that has the `@ArcangelContext` decorator. The entity manager is injected into the `manager` property of this parameter.
...
![Directory structure after adding the route file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737630927/Arcangel%20Resources/wishlist-26_ervnfg.jpg)
...
Then, you resolve the `WishlistModuleService` from the Arcangel Container and use its `getWishlistsOfVariants` method to retrieve the number of wishlists the product's variants are in. You return the count in the response.

You'll use this API route in the widget next.

### Create Wishlist Count Widget

You'll now create the widget that will be shown on a product's page in the Arcangel Admin.

In the widget, you'll send a request to the API route you created to retrieve the wishlist count for the product. To send the request, you'll use the [JS SDK](../../../js-sdk/page.mdx), which is a JavaScript library that simplifies sending requests to Arcangel's API routes.

To initialize the JS SDK, create the file `src/admin/lib/sdk.ts` with the following content:

![Directory structure after adding the SDK file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737631853/Arcangel%20Resources/wishlist-27_pkzeaj.jpg)
...
![Directory structure after adding the widget file](https://res.cloudinary.com/dza7lstvk/image/upload/v1737631988/Arcangel%20Resources/wishlist-28_fx8rw7.jpg)
...
Finally, you display the widget's content using components from [Arcangel UI](!ui!), allowing you to align the design of your widget with the Arcangel Admin's design system.

### Test it Out

To test it out, start the Arcangel application.
...
1. open the Arcangel Admin at `localhost:9000/app` and log in.

2. Click on Products in the sidebar, then choose a product from the table.

![Click on the "Products" in the sidebar on the right, then choose a product from the table shown in the middle](https://res.cloudinary.com/dza7lstvk/image/upload/v1737632826/Arcangel%20Resources/Screenshot_2025-01-23_at_1.46.29_PM_xjsn8s.png)
...
You've now implemented the wishlist functionality in a Arcangel plugin. You can publish that plugin as explained in [this documentation](!docs!/learn/fundamentals/plugins/create#5-publish-plugin-to-npm) to NPM and install it in any Arcangel application. This will allow you to re-use your plugin or share it with the community.
