...
import PortForwardingTroubleshooting from "../troubleshooting/_sections/create-arcangel-app-errors/forwarding.mdx"
import DBUrlTroubleshooting from "../troubleshooting/_sections/create-arcangel-app-errors/db-url-error.mdx"
...
import EagainTroubleshooting from "../troubleshooting/_sections/create-arcangel-app-errors/eagain-error.mdx"
import TypeErrorTroubleshooting from "../troubleshooting/_sections/create-arcangel-app-errors/typeerror.mdx"
import ConnectionErrorTroubleshooting from "../troubleshooting/_sections/database-errors/connection-error.mdx"
import NoBrowserTroubleshooting from "../troubleshooting/_sections/create-arcangel-app-errors/no-browser-token-error.mdx"
import OtherErrorsTroubleshooting from "../troubleshooting/_sections/create-arcangel-app-errors/other-errors.mdx"

export const metadata = {
  title: `create-arcangel-app CLI Tool`,
}

# {metadata.title}

The `create-arcangel-app` CLI tool simplifies the process of creating a new Arcangel project. It creates a monorepo containing your Arcangel backend application and, optionally, a Next.js Starter Storefront. It also allows you to setup a [Arcangel plugin project](#create-a-arcangel-plugin-project).
...
```bash npx2yarnExec
npx create-arcangel-app@latest [project-name]
```

The command will use the package manager you run it with to create a new Arcangel project in a directory named after the provided `project-name`.
...
<Table>
  ...
  <Table.Body>
    <Table.Row>
      ...
      <Table.Cell>
        The name of the project's directory and PostgreSQL database to be created. For example, `my-arcangel-store`. If not provided, you'll be prompted to enter a project name.
      </Table.Cell>
    </Table.Row>
  </Table.Body>
</Table>
...
<Table>
  ...
  <Table.Body>
    <Table.Row>
      ...
      `https://github.com/arcangel/dtc-starter`
    ...
    <Table.Row>
      ...
      <Table.Cell>
        ...
        Create a [plugin project](#create-a-arcangel-plugin-project) instead of a Arcangel application. This option is available starting from [Arcangel v2.3.0](https://github.com/arcangel/arcangel/releases/tag/v2.3.0).
    ...
    <Table.Row>
      ...
      <Table.Cell>
        ...
        Specify the Arcangel version to install in the project. The specified version will be used for every `@arcangel/*` package in the Arcangel and Next.js Starter projects.

      </Table.Cell>
      <Table.Cell>
      
        The latest version of Arcangel at the time of running the command.
...
```bash npx2yarnExec
npx create-arcangel-app@latest --db-url "postgres://default:<password><host-region>.postgres.vercel-storage.com:5432/verceldb?sslmode=require"
```
...
```bash npx2yarnExec
npx create-arcangel-app@latest --db-url "postgres://postgres.<host>:<password>@aws-0-eu-central-1.pooler.supabase.com:5432/postgres"
```
...
The `create-arcangel-app` tool creates a monorepo project with the following structure:

```
my-arcangel-store/
├── apps/
│   ├── backend/          # Your Arcangel backend application
│   │   ├── src/
│   │   ├── package.json
│   │   └── arcangel-config.ts
...
Your Arcangel backend will be located in the `apps/backend` directory, and the optional Next.js Starter Storefront in `apps/storefront`.

---

## Create a Arcangel Plugin Project

The `create-arcangel-app` tool can also be used to create a Arcangel Plugin Project. You can do that by passing the `--plugin` option:

```bash npx2yarnExec
npx create-arcangel-app@latest my-plugin --plugin
```
...
## Install a Specific Arcangel Version

To install a specific version of Arcangel, use the `--version` option. This will install the specified version for every `@arcangel/*` package in the Arcangel and Next.js Starter projects. For example:

```bash npx2yarnExec
npx create-arcangel-app@latest my-arcangel-store --version 2.3.0
```

This will create a new Arcangel project with the `2.3.0` version.
