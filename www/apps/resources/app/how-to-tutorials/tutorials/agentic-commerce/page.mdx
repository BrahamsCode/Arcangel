...
In this tutorial, you'll learn how to implement [Agentic Commerce](https://developers.openai.com/commerce) specifications in Arcangel that allow you to sell through ChatGPT.

When you install a Arcangel application, you get a fully-fledged commerce platform with the Framework for customization. The Arcangel application's commerce features are built around [Commerce Modules](../../../commerce-modules/page.mdx), which are available out-of-the-box.

The [Agentic Commerce Protocol](https://developers.openai.com/commerce) supports instant checkout experiences within AI agents. By implementing Agentic Commerce specifications in your Arcangel application, customers can purchase products through ChatGPT and other AI agents.
...
![Diagram showing the Agentic Commerce integration between user, ChatGPT, and Arcangel application](https://res.cloudinary.com/dza7lstvk/image/upload/v1759333742/Arcangel%20Resources/agentic-commerce_jqr5wn.jpg)

<CardList items={[
  {
    href: "https://github.com/arcangel/examples/tree/main/agentic-commerce",
    title: "Full Code",
...
Begin by installing the Arcangel application on your machine with the following command:
...
After that, the installation process will begin. This will install the Arcangel application as a monorepository in a directory with your project's name. The backend is installed in the `apps/backend` directory. If you chose to install the Next.js Starter Storefront, it'll be in the `apps/storefront` directory.

Once the installation finishes successfully, the Arcangel Admin dashboard will open with a form to create a new user. Enter the user's credentials and submit the form. Then, you can log in with the new user and explore the dashboard.
...
A module is created under the `src/modules` directory of your Arcangel application. So, create the directory `src/modules/agentic-commerce`.
...
- The module's options that you'll later pass when registering the module in the Arcangel application. You can add more options based on your integration.
...
Next, you'll add the `getSignature` method to the service. This method generates a base64 signature for a payload in the same format that AI agents use for inbound requests, which is useful to test the checkout APIs locally.
...
This method receives a payload and generates a signature using HMAC-SHA256 with the module's `signatureKey` option.
...
This type defines the structure of webhook events that you can send to AI agents based on the [ACP `2026-04-17` webhook schema](https://github.com/agentic-commerce-protocol/agentic-commerce-protocol/blob/main/spec/2026-04-17/openapi/openapi.agentic_checkout_webhook.yaml).
...
Outbound webhook events use a different signature format than inbound requests. This method builds the `Merchant-Signature` header value that the ACP `2026-04-17` release expects, which is a hexadecimal HMAC-SHA256 digest of the timestamp and the raw request body, joined by a period.

<Note title="Tip">

Sign the exact raw body string that you send in the request, not a re-serialized copy of the payload. Any difference in key order or whitespace produces a different digest that the receiver can't verify.

</Note>

Finally, add the following method to the `AgenticCommerceService` class:
...
export default class AgenticCommerceService {
  ...
  async sendWebhookEvent({
    ...
    console.log(
      `Prepared order webhook event ${type} with Merchant-Signature ${signature}`
    )
  }
}
```

This method receives webhook event types and data, serializes the payload once, signs that exact string using the `getWebhookSignature` method, and logs events to the console.
...
The final piece of a module is its definition, which you export in an `index.ts` file at the root directory. This definition tells Arcangel the module name and its service.
...
### d. Add Module to Arcangel's Configurations
...
import { getVariantAvailability, QueryContext } from "@arcangel/framework/utils"
import { CalculatedPriceSet, ShippingOptionDTO } from "@arcangel/framework/types"
    ...
    for (const product of products) {
      ...
      for (const variant of product.variants) {
        ...
        feedItems.push({
          ...
          size: size || undefined,
          seller_name: "Arcangel", // TODO add seller name if supported
    ...
    const itemsXml = input.items.map((item) => {
      return (
        `<item>` +
          ...
          `<brand>${escape(item.brand || "Arcangel")}</brand>` +
...
If you get a type error on resolving the Agentic Commerce Module, run the Arcangel application once with the `npm run dev` or `yarn dev` command to generate the necessary type definitions, as explained in the [Automatically Generated Types guide](!docs!/learn/fundamentals/generated-types).
...
In the step, you resolve the Agentic Commerce Module's service from the Arcangel container and call its `sendProductFeed` method to send the product feed to the AI agent.
...
Next, you'll create a [scheduled job](!docs!/learn/fundamentals/scheduled-jobs) that executes the `sendProductFeedWorkflow` every fifteen minutes. A scheduled job is an asynchronous function that the Arcangel application runs at the interval you specify during the Arcangel application's runtime.
...
export default async function syncProductFeed(container: ArcangelContainer) {
...
1. An asynchronous function that holds the job's logic. The function receives the [Arcangel container](!docs!/learn/fundamentals/arcangel-container) as a parameter.
...
In the scheduled job function, you use Query to retrieve regions in your Arcangel application, including their countries and currency codes.
...
To use the scheduled job, start the Arcangel application with the following command:
...
You'll implement the `POST /checkout_sessions` API route for creating checkout sessions. AI agents call this endpoint when customers want to purchase products. This is equivalent to creating a new cart in Arcangel.
  ...
  workflow={{
    ...
    steps: [
      ...
      },
      {
        ...
        link: "/references/arcangel-workflows/listShippingOptionsForCartWithPricingWorkflow",
        depth: 2
      }
    ]
  }}
...
These steps and workflows are available in Arcangel out-of-the-box. So, you can implement the workflow without creating custom steps.
...
- `cart_id`: Cart ID in Arcangel, which is also the checkout session ID.
...
const responseData = transform({
  ...
  return {
    ...
    links: [
      {
        type: "terms_of_use",
        value: "https://www.arcangel-commerce.com/terms-of-use", // TODO: replace with actual terms of use
      },
      {
        type: "privacy_policy",
        value: "https://www.arcangel-commerce.com/privacy-policy", // TODO: replace with actual privacy policy
      },
      {
        type: "seller_shop_policy",
        value: "https://www.arcangel-commerce.com/seller-shop-policy", // TODO: replace with actual seller shop policy
      },
    ],
  }
})
  ...
  workflow={{
    ...
    steps: [
        ...
        steps: [
          {
            ...
            link: "/references/arcangel-workflows/createCustomersWorkflow",
            depth: 1
          }
        ],
        depth: 6
      },
      {
        ...
        link: "/references/arcangel-workflows/createCartWorkflow",
        depth: 7
      },
      {
        type: "when",
        ...
        steps: [
          ...
          },
          {
            ...
            link: "/references/arcangel-workflows/addShippingMethodToCartWorkflow",
            depth: 3
          },
          {
            type: "step",
...
- Retrieve the shipping options using the [listShippingOptionsForCartWithPricingWorkflow](/references/arcangel-workflows/listShippingOptionsForCartWithPricingWorkflow).
...
- Add the cheapest shipping option to the cart using the [addShippingMethodToCartWorkflow](/references/arcangel-workflows/addShippingMethodToCartWorkflow).
...
export const POST = async (
  ...
  try {
    ...
    const arcangelError = error as ArcangelError
    logger.error(arcangelError)
...
import { ArcangelNextFunction, ArcangelRequest, ArcangelResponse } from "@arcangel/framework"
import { AGENTIC_COMMERCE_MODULE } from "../../modules/agentic-commerce"
...
The headers are expected based on [Agentic Commerce specifications](https://developers.openai.com/commerce/specs/checkout#rest-endpoints). You can create an API key that AI agents can use in the [Secret API Key Settings](!user-guide!/settings/developer/secret-api-keys) of the Arcangel Admin dashboard.
...
- Create an API key in the [Secret API Key Settings](!user-guide!/settings/developer/secret-api-keys) of the Arcangel Admin dashboard.
...
Then, start the Arcangel application with the following command:
...
Make sure you [have a region with the US added to its countries](!user-guide!/settings/regions#create-region) in your Arcangel store.
...
- `{your_api_key}` with the API key you created in the Arcangel Admin dashboard.
  ...
  workflow={{
    ...
    steps: [
      ...
      },
      {
        ...
        link: "/references/arcangel-workflows/updateCartWorkflow",
        depth: 6
      },
      {
        type: "when",
...
import { refreshPaymentCollectionForCartWorkflow } from "@arcangel/arcangel/core-flows"
      ...
      .run({
        input: {
          ...
          messages: [
            {
              type: "error",
              code: arcangelError.type === ArcangelError.Types.PAYMENT_AUTHORIZATION_ERROR ? 
...
- `{api_key}` with the API key you created in the Arcangel Admin dashboard.
...
export const GET = async (
  ...
  try {
    ...
    const arcangelError = error as ArcangelError
    const statusCode = arcangelError.type === ArcangelError.Types.NOT_FOUND ? 404 : 500
        ...
        steps: [
          {
            ...
            link: "/references/arcangel-workflows/createPaymentCollectionForCartWorkflow",
            depth: 1
          },
          {
            type: "workflow",
            ...
            link: "/references/arcangel-workflows/createPaymentSessionsWorkflow",
            depth: 2
          },
          {
            type: "workflow",
...
You use `transform` to prepare the input for the `prepareCheckoutSessionDataWorkflow` workflow and to map the payment provider from the input to the payment provider ID used in your Arcangel store.

Then, you use the `when` function to check if the payment provider is valid for the cart's region. If so, you:

- Create a payment collection for the cart using the [createPaymentCollectionForCartWorkflow](/references/arcangel-workflows/createPaymentCollectionForCartWorkflow).
- Create payment sessions in the payment collection using the [createPaymentSessionsWorkflow](/references/arcangel-workflows/createPaymentSessionsWorkflow).
- Complete the cart using the [completeCartWorkflow](/references/arcangel-workflows/completeCartWorkflow).
...
To support payments with Stripe, you'll need to set up the [Stripe Payment Module Provider](../../../commerce-modules/payment/payment-provider/stripe/page.mdx) in your Arcangel store.

In `arcangel-config.ts`, add a new entry to the `modules` array:
  ...
  modules: [
    // ...
    {
      ...
      options: {
        providers: [
          {
            resolve: "@arcangel/arcangel/payment-stripe",
            id: "stripe",
            options: {
...
This will add Stripe as a payment provider in your Arcangel store.
...
You only need to implement the `validateCartCancelationStep` and `cancelPaymentSessionsStep` steps. The other steps and workflows are available in Arcangel out-of-the-box.
...
import { CartDTO, OrderDTO, PaymentCollectionDTO } from "@arcangel/framework/types"
import { ArcangelError } from "@arcangel/framework/utils"
...
import { acquireLockStep, releaseLockStep, updateCartWorkflow, useQueryGraphStep } from "@arcangel/arcangel/core-flows"
...
export default async function orderWebhookHandler({
  ...
  if (!storefrontUrl) {
    throw new Error(
      "A storefront URL is required to send ACP order webhooks"
    )
  }
  ...
  // set status based on order and fulfillments
  const fulfillments = (order.fulfillments || []).filter(
    (fulfillment) => !!fulfillment
  )
  let status: AgenticCommerceWebhookEvent["data"]["status"] =
    "confirmed"
  ...
  if (order.status === "canceled") {
    ...
    fulfillments.every((f) => !!f.delivered_at)
  ) {
    status = "completed"
  } else if (
    fulfillments.length > 0 &&
    fulfillments.every((f) => !!f.shipped_at)
  ) {
    status = "shipped"
  }
...
The subscriber function receives an object as a parameter that has a `container` property, which is the [Arcangel container](!docs!/learn/fundamentals/arcangel-container).
...
- Map the order's status and fulfillments to an ACP order status. You check the `completed` case before the `shipped` case, since delivered fulfillments are also shipped. You also require at least one fulfillment, because `Array.prototype.every` returns `true` for an empty array and would otherwise report an unfulfilled order as shipped.
- Map refund transactions to the `adjustments` array. Arcangel stores refunds as negative amounts, so you use the absolute value.
- Prepare webhook event payloads based on order data, following the [ACP `2026-04-17` webhook schema](https://github.com/agentic-commerce-protocol/agentic-commerce-protocol/blob/main/spec/2026-04-17/openapi/openapi.agentic_checkout_webhook.yaml).
- Send webhook events to AI agents using the `sendWebhookEvent` method of the Agentic Commerce Module's service.

<Note title="Tip">

Extract the payload-building logic into an exported function, such as `buildOrderWebhookEvent`, if you want to unit test the status and adjustment mapping without emitting events. You can find an example in the [tutorial's repository](https://github.com/arcangel/examples/tree/main/agentic-commerce).
...
4. Check the logs of your Arcangel server to see that `order.placed` events were emitted and webhook events were sent to AI agents.

---

## Next Steps

You've now built Agentic Commerce integration in your Arcangel store. You can use it once you apply to ChatGPT's [Instant Checkout](https://chatgpt.com/merchants) and set up the integration in Instant Checkout settings.

If you're new to Arcangel, check out the [main documentation](!docs!/learn), where you'll get more in-depth understanding of all concepts used in this guide and more.

To learn more about commerce features that Arcangel provides, check out Arcangel's [Commerce Modules](../../../commerce-modules/page.mdx).
