...
import { Github, PlaySolid, EllipsisHorizontal } from "@arcangel/icons"
import { Prerequisites, WorkflowDiagram, CardList, InlineIcon } from "docs-ui"

export const metadata = {
  title: `Implement Product Rentals in Arcangel`,
}

# {metadata.title}

In this tutorial, you'll learn how to implement product rentals in Arcangel.
...
![Diagram of the Rentals Module and its connection with Arcangel's Product, Order, and Customer Modules](https://res.cloudinary.com/dza7lstvk/image/upload/v1761722702/Arcangel%20Resources/product-rentals-summary_rhjwjn.jpg)

<CardList items={[
  {
    href: "https://github.com/arcangel/examples/tree/main/product-rentals",
    title: "Full Code",
...
Afterward, the installation process will start, which will install the Arcangel application as a monorepository in a directory with your project's name. The backend is installed in the `apps/backend` directory, and the Next.js Starter Storefront is installed in the `apps/storefront` directory.
...
In Arcangel, you can build custom features in a [module](!docs!/learn/fundamentals/modules). A module is a reusable package with the data models and functionalities related to a single feature or domain. Arcangel integrates the module into your application without implications or side effects on your setup.
...
- `product_id`: The ID of the Arcangel product associated with the rental configuration.
...
Notice that you'll handle pricing and inventory through Arcangel's existing [Product](../../../commerce-modules/product/page.mdx) and [Inventory](../../../commerce-modules/inventory/page.mdx) modules.
...
- `id`: The primary key of the table.
- `variant_id`: The ID of the Arcangel product variant being rented.
- `customer_id`: The ID of the customer renting the product.
- `order_id`: The ID of the Arcangel order associated with the rental.
- `line_item_id`: The ID of the Arcangel line item associated with the rental.
...
class RentalModuleService extends ArcangelService({
  Rental,
  RentalConfiguration,
}) {
...
The `RentalModuleService` extends `ArcangelService`, which generates a class with data-management methods for your module's data models. This saves you time on implementing Create, Read, Update, and Delete (CRUD) methods.

So, the `RentalModuleService` class now has methods like `createRentals` and `retrieveRentalConfiguration`.

<Note>

Find all methods generated by the `ArcangelService` in [the Service Factory](../../../service-factory-reference/page.mdx) reference.
...
In `arcangel-config.ts`, add a `modules` property and pass it an array with your custom module:
...
Arcangel's CLI tool can generate the migrations for you. To generate a migration for the Rental Module, run the following command in the `apps/backend` directory:

```bash npx2yarn
npx arcangel db:generate rental
```

The `db:generate` command of the Arcangel CLI accepts the name of the module to generate the migration for. You'll now have a `migrations` directory under `src/modules/rental` that holds the generated migration.
...
In this step, you'll define links between the data models in the Rental Module and the data models in Arcangel's modules:
...
To sync the links to the database, run the migrations command again in the `apps/backend` directory:
...
In this step, you'll implement the logic to create or update a rental configuration for a product. Later, you'll execute this logic from an API route, and allow admin users to manage rental configurations from the Arcangel Admin dashboard.
...
Arcangel provides the `useQueryGraphStep` and `createRemoteLinkStep` out-of-the-box. So, you only need to implement the other steps.
...
In the step function, you resolve the Rental Module's service from the Arcangel container, and use it to create a rental configuration.
...
In this step, you'll create API routes that allow you to retrieve and manage rental configurations. Later, you'll use these API routes in the Arcangel Admin dashboard to allow admin users to manage rental configurations.
...
export const POST = async (
  req: ArcangelRequest<z.infer<typeof PostRentalConfigBodySchema>>,
...
In the API route handler, you execute the `upsertRentalConfigWorkflow` by invoking it, passing it the Arcangel container from the request's scope. Then, you call its `run` method, passing the workflow's input from the request's parameters and validated body.
...
You expose a `GET` API route at `/admin/products/:id/rental-config`. In the route handler, you resolve [Query](!docs!/learn/fundamentals/module-links/query) from the Arcangel container and use it to query the rental configuration for the product.

Then, you return the rental configuration in the response.

In the next step, you'll consume these API routes in the Arcangel Admin dashboard.

---

## Step 6: Manage Rental Configurations in Arcangel Admin

In this step, you'll customize the Arcangel Admin dashboard to allow admin users to manage rental configurations for products.
...
export const sdkHighlights = [
  ["3", "new Arcangel", "Initialize the Arcangel JS SDK."],
...
export const sdk = new Arcangel({
  baseUrl: process.env.ARCANGEL_BACKEND_URL || "http://localhost:9000",
...
import { DetailWidgetProps, AdminProduct } from "@arcangel/framework/types"
...
- A `config` object created with `defineWidgetConfig` from the Admin SDK. It accepts an object with the `zone` property that indicates where the widget will be rendered in the Arcangel Admin dashboard.
...
2. `confirm` to show confirmation prompts using [Arcangel's UI package](!ui!/components/prompt).
...
You can now test the rental configuration widget in the Arcangel Admin dashboard.

Run the following command in the `apps/backend` directory to start the Arcangel server:

```bash npm2yarn
npm run dev
```

Then, open the Arcangel Admin dashboard in your browser at `http://localhost:9000/app` and login with the user you created in the first step.
...
![Rental Configuration Widget](https://res.cloudinary.com/dza7lstvk/image/upload/v1761650341/Arcangel%20Resources/CleanShot_2025-10-28_at_13.17.54_2x_ip1stl.png)
...
![Rental configuration form with minimum and maximum rental days fields](https://res.cloudinary.com/dza7lstvk/image/upload/v1761650449/Arcangel%20Resources/CleanShot_2025-10-28_at_13.20.02_2x_zvixfc.png)
...
![Rental configuration details in the widget](https://res.cloudinary.com/dza7lstvk/image/upload/v1761650526/Arcangel%20Resources/CleanShot_2025-10-28_at_13.21.43_2x_t7ktgq.png)
...
If any validation fails, you throw a `ArcangelError` with the `INVALID_DATA` type.
...
import { ArcangelError, QueryContext } from "@arcangel/framework/utils"
import { z } from "@arcangel/framework/zod"
import { RENTAL_MODULE } from "../../../../../modules/rental"
import RentalModuleService from "../../../../../modules/rental/service"
...
export const GET = async (
  req: ArcangelRequest<{}, z.infer<typeof GetRentalAvailabilitySchema>>, 
  res: ArcangelResponse
) => {
...
First, run the following command in the `apps/backend` directory to start the Arcangel server:
...
![Rental options on product details page in the storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1761653559/Arcangel%20Resources/CleanShot_2025-10-28_at_14.12.25_2x_g09h85.png)

---

## Step 9: Add Rental Products to Cart

In this step, you'll implement the logic to add rentable products to the cart in the Arcangel application. You'll wrap Arcangel's existing add-to-cart logic to include rental-specific data and validation.
  ...
  workflow={{
    ...
    steps: [
      ...
      },
      {
        ...
        link: "/references/arcangel-workflows/addToCartWorkflow",
        depth: 5,
      },
      {
        type: "step",
...
Arcangel provides all the steps and workflows out-of-the-box, except for the `validateRentalCartItemStep` step, which you'll implement.
...
```ts title="src/utils/has-cart-overlap.ts" badgeLabel="Arcangel Application" badgeColor="green"
...
```ts title="src/workflows/steps/validate-rental-cart-item.ts" badgeLabel="Arcangel Application" badgeColor="green" collapsibleLines="1-8" expandButtonLabel="Show Imports"
import { createStep, StepResponse } from "@arcangel/framework/workflows-sdk"
import { ArcangelError } from "@arcangel/framework/utils"
import { RENTAL_MODULE } from "../../modules/rental"
import RentalModuleService from "../../modules/rental/service"
import { InferTypeOf, ProductVariantDTO } from "@arcangel/framework/types"
...
```ts title="src/workflows/steps/validate-rental-cart-item.ts" badgeLabel="Arcangel Application" badgeColor="green"
// Validate metadata
...
```ts title="src/workflows/add-to-cart-with-rental.ts" badgeLabel="Arcangel Application" badgeColor="green" collapsibleLines="1-18" expandButtonLabel="Show Imports"
import { 
  createWorkflow, 
  WorkflowResponse, 
  transform,
...
export const addToCartWithRentalWorkflow = createWorkflow(
  ...
  (input: AddToCartWorkflowInput) => {
    ...
    const itemToAdd = transform({
      ...
      // For non-rental products, don't specify unit_price (let Arcangel calculate it)
      return [baseItem]
    })
...
5. Prepare the item to add to the cart.
    ...
    - For non-rentable products, you don't specify the `unit_price`; Arcangel will use the variant's price.
...
```ts title="src/api/store/carts/[id]/line-items/rentals/route.ts" badgeLabel="Arcangel Application" badgeColor="green" collapsibleLines="1-9" expandButtonLabel="Show Imports"
import type { 
  ArcangelRequest, 
  ArcangelResponse,
} from "@arcangel/framework/http"
...
export const POST = async (
  req: ArcangelRequest<z.infer<typeof PostCartItemsRentalsBody>>,
  res: ArcangelResponse
) => {
...
First, run both the Arcangel server and the Next.js Starter Storefront.
...
![Rental product added to cart in the storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1761660048/Arcangel%20Resources/CleanShot_2025-10-28_at_15.38.24_2x_yfo50v.png)

---

## Step 11: Create Rental Orders

In this step, you'll implement the logic to create rental orders in the Arcangel application. You'll wrap Arcangel's existing order creation logic to handle rental-specific data and validation.
...
You'll implement the `validateRentalStep` and `createRentalsStep` steps used in the workflow. The rest are provided by Arcangel out-of-the-box.
...
```ts title="src/workflows/steps/validate-rental.ts" badgeLabel="Arcangel Application" badgeColor="green"
import { createStep, StepResponse } from "@arcangel/framework/workflows-sdk"
import { ArcangelError } from "@arcangel/framework/utils"
import { RENTAL_MODULE } from "../../modules/rental"
import RentalModuleService from "../../modules/rental/service"
...
```ts title="src/workflows/steps/create-rentals-for-order.ts" badgeLabel="Arcangel Application" badgeColor="green"
import { createStep, StepResponse } from "@arcangel/framework/workflows-sdk"
import { RENTAL_MODULE } from "../../modules/rental"
import RentalModuleService from "../../modules/rental/service"
...
```ts title="src/workflows/create-rentals.ts" badgeLabel="Arcangel Application" badgeColor="green" collapsibleLines="1-21" expandButtonLabel="Show Imports"
import { 
  createWorkflow, 
  WorkflowResponse, 
  transform,
...
```ts title="src/api/store/rentals/[cart_id]/route.ts" badgeLabel="Arcangel Application" badgeColor="green"
import type { ArcangelRequest, ArcangelResponse } from "@arcangel/framework/http"
...
![Rental order confirmation in the storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1761661870/Arcangel%20Resources/CleanShot_2025-10-28_at_16.30.14_2x_v8wbb7.png)

---

## Step 13: Manage Rentals in Admin

In this step, you'll allow admin users to manage rentals in the Arcangel Admin Dashboard. You will:
...
```ts title="src/api/admin/orders/[id]/rentals/route.ts" badgeLabel="Arcangel Application" badgeColor="green"
import type { ArcangelRequest, ArcangelResponse } from "@arcangel/framework/http"
...
```ts title="src/workflows/steps/update-rental.ts" badgeLabel="Arcangel Application" badgeColor="green"
import { createStep, StepResponse } from "@arcangel/framework/workflows-sdk"
import { RENTAL_MODULE } from "../../modules/rental"
import RentalModuleService from "../../modules/rental/service"
import { ArcangelError } from "@arcangel/framework/utils"
...
```ts title="src/workflows/update-rental.ts" badgeLabel="Arcangel Application" badgeColor="green"
...
```ts title="src/api/admin/rentals/[id]/route.ts" badgeLabel="Arcangel Application" badgeColor="green"
import type { ArcangelRequest, ArcangelResponse } from "@arcangel/framework/http"
...
export const POST = async (
  req: ArcangelRequest<z.infer<typeof PostRentalStatusBodySchema>>,
  res: ArcangelResponse
) => {
  const { id } = req.params
...
```tsx title="src/admin/widgets/order-rental-items.tsx" badgeLabel="Arcangel Application" badgeColor="green" collapsibleLines="1-18" expandButtonLabel="Show Imports"
import { defineWidgetConfig } from "@arcangel/admin-sdk"
import {
  Container,
  Heading,
...
import { useQuery, useMutation } from "@tanstack/react-query"
import { sdk } from "../lib/sdk"
import { DetailWidgetProps, AdminOrder } from "@arcangel/framework/types"
import { useEffect, useState } from "react"
...
```tsx title="src/admin/widgets/order-rental-items.tsx" badgeLabel="Arcangel Application" badgeColor="green"
...
You can now test managing rentals in the Arcangel Admin Dashboard.

First, start the Arcangel application and log in.
...
![Rental items in admin order details page](https://res.cloudinary.com/dza7lstvk/image/upload/v1761662993/Arcangel%20Resources/CleanShot_2025-10-28_at_16.49.29_2x_usufnn.png)

You can click the "Update Status" button for a rental item to open the drawer and edit its status.

![Update rental status drawer in admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1761663069/Arcangel%20Resources/CleanShot_2025-10-28_at_16.50.35_2x_bzmaoq.png)
...
Arcangel Admin users can cancel orders. So, in this step, you'll customize the order cancellation flow to validate that rental items in the order can be cancelled based on their rental status. You'll also update the rental statuses when an order is cancelled.
...
```ts title="src/workflows/hooks/validate-order-cancel.ts" badgeLabel="Arcangel Application" badgeColor="green"
import { cancelOrderWorkflow } from "@arcangel/arcangel/core-flows"
import { ArcangelError, ContainerRegistrationKeys } from "@arcangel/framework/utils"
...
To test the order cancellation validation, try to cancel an order from the Arcangel Admin that has rental items with `returned` status. The order cancellation should fail.

### b. Update Rental Statuses on Order Cancellation

Next, you'll update the rental statuses when an order is cancelled.

When an order is cancelled, Arcangel emits an `order.canceled` event. You can handle this event in a [subscriber](!docs!/learn/fundamentals/events-and-subscribers).
...
```ts title="src/subscribers/order-canceled.ts" badgeLabel="Arcangel Application" badgeColor="green"
...
To test the rental status update on order cancellation, cancel an order from the Arcangel Admin that has rental items with `pending` or `active` status.

Then, refresh the page. You'll see that the rental items' statuses have been updated to `cancelled`.

![Cancelled rentals in admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1761665415/Arcangel%20Resources/CleanShot_2025-10-28_at_17.29.55_2x_apkc5n.png)
...
A scheduled job is an asynchronous function that runs tasks at specific intervals while the Arcangel application is running. You can use scheduled jobs to change a rental's status based on the rental period.
...
```ts title="src/jobs/activate-rentals.ts" badgeLabel="Arcangel Application" badgeColor="green"
import { ArcangelContainer } from "@arcangel/framework/types"
import { ContainerRegistrationKeys } from "@arcangel/framework/utils"
import { updateRentalWorkflow } from "../workflows/update-rental"

export default async function activateRentalsJob(container: ArcangelContainer) {
  const logger = container.resolve("logger")
  const query = container.resolve(ContainerRegistrationKeys.QUERY)
...
Then, start the Arcangel application and wait for a minute. You should see log messages indicating that the job is running and has activated any pending rentals whose start date is today.

### b. Event-Driven Rental Status Updates

You can also update rental statuses based on specific [events](/references/events) in Arcangel.
...
```ts title="src/subscribers/shipment-created.ts" badgeLabel="Arcangel Application" badgeColor="green"
import { SubscriberArgs, type SubscriberConfig } from "@arcangel/framework"
import { ContainerRegistrationKeys } from "@arcangel/framework/utils"
import { updateRentalWorkflow } from "../workflows/update-rental"
...
To test the event-driven rental status update, create a fulfillment for an order that has rental items with `pending` status from the Arcangel Admin. Then, mark the fulfillment as shipped.

If you refresh the order details page, you should see that the rental items' statuses have been updated to `active`.

![Activated rentals after shipment creation in admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1761665929/Arcangel%20Resources/CleanShot_2025-10-28_at_17.38.04_2x_xgwwan.png)
...
1. Removing the associated shipping profile of the product. You can do this from the Arcangel Admin.
...
Arcangel provides optional [inventory management for product variants](../../../commerce-modules/product/variant-inventory/page.mdx). If enabled, Arcangel will increment and decrement inventory levels for product variants when orders are placed, fulfilled, or returned.
...
Arcangel creates a reservation for the inventory of a product variant when an order is placed, and decrements the inventory when the order is fulfilled. So, you don't need to manually adjust inventory when a rental is created or activated.
...
import { updateRentalStep } from "./steps/update-rental"
import { adjustInventoryLevelsStep, useQueryGraphStep } from "@arcangel/arcangel/core-flows"
...
You have now implemented product rentals in your Arcangel application. You can expand on this foundation by adding more features, such as allowing customers to view and manage their rentals from the storefront.
