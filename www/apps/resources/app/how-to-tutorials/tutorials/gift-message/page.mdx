...
export const metadata = {
  title: `Add Gift Message to Line Items in Arcangel`,
}

# {metadata.title}

In this tutorial, you will learn how to add a gift message to items in carts and orders in Arcangel.
...
You can customize the Arcangel application and storefront to add a gift message to items in the cart. This feature allows customers to add a personalized message to their gifts, enhancing the shopping experience.
...
- Customize the Arcangel Admin to show gift items with messages in an order.
...
<Card
  ...
  icon={Github}
  href="https://github.com/arcangel/examples/tree/main/order-gift-message"
...
The Next.js Starter Storefront has an `updateLineItem` function that sends a request to the Arcangel server to update the cart item. However, it doesn't support updating the `metadata` property.
...
You pass the `metadata` property to the Arcangel server, which will update the cart item with the new metadata.
...
import { Checkbox, Textarea, Button, Label } from "@arcangel/ui"
```

You import components from the [Arcangel UI library](!ui!) that will be useful for the gift inputs.
...
First, start the Arcangel application by running the following command in the Arcangel application directory:
...
![Gift checkbox on checkout page](https://res.cloudinary.com/dza7lstvk/image/upload/v1750923117/Arcangel%20Resources/CleanShot_2025-06-26_at_10.31.25_2x_wcqiff.png)
...
![Gift item with message added](https://res.cloudinary.com/dza7lstvk/image/upload/v1750924150/Arcangel%20Resources/CleanShot_2025-06-26_at_10.32.52_2x_qir2t5.png)
...
![Gift message on order confirmation page](https://res.cloudinary.com/dza7lstvk/image/upload/v1750924429/Arcangel%20Resources/CleanShot_2025-06-26_at_10.53.32_2x_htzvtw.png)

---

## Step 4: Show Gift Options in Admin Dashboard

In this step, you'll customize the Arcangel Admin dashboard to show the gift items with their messages in an order.

### What is a Widget?

The Arcangel Admin dashboard's pages are customizable to insert [widgets](!docs!/learn/fundamentals/admin/widgets) of custom content in predefined injection zones. You create these widgets as React components that allow admin users to perform custom actions.
...
You create a widget in a `.tsx` file under the `src/admin/widgets` directory. So, in the Arcangel application directory, create the file `src/admin/widgets/order-gift-items-widget.tsx` with the following content:

```tsx title="src/admin/widgets/order-gift-items-widget.tsx" badgeLabel="Arcangel Application" badgeColor="green"
...
### Test Arcangel Admin Changes

To test out the Arcangel Admin widget, start the Arcangel application by running the following command in the Arcangel application directory:

```bash npm2yarn badgeLabel="Arcangel Application" badgeColor="green"
npm run dev
```

Then, open the Arcangel Admin at `http://localhost:9000/app` and log in with the user you created earlier.
...
![Order details page in the Arcangel Admin with the "Gift Items & Messages" section highlighted](https://res.cloudinary.com/dza7lstvk/image/upload/v1750925501/Arcangel%20Resources/CleanShot_2025-06-26_at_11.11.18_2x_jrrc2s.png)
...
```ts title="Fulfillment Module Provider's Service" badgeLabel="Arcangel Application" badgeColor="green" highlights={createFulfillmentHighlights}
...
You've now added gift messages to items in carts and orders in Arcangel.
