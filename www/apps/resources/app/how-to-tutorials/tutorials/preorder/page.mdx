...
export const metadata = {
  title: `Implement Pre-Orders in Arcangel`,
}

# {metadata.title}

In this tutorial, you'll learn how to implement pre-orders in Arcangel.
...
This tutorial will help you implement pre-orders in your Arcangel application.
...
- Customize the Arcangel Admin dashboard to allow merchants to:
...
![Diagram showcasing the pre-order features from this tutorial](https://res.cloudinary.com/dza7lstvk/image/upload/v1752756906/Arcangel%20Resources/pre-order-summary_epvssb.jpg)

<CardList items={[
  {
    href: "https://github.com/arcangel/examples/tree/main/preorders",
...
- `variant_id`: The ID of the product variant that this pre-order variant configurations applies to.
    - Later, you'll learn how to link this data model to Arcangel's `ProductVariant` data model.
...
- `id`: The primary key of the table.
- `order_id`: The ID of the Arcangel order that this pre-order belongs to.
    - Later, you'll learn how to link this data model to Arcangel's `Order` data model.
...
export default class PreorderModuleService extends ArcangelService({
  PreorderVariant,
  Preorder,
}) {}
```

The `PreorderModuleService` extends `ArcangelService`, which generates a class with data-management methods for your module's data models. This saves you time on implementing Create, Read, Update, and Delete (CRUD) methods.
...
Arcangel's CLI tool can generate the migrations for you. To generate a migration for the Preorder Module, run the following command in the `apps/backend` directory:

```bash npx2yarn
npx arcangel db:generate preorder
```

The `db:generate` command of the Arcangel CLI accepts the name of the module to generate the migration for. You'll now have a `migrations` directory under `src/modules/preorder` that holds the generated migration.
...
## Step 3: Link Preorder and Arcangel Data Models
...
In this step, you'll define a link between the data models in the Preorder Module and the data models in Arcangel's Commerce Modules.
...
Next, you'll define a read-only link between the `Preorder` data model and Arcangel's `Order` data model. Read-only links allow you to retrieve the order that a pre-order belongs to without the need to manage the link in the database.
...
When you build commerce features in Arcangel that can be consumed by client applications, such as the Arcangel Admin dashboard or a storefront, you need to implement:
...
The `useQueryGraphStep` and `createRemoteLinkStep` are available through Arcangel's `@arcangel/arcangel/core-flows` package. You'll implement other steps in the workflow.
...
If you get a type error on resolving the Preorder Module, run the Arcangel application once with the `npm run dev` or `yarn dev` command to generate the necessary type definitions, as explained in the [Automatically Generated Types guide](!docs!/learn/fundamentals/generated-types).

</Note>

export const updatePreorderVariantStepHighlights = [
  ["17", "preorderModuleService", "Resolve the Preorder Module's service from the Arcangel container."],
...
In the step function, you resolve the Preorder Module's service from the Arcangel container using its `resolve` method, passing it the module's name as a parameter.
...
import { createRemoteLinkStep, useQueryGraphStep } from "@arcangel/arcangel/core-flows"
...
export const POST = async (
  req: AuthenticatedArcangelRequest<UpsertPreorderVariantSchema>,
...
You apply Arcangel's `validateAndTransformBody` middleware to `POST` requests sent to the `/admin/variants/:id/preorders` API route.
...
You'll test out this functionality when you customize the Arcangel Admin in the next step.

---

## Step 5: Add Widget in Product Variant Admin Page

In this step, you'll customize the Arcangel Admin to allow admin users to manage pre-order configurations of product variants.
...
To send requests to the Arcangel server with support for caching and refetching capabilities, you'll wrap JS SDK methods with [Tanstack (React) Query](https://tanstack.com/query/latest).
...
- `preorderVariant`: The pre-order variant. It's retrieved from Arcangel's [Retrieve Variant](!api!/admin/products/get-variant) API route, which allows expanding linked records using the `fields` query parameter.
...
import { Calendar, EllipsisHorizontal, Pencil, Plus, XCircle } from "@arcangel/icons"
...
export const config = defineWidgetConfig({
  zone: "product_variant.details.side",
})
...
To show the pre-order configuration form, you use the [Drawer](!ui!/components/drawer) component from Arcangel UI.

### Test the Customizations

You can now test the customizations you made in the Arcangel server and admin dashboard.

Start the Arcangel application with the following command:

```bash npm2yarn
npm run dev
```

Then, open the Arcangel Admin dashboard at `http://localhost:9000/app` and log in.

Go to any product, then click on one of its variants. You'll find a new "Pre-order" section in the side column.

![Pre-order variant widget in the Arcangel Admin dashboard](https://res.cloudinary.com/dza7lstvk/image/upload/v1752735553/Arcangel%20Resources/CleanShot_2025-07-17_at_09.52.30_2x_ftojti.png)
...
![Pre-order variant widget showing the pre-order configuration](https://res.cloudinary.com/dza7lstvk/image/upload/v1752735733/Arcangel%20Resources/CleanShot_2025-07-17_at_10.01.03_2x_bdvzo9.png)
...
In this step, you'll wrap custom logic around Arcangel's cart completion logic in a workflow, then execute that workflow in a custom API route.
...
import { CartLineItemDTO, ProductVariantDTO } from "@arcangel/framework/types"
import { createStep, StepResponse } from "@arcangel/framework/workflows-sdk"
...
The step function receives the ID of the Arcangel order and the IDs of the pre-order variants in the cart.

In the step function, you create a `Preorder` record for each pre-order variant. You set the `order_id` of the `Preorder` record to the ID of the Arcangel order.
...
export const completeCartPreorderWorkflowHighlights = [
  ...
  ["94", "order", "Return the created Arcangel order."],
]
...
1. Acquire a lock on the cart using the [acquireLockStep](/references/arcangel-workflows/steps/acquireLockStep) to prevent concurrent modifications.
2. Complete the cart using the [completeCartWorkflow](/references/arcangel-workflows/completeCartWorkflow) as a step. This is Arcangel's cart completion logic.
...
8. Release the lock on the cart using the [releaseLockStep](/references/arcangel-workflows/steps/releaseLockStep).
9. Return the created Arcangel order.
...
In those cases, you can perform custom logic within Arcangel's add-to-cart logic to validate an item before it's added to the cart.
...
export const validateCartHookHighlights = [
  ...
  ["55", "ArcangelError", "Throw an error if the cart has mixed pre-order and available items."],
]
...
import { StorePrice } from "@arcangel/types"
```
...
First, you'll retrieve the pre-order variant information along with the product variant from the Arcangel server.
...
To test out the changes to the product page, start the Arcangel application with the following command:
...
![Pre-order product variant in the Next.js Starter Storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1752739520/Arcangel%20Resources/CleanShot_2025-07-17_at_11.04.56_2x_fefjo6.png)
...
To test the changes to the cart page, ensure that both the Arcangel and Next.js applications are running.
...
![Pre-order item in the cart page of the Next.js Starter Storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1752740244/Arcangel%20Resources/CleanShot_2025-07-17_at_11.17.04_2x_u4opzf.png)
...
To test the cart completion with pre-order items, ensure that both the Arcangel and Next.js applications are running.
...
To test the changes to the order confirmation page, ensure that both the Arcangel and Next.js applications are running.
...
![Pre-order item in the order confirmation page of the Next.js Starter Storefront](https://res.cloudinary.com/dza7lstvk/image/upload/v1752741220/Arcangel%20Resources/CleanShot_2025-07-17_at_11.33.17_2x_anitlq.png)
...
In this step, you'll customize the Arcangel Admin to show pre-order information in the order details page.
...
import { defineWidgetConfig } from "@arcangel/admin-sdk"
import { DetailWidgetProps, HttpTypes } from "@arcangel/framework/types"
...
The `PreordersWidget` will be injected into the side column of the order details page in the Arcangel Admin dashboard.
...
To test the pre-order widget, ensure that the Arcangel application is running.

Then, in the Arcangel Admin dashboard, go to any order that has pre-order items. You should see the "Pre-orders" section in the side column with the pre-order information.

![Pre-orders Section in the Arcangel Admin dashboard](https://res.cloudinary.com/dza7lstvk/image/upload/v1752741736/Arcangel%20Resources/CleanShot_2025-07-17_at_11.41.55_2x_w8aiw2.png)
    ...
    steps: [
      {
        type: "step",
        name: "useQueryGraphStep",
        description: "Retrieve the Arcangel order details.",
        link: "/references/helper-steps/useQueryGraphStep",
        depth: 1
      },
      {
      ...
      },
      {
        ...
        description: "Create a fulfillment for the Arcangel order.",
        link: "/references/arcangel-workflows/createOrderFulfillmentWorkflow",
        depth: 3
      },
      {
        type: "step",
...
export const retrieveItemsToFulfillHighlights = [
  ...
  ["7", "line_items", "The line items in the Arcangel order."],
...
import { InferTypeOf, OrderLineItemDTO } from "@arcangel/framework/types"
import { createStep, StepResponse } from "@arcangel/framework/workflows-sdk"
import { PreorderVariant } from "../../modules/preorder/models/preorder-variant"
...
The step receives the pre-order variant to be fulfilled as an input, and the line items in the Arcangel order.

In the step, you find the items in the Arcangel order that are associated with the pre-order variant and return them.
...
export const fulfillPreorderWorkflowHighlights = [
  ...
  ["12", "order_id", "The ID of the Arcangel order containing the pre-ordered variant."],
  ["18", "useQueryGraphStep", "Retrieve the Arcangel order."],
  ["29", "retrieveItemsToFulfillStep", "Retrieve the items to fulfill in the order."],
  ["34", "createOrderFulfillmentWorkflow.runAsStep", "Create a fulfillment for the Arcangel order."],
...
import { PreorderVariant } from "../modules/preorder/models/preorder-variant"
import { createWorkflow, WorkflowResponse } from "@arcangel/framework/workflows-sdk"
import { createOrderFulfillmentWorkflow, emitEventStep, useQueryGraphStep } from "@arcangel/arcangel/core-flows"
...
- `order_id`: The ID of the Arcangel order containing the pre-ordered variant.

In the workflow, you:

- Retrieve the Arcangel order using the [useQueryGraphStep](/references/helper-steps/useQueryGraphStep).
- Retrieve the items to fulfill in the order using the `retrieveItemsToFulfillStep`.
- Create a fulfillment for the Arcangel order using the [createOrderFulfillmentWorkflow](/references/arcangel-workflows/createOrderFulfillmentWorkflow) as a step.
...
By default, Arcangel authorizes an order's payment when it's placed. The admin user can capture the payment later manually.
...
To do that, you can use Arcangel's [capturePaymentWorkflow](/references/arcangel-workflows/capturePaymentWorkflow) to capture the payment for the order in the workflow.
...
import { PreorderVariant } from "../modules/preorder/models/preorder-variant"
import { createWorkflow, transform, when, WorkflowResponse } from "@arcangel/framework/workflows-sdk"
import { capturePaymentWorkflow, createOrderFulfillmentWorkflow, emitEventStep, useQueryGraphStep } from "@arcangel/arcangel/core-flows"
import { retrieveItemsToFulfillStep, RetrieveItemsToFulfillStepInput } from "./steps/retrieve-items-to-fulfill"
import { updatePreordersStep } from "./steps/update-preorders"
import { PreorderStatus } from "../modules/preorder/models/preorder"
...
export default async function fulfillPreordersJob(container: ArcangelContainer) {
  const query = container.resolve("query")
...
So far, you only resolve Query and the Logger utility from the Arcangel container passed as input to the scheduled job.
...
Next, start the Arcangel application:
...
The subscriber receives among its parameters the data payload of the emitted event, which includes the IDs of the Arcangel order and the pre-order variant.
...
import { emitEventStep } from "@arcangel/arcangel/core-flows"
import { updatePreordersStep } from "./steps/update-preorders"
...
To test order cancelation, start the Arcangel application.
...
![Pre-order item's status is canceled in a canceled order](https://res.cloudinary.com/dza7lstvk/image/upload/v1752752393/Arcangel%20Resources/CleanShot_2025-07-17_at_14.38.50_2x_cxqscy.png)
...
import { InferTypeOf, OrderDTO, OrderLineItemDTO, ProductVariantDTO } from "@arcangel/framework/types"
import { PreorderVariant } from "../../modules/preorder/models/preorder-variant"
import { Preorder, PreorderStatus } from "../../modules/preorder/models/preorder"
import { createStep, StepResponse } from "@arcangel/framework/workflows-sdk"
...
import { createWorkflow, transform, WorkflowResponse } from "@arcangel/framework/workflows-sdk"
import { emitEventStep, useQueryGraphStep } from "@arcangel/arcangel/core-flows"
...
import { SubscriberArgs, type SubscriberConfig } from "@arcangel/framework"
import { OrderChangeActionDTO } from "@arcangel/framework/types"
...
To test out the order edit handler, start the Arcangel application.

Then, open an order in the Arcangel Admin. It can be an order you want to remove a pre-order item from, or an order you want to add a new pre-order item to.
...
![Pre-order items updated in an order edit](https://res.cloudinary.com/dza7lstvk/image/upload/v1752754220/Arcangel%20Resources/CleanShot_2025-07-17_at_15.09.55_2x_xkkfef.png)

---

## Next Steps

You've now implemented the pre-order feature in Arcangel. You can expand on this feature based on your use case. You can add features like:
