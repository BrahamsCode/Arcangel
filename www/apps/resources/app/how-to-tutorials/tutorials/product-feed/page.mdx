...
In this tutorial, you'll learn how to create a product feed in Arcangel that can be used for Meta and Google.
...
Businesses that are selling on social media platforms like Meta (Instagram and Facebook) and Google need to upload their product catalog to those platforms and keep them in sync with their Arcangel store. Creating a product feed allows you to automate this process.
...
![Products in the Meta Catalogue](https://res.cloudinary.com/dza7lstvk/image/upload/v1756732400/Arcangel%20Resources/CleanShot_2025-09-01_at_16.12.33_2x_oszhkt.png)

<CardList items={[
  {
    href: "https://github.com/arcangel/examples/tree/main/product-feed",
    title: "Full Code",
    ...
    steps: [
      {
        ...
        description: "Retrieve Arcangel products as items for the feed.",
        depth: 1,
      },
      {
        type: "step",
...
The `getProductFeedItemsStep` will retrieve the Arcangel products with pagination, and format their product variants as items to be added to the feed.
...
import { CalculatedPriceSet } from "@arcangel/framework/types"
...
Then, you resolve the following resources from the Arcangel container:

- [Query](!docs!/learn/fundamentals/module-links/query) that allows you to retrieve data across modules.
- [Arcangel Configurations](!docs!/learn/configurations/arcangel-config) that are defined in `arcangel-config.ts`.

You use the Arcangel configurations to retrieve the storefront URL, which you'll use to build the product links in the feed. If the storefront URL is not set in the configurations, you fall back to the `STOREFRONT_URL` environment variable.
...
export const getProductFeedItemsHighlights3 = [
  ...
  ["17", "hasOriginalPrice", "Check if the product variant is on sale by comparing original and calculated amounts."],
  ["19", "originalPrice", "The regular price of the product variant."],
  ["21", "salePrice", "The sale price of the product variant."],
  ["23", "stockStatus", "The stock status of the product variant."],
  ["39", "item_group_id", "Group the variants by their product."]
]
...
Then, you execute the `generateProductFeedWorkflow` by invoking it, passing it the Arcangel container (which is `req.scope`), and calling its `run` method. You pass the workflow's input to the `run` method.
...
import { defineMiddlewares, validateAndTransformQuery } from "@arcangel/framework/http"
...
To test out the product feed API route, start the Arcangel server with the following command:
...
If your Arcangel application is [deployed](../../../deployment/page.mdx), you can now use the product feed API route on social platforms like [Meta](https://www.facebook.com/business/help/125074381480892?id=725943027795860) and [Google](https://support.google.com/merchants/answer/11586438?hl=en&sjid=6331702527227918188-EU).

<Note>

Make sure to set the [admin.storefrontUrl](!docs!/learn/configurations/arcangel-config#storefronturl) or `STOREFRONT_URL` environment variable before using the product feed API route.
...
![Meta Data Sources Setup Form](https://res.cloudinary.com/dza7lstvk/image/upload/v1756731936/Arcangel%20Resources/CleanShot_2025-09-01_at_16.04.51_2x_kht1lx.png)

5. Choose the "Use a URL or Google Sheets" option, and enter the URL to the product feed API route. For example, `https://your-arcangel-store.com/product-feed?currency_code=eur&country_code=dk`.
...
![Meta Catalogue Products](https://res.cloudinary.com/dza7lstvk/image/upload/v1756732208/Arcangel%20Resources/CleanShot_2025-09-01_at_16.09.52_2x_tbs52f.png)

---

## Next Steps

You've now set up the product feed API route for your Arcangel application. Meta and Google will pull products from this feed periodically, ensuring your product listings are always up to date.
