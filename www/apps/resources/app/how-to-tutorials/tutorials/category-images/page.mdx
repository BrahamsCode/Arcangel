...
In this tutorial, you'll learn how to add images to product categories in Arcangel.
...
Arcangel doesn't natively support adding images to product categories. However, it provides the customization capabilities you need to implement this feature.
...
- Customize the Arcangel Admin dashboard to manage category images.
...
![Diagram showing the relation between product categories and their images](https://res.cloudinary.com/dza7lstvk/image/upload/v1760522310/Arcangel%20Resources/category-images-summary_l1duwj.jpg)

<CardList items={[
  {
    href: "https://github.com/arcangel/examples/tree/main/category-images",
    title: "Full Code",
...
class ProductMediaModuleService extends ArcangelService({
...
The `ProductMediaModuleService` extends `ArcangelService`, which generates a class with data-management methods for your module's data models. This saves you time implementing Create, Read, Update, and Delete (CRUD) methods.
...
The final piece of a module is its definition, which you export in an `index.ts` file at the module's root directory. This definition tells Arcangel the module's name and its service.
...
After building the module, add it to Arcangel's configurations to start using it.

In `arcangel-config.ts`, add a `modules` property with an array containing your custom module:
...
Arcangel's CLI tool can generate migrations for you. To generate a migration for the Product Media Module, run the following command in the `apps/backend` directory:

```bash npx2yarn
npx arcangel db:generate productMedia
```

The `db:generate` command of the Arcangel CLI accepts the name of the module to generate the migration for. You'll now have a `migrations` directory under `src/modules/product-media` that holds the generated migration.
...
When building commerce features in Arcangel that client applications consume, such as the Arcangel Admin dashboard or a storefront, you need to implement:
...
You won't implement the functionality to upload the image, as Arcangel already exposes an API route to upload files. You'll use that route in the Arcangel Admin dashboard to upload images and get their URLs and file IDs.
...
2. An async function that receives two parameters:
    ...
    - An object with properties including the [Arcangel container](!docs!/learn/fundamentals/arcangel-container), which is a registry of Framework and commerce tools accessible in the step.
...
1. Resolve the `ProductMediaModuleService` from the Arcangel container to manage product category images.
...
export const createCategoryImagesStepHighlights = [
  ...
  ["36", "ArcangelError", "Throw an error if more than one thumbnail is being added for a category."],
...
export async function POST(
  req: ArcangelRequest<CreateCategoryImagesInput>,
...
You apply Arcangel's `validateAndTransformBody` middleware to `POST` requests sent to the `/admin/categories/:category_id/images` route. The middleware function accepts a Zod schema that you created in the API route's file.
...
You'll test this API route next when you customize the Arcangel Admin.

---

## Step 5: Create Product Category Images in Arcangel Admin

In this step, you'll customize the Arcangel Admin dashboard to manage a product category's images.
...
import { DetailWidgetProps, AdminProductCategory } from "@arcangel/framework/types"
...
1. `uploadFilesMutation`: A mutation that uploads files using Arcangel's existing API route for uploads. This will upload the images to the [configured File Module Provider](../../../infrastructure-modules/file/page.mdx#what-is-a-file-module-provider).
...
import { FocusModal, Button, Heading, toast } from "@arcangel/ui"
...
You render a modal using the `FocusModal` component from [Arcangel UI](!ui!). The modal displays the `CategoryImageGallery` component on the left and the `CategoryImageUpload` component on the right.
...
You can now test the media widget in the Arcangel Admin dashboard.

Run the following command in the Arcangel project directory to start the Arcangel server:
...
Images are uploaded to the [configured File Module Provider](../../../infrastructure-modules/file/page.mdx#what-is-a-file-module-provider). If you haven't configured one, images will be uploaded to the `static` folder in your Arcangel project.

</Note>

![Media widget showing images with upload form](https://res.cloudinary.com/dza7lstvk/image/upload/v1760455846/Arcangel%20Resources/CleanShot_2025-10-14_at_18.29.49_2x_vhbn6z.png)
...
![Media widget showing images after upload](https://res.cloudinary.com/dza7lstvk/image/upload/v1760456027/Arcangel%20Resources/CleanShot_2025-10-14_at_18.33.27_2x_au7nxg.png)
...
- Updating the Arcangel Admin modal to allow updating image types.
...
Arcangel provides the `useQueryGraphStep`, and you've already created the `convertCategoryThumbnailsStep` in [step 3](#convertcategorythumbnailsstep). You only need to create the `updateCategoryImagesStep`.
...
export async function POST(
  req: ArcangelRequest<UpdateCategoryImagesInput>,
  res: ArcangelResponse
): Promise<void> {
...
```tsx title="src/admin/components/category-media/category-image-item.tsx"
import { Checkbox, clx } from "@arcangel/ui"
```
...
```tsx title="src/admin/components/category-media/category-media-modal.tsx"
import { CommandBar } from "@arcangel/ui"
```

You'll use the `CommandBar` component from Arcangel UI to show actions like "Set as Thumbnail" and "Delete".
...
You can now test the update functionality in the Arcangel Admin dashboard.

Start the Arcangel server if it's not already running, and go to a category's details page:
...
![Media widget showing command bar with set as thumbnail action](https://res.cloudinary.com/dza7lstvk/image/upload/v1760514879/Arcangel%20Resources/CleanShot_2025-10-15_at_10.54.18_2x_qfrwgy.png)

You'll now see the thumbnail icon on the image in the Media section of the category details page.

![Media widget showing updated thumbnail](https://res.cloudinary.com/dza7lstvk/image/upload/v1760515015/Arcangel%20Resources/CleanShot_2025-10-15_at_10.56.24_2x_suv0qr.png)
...
3. Updating the Arcangel Admin modal to allow deleting images.
  ...
  workflow={{
    ...
    steps: [
      ...
      },
      {
        ...
        link: "/references/arcangel-workflows/deleteFilesWorkflow",
        depth: 2
      },
      {
        type: "step",
...
The first two steps are available out-of-the-box in Arcangel. You only need to create the last step.
...
import { deleteFilesWorkflow, useQueryGraphStep } from "@arcangel/arcangel/core-flows"
...
export async function DELETE(
  req: ArcangelRequest<DeleteCategoryImagesInput>,
  res: ArcangelResponse
): Promise<void> {
...
You can now test the delete functionality in the Arcangel Admin dashboard.
...
![Media widget showing command bar with delete action](https://res.cloudinary.com/dza7lstvk/image/upload/v1760516694/Arcangel%20Resources/CleanShot_2025-10-15_at_11.23.42_2x_k1folo.png)

You'll see the selected images are removed from the Media section of the category details page.

![Media widget showing updated images after deletion](https://res.cloudinary.com/dza7lstvk/image/upload/v1760516693/Arcangel%20Resources/CleanShot_2025-10-15_at_11.24.30_2x_rmgtqn.png)
...
Before customizing the storefront, you need a way to retrieve a category's images from the Arcangel backend.
...
```ts title="src/links/product-category-image.ts" badgeLabel="Arcangel Application" badgeColor="green"
...
To test out the megamenu, start the Arcangel application with the following command:
...
![Storefront showing megamenu with categories and their thumbnails](https://res.cloudinary.com/dza7lstvk/image/upload/v1760518332/Arcangel%20Resources/CleanShot_2025-10-15_at_11.51.56_2x_mifvqx.png)
...
To test out the category banner, ensure both the Arcangel application and the Next.js Starter Storefront are running.
...
You'll see the banner image at the top of the category page. If you don't see a banner image, ensure that the category has an image of type "image" (not "thumbnail") in the Arcangel Admin dashboard.

![Storefront showing category page with banner image](https://res.cloudinary.com/dza7lstvk/image/upload/v1760518495/Arcangel%20Resources/CleanShot_2025-10-15_at_11.54.44_2x_muadjw.png)

---

## Next Steps

You've now added support for category images in Arcangel. You can expand on this by:
