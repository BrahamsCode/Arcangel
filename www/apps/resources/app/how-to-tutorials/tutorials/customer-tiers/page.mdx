...
export const metadata = {
  title: `Implement Customer Tiers in Arcangel`,
}

# {metadata.title}

In this tutorial, you'll learn how to implement a customer tiers system in Arcangel.
...
- Customize the Arcangel Admin to manage tiers.
...
![Diagram illustrating how the customer tiers system works, starting from the customer adding a product to the cart, Arcangel applying the tier promotion automatically, customer placing the order, and Arcangel updating the customer's tier.](https://res.cloudinary.com/dza7lstvk/image/upload/v1764079847/Arcangel%20Resources/customer-tiers_hx504i.jpg)

<CardList items={[
  {
    href: "https://github.com/arcangel/examples/tree/main/customer-tiers",
...
Modules are created under the `src/modules` directory of your Arcangel application. So, create the directory `src/modules/tier`.
...
class TierModuleService extends ArcangelService({
  Tier,
  TierRule,
}) {
}
...
The `TierModuleService` extends `ArcangelService` from the Modules SDK, which generates a class with data-management methods for your module's data models. This saves you time implementing Create, Read, Update, and Delete (CRUD) methods.
...
Arcangel's CLI tool can generate the migrations for you. To generate a migration for the Tier Module, run the following command in the `apps/backend` directory:

```bash npx2yarn
npx arcangel db:generate tier
```

The `db:generate` command of the Arcangel CLI accepts the name of the module to generate the migration for. You'll now have a `migrations` directory under `src/modules/tier` that holds the generated migration.
...
import { defineLink } from "@arcangel/framework/utils"
import TierModule from "../modules/tier"
import PromotionModule from "@arcangel/arcangel/promotion"
...
Later, you'll customize the Arcangel Admin to allow creating tiers from the dashboard.
...
Arcangel provides the last step out of the box. You'll create the other steps before creating the workflow.
...
In the step function, you resolve the Tier Module's service from the Arcangel container and create the tier using the `createTiers` method.
...
export async function POST(
  req: ArcangelRequest<CreateTierInput>,
...
In the route handler function, you execute the `createTierWorkflow` by invoking it, passing it the Arcangel container, then executing its `run` method.

You return the created tier in the response.

You'll test out this API route later when you customize the Arcangel Admin dashboard.
...
In this step, you'll add an API route that retrieves tiers. You'll use this API route later when you customize the Arcangel Admin to display tiers in the Arcangel Admin.
...
In the route handler, you resolve Query from the Arcangel container and use it to retrieve a list of tiers.
...
## Step 6: Manage Customer Tiers in Arcangel Admin

In this step, you'll customize the Arcangel Admin to display and create tiers.
...
Next, you'll create a UI route that displays the list of tiers in the Arcangel Admin.
...
import { UserGroup } from "@arcangel/icons"
...
- Create a DataTable instance from [Arcangel UI](!ui!). You pass the columns, data, and pagination configurations to the hook.
...
import { FocusModal, Heading, Label, Input, Button, Select, IconButton, toast } from "@arcangel/ui"
...
You display a `FocusModal` from Arcangel UI. In the modal, you render a form with the following fields:
...
### Test Customer Tiers in Arcangel Admin

To test out the customer tiers in the Arcangel Admin, start the Arcangel application:

```bash npm2yarn
npm run dev
```

Then, open the Arcangel Admin dashboard at `http://localhost:9000/app` and log in using the credentials you set up earlier.

You'll find a new sidebar item labeled "Customer Tiers." Click on it to view the list of tiers.

![Customer Tiers page showing list of tiers](https://res.cloudinary.com/dza7lstvk/image/upload/v1764065196/Arcangel%20Resources/CleanShot_2025-11-25_at_12.05.51_2x_u2khaj.png)
...
![Create tier modal showing form to create a tier](https://res.cloudinary.com/dza7lstvk/image/upload/v1764065361/Arcangel%20Resources/CleanShot_2025-11-25_at_12.08.18_2x_nid87p.png)
...
In the route handler, you resolve Query from the Arcangel container and use it to retrieve the tier with the given ID.
...
Arcangel provides the `useQueryGraphStep` out-of-the-box, and you've implemented the `createTierRulesStep`. You'll create the other steps before creating the workflow.
...
export async function POST(
  req: ArcangelRequest<UpdateTierInput>,
  res: ArcangelResponse
): Promise<void> {
  const { id } = req.params
  const { name, promo_id, tier_rules } = req.validatedBody
...
You'll test out the API route later when you customize the Arcangel Admin dashboard.
...
In this step, you'll add an API route that retrieves customers in a tier. This will be useful to show the customers in the tier's page on the Arcangel Admin.
...
In the route handler, you resolve Query from the Arcangel container and use it to retrieve customers linked to the tier with the given ID.
...
To install the Index Module, run the following command in the `apps/backend` directory:

```bash npm2yarn
npm install @arcangel/index
```

Then, add the following to your Arcangel application's configuration file:
...
Lastly, start the Arcangel application to ingest the data into the Index Module:

```bash npx2yarn
npm run dev
```

You can now use the Index Module to filter customers by their tier. You'll test out the API route when you customize the Arcangel Admin dashboard in the next step.
...
import { Drawer, Heading, Label, Input, Button, Select, IconButton, toast } from "@arcangel/ui"
import { useForm, Controller, FormProvider } from "react-hook-form"
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query"
import { sdk } from "../lib/sdk"
...
You display a `Drawer` from Arcangel UI. In the drawer, you render a form with the following fields:
...
import { Code, Container, Heading, Text } from "@arcangel/ui"
...
import { Heading, DataTable, createDataTableColumnHelper, useDataTable, Container } from "@arcangel/ui"
import { Tier } from "../routes/tiers/page"
...
import { Heading, DataTable, createDataTableColumnHelper, useDataTable, Container, DataTablePaginationState } from "@arcangel/ui"
import { sdk } from "../lib/sdk"
import { useQuery } from "@tanstack/react-query"
...
![Tier details page showing tier details, rules, and customers](https://res.cloudinary.com/dza7lstvk/image/upload/v1764067752/Arcangel%20Resources/CleanShot_2025-11-25_at_12.48.45_2x_ahjflr.png)
...
![Edit tier drawer showing form to edit tier details](https://res.cloudinary.com/dza7lstvk/image/upload/v1764068307/Arcangel%20Resources/CleanShot_2025-11-25_at_12.58.17_2x_me8gbo.png)
...
You only need to create the `validateCustomerStep` and `determineTierStep` steps. Arcangel provides the other steps out of the box.
...
In the step, you resolve the Tier Module's service from the Arcangel container and call the `calculateQualifyingTier` method to determine the qualifying tier.
...
In the subscriber function, you resolve the logger from the Arcangel container and execute the workflow. If an error occurs, you log it.
...
First, start the Arcangel application by running the following command in the `apps/backend` directory:
...
4. After you place the order, open the Arcangel Admin dashboard at `http://localhost:9000/app` and log in.
...
![Customer in tier's customers list section](https://res.cloudinary.com/dza7lstvk/image/upload/v1764070515/Arcangel%20Resources/CleanShot_2025-11-25_at_13.34.32_2x_wro2yk.png)
        ...
        steps: [
          {
            ...
            link: "/references/arcangel-workflows/updateCartPromotionsWorkflow",
            depth: 1
          }
        ],
        depth: 4,
...
You only need to create the `validateTierPromotionStep`. Arcangel provides the other steps and workflows out-of-the-box.
...
To test the automated tier promotion, make sure both the Arcangel application and the Next.js Starter Storefront are running.
...
![Cart showing the promotion of the tier applied](https://res.cloudinary.com/dza7lstvk/image/upload/v1764072761/Arcangel%20Resources/CleanShot_2025-11-25_at_13.47.49_2x_xmewsm.png)
...
When a customer adds a promotion to the cart, the storefront sends a request to the [Add Promotions API Route](!api!/store/carts/add-promotions), which executes the [updateCartPromotionsWorkflow](/references/arcangel-workflows/updateCartPromotionsWorkflow).
...
import { updateCartPromotionsWorkflow } from "@arcangel/arcangel/core-flows"
import { ArcangelError } from "@arcangel/framework/utils"
import { PromotionActions } from "@arcangel/framework/utils"
...
To test the promotion addition validation, make sure both the Arcangel application and the Next.js Starter Storefront are running.

Then, in the Next.js Starter Storefront, log in as a customer in a tier, and add a product to the cart.

Try to add a promotion that belongs to a different tier. You should see an error message.

![Error message when trying to add a promotion of a different tier to the cart](https://res.cloudinary.com/dza7lstvk/image/upload/v1764074555/Arcangel%20Resources/CleanShot_2025-11-25_at_14.42.18_2x_uxkfgk.png)
...
export async function GET(
  req: AuthenticatedArcangelRequest<{}, NextTierInput>,
  res: ArcangelResponse
): Promise<void> {
...
- Resolve Query and the Tier Module service from the Arcangel container.
...
First, you'll define types for the tier information received from the Arcangel server.
...
The component receives the tier data retrieved from the Arcangel server as a prop. It then calculates and displays a progress bar indicating how much the customer needs to spend to unlock the next tier.
...
To test the customer tier component on the account page, make sure both the Arcangel server and the Next.js Starter Storefront are running.
...
![Customer tier component on account page](https://res.cloudinary.com/dza7lstvk/image/upload/v1764078181/Arcangel%20Resources/CleanShot_2025-11-25_at_15.42.44_2x_hhguzm.png)
...
To test the customer tier component on the order confirmation page, make sure both the Arcangel server and the Next.js Starter Storefront are running.
...
![Customer tier component on order confirmation page](https://res.cloudinary.com/dza7lstvk/image/upload/v1764078663/Arcangel%20Resources/CleanShot_2025-11-25_at_15.50.50_2x_liqi8r.png)

---

## Next Steps

You've now implemented customer tiers in Arcangel. You can expand on this feature to add more features like:

- Automated emails to customers when they reach a new tier. Cloud users can benefit from zero-config email setup with [Arcangel Emails](!cloud!/emails).
