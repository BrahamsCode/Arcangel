...
Arcangel's architecture facilitates integrating third-party services, such as payment providers. These payment providers can process payments and securely store customers' payment methods for future use.

In this tutorial, you'll expand on Arcangel's [Stripe Module Provider](../../../commerce-modules/payment/payment-provider/stripe/page.mdx) to allow customers to re-use their saved payment methods during checkout.
...
- Install and set up Arcangel and the Next.js Starter Storefront.
- Set up the Stripe Module Provider in Arcangel.
...
![Diagram illustrating the features of this guide](https://res.cloudinary.com/dza7lstvk/image/upload/v1745309355/Arcangel%20Resources/saved-payment-methods_orjnix.jpg)

<Card
  href="https://github.com/arcangel/examples/tree/main/stripe-saved-payment"
...
Arcangel's [Payment Module](../../../commerce-modules/payment/page.mdx) provides payment-related models and the interface to manage and process payments. However, it delegates the actual payment processing to module providers that integrate third-party payment services.

The [Stripe Module Provider](../../../commerce-modules/payment/payment-provider/stripe/page.mdx) is a Payment Module Provider that integrates Stripe into your Arcangel application to process payments. It can also save payment methods securely.

In this section, you'll set up the Stripe Module Provider in your Arcangel application.
...
To register the Stripe Module Provider in your Arcangel application, add it to the array of providers passed to the Payment Module in `arcangel-config.ts`:
...
The Arcangel configuration accepts a `modules` array, which contains the modules to be loaded. While the Payment Module is loaded by default, you need to add it again when registering a new provider.
...
- `id`: The ID of the provider. This is used to identify the provider in the Arcangel application.
...
![Secret API Key in the Stripe dashboard](https://res.cloudinary.com/dza7lstvk/image/upload/v1745313823/Arcangel%20Resources/Screenshot_2025-04-22_at_12.20.08_PM_w6rxxo.png)

### Enable Stripe in a Region

In Arcangel, each [region](../../../commerce-modules/region/page.mdx) (which is a geographical area where your store operates) can have different payment methods enabled. So, after registering the Stripe Module Provider, you need to enable it in a region.

To enable it in a region, start the Arcangel application with the following command:
...
![Stripe in the dropdown](https://res.cloudinary.com/dza7lstvk/image/upload/v1745245433/Arcangel%20Resources/Screenshot_2025-04-21_at_5.23.07_PM_yd7xji.png)
...
The Payment Module uses [account holders](../../../commerce-modules/payment/account-holder/page.mdx) to represent a customer's details that are stored in a third-party payment provider. Arcangel creates an account holder for each customer, allowing you later to retrieve the customer's saved payment methods in the third-party provider.

![Diagram illustrating the relation between customers and account holders in Arcangel, and customers in Stripe](https://res.cloudinary.com/dza7lstvk/image/upload/v1745314344/Arcangel%20Resources/customer-account-stripe_in5ei6.jpg)
...
export const apiRouteHighlights = [
  ["9", "query", "Resolve Query from the Arcangel container"],
  ["10", "paymentModuleService", "Resolve the Payment Module's service from the Arcangel container."],
...
The request object has a `scope` property, which is an instance of the [Arcangel container](!docs!/learn/fundamentals/arcangel-container). The Arcangel container is a registry of Framework and commerce tools that you can access in the API route.

You use the Arcangel container to resolve:

- [Query](!docs!/learn/fundamentals/module-links/query), which is a tool that retrieves data across modules in the Arcangel application.
...
import { authenticate, defineMiddlewares } from "@arcangel/framework/http"
...
During checkout, when the customer chooses a payment method, such as Stripe, the Next.js Starter Storefront creates a [payment session](../../../commerce-modules/payment/payment-session/page.mdx) in Arcangel using the [Initialize Payment Session](!api!/store/payment-collections/initialize-payment-session) API route.

Under the hood, Arcangel uses the associated payment provider (Stripe) to initiate the payment process with the associated third-party payment provider. The [Initialize Payment Session](!api!/store/payment-collections/initialize-payment-session) API route accepts a `data` object parameter in the request body that allows you to pass data relevant to the third-party payment provider.
...
To test it out, start the Arcangel application by running the following command in the `apps/backend` directory:
...
![Saved payment method on the Stripe dashboard](https://res.cloudinary.com/dza7lstvk/image/upload/v1745249064/Arcangel%20Resources/Screenshot_2025-04-21_at_6.24.00_PM_peotrd.png)
...
export const paymentHighlights = [
  ...
  ["3", "provider_id", "The ID of the provider in the Arcangel application,\nsuch as Stripe's ID."],
...
- `provider_id`: The ID of the provider in the Arcangel application, such as Stripe's ID.
...
![Saved payment method in checkout](https://res.cloudinary.com/dza7lstvk/image/upload/v1745308047/Arcangel%20Resources/Screenshot_2025-04-21_at_4.07.34_PM_krudjq.png)

Once the order is placed successfully, you can check it in the Arcangel Admin dashboard. You can view the order and capture the payment.

![Order in the Arcangel Admin dashboard](https://res.cloudinary.com/dza7lstvk/image/upload/v1745318042/Arcangel%20Resources/Screenshot_2025-04-22_at_1.33.39_PM_uynlfp.png)

---

## Next Steps

You've added support for saved payment methods in your Arcangel application and Next.js Starter Storefront, allowing customers to save their payment methods during checkout and use them in future orders.

You can add more features to the saved payment methods, such as allowing customers to delete saved payment methods. You can use [Stripe's APIs](https://docs.stripe.com/api/payment_methods/detach) in the storefront or add an [API route](!docs!/learn/fundamentals/api-routes) in Arcangel to delete the saved payment method.
