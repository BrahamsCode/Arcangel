...
export const metadata = {
  title: `Implement Loyalty Points System in Arcangel`,
}

# {metadata.title}

In this tutorial, you'll learn how to implement a loyalty points system in Arcangel.
...
A loyalty point system allows customers to earn points for purchases, which can be redeemed for discounts or rewards. In this tutorial, you'll learn how to customize the Arcangel application to implement a loyalty points system.
...
![Diagram illustrating redeem loyalty points flow](https://res.cloudinary.com/dza7lstvk/image/upload/v1744126213/Arcangel%20Resources/redeem-points-flow_kzgkux.jpg)

<CardList items={[
  {
    href: "https://github.com/arcangel/examples/tree/main/loyalty-points",
...
Modules are created under the `src/modules` directory of your Arcangel application. So, create the directory `src/modules/loyalty`.
...
import { ArcangelError, ArcangelService } from "@arcangel/framework/utils"
...
The `LoyaltyModuleService` extends `ArcangelService` from the Modules SDK which generates a class with data-management methods for your module's data models. This saves you time on implementing Create, Read, Update, and Delete (CRUD) methods.
...
In the method, you retrieve the customer's existing loyalty points using the `listLoyaltyPoints` method, which is automatically generated by the `ArcangelService`. If the customer has existing points, you update them with the new points using the `updateLoyaltyPoints` method.
...
Otherwise, you update the customer's loyalty points with the new value using the `updateLoyaltyPoints` method, which is automatically generated by `ArcangelService`.
...
Arcangel's CLI tool can generate the migrations for you. To generate a migration for the Loyalty Module, run the following command in the `apps/backend` directory:

```bash npx2yarn
npx arcangel db:generate loyalty
```

The `db:generate` command of the Arcangel CLI accepts the name of the module to generate the migration for. You'll now have a `migrations` directory under `src/modules/loyalty` that holds the generated migration.
      ...
      },
      {
        ...
        steps: [
          ...
          },
          {
            ...
            link: "/references/arcangel-workflows/steps/updatePromotionsStep",
            depth: 2,
          }
        ],
        depth: 1
...
Arcangel provides the `useQueryGraphStep` and `updatePromotionsStep` in its `@arcangel/arcangel/core-flows` package. So, you'll only implement the other steps.
...
import { PromotionDTO, CustomerDTO, CartDTO } from "@arcangel/framework/types"
...
If you get a type error on resolving the Loyalty Module, run the Arcangel application once with the `npm run dev` or `yarn dev` command to generate the necessary type definitions, as explained in the [Automatically Generated Types guide](!docs!/learn/fundamentals/generated-types).
...
In the step, you resolve the Loyalty Module's service from the Arcangel container. Then, you use the `calculatePointsFromAmount` method to calculate the points to deduct from the promotion's amount.
...
In the compensation function, you resolve the Loyalty Module's service from the Arcangel container. Then, you call the `addPoints` method to restore the points deducted from the customer's loyalty points if an error occurs.
...
In the step, you resolve the Loyalty Module's service from the Arcangel container. Then, you use the `calculatePointsFromAmount` method to calculate the points to add from the order's amount.
...
import { updatePromotionsStep, useQueryGraphStep } from "@arcangel/arcangel/core-flows"
...
Arcangel has an event system that allows you to listen to events emitted by the Arcangel server using a [subscriber](!docs!//learn/fundamentals/events-and-subscribers). A subscriber is an asynchronous function that's executed when its associated event is emitted. In a subscriber, you can execute a workflow that performs actions in result of the event.
...
- `container`: The Arcangel container, which you can use to resolve services and tools.

In the subscriber function, you execute the `handleOrderPointsWorkflow` by invoking it, passing it the Arcangel container, then using its `run` method, passing it the workflow's input.
...
So, run the following command in the `apps/backend` directory to start the Arcangel server:
...
The Next.js Starter Storefront will be running on `http://localhost:8000`, and the Arcangel server will be running on `http://localhost:9000`.
...
After you place the order, you'll see the following message in your Arcangel application's terminal:
...
In the route handler, you resolve the Loyalty Module's service from the Arcangel container (which is available at `req.scope`).
...
To test out the customizations to the checkout flow, make sure both the Arcangel application and Next.js Starter Storefront are running.
...
![Loyalty Points Section at the end of the summary section at the right](https://res.cloudinary.com/dza7lstvk/image/upload/v1744195223/Arcangel%20Resources/Screenshot_2025-04-09_at_1.39.34_PM_l5oltc.png)
  ...
  workflow={{
    ...
    steps: [
      ...
      },
      {
        ...
        link: "/references/arcangel-workflows/steps/createPromotionsStep",
        depth: 6,
      },
      {
        type: "workflow",
      ...
      },
      {
        ...
        link: "/references/arcangel-workflows/steps/updateCartsStep",
        depth: 8,
      },
      {
        type: "step",
...
Most of the workflow's steps are either provided by Arcangel in the `@arcangel/arcangel/core-flows` package or steps you've already implemented. You only need to implement the `getCartLoyaltyPromoAmountStep` step.
...
import { PromotionDTO, CustomerDTO } from "@arcangel/framework/types"
import { ArcangelError } from "@arcangel/framework/utils"
import { createStep, StepResponse } from "@arcangel/framework/workflows-sdk"
import LoyaltyModuleService from "../../modules/loyalty/service"
import { LOYALTY_MODULE } from "../../modules/loyalty"
...
In the step, you resolve the Loyalty Module's service from the Arcangel container. Then, you call the `getPoints` method to retrieve the customer's loyalty points. If the customer has no loyalty points, you throw an error.
...
To test out applying the loyalty points on the cart, start the Arcangel application and Next.js Starter Storefront.
...
![Discounted amount is shown as part of the summary and the promotion is shown as part of the applied promotions](https://res.cloudinary.com/dza7lstvk/image/upload/v1744200895/Arcangel%20Resources/Screenshot_2025-04-09_at_3.14.19_PM_abmtjh.png)
...
To test out removing the loyalty points from the cart, start the Arcangel application and Next.js Starter Storefront.
...
![The "Remove Loyalty Points" button is shown in the "Loyalty Points" section](https://res.cloudinary.com/dza7lstvk/image/upload/v1744204436/Arcangel%20Resources/Screenshot_2025-04-09_at_4.13.24_PM_xt5trh.png)
...
So, in this step, you'll hook into Arcangel's cart completion flow to perform the validation.

Since Arcangel uses workflows in its API routes, it allows you to hook into them and perform custom functionalities using [Workflow Hooks](!docs!/learn/fundamentals/workflows/workflow-hooks). A workflow hook is a point in a workflow where you can inject custom functionality as a step function, called a hook handler.

Arcangel uses the [completeCartWorkflow](/references/arcangel-workflows/completeCartWorkflow) hook to complete the cart and place an order. This workflow has a `validate` hook that allows you to perform custom validation before the cart is completed.
...
export const completeCartWorkflowHookHighlights = [
  ...
  ["49", "ArcangelError", "Throw an error if the customer doesn't have enough loyalty points."],
]
...
- The step context object, which contains the Arcangel container. You can use it to resolve services and perform actions.
...
You've now implement a loyalty points system in Arcangel. There's still more that you can implement based on your use case:
...
- Customize the Arcangel Admin to show a new page or [UI Route](!docs!/learn/fundamentals/admin/ui-routes) for loyalty points information and analytics.
