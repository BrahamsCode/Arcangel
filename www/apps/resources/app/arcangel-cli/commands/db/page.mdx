...
export const metadata = {
  title: `db Commands - Arcangel CLI Reference`,
}

# {metadata.title}

Commands in the Arcangel CLI starting with `db:` perform actions on the database.

## db\:setup

Creates a database for the Arcangel application with the specified name, if it doesn't exist. Then, it runs migrations and syncs links.

It also updates your `.env` file with the database name.

```bash npx2yarn
npx arcangel db:setup --db <name>
```

<Note title="Tip">

Use this command if you're setting up a Arcangel project or database manually.
...
Creates a database for the Arcangel application with the specified name, if it doesn't exist.

It also updates your `.env` file with the database name.

```bash npx2yarn
npx arcangel db:create --db <name>
```
...
```bash npx2yarn
npx arcangel db:generate <module_names...>
```
...
Use this command if you've updated the Arcangel packages, or you've created customizations and want to reflect them in the database.
...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      [Arcangel v2.3.0](https://github.com/arcangel/arcangel/releases/tag/v2.3.0).
...
```bash npx2yarn
npx arcangel db:rollback <module_names...>
```
...
Sync the database with the link definitions in your application, including the definitions in Arcangel's modules.
...
The Arcangel CLI does not provide a dedicated command to drop or reset the database. To reset your database, drop it manually using PostgreSQL tools, then run `db:setup` to recreate and reinitialize it.
...
```bash npx2yarn
npx arcangel db:setup --db <db_name>
```
