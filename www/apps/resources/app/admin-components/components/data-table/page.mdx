...
This component is available after [Arcangel v2.4.0+](https://github.com/arcangel/arcangel/releases/tag/v2.4.0).

</Note>

The [DataTable component in Arcangel UI](!ui!/components/data-table) allows you to display data in a table with sorting, filtering, and pagination. It's used across the Arcangel Admin dashboard to showcase a list of items, such as a list of products.

![Example of a table in the product listing page](https://res.cloudinary.com/dza7lstvk/image/upload/v1728295658/Arcangel%20Resources/list_ddt9zc.png)

You can use this component in your Admin Extensions to display data in a table format, especially if you're retrieving them from API routes of the Arcangel application.

This guide focuses on how to use the `DataTable` component while fetching data from the backend. Refer to the [Arcangel UI documentation](!ui!/components/data-table) for detailed information about the DataTable component and its different usages.
...
Start by initializing the columns in the data table. To do that, use the `createDataTableColumnHelper` from Arcangel UI:
...
Next, you'll fetch the products from the Arcangel application. Assuming you have the JS SDK configured as explained in [this guide](../../../js-sdk/page.mdx), add the following imports at the top of the file:
...
You use the `useQuery` hook to fetch the products from the Arcangel application. In the `queryFn`, you call the `sdk.admin.product.list` method to fetch the products. You pass the following query parameters to the method:
...
Next, you'll configure the data table. Arcangel UI provides a `useDataTable` hook that helps you configure the data table. Add the following imports at the top of the file:
...
<TypeList
  types={[
    ...
    { name: "data", type: "`array`", description: "The products fetched from the Arcangel application." },
...
If you start your Arcangel application and go to `localhost:9000/app/custom`, you'll see the data table showing the list of products with pagination, filtering, searching, and sorting functionalities.
...
import { useMemo, useState } from "react"
import { Container } from "../../components/container"
import { HttpTypes, ProductStatus } from "@arcangel/framework/types"
