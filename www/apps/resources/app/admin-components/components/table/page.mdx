...
If you're using [Arcangel v2.4.0+](https://github.com/arcangel/arcangel/releases/tag/v2.4.0), it's recommended to use the [Data Table](../data-table/page.mdx) component instead as it provides features for sorting, filtering, pagination, and more with a simpler API.

</Note>

You can use the `Table` component from Arcangel UI to display data in a table. It's mostly recommended for simpler tables.
...
This section shows you how to use the `Table` component when fetching data from the Arcangel application's API routes.
...
export const tableExampleHighlights = [
  ...
  ["18", "useQuery", "Fetch products from the Arcangel application."]
]
...
To test it out, log into the Arcangel Admin and open `http://localhost:9000/app/custom`. You'll find a table of products with pagination.
