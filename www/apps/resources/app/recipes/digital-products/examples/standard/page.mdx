...
In this guide, you'll learn how to support digital products in Arcangel.

When you install a Arcangel application, you get a fully-fledged commerce platform with support for customizations. Arcangel provides all features related to products and managing them, and the Arcangel Framework allows you to extend those features and implement your custom use case.

You can extend Arcangel's product features to support selling, storing, and fulfilling digital products. In this guide, you'll customize Arcangel to add the following features:
...
5. All other commerce features that Arcangel provides.

<Note>

This guide provides an example of an approach to implement digital products. You're free to choose a different approach using the Arcangel Framework.

</Note>

<CardList items={[
  {
    href: "https://github.com/arcangel/examples/tree/main/digital-product",
...
Arcangel creates commerce features in modules. For example, product features and data models are created in the Product Module.

You also create custom commerce data models and features in custom modules. They're integrated into the Arcangel application similar to Arcangel's modules without side effects.
...
class DigitalProductModuleService extends ArcangelService({
  DigitalProduct,
  DigitalProductMedia,
  DigitalProductOrder,
}) {
...
### Add Module to Arcangel Configuration

Finally, add the module to the list of modules in `arcangel-config.ts`:
...
Modules are isolated in Arcangel, making them reusable, replaceable, and integrable in your application without side effects.
...
In this step, you’ll define links between your module’s data models and data models from Arcangel’s Commerce Modules:
...
export const orderLinkHighlights = [
  ["8", "deleteCascade", "Delete the digital product order when its linked Arcangel order is deleted."]
]
...
`deleteCascades` is enabled on the `digitalProductOrder` so that when a Arcangel order is deleted, its linked digital product order is also deleted.
...
```bash npx2yarn
npx arcangel db:generate digitalProductModuleService
```
...
To expose custom commerce features to frontend applications, such as the Arcangel Admin dashboard or a storefront, you expose an endpoint by creating an API route.
...
```mermaid
graph TD
  createProductsWorkflow["createProductsWorkflow (Arcangel)"] --> createDigitalProductStep
  createDigitalProductStep --> createDigitalProductMediasStep
  createDigitalProductMediasStep --> createRemoteLinkStep["createRemoteLinkStep (Arcangel)"]
```

1. `createProductsWorkflow`: Create the Arcangel product that the digital product is associated with its variant. Arcangel provides this workflow through the `@arcangel/arcangel/core-flows` package, which you can use as a step.
...
4. `createRemoteLinkStep`: Create the link between the digital product and the product variant. Arcangel provides this step through the `@arcangel/arcangel/core-flows` package.
...
export const createDpWorkflowHighlights = [
  ["34", "createProductsWorkflow", "Create the Arcangel product."],
...
This creates the `createDigitalProductWorkflow`. The workflow accepts as a parameter the digital product and the Arcangel product to create.

In the workflow, you run the following steps:

1. `createProductsWorkflow` as a step to create a Arcangel product.
...
```ts title="src/api/admin/digital-products/route.ts"
// other imports...
import type { CreateProductWorkflowInputDTO } from "@arcangel/types"
...
export const POST = async (
  req: AuthenticatedArcangelRequest<CreateRequestBody>,
  res: ArcangelResponse
) => {
  const query = req.scope.resolve(ContainerRegistrationKeys.QUERY)
...
To upload the digital product media files, use Arcangel’s File Module.

<Note>

Your Arcangel application uses the local file module provider by default, which uploads files to a local directory. However, you can use other file module providers, such as the [S3 module provider](../../../../infrastructure-modules/file/s3/page.mdx).
...
import { uploadFilesWorkflow } from "@arcangel/arcangel/core-flows"
...
In the route handler, you use `uploadFilesWorkflow` from Arcangel's core workflows to upload the file. If the file type is `main`, it’s uploaded with private access, as only customers who purchased it can download it. Otherwise, it’s uploaded with `public` access.
...
The Arcangel Admin is customizable, allowing you to inject widgets into existing pages or add UI routes to create new pages.

In this step, you’ll add a UI route to the Arcangel Admin that displays a list of digital products.
...
import { PhotoSolid } from "@arcangel/icons"
import { Container, Heading, Table } from "@arcangel/ui"
...
This creates a UI route that's displayed at the `/digital-products` path in the Arcangel Admin. The UI route also adds a sidebar item with the label “Digital Products" pointing to the UI route.
...
export const fetchDigitalProductsHighlights = [
  ["7", "fetchProducts", "A function that fetches the digital products from the Arcangel application."],
...
To test the UI route out, start the Arcangel application, go to `localhost:9000/app`, and log in as an admin user.
...
import { useState } from "react"
import { Input, Button, Select, toast } from "@arcangel/ui"
import { MediaType } from "../../types"
...
```tsx title="src/admin/routes/digital-products/page.tsx"
// other imports...
import { Drawer } from "@arcangel/ui"
...
To test the form, open the Digital Products page in the Arcangel Admin. There, you’ll find a new Create button.
...
When a product is deleted, Arcangel emits a `product.deleted` event. You can handle this event with a subscriber. A subscriber is an asynchronous function that, when an event is emitted, is executed. You can implement in subscribers features that aren't essential to the original flow that emitted the event.
...
import { SubscriberArgs, SubscriberConfig } from "@arcangel/framework"
import { 
...
- `container`: Instance of the [Arcangel Container](!docs!/learn/fundamentals/arcangel-container).

In the subscriber, you execute the workflow by invoking it, passing the Arcangel container as an input, then executing its `run` method. You pass the product's ID, which is received through the event's data payload, as an input to the workflow.

### Test it Out

To test this out, start the Arcangel application and, from the Arcangel Admin dashboard, delete a product that has digital products. You can confirm that the digital product was deleted by checking the Digital Products page.
...
import { ModuleProviderExports } from "@arcangel/framework/types"
...
### Register Module Provider in Arcangel's Configurations

Finally, register the module provider in `arcangel-config.ts`:
...
In the Arcangel Admin, go to Settings -> Location & Shipping, and add the fulfillment provider and a shipping option for it in a location.
...
In this step, you’ll create a new cart completion flow that not only creates a Arcangel order, but also create a digital product order.
...
```mermaid
graph TD
  completeCartWorkflow["completeCartWorkflow (Arcangel)"] --> useQueryGraphStep["useQueryGraphStep (Arcangel)"]
  useQueryGraphStep --> when{order has digital products?}
  when -->|Yes| createDigitalProductOrderStep
  createDigitalProductOrderStep --> createRemoteLinkStep["createRemoteLinkStep (Arcangel)"]
  createRemoteLinkStep --> createOrderFulfillmentWorkflow["createOrderFulfillmentWorkflow (Arcangel)"]
  createOrderFulfillmentWorkflow --> emitEventStep["emitEventStep (Arcangel)"]
  emitEventStep --> End
  when -->|No| End
```
...
1. `completeCartWorkflow` to create a Arcangel order from the cart. Arcangel provides this workflow through the `@arcangel/arcangel/core-flows` package and you can use it as a step.
2. `useQueryGraphStep` to retrieve the order’s items with the digital products associated with the purchased product variants. Arcangel provides this step through the `@arcangel/arcangel/core-flows` package.
3. If the order has digital products, you:
    1. create the digital product order.
    2. link the digital product order with the Arcangel order. Arcangel provides a `createRemoteLinkStep` in the `@arcangel/arcangel/core-flows` package that can be used here.
    3. Create a fulfillment for the digital products in the order. Arcangel provides a `createOrderFulfillmentWorkflow` in the `@arcangel/arcangel/core-flows` package that you can use as a step here.
    4. Emit the `digital_product_order.created` custom event to handle it later in a subscriber and send the customer an email. Arcangel provides a `emitEventStep` in the `@arcangel/arcangel/core-flows` that you can use as a step here.
...
export const createDpoWorkflowHighlights = [
  ...
  ["37", "completeCartWorkflow", "Completes the cart and creates the Arcangel order."],
  ["43", "useQueryGraphStep", "Retrieve the order's items with their associated variants and linked digital products."],
  ["60", "useQueryGraphStep", "Retrieve any existing links between the digital product order and the Arcangel order."],
  ...
  ["93", "createRemoteLinkStep", "Link the digital product order to the Arcangel order."],
...
2. `completeCartWorkflow` as a step to create the Arcangel order.
3. `useQueryGraphStep` to retrieve the order’s items with their associated variants and linked digital products.
4. `useQueryGraphStep` to retrieve any existing links between the digital product order and the Arcangel order.
...
6. Use `when` to check whether the order has digital products and no existing links. If so:
    ...
    2. Use the `createRemoteLinkStep` to link the digital product order to the Arcangel order.
...
The workflow returns the Arcangel order and the digital product order, if created.
...
Then, run the following command in the Arcangel application directory to start the Arcangel application:
...
1. Retrieve the digital product order's details. For this, you'll use `useQueryGraphStep` from Arcangel's core workflows.
2. Send a notification to the customer with the digital products to download.
3. Mark the Arcangel order's fulfillment as delivered. For this, you'll use `markOrderFulfillmentAsDeliveredWorkflow` from Arcangel's core workflows.
...
```ts
import { OrderDTO, InferTypeOf } from "@arcangel/framework/types"
import DigitalProductOrder from "../models/digital-product-order"
...
1. Retrieve the digital product order's details using `useQueryGraphStep` from Arcangel's core workflows.
2. Send a notification to the customer with the digital product download links using the `sendDigitalOrderNotificationStep`.
3. Check whether the Arcangel order has fulfillments. If so, mark the order's fulfillment as delivered using `markOrderFulfillmentAsDeliveredWorkflow` from Arcangel's core workflows. The `no_notification` parameter is set to `true` to prevent duplicate notifications since the customer already received the digital products via the custom notification step.
...
For testing purposes, add to `arcangel-config.ts` the following to use the Local Notification Module Provider:
...
To test it out, run the Next.js starter with the Arcangel application, then open the details page of a product that’s digital. You should see a “Download Preview” button to download the preview media of the product.
