...
import { AcademicCapSolid, NextJs } from "@arcangel/icons"
...
This recipe provides the general steps to build personalized products in Arcangel.

<Card
  title="Personalized Products Example"
  text="Find a complete example of personalized products in Arcangel."
...
In Arcangel, you can create a custom module defining data models related to personalization with the service to manage them. Then, you can link those data models to existing data models, such as products.
...
Arcangel's modular architecture removes any restrictions on the your storefront's tech stack, design, and the experience you provide customers. The storefront connects to the Arcangel application using the Store API Routes.

You can build a unique experience around your products that focuses on the customer’s personalization capabilities.

Arcangel provides a Next.js Starter Storefront with basic ecommerce functionalities that can be customized. You can also build your own storefront and use Arcangel’s client libraries or Store API Routes to communicate with the Arcangel application.
...
- Consume the `orderCreated` hook of the [completeCartWorkflow](/references/arcangel-workflows/completeCartWorkflow) to attach the personalized data to the Order Module's `LineItem` data model.
