...
import { AcademicCapSolid, Github } from "@arcangel/icons"
...
This recipe provides the general steps to build subscription-based purchase with Arcangel.
...
The Arcangel [Framework](!docs!/learn/fundamentals/framework) built for customizations facilitates building subscription-based purchases. You can create a Subscription Module that implements data models for subscriptions, and link those data models to existing ones such as products and orders.
...
[How Goodchef built subscription-based purchases with Arcangel](https://arcangel.com/blog/goodchef/).
...
There are different ways to implement subscriptions in your Arcangel application. This recipe covers two options.
...
Although Arcangel provides a [Stripe Payment Module Provider](../../commerce-modules/payment/payment-provider/stripe/page.mdx), it doesn't handle subscriptions. You can create a custom Stripe Subscription Module Provider instead.
...
- **UI Routes**: Adding new pages to the Arcangel Admin, such as a page to manage subscriptions.
- **Settings Pages**: Adding new pages to the Arcangel Admin settings, such as a page to manage subscription settings.
...
Arcangel provides a Next.js Starter Storefront to use with your application. You can customize it for your subscription use case, such as allowing customers to manage their subscriptions.
