...
In this guide, you'll learn how to support subscription purchases with Arcangel.

When you install a Arcangel application, you get a fully-fledged commerce platform with support for customizations. While Arcangel doesn't provide subscription-based purchases natively, it provides the Framework to support you in implementing this feature.

In this guide, you'll customize Arcangel to implement subscription-based purchases with the following features:
...
This guide provides an example of an approach to implement subscriptions. You're free to choose a different approach using the Arcangel Framework.

</Note>

<CardList items={[
  {
    href: "https://github.com/arcangel/examples/tree/main/subscription",
...
Once the installation finishes successfully, the Arcangel Admin dashboard will open with a form to create a new user. Enter the user's credentials and submit the form.
...
As mentioned in the introduction, you'll use Stripe as the payment provider for the subscription payments. In this step, you'll configure the [Stripe Module Provider](../../../../commerce-modules/payment/payment-provider/stripe/page.mdx) in your Arcangel application.
...
To add the Stripe Module Provider to the Arcangel configurations, add the following to the `arcangel-config.ts` file:
...
The Arcangel configurations accept a `modules` property to add modules to your application. You'll learn more about modules in the next section.
...
To allow customers to use Stripe during checkout, you must enable it in at least one region. Customers can only choose from payment providers available in their region. You can enable the payment provider in the Arcangel Admin dashboard.

To do that, start the Arcangel application:
...
![Choose "Stripe (STRIPE)" in the payment provider dropdown](https://res.cloudinary.com/dza7lstvk/image/upload/v1740654408/Arcangel%20Resources/Screenshot_2025-02-27_at_1.06.12_PM_esql1c.png)
...
class SubscriptionModuleService extends ArcangelService({
  Subscription,
}) {
}
...
### Register Module in Arcangel’s Configuration

Finally, add the module into `arcangel-config.ts`:
...
In this step, you’ll define links between the Subscription Module’s `Subscription` data model and the data models of Arcangel’s Commerce Modules:
...
```bash npx2yarn
npx arcangel db:generate subscriptionModuleService
```
...
Arcangel provides the first and last steps in the `@arcangel/arcangel/core-flows` package, so you only need to implement the second step.
...
import { LinkDefinition } from "@arcangel/framework/types"
...
1. [acquireLockStep](/references/arcangel-workflows/steps/acquireLockStep) to acquire a lock on the cart to prevent race conditions.
2. [completeCartWorkflow](/references/arcangel-workflows/completeCartWorkflow) that completes a cart and creates an order.
...
6. [releaseLockStep](/references/arcangel-workflows/steps/releaseLockStep) to release the lock on the cart.
...
In this step, you’ll create a custom API route similar to the [Complete Cart API route](!api!/store/carts/complete-cart) that uses the workflow you previously created to complete the customer's purchase and create a subscription.
...
![Diagram showcasing payment flow overview](https://res.cloudinary.com/dza7lstvk/image/upload/v1740657136/Arcangel%20Resources/subscriptions-stripe_ycw4ig.jpg)

To support subscriptions, you need to support capturing the payment each time the subscription renews. When creating the payment session using the [Initialize Payment Session API route](!api!/store/payment-collections/initialize-payment-session), you must pass the data that your payment provider requires to support capturing the payment again in the future. You can pass the data that the provider requires in the `data` property.
...
When you create the payment session, Arcangel creates an account holder for the customer. An account holder represents a customer's saved payment information, including saved methods, in a third-party provider and may hold data from that provider. Learn more in the [Account Holder](../../../../commerce-modules/payment/account-holder/page.mdx) documentation.
...
import { Button, clx, Heading, Text } from "@arcangel/ui"
...
This adds a component that displays a form to choose the subscription's interval and period during checkout. When the customer submits the form, you use the `updateSubscriptionData` function that sends a request to the Arcangel application to update the cart with the subscription details.
...
1. In the `apps/backend` directory, run the following command to start the application:

```bash npm2yarn
npm run dev
```

2. In the `apps/storefront` directory, run the following command to start the storefront:
...
- A validation schema indicating which query parameters are accepted. You create the schema with [Zod](https://zod.dev/). Arcangel has a createFindParams utility that generates a Zod schema accepting four query parameters:
...
In the next section, you’ll extend the Arcangel Admin and use these API routes to show the subscriptions.
...
Arcangel provides a [JS SDK](../../../../js-sdk/page.mdx) that facilitates sending requests to the server. You can use this SDK in any JavaScript client-side application, including your admin customizations.
...
- `baseUrl`: The URL of the Arcangel server. You use the Vite environment variable `VITE_BACKEND_URL`.
...
You'll now create the subscriptions list UI route. Since you'll show the subscriptions in a table, you'll use the [DataTable component](!ui!/components/data-table) from Arcangel UI. It facilitates displaying data in a tabular format with sorting, filtering, and pagination.
...
import { ClockSolid } from "@arcangel/icons"
import { Container, Heading, Badge, createDataTableColumnHelper, useDataTable, DataTablePaginationState, DataTable } from "@arcangel/ui"
...
To test the UI routes, run the Arcangel application and go to `http://localhost:9000/app`.
...
```mermaid
graph TD
  useQueryGraphStep["Retrieve Cart (useQueryGraphStep by Arcangel)"] --> createPaymentCollectionStep["createPaymentCollectionStep (Arcangel)"]
  createPaymentCollectionStep["createPaymentCollectionStep (Arcangel)"] --> getPaymentMethodStep
  getPaymentMethodStep --> createPaymentSessionsWorkflow["createPaymentSessionsWorkflow (Arcangel)"]
  createPaymentSessionsWorkflow["createPaymentSessionsWorkflow (Arcangel)"] --> authorizePaymentSessionStep["authorizePaymentSessionStep (Arcangel)"]
  authorizePaymentSessionStep["authorizePaymentSessionStep (Arcangel)"] --> createSubscriptionOrderStep
  createSubscriptionOrderStep --> createRemoteLinkStep["Create Links (createRemoteLinkStep by Arcangel)"]
  createRemoteLinkStep["Create Links (createRemoteLinkStep by Arcangel)"] --> capturePaymentStep["capturePaymentStep (Arcangel)"]
  capturePaymentStep["capturePaymentStep (Arcangel)"] --> updateSubscriptionStep
```

1. Retrieve the subscription’s linked cart. Arcangel provides a `useQueryGraphStep` in the `@arcangel/arcangel/core-flows` package that can be used as a step.
2. Create a payment collection for the new order. Arcangel provides a `createPaymentCollectionsStep` in the `@arcangel/arcangel/core-flows` package that you can use.
3. Get the customer's saved payment method. This payment method will be used to charge the customer.
4. Create payment sessions in the payment collection. Arcangel provides a `createPaymentSessionsWorkflow` in the `@arcangel/arcangel/core-flows` package that can be used as a step.
5. Authorize the payment session. Arcangel also provides the `authorizePaymentSessionStep` in the `@arcangel/arcangel/core-flows` package, which can be used.
6. Create the subscription’s new order.
7. Create links between the subscription and the order using the `createRemoteLinkStep` provided in the `@arcangel/arcangel/core-flows` package.
8. Capture the order’s payment using the `capturePaymentStep` provided by Arcangel in the `@arcangel/arcangel/core-flows` package.
...
import { AccountHolderDTO, CustomerDTO, PaymentMethodDTO } from "@arcangel/framework/types"
import { createStep, StepResponse } from "@arcangel/framework/workflows-sdk"
...
Then, you create the `getPaymentMethodStep` that receives the customer's data and account holders as an input.
...
In the step, you first find the account holder for the Stripe payment provider from the customer's `account_holders` list, and throw an error if none is found. Then, you list the customer's payment methods using the Payment Module service's `listPaymentMethods` method. If you're using a different payment provider, adjust the `provider_id` value.
...
export const createSubscriptionOrderStep1Highlights = [
  ...
  ["33", "createOrderWorkflow", "Use Arcangel's workflow to create the order."],
...
This creates a `createSubscriptionOrderStep` that uses the `createOrdersWorkflow`, which Arcangel provides in the `@arcangel/arcangel/core-flows` package. The step returns the created order and an array of links to be created.
...
To manage the orders created for a subscription, or other functionalities, use Arcangel’s existing [Admin API routes](!api!/admin).
...
Arcangel provides a Next.js Starter storefront that you can customize to your use case. You can also create a custom storefront. To learn how visit the [Storefront Development](../../../../storefront-development/page.mdx) section.
