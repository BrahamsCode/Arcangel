...
This recipe provides the general steps to implement bundled products in your Arcangel application.

<Note>

Follow the step-by-step [Bundled Products Example](./examples/standard/page.mdx) to learn how to implement bundled products in your Arcangel application.
...
Arcangel provides an [inventory kit](../../commerce-modules/inventory/inventory-kit/page.mdx) feature that allows you to create bundled products. However, it doesn't support all bundled-product features. For example, you can't set a different price for the bundle, or fulfill items within the same bundle separately.

To support more bundled-product features, you can customize the Arcangel application by creating a Bundled Product Module and building flows around it.
...
<CardList itemsPerRow={2} items={[
  ...
  },
  {
    ...
    text: "Learn how to fetch data across modules with Arcangel's Query.",
    icon: AcademicCapSolid,
  },
]} />
...
You can extend the Arcangel Admin to provide merchants with an interface to manage bundled products. You can inject widgets into existing pages or create new pages.
...
Arcangel provides a Next.js Starter Storefront with standard commerce features including listing products, placing orders, and managing accounts. You can customize the storefront and cater its functionalities to support bundled products.
