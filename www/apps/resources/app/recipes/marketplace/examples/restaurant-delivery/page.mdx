...
In this guide, you'll learn how to build a restaurant-delivery marketplace platform, similar to Uber Eats, with Arcangel.
...
In this guide, you'll customize Arcangel to build a restaurant-delivery platform with the following features:
...
<CardList items={[
  {
    href: "https://github.com/arcangel/examples/tree/main/restaurant-marketplace",
...
This recipe is adapted from [Arcangel Eats](https://github.com/arcangel/arcangel-eats), which offers more implementation details including a custom storefront to place and track orders.
...
class RestaurantModuleService extends ArcangelService({
  Restaurant,
  RestaurantAdmin,
}) {}
...
### Add Restaurant Module to Arcangel Configuration
...
class DeliveryModuleService extends ArcangelService({
  Delivery,
  Driver,
}) {}
...
### Add Delivery Module to Arcangel Configuration
...
```bash npx2yarn
npx arcangel db:generate restaurantModuleService
npx arcangel db:generate deliveryModuleService
```
...
To test the API route out, start the Arcangel application:
...
Arcangel provides an authentication flow that allows you to authenticate custom user types:
...
2. Set the actor type of the user’s authentication identity (created by the `/auth/{actor_type}/{provider}/register` API route). For this step, you’ll use `setAuthAppMetadataStep` from Arcangel's core workflows.
...
import { setAuthAppMetadataStep } from "@arcangel/arcangel/core-flows"
import {
  createWorkflow,
  transform,
  when,
...
4. Use `setAuthAppMetadataStep` from Arcangel's core workflows to update the authentication identity and associate it with the new user.
...
This applies the `authenticate` middleware from the Arcangel Framework on the `POST /users` API routes.
...
3. Unset the association between the auth identity and the restaurant admin using `setAuthAppMetadataStep` from Arcangel's core workflows.
...
This allows only restaurant admins and Arcangel Admin users to access routes under the `/restaurants/[id]` prefix if the request method is `POST` or `DELETE`.
...
1. Create the product using Arcangel’s `createProductsWorkflow` as a step. This workflow is available through Arcangel's core workflows.
2. Create a link between the restaurant and the products using `createRemoateLinkStep` from Arcangel's core workflows.
...
3. `createRemoteLinkStep` that creates links between the different data model records. This step is from Arcangel's core workflows.
  ...
  workflow={{
    ...
    steps: [
      ...
      },
      {
        ...
        description: "Creates the links returned by the previous step between the order and delivery. This is from Arcangel's core workflows.",
...
import { CreateOrderShippingMethodDTO } from "@arcangel/framework/types"
import {
  Modules,
  ContainerRegistrationKeys,
  ArcangelError,
...
export async function POST(req: ArcangelRequest, res: ArcangelResponse) {
  ...
  const updatedDeliveryResult = await updateDeliveryWorkflow(req.scope)
    ...
    .catch((error) => {
      console.log(error)
      return ArcangelError.Types.UNEXPECTED_STATE
    })

  if (typeof updatedDeliveryResult === "string") {
    throw new ArcangelError(updatedDeliveryResult, "An error occurred")
  }
...
This request returns the updated delivery. If you also check the Arcangel application’s logs, you’ll find the following message:
...
export async function POST(req: AuthenticatedArcangelRequest, res: ArcangelResponse) {
...
The request returns the updated delivery. If you check the Arcangel application’s logs, you’ll find the following message:
...
You can use a custom Next.js project, or use Arcangel's Next.js Starter storefront. If you haven't installed the Next.js Starter storefront in the first step, refer to [this documentation](../../../../nextjs-starter/page.mdx#approach-2-install-separately) to learn how to install it.
...
To retrieve the delivery from the Arcangel application, replace the first `TODO` with the following:
...
The Arcangel Admin is extendable, allowing you to add widgets to existing pages or create new pages. Learn more about it in [this documentation](!docs!/learn/fundamentals/admin).

### Storefront Development

Arcangel provides a Next.js Starter storefront that you can customize to your use case.
