...
In this guide, you'll learn how to build a marketplace with Arcangel.
...
In this guide, you'll customize Arcangel to build a marketplace with the following features:
...
This guide provides an example of an approach to implement marketplaces. You're free to choose a different approach using the Arcangel Framework.

</Note>

<CardList items={[
  {
    href: "https://github.com/arcangel/examples/tree/main/marketplace",
...
A module is created under the `src/modules` directory of your Arcangel application. So, create the directory `src/modules/marketplace`.
...
class MarketplaceModuleService extends ArcangelService({
  Vendor,
  VendorAdmin,
}) { }
...
The `MarketplaceModuleService` extends `ArcangelService` from the Modules SDK which generates a class with data-management methods for your module's data models. This saves you time on implementing Create, Read, Update, and Delete (CRUD) methods.
...
Arcangel's CLI tool generates the migrations for you. To generate a migration for the Marketplace Module, run the following command in your Arcangel application's directory:

```bash npx2yarn
npx arcangel db:generate marketplace
```

The `db:generate` command of the Arcangel CLI accepts the name of the module to generate the migration for. You'll now have a `migrations` directory under `src/modules/marketplace` that holds the generated migration.
...
Arcangel represents the links you define in link tables similar to pivot tables. So, to sync the defined links to the database, run the `db:migrate` command:
...
An [actor type](../../../../commerce-modules/auth/auth-identity-and-actor-types/page.mdx#actor-types) is a type of user that can send an authenticated requests. Arcangel has two default actor types: `customer` for customers, and `admin` for admin users.
...
1. Retrieve a registration JWT token. Arcangel has a `/auth/{actor_type}/emailpass/register` route to retrieve a registration JWT token for the specified actor type.
...
3. Retrieve an authenticated JWT token using Arcangel's `/auth/{actor_type}/emailpass` route, which retrieves the token for the specified actor type if the credentials in the request body match a user in the database.

In the next steps, you'll implement the logic to create a vendor and its admin around the above authentication flow. You can also refer to the following documentation pages to learn more about authentication in Arcangel:
...
Arcangel provides the last two steps through its `@arcangel/arcangel/core-flows` package. So, you only need to implement the first two steps.
...
export const POST = async (
  req: AuthenticatedArcangelRequest<RequestBody>,
...
You define middlewares in Arcangel in the `src/api/middlewares.ts` special file. So, create the file `src/api/middlewares.ts` with the following content:
...
To test out the above API route, start the Arcangel application:
...
The workflow's steps are all provided by Arcangel's `@arcangel/arcangel/core-flows` package. So, you can create the workflow right away.
...
export const POST = async (
  req: AuthenticatedArcangelRequest<HttpTypes.AdminCreateProduct>,
  res: ArcangelResponse
) => {
...
```ts title="src/api/middlewares.ts"
// other imports...
import { AdminCreateProduct } from "@arcangel/arcangel/api/admin/products/validators"
...
Similar to before, you apply the `validateAndTransformBody` middleware on the `POST /vendors/products` API route. You pass to the middleware the `AdminCreateProduct` schema that Arcangel uses to validate the request body of the [Create Product Admin API Route](!api!/admin/products/create-product).
...
To test out the new API routes, start the Arcangel application:
  ...
  workflow={{
    ...
    steps: [
      ...
      },
      {
        ...
        depth: 5,
        link: "/references/arcangel-workflows/getOrderDetailWorkflow"
      },
      {
        type: "when",
...
You only need to implement the `groupVendorItemsStep` and `createVendorOrdersStep` steps, as Arcangel provides the rest of the steps in its `@arcangel/arcangel/core-flows` package.
...
import { CartLineItemDTO } from "@arcangel/framework/types"
...
If an error occurs, the created orders in the `createdOrders` array are canceled using Arcangel's `cancelOrderWorkflow` from the `@arcangel/arcangel/core-flows` package.
...
The compensation function cancels all child orders received from the step. It uses the `cancelOrderWorkflow` that Arcangel provides in the `@arcangel/arcangel/core-flows` package.
...
Then, you need to customize the storefront to use your complete cart API route rather than Arcangel's. In `src/lib/data/cart.ts`, find the following lines in the `src/lib/data/cart.ts`:
...
Now, the checkout flow uses your custom API route to place the order instead of Arcangel's.
...
export const getOrderHighlights = [
  ...
  ["19", "getOrdersListWorkflow", "Use Arcangel's workflow to retrieve the list of orders."],
]
...
import { getOrdersListWorkflow } from "@arcangel/arcangel/core-flows"
...
You add a `GET` API route at `/vendors/orders`. In the route handler, you first use Query to retrieve the orders of the authenticated admin's vendor. Then, you use Arcangel's `getOrdersListWorkflow` to retrieve the list of orders with the specified fields.
...
If you want vendors to perform actions that are available for admin users through Arcangel's [Admin API routes](!api!/admin), such as managing their orders, you need to recreate them similar to the create product API route you created earlier.
...
Arcangel provides a Next.js Starter storefront, which you can customize to fit your specific use case.
...
The Arcangel Admin is extendable, allowing you to add custom widgets to existing pages or create entirely new pages. For example, you can add a new page showing the list of vendors. Learn more about it in [this documentation](!docs!/learn/fundamentals/admin).

Only super admins can access the Arcangel Admin, not vendor admins. So, if you need a dashboard specific to each vendor admin, you will need to build a custom dashboard with the necessary features.
