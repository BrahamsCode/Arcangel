...
export const metadata = {
  title: `Implement a Ticket Booking System with Arcangel`,
}

# {metadata.title}

In this tutorial, you'll learn how to implement a ticket booking system using Arcangel.
...
When you install a Arcangel application, you get a fully-fledged commerce platform with a Framework for customization. The Arcangel application's commerce features are built around [Commerce Modules](../../../commerce-modules/page.mdx) that are available out-of-the-box.

The Arcangel [Framework](!docs!/learn/fundamentals/framework) facilitates customizing Arcangel's core features for your specific use case, such as ticket booking.

<Note>

This tutorial provides an approach to implement a ticket booking system using Arcangel. Depending on your specific requirements, you might need to adjust the implementation details or explore a different approach.
...
- Install and set up Arcangel with the Next.js Starter Storefront.
- Create data models for venues and tickets, and link them to Arcangel's data models.
- Customize the Arcangel Admin to manage venues and shows or events.
...
![Diagram showing the architecture of the ticket booking system with Arcangel](https://res.cloudinary.com/dza7lstvk/image/upload/v1757512301/Arcangel%20Resources/ticket-booking-diagram_egbpye.jpg)

<CardList items={[
  {
    href: "https://github.com/arcangel/examples/tree/main/ticket-booking",
    title: "Full Code",
...
the installation process will start, which will install the Arcangel application as a monorepository in a directory with your project's name. The backend is installed in the `apps/backend` directory, and the Next.js Starter Storefront is installed in the `apps/storefront` directory.
...
In Arcangel, you can build custom features in a [module](!docs!/learn/fundamentals/modules). A module is a reusable package with the data models and functionality related to a single feature or domain. Arcangel integrates the module into your application without implications or side effects on your setup.
...
You'll define data models to represent venues, tickets, and purchases. Later, you'll link these data models to Arcangel's data models, such as products and orders.
...
The `TicketProduct` data model represents a product purchased as a ticket, such as a show or event. It will be linked to Arcangel's `Product` data model.
...
- `id`: The primary key of the table.
- `product_id`: The ID of the linked product in Arcangel's `Product` data model.
...
The `TicketProductVariant` data model represents a variant of a ticket product, such as a specific row type. It will be linked to Arcangel's `ProductVariant` data model.
...
- `id`: The primary key of the table.
- `product_variant_id`: The ID of the linked product variant in Arcangel's `ProductVariant` data model.
...
The `TicketPurchase` data model represents the purchase of a seat for a specific `TicketProduct`. It will be linked to Arcangel's `Order` data model.
...
- `id`: The primary key of the table.
- `order_id`: The ID of the linked order in Arcangel's `Order` data model.
...
export class TicketBookingModuleService extends ArcangelService({
  Venue,
  VenueRow,
  TicketProduct,
...
The `TicketBookingModuleService` extends `ArcangelService`, which generates a class with data-management methods for your module's data models. This saves you time on implementing Create, Read, Update, and Delete (CRUD) methods.

The `TicketBookingModuleService` class now has methods like `createTicketProducts` and `retrieveVenue`.

<Note>

Find all methods generated by the `ArcangelService` in [the Service Factory reference](../../../service-factory-reference/page.mdx).

</Note>

### d. Export Module Definition

The final piece of a module is its definition, which you export in an `index.ts` file at its root directory. This definition tells Arcangel the name of the module and its service.
...
Arcangel's CLI tool can generate the migrations for you. To generate a migration for the Ticket Booking Module, run the following command in your Arcangel application's directory:

```bash npx2yarn
npx arcangel db:generate ticketBooking
```

The `db:generate` command of the Arcangel CLI accepts the name of the module to generate the migration for. You'll now have a `migrations` directory under `src/modules/ticket-booking` that holds the generated migration.
...
## Step 3: Link Ticket Booking to Arcangel Data Models

Since Arcangel isolates modules to integrate them into your application without side effects, you can't directly create relationships between data models of different modules.

Instead, Arcangel provides a mechanism to define links between data models and retrieve and manage linked records while maintaining module isolation.
...
In this step, you'll define a link between the data models in the Ticket Booking Module and Arcangel's Commerce Modules.

### a. TicketProduct \<\> Product Link

To define a link between the `TicketProduct` data model and Arcangel's `Product` data model, create the file `src/links/ticket-product.ts` with the following content:
...
In later steps, you'll learn how this link allows you to retrieve and manage ticket products and their related Arcangel products.
...
To define a link between the `TicketProductVariant` data model and Arcangel's `ProductVariant` data model, create the file `src/links/ticket-product-variant.ts` with the following content:
...
Finally, to define a link between the `TicketPurchase` data model and Arcangel's `Order` data model, create the file `src/links/ticket-purchase-order.ts` with the following content:
...
To sync the links to the database, run the migrations command again in the Arcangel application's directory:

```bash npx2yarn
npx arcangel db:migrate
```

This command will create the necessary tables to manage the links between the Ticket Booking Module's data models and Arcangel's data models.
...
When you build commerce features in Arcangel that can be consumed by client applications, such as the Arcangel Admin dashboard or storefront, you need to implement:
...
The `useQueryGraphStep` is available through Arcangel's `@arcangel/arcangel/core-flows` package. You'll implement other steps in the workflow.
...
In the step function, you resolve the Ticket Booking Module's service from the Arcangel container using its `resolve` method, passing it the module's name as a parameter.
...
export async function POST(
  req: ArcangelRequest<CreateVenueSchema>,
...
You'll test this API route later when you customize the Arcangel Admin.
...
You apply Arcangel's `validateAndTransformBody` middleware to `POST` requests sent to the `/admin/venues` route.
...
In this step, you'll add an API route to retrieve a list of venues. You'll use this API route later to display a list of venues in the Arcangel Admin.
...
In the route handler, you resolve Query from the Arcangel container and use it to retrieve a list of venues.
...
You'll test out this API route later when you customize the Arcangel Admin.
...
1. A Zod schema to validate the query parameters. You use Arcangel's `createFindParams` utility function to create a schema that validates common query parameters like `limit`, `offset`, `fields`, and `order`.
...
## Step 6: Manage Venues in Arcangel Admin

In this step, you'll customize the Arcangel Admin to allow admin users to manage venues.
...
In this step, you'll create a new page or UI route in the Arcangel Admin to view a list of venues and create new venues.

### a. Initialize JS SDK

To send requests to the Arcangel server, you'll use the [JS SDK](../../../js-sdk/page.mdx). It's already installed in your Arcangel project, but you need to initialize it before using it in your customizations.
...
Then, you use the `useDataTable` hook from Arcangel UI to create a data table instance with the fetched venues and the columns you defined earlier.
...
You use Arcangel UI's `FocusModal` component to render the modal. Inside the modal, you render a form with input fields for the venue's name and address, and fields to add rows.
...
You can now test the venues page in the Arcangel Admin.

Run the following command to start the Arcangel server:

```bash npm2yarn
npm run dev
```

Then, open `http://localhost:9000/app` in your browser to access the Arcangel Admin. Log in with the admin user you created earlier.
...
![Venues Page in the Arcangel Admin with an empty table](https://res.cloudinary.com/dza7lstvk/image/upload/v1757430925/Arcangel%20Resources/CleanShot_2025-09-09_at_18.15.02_2x_o0pf16.png)
...
![Create Venue Modal in the Arcangel Admin](https://res.cloudinary.com/dza7lstvk/image/upload/v1757431196/Arcangel%20Resources/CleanShot_2025-09-09_at_18.19.45_2x_qyov67.png)
...
The workflow that creates a ticket product will also create the Arcangel product and its variants. It will create a variant for each show date and row type. For example, if a show has two dates and three row types, the workflow will create six variants for the Arcangel product.
  ...
  workflow={{
    ...
    steps: [
      ...
      },
      {
        type: "step",
        name: "useQueryGraphStep",
        description: "Retrieve default store configuration that are useful for creating the Arcangel product.",
        depth: 2,
        link: "/references/helper-steps/useQueryGraphStep",
      },
      {
        ...
        description: "Creates inventory items for the Arcangel product.",
        depth: 3,
        link: "/references/arcangel-workflows/createInventoryItemsWorkflow"
      },
      {
        type: "step",
        name: "createProductsWorkflow",
        description: "Creates the Arcangel product and its variants.",
        depth: 4,
        link: "/references/arcangel-workflows/createProductsWorkflow"
      },
      {
        type: "step",
      ...
      },
      {
        ...
        description: "Create links between the ticket product and variants and the Arcangel product and variants.",
        depth: 7
      },
      {
        type: "step",
...
You only need to implement the `validateVenueAvailabilityStep`, `createTicketProductsStep`, and `createTicketProductVariantsStep` steps. The other steps are provided by Arcangel.
...
export const createTicketProductVariantsStep = createStep(
  ...
  async (input: CreateTicketProductVariantsStepInput, { container }) => {
    ...
    // Create ticket product variants for each Arcangel variant
...
import { useQueryGraphStep, createProductsWorkflow, createRemoteLinkStep, createInventoryItemsWorkflow } from "@arcangel/arcangel/core-flows"
import { CreateProductWorkflowInputDTO, CreateMoneyAmountDTO } from "@arcangel/framework/types"
...
Then, you retrieve the default store configuration using the `useQueryGraphStep`. These configurations are useful when creating the Arcangel inventory items and product.

<Note>

Make sure you have [default location and sales channel set up in your Arcangel store](!user-guide!/settings/store).

</Note>

Next, you'll create inventory items for each variant before creating the Arcangel product. Replace the `TODO` with the following:
...
// TODO create the Arcangel product
```
...
Next, you'll create the Arcangel product and variants. Replace the `TODO` with the following:
...
const arcangelProduct = createProductsWorkflow.runAsStep({
  input: {
...
You prepare the Arcangel product data using the `transform` function. You create options for "Date" and "Row Type" and create a variant for each combination of show date and row type.
...
Then, you create the Arcangel product using the `createProductsWorkflow`.
...
const ticketProductData = transform({
  ...
  return {
    ticket_products: data.arcangelProduct.map((product: any) => ({
...
const ticketVariantsData = transform({
  ...
  return {
    variants: data.arcangelProduct[0].variants.map((variant: any) => {
...
Finally, you'll create links between the ticket product and variants and the Arcangel product and variants, and retrieve the created ticket product with its relations. Replace the `TODO` with the following:
...
const linksData = transform({
  ...
  // Create links between ticket product and Arcangel product
  const productLinks = [{
    [TICKET_BOOKING_MODULE]: {
      ticket_product_id: data.ticket_products[0].id,
    },
    [Modules.PRODUCT]: {
      product_id: data.arcangelProduct[0].id,
    },
  }]

  // Create links between ticket variants and Arcangel variants
...
You create links between the ticket product and the Arcangel product, and between the ticket product variants and the Arcangel product variants using the `createRemoteLinkStep`.
...
export async function POST(
  req: ArcangelRequest<CreateTicketProductSchema>,
  res: ArcangelResponse
) {
...
You'll test the API route when you customize the Arcangel Admin later.
...
In this step, you'll add an API route to list ticket products. This will be useful when you customize the Arcangel Admin to display ticket products.
...
## Step 9: Manage Ticket Products in Arcangel Admin

In this step, you'll customize the Arcangel Admin to add a new page that shows a list of ticket products and allows creating new ticket products.

### a. Define Ticket Product Type

Before you customize the Arcangel Admin, you'll define a type for the ticket product to use in your customizations.
...
import { defineRouteConfig } from "@arcangel/admin-sdk"
import { ReceiptPercent } from "@arcangel/icons"
import {
  createDataTableColumnHelper,
  Container,
  DataTable,
...
First, you define the columns for the data table that will display ticket products. You create columns for the ticket product's name, venue, dates, and a link to view the associated Arcangel product.
...
Then, you use Arcangel UI's `DataTable` component to render the table of ticket products.
...
In Arcangel, you can set the price of a product variant in multiple currencies and regions. This allows you to sell products in different markets with localized pricing. Learn more in the [Pricing documentation](../../../commerce-modules/pricing/concepts/page.mdx).
...
Next, you'll fetch the list of venues, regions, and store currencies from the Arcangel backend. Replace the `TODO` with the following:
...
You can now test the ticket products page in the Arcangel Admin.

Start the Arcangel server if it isn't already running. If you open the Arcangel Admin and log in, you should see a new "Shows" item in the sidebar. If you click on it, you can see a table of ticket products with a button to create a new show.

![Shows page with the table of ticket products and a button to create a new show](https://res.cloudinary.com/dza7lstvk/image/upload/v1757490157/Arcangel%20Resources/CleanShot_2025-09-10_at_10.42.00_2x_jsaq7v.png)
...
![Create Show modal with the Product Details step](https://res.cloudinary.com/dza7lstvk/image/upload/v1757490395/Arcangel%20Resources/CleanShot_2025-09-10_at_10.46.23_2x_v2pbuo.png)
...
![Create Show modal with the Pricing step](https://res.cloudinary.com/dza7lstvk/image/upload/v1757490725/Arcangel%20Resources/CleanShot_2025-09-10_at_10.51.55_2x_bijkab.png)

This will create a new ticket product with a Arcangel product. You can view the ticket product in the table, and you can click the link to view the associated Arcangel product.

You can edit the associated Arcangel product to add images, descriptions, and other details for the show.
...
Arcangel implements cart operations in workflows. Specifically, you'll focus on the `addToCartWorkflow`. Arcangel allows you to inject custom logic into workflows using [hooks](!docs!/learn/fundamentals/workflows/workflow-hooks).
  ...
  workflow={{
    ...
    steps: [
      ...
      },
      {
        ...
        description: "Complete the cart using Arcangel's default completeCartWorkflow",
        depth: 2,
        link: "/references/arcangel-workflows/completeCartWorkflow"
      },
      {
        type: "step",
        name: "useQueryGraphStep",
...
You only need to implement the `createTicketPurchasesStep` and `validateTicketOrderStep` steps, as the other steps and workflows are provided by Arcangel.
...
2. Complete the cart using Arcangel's `completeCartWorkflow`.
...
export const GET = async (req: ArcangelRequest, res: ArcangelResponse) => {
  ...
  if (!ticketProduct) {
    throw new ArcangelError(ArcangelError.Types.NOT_FOUND, "Ticket product not found")
  }
...
1. Retrieve the ticket product by its associated Arcangel product ID, including its dates, venue, variants, and purchases.
...
1. Retrieve the ticket product by its associated Arcangel product ID, including its venue, rows, variants, and purchases.
...
To send an email in Arcangel when an order is placed, you'll need:
...
First, set up a Notification Module Provider in your Arcangel application.

For example, to set up the SendGrid Notification Module Provider, add it to `arcangel-config.ts`:
  ...
  modules: [
    // ...
    {
      ...
      options: {
        providers: [
          {
            resolve: "@arcangel/arcangel/notification-sendgrid",
            id: "sendgrid",
            options: {
              channels: ["email"],
...
Start by installing the `qrcode` package in your Arcangel application with the following command:
...
import { SubscriberArgs, type SubscriberConfig } from "@arcangel/arcangel"
import { TICKET_BOOKING_MODULE } from "../modules/ticket-booking"
...
To test the order confirmation email, [customize the storefront](./storefront/page.mdx) first. Then, start the Arcangel application and the Next.js Starter Storefront.

Place an order with at least one ticket product. After placing the order, you'll see the following message in your Arcangel application's terminal:
...
To test the ticket verification functionality, start the Arcangel application.
