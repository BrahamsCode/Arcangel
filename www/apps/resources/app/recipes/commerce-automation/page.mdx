...
This recipe provides the general steps to implement commerce automation with Arcangel.
...
Arcangel provides the necessary architecture and tools to implement commerce automation for order management, customer service, and more. You can perform an asynchronous action when an event is triggered, schedule a job that runs at a specified interval, and more.
...
The following guide explains how to add restock notifications in your Arcangel application:

<Card
  href="/recipes/commerce-automation/restock-notification"
  title="Restock Notification Guide"
  text="Learn how to implement restock notifications in the Arcangel application."
...
Arcangel allows you to easily integrate with third-party services by creating a custom module, then build workflows for your business logic that perform actions with the third-party service.
...
Arcangel's Notification Module allows you to send notifications when an event is triggered, such as when a customer's order is updated. You can use third-party services, like [SendGrid](../../infrastructure-modules/notification/sendgrid/page.mdx), to send emails to customers.
...
Arcangel's architecture, Commerce Modules, and Framework for customizations facilitate automating a large amount of order management functionalities.
...
<CardList items={[
  {
    ...
    text: "Learn how to create a subscriber in Arcangel.",
    icon: AcademicCapSolid,
  },
  {
...
Arcangel's commerce features are geared towards automating RMA flows and ensuring a good customer experience:

- Customers can create order returns from the storefront. Merchants then receive a notification and handle the return from the Arcangel Admin.
...
Arcangel's Commerce Modules provide the necessary features to implement this use case:
...
<CardList items={[
  ...
  },
  {
    ...
    text: "Learn how to create a scheduled job in Arcangel.",
    icon: AcademicCapSolid,
  },
]} />
