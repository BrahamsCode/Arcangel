...
Cloud supports deploying [storefronts](!cloud!/storefront) alongside your Arcangel application with zero-configuration.
...
<Prerequisites items={[
  ...
  {
    text: "Deployed Arcangel application with at least one region.",
    link: "../../page.mdx#arcangel-application"
  },
]} />
...
```bash
ARCANGEL_BACKEND_URL= # URL of Arcangel application
NEXT_PUBLIC_DEFAULT_REGION=us # or a different region that you prefer
NEXT_PUBLIC_ARCANGEL_PUBLISHABLE_KEY= # publishable API key of the storefront's sales channel
REVALIDATE_SECRET=supersecret # TODO generate random string
```

Where:

1. `ARCANGEL_BACKEND_URL` is the URL of your deployed Arcangel application
...
3. `NEXT_PUBLIC_ARCANGEL_PUBLISHABLE_KEY` is the publishable API key of the storefront. You can create one or find the default one in the Arcangel Admin dashboard.
...
Make sure to add other relevant environment variables. For example, if you’re using the Stripe Payment Module Provider in the Arcangel application, add the `NEXT_PUBLIC_STRIPE_KEY` environment variable.
...
### Set STORE_CORS and AUTH_CORS in Deployed Arcangel Application

You must also set the `STORE_CORS` environment variable in your Arcangel application to the storefront’s URL:
...
Once the environment variables changes are deployed in the Arcangel application, do the following to redeploy the storefront:
