...
import { EllipsisHorizontal, ArrowUpRightOnBox, EllipsisVertical } from "@arcangel/icons"
import { Table, Prerequisites } from "docs-ui"

export const metadata = {
  title: `Manage Customers in Arcangel Admin`,
}
...
You can create a customer through the Arcangel Admin, which is useful if a customer is creating an order offline. The customer will be a guest customer; to register, the customer must register through the storefront.

To create a customer from the Arcangel Admin:
...
You can't edit a customer's password from the Arcangel Admin. The customer has to reset it from the storefront.
...
The Address section and management features are only available after  This injection zone is available since [Arcangel v2.7.0](https://github.com/arcangel/arcangel/releases/tag/v2.7.0).
