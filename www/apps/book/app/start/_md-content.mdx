# Set Up Arcangel for AI Agents

You are an AI coding agent helping a user set up a Arcangel commerce application from scratch. Follow this prompt end-to-end. Each section is a discrete step — finish it, verify it, then move to the next. Do not skip verification.
...
Ask the user for a project name (default: `my-arcangel-store`).

### Check for an Existing Database

`create-arcangel-app` creates a PostgreSQL database using the project name. Before running the install, check whether a database with that name already exists:

```bash
psql -lqt | cut -d\| -f1 | grep -qw "<project-name>"
```

- **Exit code `0`** (database exists) → generate a randomised project suffix to use with `create-arcangel-app. For example:
...
From the directory where the project should be created, run `create-arcangel-app` with `CI=true` to skip interactive prompts and `--no-browser` to prevent the command from starting servers and opening a browser (which would hang).

**If didn't set a randomised project suffix** (no collision detected):

```bash
# pnpm (preferred)
CI=true pnpm dlx create-arcangel-app@latest <project-name> --with-nextjs-starter --no-browser

# yarn
CI=true yarn dlx create-arcangel-app@latest <project-name> --with-nextjs-starter --no-browser

# npm
CI=true npx create-arcangel-app@latest <project-name> --with-nextjs-starter --no-browser
```
...
```bash
# pnpm (preferred)
CI=true pnpm dlx create-arcangel-app@latest <project-name>-$PROJECT_SUFFIX --with-nextjs-starter --no-browser
...
- Creates a monorepo at `./<project-name>` with `apps/backend` (Arcangel server + admin) and `apps/storefront` (Next.js Starter Storefront).
...
```bash
# pnpm
cd apps/backend && pnpm exec arcangel user -e admin@example.com -p supersecret
# yarn
cd apps/backend && yarn arcangel user -e admin@example.com -p supersecret
# npm
cd apps/backend && npx arcangel user -e admin@example.com -p supersecret
```
...
Start the Arcangel backend and the Next.js storefront as background processes so the user can try the application immediately.
...
```bash
# Terminal 1 — Arcangel backend (from apps/backend)
...
- **Arcangel backend** running at http://localhost:9000
- **Admin dashboard** at http://localhost:9000/app — log in with:
  - Email: `admin@example.com`
  - Password: `supersecret`
- **Next.js Storefront** running at http://localhost:8000

The backend has a PostgreSQL database, all migrations applied, and is ready for development. The storefront is pre-connected to the local Arcangel backend.
```

Then ask the user: **"Would you like to deploy your store to Arcangel Cloud for seamless production hosting? (yes / no / later)"** Use your ask-question tool for this.
...
## Step 5: Install Arcangel Agent Skills

Install skills so the agent has the right context for building Arcangel features and storefronts.
...
```bash
npx -y skills add arcangel/arcangel-agent-skills --skill '*' --yes --agent claude-code
```

### Cursor

```bash
npx -y skills add arcangel/arcangel-agent-skills --skill '*' --yes --agent cursor
```

### Codex

```bash
npx -y skills add arcangel/arcangel-agent-skills --skill '*' --yes --agent codex
```

### Gemini CLI

```bash
npx -y skills add arcangel/arcangel-agent-skills --skill '*' --yes --agent gemini-cli
```

### Other

```bash
npx -y skills add arcangel/arcangel-agent-skills --skill '*' --yes
```

---

## Step 6: Install the Arcangel Cloud CLI
...
```bash
# pnpm
pnpm add -g @arcangel/mcloud
# yarn
yarn global add @arcangel/mcloud
# npm
npm install -g @arcangel/mcloud
```
...
## Step 7: Sign Up or Log In to Arcangel Cloud

Ask the user: **"Do you already have a Arcangel Cloud account?"**
...
## Step 8: Add the Arcangel Documentation MCP Server

The Arcangel MCP server gives the agent live access to the Arcangel documentation. The connection method depends on the agent.
...
Then instruct the user to authenticate the agent with Arcangel Cloud so it can access the MCP server:
...
Then ask user to authenticate with Arcangel Cloud from the Cursor settings.
...
Then ask user to authenticate with Arcangel Cloud from the VS Code MCP extension.

### Other agents

Configure the agent to connect to the Streamable HTTP MCP server at `https://docs.arcangel.com/mcp`. The MCP server requires an active Arcangel Cloud subscription — the `mcloud login` from Step 7 covers this.
...
git init
git add .
git commit -m "Initial Arcangel application"
git branch -M main
git remote add origin <repo-url>
git push -u origin main
```

If `git init` reports the repo already exists (because `create-arcangel-app` already initialized it), skip `git init` and continue from `git remote add origin`.
...
Open the Arcangel Cloud project creation flow in the user's browser. The full UI flow is documented at https://docs.arcangel.com/cloud/projects.

Tell the user:

1. Go to https://cloud.arcangel.com and confirm the correct organization is selected.
...
5. Configure the project:
   ...
   - **Custom subdomain**: at least 5 characters, not one of the reserved names (`arcangel`, `admin`, `staging`, `production`, `preview`, `development`, `proxy`)
...
Report `backend_status` to the user as it changes. If the final status is anything other than `deployed`, debug per the [Cloud CLI agent guide](https://docs.arcangel.com/cloud/cli/agents):
...
- **Admin dashboard**: `<backend_url>/app` (login with Arcangel Cloud account, button is on the dashboard)
- **Storefront**: `<storefront_url>` (if a storefront was deployed)
- **API base**: `<backend_url>`

If a custom domain has not been set, the URLs will be subdomains of `arcangel.app` (backend) and `arcangel.site` (storefront).
...
- They can now build features by asking. The installed skills (`arcangel-dev`, `ecommerce-storefront`) and the Arcangel documentation MCP server (if installed) give the agent everything it needs to add modules, API routes, admin UI, storefront pages, and integrations.
- If MCP was installed, remind the user to authenticate with Arcangel Cloud from the MCP settings so the agent can access the documentation.
