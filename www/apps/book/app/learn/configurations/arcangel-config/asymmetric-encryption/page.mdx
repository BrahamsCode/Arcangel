...
In this chapter, you'll learn how to configure asymmetric encryption in Arcangel using public/private key pairs instead of a shared secret.

## What is Asymmetric Encryption?

By default, Arcangel uses symmetric JWT authentication, where the same secret signs and verifies tokens. With asymmetric encryption, you use a private key to sign tokens and a public key to verify them.
...
<Table>
  ...
  <Table.Body>
    <Table.Row>
      ...
      <Table.Cell>
        Running multiple Arcangel instances behind a load balancer.
      </Table.Cell>
      <Table.Cell>
    ...
    <Table.Row>
      ...
      <Table.Cell>
        Arcangel as part of a larger microservices ecosystem.
      </Table.Cell>
      <Table.Cell>
...
Run the following command in your Arcangel application:
...
To configure asymmetric encryption, you need to set up both signing and verification options in your `arcangel-config.ts` file.

In `arcangel-config.ts`, create a helper function to load the JWT configuration, and use it in the exported configuration:
...
1. [http options](../page.mdx#http): You set the global JWT options for Arcangel's HTTP layer, which are used to sign and verify JWT authentication tokens.
    - Refer to the [Arcangel Configuration chapter](../page.mdx#http) for more details on these options and their default values.
...
JWT_EXPIRES_IN=1d
JWT_ISSUER=arcangel
JWT_AUDIENCE=arcangel-api
JWT_KEYID=arcangel-key-1
```
...
This section explains how to set up JWKS support in Arcangel and verify tokens using JWKS.

### Step 1: Install Required Packages

First, install the packages for handling JWKS and JWT verification. Run the following command in your Arcangel application:
...
```bash
JWKS_CONTENT='{"keys":[{"kty":"RSA","use":"sig","kid":"arcangel-key-1","n":"vTtLGDIK...","e":"AQAB"}]}'
```
...
If you prefer not to use an environment variable, manually convert your public key to JWK format using the JWT configurations you set in `arcangel-config.ts`.
  ...
  try {
    ...
    const jwks = {
      keys: [{
        ...jwk,
        use: "sig",
        kid: projectConfig.http.jwtOptions?.keyid || "arcangel-key-1",
...
2. If not, retrieve the public key from the Arcangel configuration and convert it to JWK format using Node's `crypto` module.
...
const ARCANGEL_BACKEND_URL = process.env.ARCANGEL_BACKEND_URL || "http://localhost:9000"

// Create JWKS client with caching
const jwksClient = new JwksClient({
  // This is the API route where your JWKS is exposed
  jwksUri: `${ARCANGEL_BACKEND_URL}/.well-known/jwks.json`,
  cache: true,
  rateLimit: true,
...
To test the JWKS verification, start the Arcangel application:
...
Make sure to replace the email and password with valid credentials for your Arcangel application.
