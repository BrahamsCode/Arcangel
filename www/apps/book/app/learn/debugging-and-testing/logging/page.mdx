...
In this chapter, you’ll learn how to use Arcangel’s logging utility.

## Logger Class

Arcangel provides a `Logger` class with advanced logging functionalities. This includes configuring logging levels or saving logs to a file.
...
The Arcangel application registers the `Logger` class in the Arcangel container and each module's container as `logger`. 

---

## How to Log a Message

Resolve the `logger` using the Arcangel container to log a message in your resource.
...
This creates a scheduled job that resolves the `logger` from the Arcangel container and uses it to log a message.

### Test the Scheduled Job

To test out the above scheduled job, start the Arcangel application:
...
Aside from showing the logs in the terminal, you can save the logs in a file by setting the `LOG_FILE` environment variable to the path of the file relative to the Arcangel server’s root directory.
...
Your logs are now saved in the `all.log` file at the root of your Arcangel application.
...
The `Logger` class has an `activity` method used to log a message of level `info`. If the Arcangel application is running in a development environment, a spinner starts to show the activity's progress.
