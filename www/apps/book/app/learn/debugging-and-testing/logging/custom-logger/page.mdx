...
In this guide, you'll learn how to override the default [Logger](../page.mdx) in Arcangel with your custom implementation.

## Why Override the Logger?

Arcangel's default `Logger` logs your application's events, errors, and general log messages into the console. However, you might want to customize this behavior for your specific use case.
...
After overriding the `Logger`, Arcangel will use your custom logger whenever it needs to log a message.
...
1. Create a class that extends the `Logger` interface from `@arcangel/framework/types`.
2. Pass the class in the Arcangel configurations.
...
Notice that you must export an instance of your custom logger class to use it in the Arcangel configurations.
...
### Step 2: Pass the Custom Logger in Arcangel Configurations

Next, to use the custom logger, set the `logger` configuration in your `arcangel-config.ts` file:
...
To test that your custom logger is working, start the Arcangel application:
