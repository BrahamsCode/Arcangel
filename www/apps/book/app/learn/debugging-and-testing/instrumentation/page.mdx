...
In this chapter, you'll learn about observability in Arcangel and how to configure instrumentation with OpenTelemetry.
...
Arcangel uses [OpenTelemetry](https://opentelemetry.io/) for instrumentation and reporting. When configured, it reports traces for:
...
## How to Configure Instrumentation in Arcangel?
...
As of [Arcangel v2.11.0](https://github.com/arcangel/arcangel/releases/tag/v2.11.0), OpenTelemetry dependencies are installed by default in new Arcangel projects. If you're using an older version of Arcangel, you need to install the `@opentelemetry/sdk-node`, `@opentelemetry/resources`, `@opentelemetry/sdk-trace-node`, and `@opentelemetry/instrumentation-pg` dependencies.
...
const exporter = new ZipkinExporter({
  serviceName: "my-arcangel-project",
})
...
In the `instrumentation.ts` file, you export a `register` function that uses Arcangel's `registerOtel` utility function. You also initialize an instance of the exporter, such as Zipkin, and pass it to the `registerOtel` function.
...
The `NodeSDKConfiguration` properties are accepted since Arcangel v2.5.1.
...
Then, start your Arcangel application:

```bash npm2yarn
npm run dev
```

Try to open the Arcangel Admin or send a request to an API route.
...
- [Integrate Sentry with Arcangel](!resources!/integrations/guides/sentry)