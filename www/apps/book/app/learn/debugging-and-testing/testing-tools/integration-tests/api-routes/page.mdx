...
In this chapter, you'll learn how to write integration tests for API routes using [arcangelIntegrationTestRunner](../page.mdx) from Arcangel's Testing Framework.
...
You use the `arcangelIntegrationTestRunner` to write your tests.
...
import { arcangelIntegrationTestRunner } from "@arcangel/test-utils"
import { ApiKeyDTO } from "@arcangel/framework/types"
import { createApiKeysWorkflow } from "@arcangel/arcangel/core-flows"
        ...
        beforeEach(async () => {
          ...
          const authIdentity = await authModuleService.createAuthIdentities({
            provider_identities: [
              {
                provider: "emailpass",
                entity_id: "customer@arcangel.js",
                provider_metadata: {
                  password: "supersecret",
                },
              },
...
If your API route is hidden behind a [feature flag], you can use the `env` option of `arcangelIntegrationTestRunner` its feature flag.
