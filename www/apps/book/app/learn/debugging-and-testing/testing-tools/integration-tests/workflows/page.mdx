...
In this chapter, you'll learn how to write integration tests for workflows using [arcangelIntegrationTestRunner](../page.mdx) from Arcangel's Testing Framework.
...
You use the `arcangelIntegrationTestRunner` to write an integration test for the workflow. The test passes if the workflow returns the string `"Hello, World!"`.
...
This runs your Arcangel application and runs the tests available under the `integration-tests/http` directory.
...
For example, if you have the following step in your workflow that throws a `ArcangelError`:
...
const step1 = createStep("step-1", () => {
  throw new ArcangelError(ArcangelError.Types.NOT_FOUND, "Item doesn't exist")
})
```
...
If you threw a `ArcangelError`, then you can check the error message in `errors[0].error.message`.
...
2. Resolve the [Workflow Engine Module](!resources!/infrastructure-modules/workflow-engine)'s service from the Arcangel container.
