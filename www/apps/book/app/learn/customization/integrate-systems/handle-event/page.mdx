...
export const metadata = {
  title: `${pageNumber} Guide: Sync Brands from Arcangel to Third-Party`,
}
...
Arcangel has an event system that emits events when an operation is performed. It allows you to listen to those events and perform an asynchronous action in a function called a [subscriber](../../../fundamentals/events-and-subscribers/page.mdx). This is useful to perform actions that aren't integral to the original flow, such as syncing data to a third-party system.

<Note>

Learn more about Arcangel's event system and subscribers in [this chapter](../../../fundamentals/events-and-subscribers/page.mdx).
...
Arcangel provides an `emitEventStep` that allows you to emit an event in your workflows. So, in the `createBrandWorkflow` defined in `src/workflows/create-brand.ts`, use the `emitEventStep` helper step after the `createBrandStep`:
...
- `useQueryGraphStep`: a step that Arcangel provides to retrieve data using [Query](../../../fundamentals/module-links/query/page.mdx). You'll use this to retrieve the brand's details using its ID.
...
![Directory structure of the Arcangel application after adding the file](https://res.cloudinary.com/dza7lstvk/image/upload/v1733493547/Arcangel%20Book/cms-dir-overview-4_u5t0ug.jpg)
...
You create the `syncBrandToCmsStep` that accepts a brand as an input. In the step, you resolve the CMS Module's service from the [Arcangel container](../../../fundamentals/arcangel-container/page.mdx) and use its `createBrand` method. This method will create the brand in the third-party CMS.
...
![Directory structure of the Arcangel application after adding the subscriber](https://res.cloudinary.com/dza7lstvk/image/upload/v1733493774/Arcangel%20Book/cms-dir-overview-5_iqqwvg.jpg)
...
export const subscriberHighlights = [
  ...
  ["9", "container", "The Arcangel container used to resolve resources."],
...
- `container`: The Arcangel container used to resolve Framework and commerce tools.

In the function, you execute the `syncBrandToCmsWorkflow`, passing it the data payload as an input. So, every time a brand is created, Arcangel will execute this function, which in turn executes the workflow to sync the brand to the CMS.
...
## Next Chapter: Sync Brand from Third-Party CMS to Arcangel

You can also automate syncing data from a third-party system to Arcangel at a regular interval. In the next chapter, you'll learn how to sync brands from the third-party CMS to Arcangel once a day.
