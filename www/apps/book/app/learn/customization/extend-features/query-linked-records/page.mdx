...
In the previous chapters, you [defined a link](../define-link/page.mdx) between the [custom Brand Module](../../custom-features/module/page.mdx) and Arcangel's [Product Module](!resources!/commerce-modules/product), then [extended the create-product flow](../extend-create-product/page.mdx) to link a product to a brand.

In this chapter, you'll learn how to retrieve a product's brand (and vice-versa) in two ways: Using Arcangel's existing API route, or in customizations, such as a custom API route.
...
You can also retrieve linked records using [Query](../../../fundamentals/module-links/query/page.mdx). Query allows you to retrieve data across modules with filters, pagination, and more. You can resolve Query from the Arcangel container and use it in your API route or workflow.
...
export const highlights = [
  ["11", "resolve", "Resolve Query from the Arcangel container."],
...
This adds a `GET` API route at `/admin/brands`. In the API route, you resolve Query from the Arcangel container. Query has a `graph` method that runs a query to retrieve data. It accepts an object having the following properties:
...
## Next Steps: Customize Arcangel Admin

Client applications, such as the Arcangel Admin dashboard, can now use brand-related features, such as creating a brand or setting the brand of a product.

In the next chapters, you'll learn how to customize the Arcangel Admin to show a product's brand on its details page, and to show a new page with the list of brands in your store.
