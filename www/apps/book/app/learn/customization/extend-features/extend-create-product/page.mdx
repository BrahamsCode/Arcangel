...
After linking the [custom Brand data model](../../custom-features/module/page.mdx) and Arcangel's [Product Module](!resources!/commerce-modules/product) in the [previous chapter](../define-link/page.mdx), you'll extend the create product workflow and API route to allow associating a brand with a product.

Some API routes, including the [Create Product API route](!api!/admin/products/create-product), accept an `additional_data` request body parameter. This parameter can hold custom data that's passed to the [hooks](../../../fundamentals/workflows/workflow-hooks/page.mdx) of the workflow executed in the API route, allowing you to consume those hooks and perform actions with the custom data.

So, in this chapter, to extend the create product flow and associate a brand with a product, you will:

- Consume the [productsCreated](!resources!/references/arcangel-workflows/createProductsWorkflow#productsCreated) hook of the [createProductsWorkflow](!resources!/references/arcangel-workflows/createProductsWorkflow), which is executed within the workflow after the product is created. You'll link the product with the brand passed in the `additional_data` parameter.
...
The [createProductsWorkflow](!resources!/references/arcangel-workflows/createProductsWorkflow) used in the [Create Product API route](!api!/admin/products/create-product) has a `productsCreated` hook that runs after the product is created. You'll consume this hook to link the created product with the brand specified in the request parameters.

To consume the `productsCreated` hook, create the file `src/workflows/hooks/created-product.ts` with the following content:

![Directory structure after creating the hook's file.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733384338/Arcangel%20Book/brands-hook-dir-overview_ltwr5h.jpg)
...
2. An object of properties related to the step's context. It has a `container` property whose value is the [Arcangel container](../../../fundamentals/arcangel-container/page.mdx) to resolve Framework and commerce tools.
...
![Diagram showcasing how the order of defining a link affects creating the link](https://res.cloudinary.com/dza7lstvk/image/upload/v1733386156/Arcangel%20Book/remote-link-brand-product-exp_fhjmg4.jpg)
...
![Directory structure after adding the middelwares file](https://res.cloudinary.com/dza7lstvk/image/upload/v1733386868/Arcangel%20Book/brands-middleware-dir-overview_uczos1.jpg)
...
Make sure to replace `{token}` with the token you received from the previous request, `shipping_profile_id` with the ID of a shipping profile in your application, and `{brand_id}` with the ID of a brand in your application. You can retrieve the ID of a shipping profile either from the Arcangel Admin, or the [List Shipping Profiles API route](!api!/admin/shipping-profiles/list-shipping-profiles).

The request creates a product and returns it.

In the Arcangel application's logs, you'll find the message `Linked brand to products`, indicating that the workflow hook handler ran and linked the brand to the products.
