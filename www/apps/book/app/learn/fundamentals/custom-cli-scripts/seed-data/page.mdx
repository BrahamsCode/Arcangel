...
In the CLI script, use your custom workflows or Arcangel's existing workflows, which you can browse in [this reference](!resources!/arcangel-workflows-reference), to seed data.
...
You create the generated products using the `createProductsWorkflow` imported previously from `@arcangel/arcangel/core-flows`. It accepts the product data as input, and returns the created products.
...
Finally, you use the `createInventoryLevelsWorkflow` from Arcangel's core workflows to create the inventory levels.
...
```bash npx2yarn
npx arcangel exec ./src/scripts/demo-products.ts
```

This seeds the products to your database. If you run your Arcangel application and view the products in the dashboard, you'll find fifty new products.
