...
Instead of you writing the migration manually, the Arcangel CLI tool provides a [db:generate](!resources!/arcangel-cli/commands/db#dbgenerate) command to generate a migration for a modules' data models.
...
The migration class in the file extends the `Migration` class imported from `@arcangel/framework/mikro-orm/migrations`. In the `up` and `down` method of the migration class, you use the `addSql` method provided by MikroORM's `Migration` class to run PostgreSQL syntax.
...
When you write a migration manually for a data model defined with [model.define](../page.mdx), make sure the created table includes the `created_at`, `updated_at`, and `deleted_at` columns. Arcangel adds these [default properties](../properties/page.mdx#data-models-default-properties) to every data model, and the module's service relies on them in its queries. For example, the service filters out soft-deleted records using the `deleted_at` column, so queries fail at runtime if the column doesn't exist.
...
For example, if you delete the migration of the Blog Module, then try to create a new one, Arcangel will create a brand new migration that re-creates the tables or indices. If those are still in the database, you might encounter errors.
...
To learn more about the Arcangel CLI's database commands, refer to [this CLI reference](!resources!/arcangel-cli/commands/db).
...
In those scenarios, you can instead create a data migration script. They are asynchronous function that the Arcangel application executes once when you run migrations.
...
export default async function migrateBlogData({ container }: ExecArgs) {
  ...
  if (!ArcangelModule.isInstalled(BLOG_MODULE)) {
    return
  }
...
In the above example, you default export an asynchronous function that receives an object parameter with the [Arcangel Container](../../arcangel-container/page.mdx) property.
...
Arcangel will run any pending migrations and migration scripts, including your script.

If the script runs successfully, Arcangel won't run the script again.

If there are errors in the script, you'll receive an error in the migration script logs. Arcangel will keep running the script every time you run the migration command until it runs successfully.
