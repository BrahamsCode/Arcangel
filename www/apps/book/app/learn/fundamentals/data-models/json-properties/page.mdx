...
The type argument is supported since [Arcangel v2.19.0](https://github.com/arcangel/arcangel/releases/tag/v2.19.0).
...
Most data models in Arcangel's Commerce Modules have a `metadata` property that is a JSON object. `metadata` allows you to store custom information that is not part of the core data model.
...
### How Arcangel Updates JSON Properties

Consider a Brand Module with a `Brand` data model as shown [in the previous section](#what-is-a-json-property). The [module's service](../../modules/page.mdx#2-create-service) will extend `ArcangelService`, which generates methods like [updateBrands](!resources!/service-factory-reference/methods/update) to update a brand.

When you pass a JSON property in the `updateBrands` method, Arcangel will merge the provided JSON object with the existing one in the database. So, only the properties you pass will be updated, and the rest will remain unchanged.
...
The following examples assume you have a `brandModuleService` that is resolved from the [Arcangel container](../../arcangel-container/page.mdx). For example:
