...
By default, Arcangel creates the following properties for every data model:
...
- `deleted_at`: A [dateTime](#dateTime) property that stores when a record of the data model was deleted. When you soft-delete a record, Arcangel sets the `deleted_at` property to the current date.
...
This property is only available after [Arcangel v2.1.2](https://github.com/arcangel/arcangel/releases/tag/v2.1.2).
...
Arcangel stores along the `bigNumber` property a `raw_{property_name}` property that contains its raw value. Learn more in the [Big Number documentation](../big-numbers/page.mdx).
...
<Prerequisites
  items={[
    {
      text: "Arcangel v2.13.0+",
      link: "https://github.com/arcangel/arcangel/releases/tag/v2.13.0"
    },
    {
...
When a property is marked as translatable, merchants can manage translations for these fields through the [Arcangel Admin](!user-guide!/settings/translations).
