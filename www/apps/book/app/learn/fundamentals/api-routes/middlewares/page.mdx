...
Middlewares of all types are defined in the special file `src/api/middlewares.ts`. Use the `defineMiddlewares` function from the Arcangel Framework to define the middlewares, and export its value.
...
- `matcher`: the API route path to apply the middleware on. Its value can be one of the following:
    - A string, which Arcangel compiles using [path-to-regexp](https://github.com/pillarjs/path-to-regexp). So, the string can include patterns like `/custom*`.
    - A regular expression (`RegExp`), which Arcangel matches against the request path directly. For example, `/^\/custom(\/.*)?$/`.
...
A `RegExp` matcher must account for the full request path, including the leading slash. How Arcangel matches the pattern depends on whether the middleware is a global or route middleware.

### Global Middleware

For a global middleware (a middleware without a `method`), Arcangel registers the `RegExp` using Express's [app.use](https://expressjs.com/en/4x/api.html#app.use) method. Express matches a `RegExp` in `app.use` only if the pattern matches from the start of the request path.
...
For a route middleware (a middleware with a `method`), Arcangel matches the `RegExp` against the full request path. So, an unanchored pattern like `/custom/` matches routes containing `custom`.
...
1. A request object of type `ArcangelRequest`.
2. A response object of type `ArcangelResponse`.
3. A function of type `ArcangelNextFunction` that executes the next middleware in the stack.
...
The information explained in this section is applicable starting from [Arcangel v2.6](https://github.com/arcangel/arcangel/releases/tag/v2.6).

</Note>

### Middleware and Routes Execution Order

The Arcangel application registers middlewares and API route handlers in the following order, stacking them on top of each other:

![Diagram showcasing the order in which middlewares and route handlers are registered.](https://res.cloudinary.com/dza7lstvk/image/upload/v1746776911/Arcangel%20Book/middleware-registration-overview_spc02f.jpg)

1. Global middlewares in the following order:
    1. Global middleware defined in the Arcangel's core.
...
2. Route middlewares in the following order:
    1. Route middleware defined in the Arcangel's core.
...
3. API routes in the following order:
    1. API routes defined in the Arcangel's core.
...
![Diagram showcasing the order in which middlewares and route handlers are executed when a request is sent to an API route.](https://res.cloudinary.com/dza7lstvk/image/upload/v1746776172/Arcangel%20Book/middleware-order-overview_h7kzfl.jpg)
...
On top of the previous ordering, Arcangel sorts global and route middlewares based on their matcher pattern in the following order:
...
![Diagram showcasing the order in which middlewares are sorted before being registered.](https://res.cloudinary.com/dza7lstvk/image/upload/v1746777297/Arcangel%20Book/middleware-registration-sorting_oyfqhw.jpg)
