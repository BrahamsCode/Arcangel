...
In this chapter, you'll learn the approach recommended when you need to override an existing API route in Arcangel.

## Approaches to Consider Instead of Overriding API Routes

While building customizations in your Arcangel application, you may need to make changes to existing API routes for your business use case.

Arcangel provides the following approaches to customize API routes:
...
By replicating instead of overriding, the original API route remains intact, allowing you to easily revert to the original functionality if needed. You can also update your Arcangel version without worrying about breaking changes in the original API route.

---

## How to Replicate an API Route?

Arcangel's API routes are generally slim and use logic contained in [workflows](../../workflows/page.mdx). So, creating a custom route based on the original route is straightforward.

You can view the source code for Arcangel's API routes in the [Arcangel GitHub repository](https://github.com/arcangel/arcangel/tree/develop/packages/arcangel/src/api).

For example, if you need to allow vendors to access the `POST /admin/products` API route, you can create an API route in your Arcangel project at `src/api/vendor/products/route.ts` with the [same code as the original route](https://github.com/arcangel/arcangel/blob/develop/packages/arcangel/src/api/admin/products/route.ts#L88). Then, you can make changes to it or its middlewares.
