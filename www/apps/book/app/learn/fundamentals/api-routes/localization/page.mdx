...
In this chapter, you'll learn how to handle localization in API routes of your Arcangel application to serve content in different languages.
...
Localization in API routes allows you to serve translated content based on the user's preferred language. The Arcangel application provides built-in support for handling locale information in API requests and retrieving localized data.
...
The Arcangel application automatically supports retrieving localized content from all routes under the `/store` prefix, including both core and custom store API routes.
...
2. The `x-arcangel-locale` request header
...
After applying the `applyLocale` middleware, you can access the request's locale from the `locale` property of the `ArcangelRequest` object.
