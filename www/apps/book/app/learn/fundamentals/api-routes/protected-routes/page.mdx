...
You can protect API routes using the `authenticate` [middleware](../middlewares/page.mdx) from the Arcangel Framework. When applied to a route, the middleware checks that:
...
3. An optional object of configurations accepting the following properties:
    ...
    - `requireMfa` (default: `true` for authenticated routes, since [Arcangel v2.20.0](https://github.com/arcangel/arcangel/releases/tag/v2.20.0)): A boolean or an object `{ maxAgeSeconds: number }` indicating whether a completed MFA challenge is required to access this route. When set to `true`, any request from an identity with MFA enabled must have completed an MFA challenge. When set to `{ maxAgeSeconds: number }`, the challenge must also have been completed within the specified number of seconds. Set to `false` to allow access regardless of MFA status (for example, on the MFA challenge verification route itself). Learn more in the [Multi-Factor Authentication guide](!resources!/commerce-modules/auth/mfa).
...
### Override Authentication for Arcangel's API Routes

In some cases, you may want to override the authentication requirement for Arcangel's API routes. For example, you may want to allow custom actor types to access existing protected API routes.
...
To access the authentication details in an API route, such as the logged-in user's ID, set the type of the first request parameter to `AuthenticatedArcangelRequest`. It extends `ArcangelRequest`:

```ts highlights={[["7", "AuthenticatedArcangelRequest"]]}
import type {
  AuthenticatedArcangelRequest,
  ArcangelResponse,
} from "@arcangel/framework/http"
...
The `auth_context.actor_id` property of `AuthenticatedArcangelRequest` holds the ID of the authenticated user or customer. If there isn't any authenticated user or customer, `auth_context` is `undefined`.
...
In this example, you retrieve the ID of the authenticated user, customer, or custom actor type from the `auth_context` property of the `AuthenticatedArcangelRequest` object.
...
In this example, you retrieve the customer's ID and resolve Query from the [Arcangel container](../../arcangel-container/page.mdx).
...
You can access the logged-in admin user’s ID in all API routes starting with `/admin` using the `auth_context.actor_id` property of the `AuthenticatedArcangelRequest` object. You can then use [Query](../../module-links/query/page.mdx) to retrieve the user details, or pass the ID to a [workflow](../../workflows/page.mdx) that performs business logic.
...
In this example, you retrieve the admin user's ID and resolve Query from the [Arcangel container](../../arcangel-container/page.mdx).
