...
```bash npx2yarn
npx arcangel db:sync-links
```

The Arcangel application creates a new table for your module link to store the IDs of linked records.
...
When you define a module link, the Arcangel application creates a table in the database for that module link. The table's name is a combination of the names of the two data models linked in the format `module1_table1_module2_table2`, where:
...
![Diagram illustration for module links](https://res.cloudinary.com/dza7lstvk/image/upload/v1741696766/Arcangel%20Book/custom-links_vezsx8.jpg)
...
- `alias`: A custom alias to use when querying the linked records. By default, Arcangel derives the alias from the data model's name. Available since Arcangel [v2.17.2](https://github.com/arcangel/arcangel/releases/tag/v2.17.2).
...
When you define a link using the object form, Arcangel derives the query alias for the linked data model from the data model's name. You can override this alias by passing the `alias` property.

<Note>

This option is available since Arcangel [v2.17.2](https://github.com/arcangel/arcangel/releases/tag/v2.17.2).
...
For example, if you have the link table `product_product_blog_post` and you rename the Blog Module from `blog` to `article`, Arcangel considers the old link definition deleted. Then, when you run the `db:sync-links` or `db:migrate` command, Arcangel will ask if you want to delete the old link table, and will create a new one with the new name `product_product_article_post`.
