...
In all resources that can access the [Arcangel Container](../../arcangel-container/page.mdx), such as API routes or workflows, you can resolve Query to fetch data across custom modules and Arcangel's Commerce Modules.
...
export const exampleHighlights = [
  ["13", "", "Resolve Query from the Arcangel container."],
...
In the above example, you resolve Query from the Arcangel container using the `ContainerRegistrationKeys.QUERY` (`query`) key.
...
When you use the `query.graph` method, you're running a query through an internal graph that the Arcangel application creates.
...
These data models have computed total properties, such as `total`, `subtotal`, and `tax_total`. Arcangel calculates these totals in the application, not the database, and it only calculates them when you request at least one total property, or when you pass `*`.

So, when you pass `*` in `fields`, Arcangel:
...
As mentioned in the [Module Link](../page.mdx) documentation, Arcangel creates a table for your module link. So, not only can you retrieve linked records, but you can also retrieve the records in a module link's table.
...
The `withDeleted` property is available from [Arcangel v2.8.5](https://github.com/arcangel/arcangel/releases/tag/v2.8.5).
...
Caching options are available from [Arcangel v2.11.0](https://github.com/arcangel/arcangel/releases/tag/v2.11.0).
...
In the example above, you specify the providers with ID `caching-redis` and `caching-memcached` to cache the query results. These IDs must match the IDs of the providers in `arcangel-config.ts`.
...
For API routes that retrieve a single or list of resources, Arcangel provides a `validateAndTransformQuery` middleware that:
...
1. A Zod validation schema for the query parameters, which you can learn more about in the [API Route Validation documentation](../../../fundamentals/api-routes/validation/page.mdx). Arcangel has a `createFindParams` utility that generates a Zod schema that accepts four query parameters:
...
The middleware transforms these parameters to configurations that you can pass to Query in your API route handler. These configurations are stored in the `queryConfig` parameter of the `ArcangelRequest` object.

<Note>

As of [Arcangel v2.2.0](https://github.com/arcangel/arcangel/releases/tag/v2.2.0), `remoteQueryConfig` has been deprecated in favor of `queryConfig`. Their usage is still the same, only the property name has changed.
...
To test it out, start your Arcangel application and send a `GET` request to the `/customs` API route. A list of records is retrieved with the specified fields in the middleware.
