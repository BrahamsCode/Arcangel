...
As of [Arcangel v2.2.0](https://github.com/arcangel/arcangel/releases/tag/v2.2.0), Remote Link has been deprecated in favor of Link. They have the same usage, so you only need to change the key used to resolve the tool from the Arcangel container as explained below.

</Note>

## What is Link?

Link is a class with utility methods to manage links between data models. It’s registered in the Arcangel container under the `link` registration name.
...
To perform link operations in a [workflow](../../workflows/page.mdx), Arcangel provides the following steps out-of-the-box:
...
Arcangel enforces integrity constraints on links based on the link's relation type. So, an error is thrown in the following scenarios:
...
```ts
import { Modules } from "@arcangel/framework/utils"
import { dismissRemoteLinkStep } from "@arcangel/arcangel/core-flows"
import { 
  createWorkflow,
  transform,
} from "@arcangel/framework/workflows-sdk"
...
```ts
import { Modules } from "@arcangel/framework/utils"
import { removeRemoteLinkStep } from "@arcangel/arcangel/core-flows"
import { 
  createWorkflow,
  transform,
} from "@arcangel/framework/workflows-sdk"
...
```ts
import { Modules } from "@arcangel/framework/utils"
import { updateRemoteLinksStep } from "@arcangel/arcangel/core-flows"
import { 
  createWorkflow,
  transform,
} from "@arcangel/framework/workflows-sdk"
