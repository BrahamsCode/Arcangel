...
The Index Module is experimental and still in development, so it is subject to change. However, it's being actively used in the Arcangel Admin dashboard and is increasingly stable for production use. Consider your application's tolerance for minor issues when deciding to implement it.
...
![Diagram showcasing how data is retrieved from the Index Module's data store](https://res.cloudinary.com/dza7lstvk/image/upload/v1747988533/Arcangel%20Book/index-module_epurmt.jpg)

### Ingested Data Models

By default, Arcangel only ingests the `Product`, `ProductVariant`, `Price`, `PriceSet`, and `SalesChannel` data models into the Index Module's data store.

You can also ingest custom data models into the Index Module, as explained in the [How to Ingest Custom Data Models](#how-to-ingest-custom-data-models) section. Arcangel will then ingest the custom and core data models into the Index Module's data store.

---

## How to Install the Index Module

To install the Index Module, run the following command in your Arcangel project to install its package:

```bash npm2yarn
npm install @arcangel/index
```

Then, add the Index Module to your Arcangel configuration in `arcangel-config.ts`:
...
The Index Module only ingests data when you start your Arcangel server. So, to [ingest data models](#ingested-data-models), start the Arcangel application:
...
These data models have computed total properties, such as `total`, `subtotal`, and `tax_total`. Arcangel calculates these totals in the application, not the database, and it only calculates them when you request at least one total property, or when you pass `*`. Calculating them also loads relations, such as `items` and `shipping_methods` with their tax lines and adjustments, even if you don't use them.
...
Then, start the Arcangel application:
...
The Index API routes are available from [Arcangel v2.11.2](https://github.com/arcangel/arcangel/releases/tag/v2.11.2).

</Note>

Arcangel provides API routes to view and trigger index reingestion or syncing. This is useful if you want to reingest data manually, for example, after a large data import.
