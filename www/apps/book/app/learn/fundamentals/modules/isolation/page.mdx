...
A module has its own container, as explained in the [Module Container](../container/page.mdx) chapter. This container includes the module's resources, such as services and data models, and some Framework resources that the Arcangel application provides.

<Note>

Refer to the [Module Container Resources](!resources!/arcangel-container-resources) for a list of resources registered in a module's container.
...
- Integrate your module into any Arcangel application without side-effects to your setup.
...
A module has in its container some Framework APIs and tools, such as [Logger](../../../debugging-and-testing/logging/page.mdx). You can refer to the [Module Container Resources](!resources!/arcangel-container-resources) for a list of resources registered in a module's container.
...
Some Framework APIs and tools are not registered in the module's container. For example, [Query](../../module-links/query/page.mdx) is only registered in the Arcangel container.
...
The following services are available in every module's container by default, without any `dependencies` declaration: `logger`, `event_bus`, `manager`, `configModule`, and `pgConnection`. Refer to the [Module Container Resources](!resources!/arcangel-container-resources) for the full list.

</Note>

For other Infrastructure Modules not in the default set, you can inject them into your module's service in `arcangel-config.ts` using the `dependencies` property of the module's configuration.
