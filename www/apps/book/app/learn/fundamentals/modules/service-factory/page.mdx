...
Arcangel provides a service factory that your module’s main service can extend.
...
Arcangel provides the service factory as a `ArcangelService` function your service extends. The function creates and returns a service class with generated data-management methods.
...
### ArcangelService Parameters

The `ArcangelService` function accepts one parameter, which is an object of data models for which to generate data-management methods.

In the example above, since the `BlogModuleService` extends `ArcangelService`, it has methods to manage the `Post` data model, such as `createPosts`.
...
The method names are the operation name, suffixed by the data model's key in the object parameter passed to `ArcangelService`.
...
The service factory also generates internal services for each data model passed to the `ArcangelService` function. These services are registered in the module's container and can be resolved using their camel-cased names.
