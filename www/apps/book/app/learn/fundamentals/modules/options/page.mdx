...
In this chapter, you’ll learn about passing options to your module from the Arcangel application’s configurations and using them in the module’s resources.
...
To pass options to a module, add an `options` property to the module’s configuration in `arcangel-config.ts`.
...
```ts title="arcangel-config.ts"
import { defineConfig } from "@arcangel/framework/utils"
module.exports = defineConfig({
  plugins: [
    {
...
export default async function helloWorldLoader({
  ...
  console.log(
    "[BLOG MODULE] Just started the Arcangel application!",
    options
  )
}
```
...
If you expect a certain option and want to throw an error if it's not provided or isn't valid, it's recommended to perform the validation in a loader. The module's service is only instantiated when it's used, whereas the loader runs when the Arcangel application starts.
...
Now, when the Arcangel application starts, the loader will run, validating the module's options and throwing an error if the `apiKey` option is missing.
