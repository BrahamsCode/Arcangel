...
Run the Arcangel application and send a `GET` request to `/workflow`:
...
## Resolve Resources from the Arcangel Container

The compensation function receives an object second parameter. The object has a `container` property that you use to resolve resources from the Arcangel container.
...
This feature is only available after [Arcangel v2.0.5](https://github.com/arcangel/arcangel/releases/tag/v2.0.5).
