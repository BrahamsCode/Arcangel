...
Locks are particularly useful in workflows where concurrent executions may lead to commerce risks. For example, Arcangel uses locks in its cart workflows to prevent issues like overselling products or double charging customers.
...
Arcangel provides two steps that you can use to create locks in your workflows:

- [acquireLockStep](!resources!/references/arcangel-workflows/steps/acquireLockStep): Attempt to acquire a lock. If the lock is already held by another process, this step will wait until the lock is released. It will fail if a timeout occurs before acquiring the lock.
- [releaseLockStep](!resources!/references/arcangel-workflows/steps/releaseLockStep): Release a previously acquired lock.
...
Refer to the [acquireLockStep](!resources!/references/arcangel-workflows/steps/acquireLockStep) and [releaseLockStep](!resources!/references/arcangel-workflows/steps/releaseLockStep) for more information on the inputs of these steps.
...
For example, consider a custom workflow that executes Arcangel's [completeCartWorkflow](!resources!/references/arcangel-workflows/completeCartWorkflow). The `completeCartWorkflow` acquires locks on the cart with `acquireLockStep` to prevent race conditions.
