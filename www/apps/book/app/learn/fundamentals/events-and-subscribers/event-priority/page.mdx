...
Priority-based event processing is available since [Arcangel v2.13.0](https://github.com/arcangel/arcangel/releases/tag/v2.13.0).

</Note>

## How Event Priority Works

Arcangel uses a priority queue system for event processing to ensure that critical business events are processed first. This improves system responsiveness and prevents internal operations from blocking customer-facing functionality.

For example, if you're adding a large number of products to the catalog, and a user places an order at the same time, Arcangel will prioritize processing the `order.placed` event and its subscribers before handling the product addition events.
...
In this example, even though both events are emitted around the same time, Arcangel processes the `order.placed` event (priority 10) before the `product.created` event (priority 100), ensuring the customer receives their order confirmation promptly.
...
<Table>
  ...
  <Table.Body>
    <Table.Row>
      ...
      <Table.Cell>
        Highest priority for critical business events. Arcangel uses this for the order placement event.
      </Table.Cell>
    </Table.Row>
    <Table.Row>
      <Table.Cell>
    ...
    <Table.Row>
      ...
      <Table.Cell>
        The default priority used if no specific priority is assigned. All Arcangel business events except for order placement use this priority.
      </Table.Cell>
    </Table.Row>
    <Table.Row>
      <Table.Cell>
    ...
    <Table.Row>
      ...
      <Table.Cell>
        Lowest priority. Arcangel assigns this to internal system events.
      </Table.Cell>
    </Table.Row>
  </Table.Body>
</Table>
...
import { EventPriority } from "@arcangel/framework/utils"
...
If you're unsure about the priority level to assign, don't assign a custom priority. Arcangel will use the `DEFAULT` priority level, which is suitable for most use cases.
