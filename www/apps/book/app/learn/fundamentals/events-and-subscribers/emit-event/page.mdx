...
There are two types of events in Arcangel:

1. Workflow event: an event that's emitted in a workflow after a commerce feature is performed. For example, Arcangel emits the `order.placed` event after a cart is completed.
...
To emit a workflow event, use the `emitEventStep` helper step provided in the `@arcangel/arcangel/core-flows` package. It allows you to emit an event from within a workflow, and it only emits the event after the workflow has finished successfully.
...
Event priority is available since [Arcangel v2.13.0](https://github.com/arcangel/arcangel/releases/tag/v2.13.0).
...
The Event Module's service (`event_bus`) is available in every module's container by default. You don't need to add it to `dependencies` in `arcangel-config.ts`.
