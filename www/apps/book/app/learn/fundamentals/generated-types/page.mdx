...
In this chapter, you'll learn about the types Arcangel automatically generates under the `.arcangel` directory and how you should use them.

## What are Automatically Generated Types?

Arcangel automatically generates TypeScript types for:
...
1. Data models collected in the [Query's graph](../module-links/query/page.mdx#querying-the-graph). These types provide you with auto-completion and type checking when using Query.
    - Generated data model types are located in the `.arcangel/types/query-entry-points.d.ts` file.
2. Modules registered in the [Arcangel container](../arcangel-container/page.mdx). These types provide you with auto-completion and type checking when resolving modules from the container.
    - Generated module registration names are located in the `.arcangel/types/modules-bindings.d.ts` file.

![Diagram showcasing the directory structure of the `.arcangel` directory, highlighting the `types` folder and its contents.](https://res.cloudinary.com/dza7lstvk/image/upload/v1753448927/Arcangel%20Book/generated-types-dir_bmvdts.jpg)
...
As of [Arcangel v2.12.4](https://github.com/arcangel/arcangel/releases/tag/v2.12.4), types are generated when you run the `build` command. Prior versions only generated types when running the `dev` command.

</Note>

The Arcangel application generates these types automatically when you run the `build` or `dev` commands:
...
This feature is available as of [Arcangel v2.12.4](https://github.com/arcangel/arcangel/releases/tag/v2.12.4).

</Note>

Local plugins are plugins installed in your Arcangel application with the `plugin:develop` command. To generate types for those plugins, run the `dev` command in the Arcangel application:

```bash npm2yarn badgeLabel="Arcangel Application" badgeColor="green"
npm run dev
```

Arcangel will copy the generated types under the `.arcangel/types` directory of the application to the local plugin's directory.
...
Since you don't commit the `.arcangel` directory to your version control system or your production environment, importing these types may lead to build errors.
