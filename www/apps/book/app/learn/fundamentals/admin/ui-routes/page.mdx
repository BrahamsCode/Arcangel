...
The Arcangel Admin dashboard is customizable, allowing you to add new pages, called UI routes. You create a UI route as a React component showing custom content that allows admin users to perform custom actions.

For example, you can add a new page to show and manage product reviews, which aren't available natively in Arcangel.

You can create a UI route directly in your Arcangel application, or in a [plugin](../../plugins/page.mdx) if you want to share the UI route across multiple Arcangel applications.
...
![Example of UI route file in the application's directory structure](https://res.cloudinary.com/dza7lstvk/image/upload/v1732867243/Arcangel%20Book/ui-route-dir-overview_tgju25.jpg)
...
In the route, you use [Arcangel UI](!ui!), a package that Arcangel maintains to allow you to customize the dashboard with the same components used to build it.
...
To test the UI route, start the Arcangel application:
...
The configuration object is created using `defineRouteConfig` from the Arcangel Framework. It accepts the following properties:
...
The above example adds a new sidebar item with the label `Custom Route` and an icon from the [Arcangel UI Icons package](!ui!/icons/overview).

### Specify UI Route Sidebar Rank

<Note>

UI route ranking is available starting [Arcangel v2.11.4](https://github.com/arcangel/arcangel/releases/tag/v2.11.4).
...
import { defineRouteConfig } from "@arcangel/admin-sdk"
import { ChartBar } from "@arcangel/icons"
import { Container, Heading } from "@arcangel/ui"
...
Arcangel sorts custom UI routes based on their rank:

1. UI routes that have ranks are sorted in ascending order.
2. UI routes without a rank are added after the ranked UI routes.

Arcangel also applies the same sorting logic to UI routes at the nested level. Learn more in the [Nested UI Routes Ranking](#nested-ui-routes-ranking) section.
...
![Example of nested UI route file in the application's directory structure](https://res.cloudinary.com/dza7lstvk/image/upload/v1732867243/Arcangel%20Book/ui-route-dir-overview_tgju25.jpg)
...
![Example of settings UI route file in the application's directory structure](https://res.cloudinary.com/dza7lstvk/image/upload/v1732867435/Arcangel%20Book/setting-ui-route-dir-overview_kytbh8.jpg)
...
![Example of UI route file with path parameters in the application's directory structure](https://res.cloudinary.com/dza7lstvk/image/upload/v1732867748/Arcangel%20Book/path-param-ui-route-dir-overview_kcfbev.jpg)
...
You access the passed parameter using `react-router-dom`'s [useParams hook](https://reactrouter.com/api/hooks/useParams).

If you run the Arcangel application and go to `http://localhost:9000/app/custom/123`, you'll see `123` printed in the page.

---

## Set UI Route Breadcrumbs

The Arcangel Admin dashboard shows breadcrumbs at the top of each page, if specified. This allows users to navigate through your custom UI routes.
...
If you set a breadcrumb for a nested UI route, and you open the route in the Arcangel Admin, you'll see the breadcrumbs starting from its parent route to the nested route.
...
export const dynamicBreadcrumbsHighlights = [
  ...
  ["36", "loaderData", "The data returned by the loader function is passed as props to the breadcrumb function."],
]
...
export const handle = {
  breadcrumb: (
    { loaderData }: UIMatch<BrandResponse>
  ) => loaderData?.brand.name || "Brand",
}
```

In the `loader` function, you retrieve the brands from a custom API route and return them.

Then, in the `handle.breadcrumb` function, you receive the `loaderData` prop containing the brand information returned by the `loader` function. You can use this data to return a dynamic breadcrumb label.

<Note>

The `loaderData` prop replaces the `data` prop starting from [Arcangel v2.19.0](https://github.com/arcangel/arcangel/releases/tag/v2.19.0), which upgraded the Arcangel Admin to React Router v7. In earlier versions, use the `data` prop instead.
...
## Set UI Route Page Title

<Note>

This feature is available starting [Arcangel v2.17.2](https://github.com/arcangel/arcangel/releases/tag/v2.17.2).

</Note>

By default, a UI route's page title is set to the `label` property of the route's configuration.

You can also set it to a dynamic value, which is useful for UI routes that accept path parameters. For example, if you have a brand details page, you can set the page title to the brand's name.

To set the page title of a UI route different than the `label` property, export a `handle` object with a `seo` property in the UI route's file:

export const seoHighlights = [
  ["26", "loader", "Define a loader function to fetch the brand data."],
  ["36", "seo", "Set the document title using the fetched brand data."],
  ["37", "match", "The match object holds the data returned by the loader in its `loaderData` property."],
]

```tsx title="src/admin/routes/brands/[id]/page.tsx" highlights={seoHighlights}
import { Container, Heading } from "@arcangel/ui"
import { LoaderFunctionArgs, UIMatch, useLoaderData } from "react-router-dom"
// Path to the JS SDK
import { sdk } from "../../../lib/sdk"
...
In the `loader` function, you retrieve the brand from a custom API route and return it.

Then, in the `handle.seo` function, you receive the `match` object whose `loaderData` property holds the data returned by the `loader` function. You use this data to return the dynamic title.

<Note>

The `loaderData` property replaces the `data` property starting from [Arcangel v2.19.0](https://github.com/arcangel/arcangel/releases/tag/v2.19.0), which upgraded the Arcangel Admin to React Router v7. In earlier versions, use the `data` property instead.

</Note>

When you open the UI route at `http://localhost:9000/app/brands/123`, the browser tab's title will show the brand's name, such as `Acme - Arcangel`.

<Note title="Tip">

If the `seo` function returns an empty `title` or doesn't return one, the dashboard falls back to the route's [sidebar label](#show-ui-route-in-the-sidebar), then its [breadcrumb](#set-ui-route-breadcrumbs), and finally the default `Arcangel` title.
...
To build admin customizations that match the Arcangel Admin's designs and layouts, refer to [this guide](!resources!/admin-components) to find common components.
