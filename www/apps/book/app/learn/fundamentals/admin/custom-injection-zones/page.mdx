...
Custom injection zones are available since Arcangel [v2.16.0](https://github.com/arcangel/arcangel/releases/tag/v2.16.0).

</Note>

## What are Custom Injection Zones?

The Arcangel Admin has [pre-defined injection zones](!resources!/admin-widget-injection-zones) where you insert [widgets](../widgets/page.mdx). When you build a plugin that adds custom admin pages, you can also register custom injection zones to allow other developers to inject widgets into your plugin's pages.
...
When you build a custom admin page in a plugin, use the [LayoutComposer](!resources!/admin-components/components/layout-composer) component from `@arcangel/dashboard/components` to structure the page's layout. This component also exposes injection zones based on the prefix you pass to its `widgetsZonePrefix` prop.
...
Don't add a `.before` or `.after` suffix to your zone names. These suffixes are deprecated since [Arcangel v2.17.2](https://github.com/arcangel/arcangel/releases/tag/v2.17.2), as admin users now change a widget's placement with [Layout Configurations](!user-guide!/tips/layout-configurations).
...
"include": [
  ".",
  "../../.arcangel/types/augmentation-refs.d.ts"
]
```
