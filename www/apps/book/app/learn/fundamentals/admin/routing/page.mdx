...
The Arcangel Admin dashboard uses [React Router](https://reactrouter.com) under the hood to manage routing. This gives you more flexibility in routing-related customizations using React Router's utilities, hooks, and components.
...
`react-router-dom` is available in your project by default through the Arcangel packages. You don't need to install it separately.

</Note>

<Note title="React Router v7">

Starting from [Arcangel v2.19.0](https://github.com/arcangel/arcangel/releases/tag/v2.19.0), the Arcangel Admin uses React Router v7. If your project has its own `react-router-dom` dependency, update it to v7 to avoid installing two versions of React Router, which leads to a `useNavigate() may be used only in the context of a Router` error in your admin customizations.
...
Route loaders are available starting from Arcangel v2.5.1.
...
![Timeline comparison of a UI route with and without a route loader](https://res.cloudinary.com/dza7lstvk/image/upload/v1753428567/Arcangel%20Book/ui-route-loading_vycev8.jpg)
...
Route handles are available starting from Arcangel v2.5.1.
