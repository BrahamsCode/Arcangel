...
In this chapter, you'll learn about best practices for syncing data between Arcangel and third-party systems.

## Common Issues with Third-Party Syncing Implementation

Syncing data between Arcangel and external systems is a common use case for commerce applications. For example, if your commerce ecosystem includes an external CMS or inventory management system, you may need to sync product data between Arcangel and these systems.
...
This chapter covers best practices to avoid these issues when syncing data between Arcangel and third-party systems. These best practices are general programming patterns that aren't Arcangel-specific, but they're essential for building robust third-party syncs.

---

## How to Sync Data Between Systems

Before diving into best practices, it's important to understand the general approach for syncing data between Arcangel and third-party systems. Third-party syncing typically involves two main steps:
...
When defining your syncing logic, such as pushing product data to a third-party service or pulling inventory data into Arcangel, you should define a workflow that encapsulates this logic.

Arcangel also exposes [built-in workflows](!resources!/arcangel-workflows-reference) for common commerce operations, like creating or updating products, that you can leverage in your syncing logic.

For example, you can use Arcangel's built-in [batchProductsWorkflow](!resources!/references/arcangel-workflows/batchProductsWorkflow) to create or update products in batches:

```ts title="src/jobs/sync-products.ts"
import { ArcangelContainer } from "@arcangel/framework/types"
import { batchProductsWorkflow } from "@arcangel/arcangel/core-flows"
...
After defining your syncing logic in a workflow, or choosing a Arcangel workflow to use, you need to execute it based on your syncing requirements:
...
- [Subscribers](../../fundamentals/events-and-subscribers/page.mdx): Use subscribers for event-driven syncs, such as syncing data when a product is updated in Arcangel or when an order is placed. Subscribers listen for specific events and can trigger your workflow in response.
...
In the scheduled job or subscriber, you retrieve the data to be synced from the third-party service or from Arcangel itself. Then, you execute the workflow, passing it the data to be synced.

For example, the following scheduled job fetches products from a third-party service and syncs them to Arcangel using a workflow:
...
```ts title="src/jobs/sync-products.ts"
import { ArcangelContainer } from "@arcangel/framework/types"
import { ContainerRegistrationKeys, ArcangelError, Modules } from "@arcangel/framework/utils"
import { batchProductsWorkflow } from "@arcangel/arcangel/core-flows"
...
export default async function syncProductsJob(container: ArcangelContainer) {
  ...
  for await (const batch of batchedProducts) {
    ...
    // Extract external IDs from this batch to look up in Arcangel
    const externalIds = batch.map((p) => p.id)

    // Query Arcangel for products matching these external IDs
...
![Diagram showcasing object in memory when loading entire JSON response vs streaming JSON parsing](https://res.cloudinary.com/dza7lstvk/image/upload/v1764761087/Arcangel%20Book/stream-vs-full_iz2fyv.jpg)

First, install the `stream-json` library in your Arcangel project:
...
A common performance pitfall when syncing data is retrieving more fields than necessary from third-party services or Arcangel's [Query](../../fundamentals/module-links/query/page.mdx). This leads to increased data size, slower performance, and higher memory usage.

When retrieving data from third-party services or with Arcangel's Query, only request the necessary fields. Then, to efficiently group existing data for updates, use a [Map](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Map) for quick lookups.
...
export const fieldsHighlights = [
  ["19", "fields", "Request only necessary fields from Arcangel"],
...
export default async function syncProductsJob(container: ArcangelContainer) {
  ...
  for await (const batch of batchedProducts) {
    ...
    // Process batch and sync to Arcangel...
  }
}
```

In the above snippet, after retrieving a batch of products from the external API, you query Arcangel's products to find existing products that match the external IDs. You only request the necessary fields for this operation.
...
![Diagram showcasing how batches are loaded into memories one at a time with async generators](https://res.cloudinary.com/dza7lstvk/image/upload/v1764761706/Arcangel%20Book/async-gen-batches_ne09bs.jpg)
...
![Diagram showcasing timeline from start to end of scheduled job execution where batches are fetched and synced one at a time](https://res.cloudinary.com/dza7lstvk/image/upload/v1764762095/Arcangel%20Book/timeline-backpressure_rtwwzt.jpg)
