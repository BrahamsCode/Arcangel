...
const HomepageCodeTabs = () => {
  ...
  .cascades({
    ...
    },
    {
      ...
      code: {
        lang: "ts",
        source: `class DigitalProductService extends ArcangelService({
    DigitalProduct,
  }) {
    ...
    res.json({ success: true })
        ...
        highlights: [
          ...
          [
            "13",
            "digitalProductModuleService",
            "Resolve the database from the Arcangel container\nin routes and other resources.",
          ],
    ...
    zone: "product.details.before",
        ...
        highlights: [
          ...
          [
            "7",
            "Container",
            "Use Arcangel's UI components in your customizations.",
          ],
          [
            "15",
    ...
    () => {
      ...
      return new WorkflowResponse({
        ...
        highlights: [
          ...
          ["6", "createBrandsInArcangelStep", "Sync data to Arcangel."],
        ],
      },
    },
  ]
  ...
  return (
    <div
      className={clsx(
        "w-full border-b border-arcangel-border-base",
        "flex gap-0 flex-col lg:flex-row"
      )}
    >
      <div className="w-full lg:w-1/2 flex flex-col gap-0 border-r border-arcangel-border-base">
        {tabs.map((tab, index) => (
          <React.Fragment key={index}>
            ...
            >
              <div className="flex items-center gap-0.5">
                <span
                  className={clsx(
                    "text-code-label group-hover:text-arcangel-fg-interactive",
                    index === selectedTabIndex && "text-arcangel-fg-interactive",
                    index !== selectedTabIndex && "text-arcangel-fg-subtle"
                  )}
                >
                  [ {index + 1} ]
                </span>
                <span className="text-medium-plus text-arcangel-fg-base">
                  {tab.title}
                </span>
              </div>
              {index === selectedTabIndex && (
                <p className="text-arcangel-fg-subtle text-medium">
...
const CodeSection = ({ tab }: CodeSectionProps) => {
  return (
    <div
      className={clsx(
        "w-full lg:w-1/2 p-2 flex flex-col gap-2 justify-center bg-arcangel-bg-component relative"
      )}
    >
