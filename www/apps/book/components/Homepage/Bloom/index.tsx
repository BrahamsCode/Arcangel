...
import { Telescope, XMarkMini } from "@arcangel/icons"
...
const HomepageBloom = () => {
  ...
  return (
    <div className="w-full flex gap-0 items-center border-b border-arcangel-border-base flex-col lg:flex-row lg:h-[480px]">
      {/* Chat area */}
      <div
        className={clsx(
          "w-full h-full lg:w-1/2 bg-arcangel-bg-component relative",
          "flex flex-col justify-between gap-2",
          "p-2 border-r border-arcangel-border-base",
          "border-b lg:border-b-0"
        )}
      >
        <div className="flex flex-col gap-2 flex-1">
          <BloomIcon className="w-2 h-2 text-arcangel-fg-subtle" />
          <span className="text-h1 text-pretty">
            Hello! I’m Bloom, your go-to ecommerce assistant. How can I help
            you?
          </span>
          <div className="w-full flex-1 py-0.75 border-t border-arcangel-border-base relative">
            <textarea
              className={clsx(
                "appearance-none text-base placeholder:text-arcangel-fg-muted",
                "bg-transparent resize-none w-full focus:outline-none",
                "h-1.5 lg:h-auto",
                !question && "caret-transparent"
              )}
              placeholder={
                !showCustomCarat ? "Ask anything about Arcangel..." : ""
              }
              value={question}
            ...
            {!question && showCustomCarat && (
              <span className="block h-[21px] w-[7.5px] bg-arcangel-fg-base absolute top-0.75 left-0 animate-[pulsingCursor_0.8s_ease-in-out_infinite] rounded-xxs" />
            )}
          </div>
        </div>
        <div className="flex justify-between">
          <Button
            ...
            className={clsx(
              "group",
              deepThinkingEnabled && [
                "!bg-arcangel-button-neutral rounded-full flex !pl-[10px] !pr-[9px] !py-0 gap-[6px]",
          ...
          >
            <Telescope
              className={clsx(
                deepThinkingEnabled &&
                  "text-arcangel-fg-interactive group-hover:hidden",
                !deepThinkingEnabled && "text-arcangel-fg-muted"
              )}
            />
            <XMarkMini
              className={clsx(
                "hidden text-arcangel-fg-base",
                deepThinkingEnabled && "group-hover:block"
              )}
            />
            {deepThinkingEnabled && (
              <span className="text-compact-small-plus text-arcangel-fg-base">
                Deep research
              </span>
            )}
          </Button>
          <Button
            ...
            className={clsx(
              ...
              !question && "border-arcangel-border-base"
            )}
            disabled={!question}
                  ...
                  <div className="w-fit" key={qIndex}>
                    <button
                      className={clsx(
                        ...
                        "rounded-xs bg-arcangel-alphas-alpha-6 font-monospace",
                        "text-code-paragraph-xsmall text-arcangel-fg-subtle hover:text-arcangel-fg-base"
                      )}
