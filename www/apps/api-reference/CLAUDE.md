...
Next.js (App Router) site that renders the Arcangel **Store** and **Admin** REST API references. Deployed to Cloudflare via OpenNext, served under the `basePath` `/api` (e.g. `https://docs.arcangel.com/api/store`).

## Pipeline: from OAS to public docs

```
packages/arcangel/src/api  (API route request/response types = source of truth)
...
# packages/arcangel/src/api) — heavy, usually CI:
