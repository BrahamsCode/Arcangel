...
<H1 className={"scroll-m-[184px] lg:scroll-m-[264px] !mb-0"} id="introduction">Arcangel V2 Store API Reference</H1>

<ViewAsMarkdown path="/store" className="mb-1" />

This API reference includes Arcangel v2's Store APIs, which are REST APIs exposed by the Arcangel application. They are used to create a storefront for your commerce store, such as a webshop or a commerce mobile app.

All API Routes are prefixed with `/store`. So, during development, the API Routes will be available under the path `http://localhost:9000/store`. For production, replace `http://localhost:9000` with your Arcangel application URL.
...
There are two ways to send authenticated requests to the Arcangel application: Using a JWT token or using a Cookie Session ID.
...
To obtain a JWT token, send a request to the [authentication route](https://docs.arcangel.com/api/store/auth/authenticate-customer) passing it the customer's email and password in the request body.
...
If you're not using the JS SDK, send a request to the [session authentication API route](https://docs.arcangel.com/api/store/auth/set-authentication-session). To view the cookie session ID, pass the `-v` option to the `curl` command.
...
Create a publishable API key either using the [admin REST APIs](https://docs.arcangel.com/api/admin/api-keys/create-api-key), or using the [Arcangel Admin](!user-guide!/settings/developer/publishable-api-keys#create-publishable-api-key).
...
- Use the JS SDK's `setLocale` method. The JS SDK will automatically include the locale in the `x-arcangel-locale` header of subsequent requests.
...
- Set the `x-arcangel-locale` header in the API request to the relevant API route. By default, all `/store` API routes support the `x-arcangel-locale` header.
