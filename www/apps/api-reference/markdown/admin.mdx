...
<H1 className={"scroll-m-[184px] lg:scroll-m-[264px] !mb-0"} id="introduction">Arcangel V2 Admin API Reference</H1>

<ViewAsMarkdown path="/admin" className="mb-1" />

This API reference includes Arcangel v2's Admin APIs, which are REST APIs exposed by the Arcangel application. They are used to perform admin functionalities or create an admin dashboard to access and manipulate your commerce store's data.

All API Routes are prefixed with `/admin`. So, during development, the API Routes will be available under the path `http://localhost:9000/admin`. For production, replace `http://localhost:9000` with your Arcangel application URL.
...
There are three ways to send authenticated requests to the Arcangel server: Using a JWT token in a bearer authorization header, using an admin user's API token, or using a cookie session ID.
...
To obtain a JWT token, send a request to the [authentication route](https://docs.arcangel.com/api/admin/auth/authenticate-user) passing it the user's email and password in the request body.
...
The `x-arcangel-access-token` header is no longer used in Arcangel v2. Pass the JWT token in the `Authorization: Bearer` header instead, as explained here.
...
Create the API key token either from the [Arcangel Admin](!user-guide!/settings/developer/secret-api-keys) or using the [Create API Key API Route](https://docs.arcangel.com/api/admin/api-keys/create-api-key).
...
Previous versions of Arcangel required passing the API key token as base64. This is still supported for backwards compatibility, but you can pass the API token without encoding it as well.
...
If you're not using the JS SDK, send a request to the [session authentication API route](https://docs.arcangel.com/api/admin/auth/set-authentication-session). To view the cookie session ID, pass the `-v` option to the `curl` command.
...
If you've enabled HTTP Compression in your Arcangel configurations, and you 
...
Many data models in Arcangel, such as products and carts, have a `metadata` field that allows you to store custom information in key-value pairs.

When setting or updating the `metadata` field using the relevant API routes, Arcangel will merge the new metadata with the existing metadata.
...
Arcangel doesn't merge nested objects, so if you pass a partial object, the existing properties in the nested object will be removed.
...
However, some API routes restrict the fields and relations you can retrieve. To learn about those API routes and how to override the allowed fields and relations, refer to the [Retrieve Custom Linked Data Models from Arcangel's API routes](!docs!/learn/fundamentals/api-routes/retrieve-custom-links) documentation.
