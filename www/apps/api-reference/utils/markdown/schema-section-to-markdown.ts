...
export default function schemaSectionToMarkdown(
  ...
  parts.push(
    `> This object's schema is as returned by Arcangel's ${ctx.area} API routes. ` +
      `However, the related model in the Arcangel application may support more fields and relations. ` +
      `To view the models in the Arcangel application and their relations, visit the ` +
      `[Commerce Modules Documentation](https://docs.arcangel.com/resources/commerce-modules).`
  )
