...
```tsx
<Copy content="yarn add @arcangel/ui" />
```
...
If you're using the `Copy` component in a project other than the Arcangel Admin, make sure to include the `TooltipProvider` somewhere up in your component tree, as the `Copy` component uses a [Tooltip](../tooltip/page.mdx#usage-outside-arcangel-admin):
