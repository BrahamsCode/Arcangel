...
In this guide, you'll learn about the managed database service that Arcangel provides as part of your project environments on Cloud.

## Managed Database Service on Cloud

Arcangel provisions and manages databases for each of your environments on Cloud.
...
Arcangel automatically configures the database for your environments on Cloud. So, you don't need to worry about setting up the database, configuring it with your Arcangel application, or monitoring performance and availability on your end.

---

## Cloud Database Backups

Arcangel provides automatic backups for your environment databases on Cloud. These backups are retained for 14 days and allow you to restore your database to a previous state if needed.
...
4. Click **Database & Storage** > **Database** in the sidebar under the environment's section.
...
Arcangel notifies you when the restore completes or if an error occurs.
...
Arcangel gives you read access to your Cloud project's database, which is useful when you need to export data, perform analytics, or troubleshoot issues.
...
For advanced use cases where you need write access to your environment's database, such as importing data or performing maintenance tasks, Arcangel allows organization owners to enable database write access.
...
Arcangel allows you to export and import database dumps for any environment on Cloud. This is useful for seeding the database with initial data, migrating from other hosting platforms, or debugging issues locally.
...
By default, when Arcangel creates a [preview environment](../environments/preview/page.mdx) on Cloud, it replicates the Production database. This allows you to test changes in a safe environment that mirrors production, without affecting the live data.
...
Arcangel automatically runs database migrations for every deployment on Cloud. This ensures that your database schema is always up-to-date with the latest changes in your Arcangel application.

When you deploy changes to an environment on Cloud, Arcangel:
...
So, if you define new database migrations in your module, define new links, or add data migration scripts, Arcangel will apply those changes automatically when you deploy the changes to Cloud.