...
import { EyeMini, SquareTwoStackMini, ArrowPathMini, Trash } from "@arcangel/icons"
...
A webhook endpoint is an HTTPS URL that receives event payloads from Arcangel when certain actions occur in your organization.
...
Arcangel generates a signing secret for each endpoint. You can use the secret to verify that incoming requests are from Arcangel.
...
Arcangel generates a new secret and the previous one is no longer valid.
...
If an endpoint is disabled, Arcangel will not send any events to it until it is re-enabled.
...
The endpoint is removed and Arcangel will stop sending events to it.
