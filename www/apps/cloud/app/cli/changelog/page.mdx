...
- `mcloud variables list` now supports `-t, --type` (`backend` or `storefront`), `--scope` (`build` and/or `runtime`) to filter which variables are shown, `--include-system` to show system variables auto-injected by Arcangel, and `--dotenv` to output variables in `.env` format. The output now includes a `SYSTEM` column. The `--limit` and `--offset` pagination flags have been removed.
