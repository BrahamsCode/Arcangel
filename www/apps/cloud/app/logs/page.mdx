...
import { ArrowDownTray, ArrowPath, Brackets } from "@arcangel/icons"
...
Logs are output messages from your Arcangel application's build and runtime processes for a Cloud environment's deployments.
...
- **Runtime logs**: Messages logged by the Arcangel backend and storefront when they start and while they're running, such as informational startup messages, HTTP requests, errors, and more. These logs are useful for debugging the application's deployment and runtime errors.
...
To view the build logs for a deployment, go to the [deployment's details page](../deployments/page.mdx#find-deployment-details), where you can see the deployment's activity:

- **Backend build logs**: Logs for building the Arcangel backend.
- **Storefront build logs**: If you deployed a storefront alongside the Arcangel application, this shows the logs for building the storefront.
...
The runtime logs page shows logs for both the Arcangel backend and, if you deployed a storefront alongside the Arcangel application, the storefront. You can switch between the two using the toggle at the top of the logs table:

- **Backend**: Logs from the Arcangel backend application, such as HTTP requests, errors, and startup messages.
- **Storefront**: Logs from the storefront application. This option is only available if you deployed a storefront alongside the Arcangel application.
