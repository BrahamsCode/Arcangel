...
In this guide, you'll find solutions to common issues that may arise when deploying your Arcangel applications on Cloud.
...
When your build or deployment fails on Cloud, you can use the "Fix with AI" feature to troubleshoot and resolve the issue with the help of the [Cloud Assistant](../../assistant/page.mdx). The AI agent will analyze the deployment and build logs, identify the root cause of the issue, and provide you with actionable steps to resolve it.
...
Before deploying your Arcangel application on Cloud, Arcangel runs the `build` script defined in your `package.json` file, then copies only the output of the [build process](!docs!/learn/build), which is the `.arcangel/server` directory.

The `.arcangel/server` directory holds the compiled JavaScript files of your project, the production build of the admin dashboard, and other necessary files to run your Arcangel application in production.

If you have custom files needed at runtime, such as a `src/data` directory that holds JSON files, modify the `build` script in your `package.json` file to copy these files to the `.arcangel/server` directory after the build process.
...
This script copies the `src/data` directory to the `.arcangel/server` directory following the build process.
...
## Missing arcangel-config.js in Build Output

If your deployment fails with an error similar to:

```bash
Invalid build output: missing /build/apps/backend/.arcangel/server/arcangel-config.js.
Ensure tsconfig compilerOptions.outDir is .arcangel/server (or unset), your package.json
build script runs arcangel build, and the Arcangel build step completed successfully.
```

Cloud validates that the Arcangel build output contains a compiled `arcangel-config.js` file at `.arcangel/server/arcangel-config.js`. This file is produced when `arcangel build` compiles your `arcangel-config.ts` file to the `.arcangel/server` directory.

The error occurs when one of the following is true:

- The `build` script in `package.json` does not run `arcangel build`.
- The `tsconfig.json` file sets `compilerOptions.outDir` to a path other than `.arcangel/server`, causing the build output to land in the wrong directory.
- The `arcangel build` step failed silently or was skipped during the build.

### Verify the build script

Open your `package.json` and confirm the `build` script calls `arcangel build`:
...
If you have a custom post-build step, chain it after `arcangel build`:
...
Open `tsconfig.json` and check the `compilerOptions.outDir` value. Either remove it to use the default (`.arcangel/server`), or set it explicitly to `.arcangel/server`:

```json title="tsconfig.json"
{
  "compilerOptions": {
    "outDir": ".arcangel/server"
  }
}
```
...
Setting `outDir` to any path other than `.arcangel/server` causes the build output to land in the wrong location and triggers this error.

</Note>

### Check the build logs

After verifying the script and `tsconfig.json`, check the build logs to confirm `arcangel build` ran and completed without errors. If you see TypeScript compilation errors in the logs, fix them locally by running:
...
For example, Arcangel supports Zod v4.2.0. If you install a different version of Zod in your project, it can lead to complex type issues during compilation, causing OOM failures.

To resolve this, ensure that your project's dependencies are compatible with those used by Arcangel or other dependencies.
...
2. Click **Projects** in the sidebar and select the project.
3. Click **Settings** in the sidebar.
4. Check the **Storefront root directory** field and confirm it matches the actual path of your storefront in the repository. For example, if your storefront is at `apps/storefront`, the field should contain `apps/storefront`.
5. If the path is incorrect, update it and save.
...
When you install a third-party package (for example, a payment provider) that requires a specific version of a Arcangel package, npm may throw an `ERESOLVE` error if the required version doesn't match the version installed in your project.
...
npm error
npm error While resolving: my-arcangel-app@1.0.0
npm error Found: @arcangel/framework@2.15.5
npm error node_modules/@arcangel/framework
npm error   @arcangel/framework@"2.15.5" from the root project
npm error
npm error Could not resolve dependency:
npm error peer @arcangel/framework@"2.13.6" from @example/arcangel-payment-example@0.1.0
```
...
1. **Update the third-party package**: Check if a newer version of the package supports your Arcangel version. Run:

```bash npm2yarn
npm install @example/arcangel-payment-example@latest
```

2. **Contact the package maintainer**: If no compatible version exists, open an issue or contact the maintainer to request support for your Arcangel version.
...
## Storefront: Cannot Find Module @opennextjs/cloudflare

If your storefront build fails with an error similar to:

```bash
Cannot find module '@opennextjs/cloudflare/overrides/...'
```

This error occurs when your storefront's `tsconfig.json` uses the legacy `moduleResolution: "node"` setting. The `@opennextjs/cloudflare` package uses subpath exports, which the `node` resolution mode cannot resolve.

### Fix the moduleResolution Setting

Open your storefront's `tsconfig.json` and update `compilerOptions.moduleResolution` to `bundler` (recommended for Next.js 13+), or to `node16`/`nodenext`:

```json title="apps/storefront/tsconfig.json"
{
  "compilerOptions": {
...
After updating the file, commit the change and push to trigger a new deployment.

---

## Next.js 15: 404 Prerender Error with Async Layouts

If your Next.js 15 storefront build fails during static prerendering with an error similar to:

```bash
Error: React error #31 on /404
```

This occurs when your `not-found.tsx` file inherits a parent layout that performs async data fetches, such as `retrieveCustomer` or `retrieveCart`. During static prerendering, Next.js 15 attempts to pre-render the `/404` route using the parent layout, which triggers the async data fetches in a context where they are not supported, causing a React error.

### Fix the 404 Prerender Error

Add the following export to your `not-found.tsx` file to opt out of static prerendering and force the page to render dynamically at request time:

```ts title="app/not-found.tsx"
export const dynamic = "force-dynamic"
```

After adding this export, commit the change and push to trigger a new deployment.
...
## Missing Root Files in Turborepo Monorepo Builds (TS5083)

If your `turbo` monorepo build fails with an error similar to the following, but succeeds locally, a file at the root of your monorepo isn't available during the build:

```bash
error TS5083: Cannot read file '/tsconfig.base.json'.
```

Arcangel prunes `turbo` monorepos down to the package it's building. The pruned output only includes the root `package.json`, the lockfile, `pnpm-workspace.yaml`, `turbo.json`, `.gitignore`, `.npmrc`, and the source code of the target package along with the workspace packages it depends on. So, a shared root file, such as a `tsconfig.base.json` file that other packages reference with `extends`, isn't copied into the build.

### Fix Missing Root Files

Move the shared file into a workspace package, then add that package as a dependency of your Arcangel application or storefront. Arcangel then includes the file in the build, since it builds the target package along with the workspace packages it depends on.

For files that your Arcangel application needs at runtime, such as JSON files or fonts, keep them within your Arcangel application and copy them to the `.arcangel/server` directory as part of your `build` script. Learn more in the [Built Assets](!docs!/learn/build#built-assets) documentation.
...
1. If you're in a different organization, [switch to the organization](../../organizations/page.mdx#switch-organization).
2. Click **Projects** in the sidebar and select the project that contains the failed deployment.
3. Click **Deployments** in the sidebar under the project.
4. Find the failed deployment and click the <InlineIcon Icon={EllipsisHorizontal} alt="three-dots" /> button to open the deployment actions menu. 
...
Arcangel automatically provisions and manages the database for each environment on Cloud. You do not need to create or configure a database yourself. The `DATABASE_URL` is set automatically by Cloud when your environment is created.
...
Cloud starts your Arcangel application by invoking `arcangel start` directly. It does not use the `start` script defined in your `package.json`. This means that any custom logic you place in your `package.json` `start` script, such as environment checks, maintenance-mode toggles, or initialization commands, is not executed on Cloud.

### Use Loaders for Custom Initialization

To run custom initialization logic when your Arcangel application starts on Cloud, use [loaders](!docs!/learn/fundamentals/modules/loaders). Loaders are executed by Arcangel as part of its startup process regardless of how the application is started.
...
- `ARCANGEL_CLOUD_PROJECT_HANDLE`: The handle of your Cloud project.
- `ARCANGEL_CLOUD_ENVIRONMENT_HANDLE`: The handle of the current Cloud environment.
...
- [Check the Cloud status page](../../status/page.mdx) for ongoing Cloud incidents.
- [Refer to Arcangel's troubleshooting guides](!resources!/troubleshooting).
