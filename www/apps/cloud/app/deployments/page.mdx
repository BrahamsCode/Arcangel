...
If you deployed both the Arcangel backend and storefront in an environment, each deployment refers to the backend and storefront. The deployment will also show the status of both the backend and storefront.

---

## How are Deployments Created?

For long-lived environments, Arcangel creates a new deployment every time you push a new commit to the environment's branch. For example, if your Production environment is connected to the `main` branch, Arcangel will create a new Production deployment every time you push a new commit to the `main` branch.

For short-lived preview environments, Arcangel creates a new environment and deployment when you open a pull request. Arcangel will redeploy the preview environment every time you push a new commit to the pull request branch.

---

## Build Process for Deployments

### Arcangel Application Only

Before deploying your application on Cloud, Arcangel runs the `build` script defined in your project's `package.json` file, which must run the `arcangel build` command, among other build steps you may have.
...
```json title="package.json"
{
  "scripts": {
    ...
    "build": "arcangel build && npm run other-build-steps"
  }
}
```
...
### Arcangel Application with Storefront

If you're deploying both a Arcangel application and a storefront on Cloud, Arcangel will run the:

1. The `build` command defined in the backend's `package.json` file, which must run the `arcangel build` command.
2. The build command relevant to the storefront, depending on the frontend framework you're using. For example, if you're using Next.js for your storefront, Arcangel will run the `next build` command in the storefront's directory.
    - Arcangel currently doesn't support custom build scripts for storefronts.

In a monorepo, Arcangel doesn't run the `build` script of your root `package.json` file. It builds each of the two packages separately, targeting only that package and the workspace packages it depends on. So, the other packages of your monorepo don't enter the build process.

<Note>

If you're using `turbo`, Arcangel prunes your monorepo down to the package it's building. This means most files at the root of your monorepo aren't available during the build. Learn more in the [Prerequisites](../projects/prerequisites/page.mdx#root-files-available-during-the-build-turborepo) guide.
...
### What Gets Deployed in the Arcangel Application?

Arcangel deploys the contents of the `.arcangel/server` directory of the Arcangel application. This directory is created by the [build process](!docs!/learn/build). It includes the compiled JavaScript files in your project, the production build of the admin dashboard, and other necessary files to run your Arcangel application in production.

<Note>

Learn more about the `.arcangel/server` directory in the [Build guide](!docs!/learn/build#output-directory-structure).

</Note>

So, if you have custom assets like JSON files that your application needs at runtime, make sure to copy them to the `.arcangel/server` directory after the `arcangel build` command in your `build` script.

For example, if your application needs a `src/data/custom.json` file at runtime, you can add a script that copies it to `.arcangel/server/src/data/custom.json` after the build process is complete.

You can also customize the Arcangel Admin build configurations using the [admin.vite](!docs!/learn/configurations/arcangel-config#adminvite) option in your `arcangel-config.ts` file.

<Note type="warning">

Do not expose sensitive files in your deployments, especially in the `.arcangel/server/public` directory, as they will be publicly accessible. Sensitive files include those containing secret API keys, database credentials, or any other confidential information.
...
Once a deployment's status is "Live", you can access its Arcangel Admin, send requests to its API routes, and access its storefront (if applicable).
