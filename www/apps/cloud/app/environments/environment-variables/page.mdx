...
You can manage an environment's variables in its **Environment variables** settings. You can manage environment variables for the environment's Arcangel backend and storefront (if applicable).
...
- `ARCANGEL_CLOUD_ENVIRONMENT_TYPE`
- `ARCANGEL_CLOUD_ENVIRONMENT_NAME`
- `ARCANGEL_CLOUD_PROJECT_HANDLE`
...
Cloud automatically provides the following system environment variables in all environments at both runtime and build time:

- `ARCANGEL_CLOUD_ENVIRONMENT_HANDLE`: The unique handle/identifier for the environment
- `ARCANGEL_CLOUD_ENVIRONMENT_TYPE`: The type of environment (`production`, `preview-instance`, or `long-lived`)
- `ARCANGEL_CLOUD_ENVIRONMENT_NAME`: The display name of the environment. For example, "Production" or "Staging".
- `ARCANGEL_CLOUD_PROJECT_HANDLE`: The unique handle/identifier for the project

These variables are automatically set by Cloud and don't need to be configured manually. You can use them in your application code to access information about the current environment context.
...
In addition to the [system environment variables](#system-environment-variables) documented above, the **Backend** view displays all system-provided variables alongside your user-defined ones when the **System Variables** toggle is enabled. These are variables that Arcangel adds to your environment, such as database URL, Redis URL, and other relevant variables.

System variables are visually distinguished from user-defined variables with a subtle background and muted text. Because Arcangel manages those:
...
System variables are useful to understand what variables Arcangel sets for you so that you don't add them as well.
...
1. In the [environment's dashboard](../long-lived/page.mdx#open-environment-dashboard), click **Environment variables** in the sidebar.
2. If you deployed both the Arcangel backend and storefront in the environment, use the **Backend** or **Storefront** tabs to switch between managing the backend and storefront environment variables.
3. In the "Add new environment variables" section, you can add environment variables either for the backend or storefront. There are different ways you can add environment variables:
    - **Manually**:
      - Enter the key and value of the environment variable in the respective fields.
      - Use the **Sensitive** toggle to mark variables containing secrets that should be masked in the interface.
      - Use the **Build** toggle to make the variable available during build time.
      - Use the **Runtime** toggle to make the variable available during runtime (enabled by default).
...
6. In the side window that opens, you can edit the variable's key, value, and configure its settings:
   - **Sensitive**: Whether the variable contains secrets that should be masked in the interface
   - **Build**: Whether the variable is available during build time
   - **Runtime**: Whether the variable is available during runtime
