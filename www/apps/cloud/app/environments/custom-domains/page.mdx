...
import { EllipsisHorizontal, TriangleRightMini } from "@arcangel/icons"
...
By default, Arcangel environments are deployed with `arcangel.app` (backend) and `arcangel.site` (storefront) subdomains. You can configure custom domains to use your own branded URLs for both your backend and storefront.

<Note>

To update the default `arcangel.app` and `arcangel.site` subdomains instead, see the [Update Subdomains](../subdomains/page.mdx) guide.
...
Backend custom domains allow you to serve your Arcangel backend from your own domain instead of the default `arcangel.app` subdomain.
...
To avoid downtime in you storefront while DNS changes propagate for your backend's custom domain, Arcangel doesn't update the backend URL set in the storefront automatically. The storefront will still point to the backend's original subdomain.

Once you've verified that DNS changes have propagated and the custom domain is correctly pointing to the Arcangel backend, you need to manually update the backend URL in your storefront's configuration to point to the new custom domain.
...
# Update any other envionment variable that uses the Arcangel backend URL
# For example:
...
After updating the backend URL, [redeploy your environment](../../deployments/page.mdx#redeploy-a-deployment). Your live storefront will now connect to your Arcangel backend through the new custom domain without any downtime.

---

## Storefront Custom Domains

Storefront custom domains allow you to serve your storefront from your own domain instead of the default `arcangel.site` subdomain.
...
Arcangel supports multiple domains on the [Scale and Enterprise plans](../../pricing/page.mdx), with a three-domain limit per project for Scale plans. If you're on these plans, you can add another domain by following the same flow for [storefront](#storefront-custom-domains) and [backend](#backend-custom-domains).
