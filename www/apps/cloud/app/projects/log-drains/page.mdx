...
- **Resource Attributes**: OpenTelemetry resource attribute key/value pairs attached to every exported log. Several Arcangel environment attributes are pre-populated. You can add or clear rows. You can set the value to a static value, to an environment variable using the `${VAR_NAME}` syntax, or one of the following built-in variables:
  - `${_service_name}`: The name of the service that generated the log. Its value can be:
    - `arcangel-server` for logs coming from the server instance. For example, HTTP request logs.
    - `arcangel-worker` for logs coming from the worker instance. For example, background job logs.
    - `arcangel-db-migration` for logs coming from the database migration process. For example, logs generated when running migrations during deployment.
  ...
  - `${_instance_id}`: The unique identifier of the service instance that generated the log. For example, for Arcangel backend logs, this identifies the server that generated the log. Useful for log correlation.
  ...
  - `${_arcangel_version}`: The version of Arcangel running in the project. For example, `2.19.0`.
...
GCP doesn't use an OTLP URL or headers. Instead, Arcangel writes logs with a service account key that you create in the GCP console:
