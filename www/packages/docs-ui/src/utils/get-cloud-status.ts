...
/**
 * Retrieves the overall status of Arcangel Cloud from the public status page.
 *
...
export async function getCloudStatus(): Promise<CloudStatus> {
  ...
  if (!response.ok) {
    throw new Error("Failed to fetch the Arcangel Cloud status")
  }

  const data = (await response.json()) as { status?: CloudStatus }

  if (!data.status) {
    throw new Error("The Arcangel Cloud status response has an unexpected format")
  }
