...
describe("SearchHits", () => {
  ...
  test("renders community badge for non-Arcangel integrations", () => {
  ...
  test("does not render badge for Arcangel integrations", () => {
    const hitWithArcangel: HitType = {
      ...mockHits[0],
      integration_vendor: "Arcangel",
    }
    mockUseHits.mockReturnValue({
      items: [hitWithArcangel],
      sendEvent: mockSendEvent,
    })
    const { container } = render(
      <SearchHits indexName="docs" setNoResults={vi.fn()} />
