...
> = ({
  ...
  return (
    <div
      className="bg-arcangel-bg-base w-full h-full shadow-elevation-flyout dark:shadow-elevation-flyout-dark rounded-docs_DEFAULT"
    ...
    >
      <div className={clsx("flex gap-docs_1 p-docs_1")}>
        {type !== "none" && (
          ...
          >
            {type === "info" && (
              <InformationCircleSolid className="text-arcangel-tag-blue-icon" />
            )}
            {type === "error" && (
              <XCircleSolid className="text-arcangel-tag-red-icon" />
            )}
            {type === "warning" && (
              <ExclamationCircleSolid className="text-arcangel-tag-orange-icon" />
            )}
            {type === "success" && (
              <CheckCircleSolid className="text-arcangel-tag-green-icon" />
            )}
            {type === "custom" && CustomIcon}
          </div>
        )}
        <span
          className={clsx("text-compact-medium-plus", "text-arcangel-fg-base")}
      ...
      {(text || children) && (
        ...
        >
          ...
          <div className={clsx("flex flex-col", children && "gap-docs_1")}>
            {text && (
              <span className={clsx("text-medium text-arcangel-fg-subtle")}>
                {text}
              </span>
            )}
            {children}
