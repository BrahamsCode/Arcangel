      ...
      {tags.map((tag, index) => {
        return (
          <React.Fragment key={index}>
            {typeof tag === "string" ? (
              <span className=" text-arcangel-fg-subtle">[{tag}]</span>
            ) : (
              <Link
                href={tag.link}
                className="text-arcangel-fg-interactive hover:text-arcangel-fg-interactive-hover"
              >
                [{tag.text}]
              </Link>
            )}
            {index !== tags.length - 1 && (
              <span className="text-arcangel-fg-subtle">·</span>
            )}
          </React.Fragment>
        )
      })}
