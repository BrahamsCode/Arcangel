...
vi.mock("@/components/Icons", () => ({
  ColoredArcangelIcon: ({ variant }: { variant?: string }) => (
...
describe("rendering", () => {
  ...
  test("always shows border-b on nav content regardless of collapse state", () => {
    ...
    expect(topBar).toHaveClass("border-arcangel-border-base")
  })
  ...
  test("adjusts layout when collapsed", () => {
    ...
    const topBar = container.querySelector("[data-testid='main-nav-content']")
    expect(topBar).toBeInTheDocument()
    expect(topBar).toHaveClass("border-b border-arcangel-border-base")
    expect(topBar).toHaveClass("relative")
    expect(topBar).toHaveClass("z-50")
