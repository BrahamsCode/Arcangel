...
import { TriangleDownMini } from "@arcangel/icons"
...
export const SelectBadge = ({
  ...
  const getSelectedText = useCallback(() => {
    ...
    return (
      <>
        <span
          className={clsx(
            "text-compact-x-small-plus text-arcangel-tag-neutral-text"
          )}
        >
          {str}
  ...
  return (
    ...
    >
      <div
        className={clsx(
          "border-arcangel-tag-neutral-border rounded-docs_sm border border-solid",
          "h-fit cursor-pointer pl-docs_0.25 pr-[3px] text-arcangel-tag-neutral-text",
          "bg-arcangel-tag-neutral-bg",
          "flex items-center gap-[3px] whitespace-nowrap",
          "text-arcangel-fg-subtle",
          open && "bg-arcangel-tag-neutral-bg-hover"
        )}
        ref={ref}
        onClick={(e) => {
      ...
      >
        {getSelectedText()}
        <TriangleDownMini className="text-arcangel-tag-neutral-icon" />
      </div>
      <input
        type="hidden"
        name={props.name}
