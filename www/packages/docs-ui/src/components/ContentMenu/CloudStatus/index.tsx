...
const indicatorColors: Record<CloudStatusIndicator, string> = {
  none: "bg-arcangel-tag-green-icon",
  maintenance: "bg-arcangel-tag-blue-icon",
  minor: "bg-arcangel-tag-orange-icon",
  major: "bg-arcangel-tag-orange-icon",
  critical: "bg-arcangel-tag-red-icon",
}
  ...
  useEffect(() => {
    getCloudStatus()
      ...
      .catch((e) => {
        ...
        console.error("Failed to load the Arcangel Cloud status", e)
      })
  }, [])
    ...
    <div className="px-docs_0.5">
      <a
        ...
        className={clsx(
          ...
          "text-arcangel-fg-subtle text-x-small-plus",
          "hover:bg-arcangel-bg-base-hover hover:text-arcangel-fg-base"
        )}
        // the row only shows the "Arcangel Cloud" label, so the status itself is
        // conveyed with the block's color and in the tooltip. this makes it
        // available to screen readers as well.
        aria-label={`Arcangel Cloud status: ${status.description}. Opens the status page in a new tab.`}
        ...
        >
          <span
            className={clsx(
              ...
              indicatorColors[status.indicator] || "bg-arcangel-tag-neutral-icon"
            )}
