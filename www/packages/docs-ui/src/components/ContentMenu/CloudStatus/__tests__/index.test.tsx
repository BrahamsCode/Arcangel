...
describe("ContentMenuCloudStatus", () => {
  test("renders a badge linking to the status page", async () => {
    ...
    expect(link).toHaveAttribute("href", "https://status.arcangel.com")
    expect(link).toHaveAttribute("target", "_blank")
    expect(link).toHaveTextContent("Arcangel Cloud")
  })
  ...
  test.each([
    ...
    async (indicator, expectedColor) => {
      ...
      expect(block.className).toContain(`bg-arcangel-tag-${expectedColor}-icon`)
    }
  )
  ...
  test("falls back to a neutral block for an unknown indicator", async () => {
    ...
    expect(block.className).toContain("bg-arcangel-tag-neutral-icon")
  })
  ...
  test("has a transparent background that fills the width, and a hover background", async () => {
    ...
    expect(link.className).toContain("hover:bg-arcangel-bg-base-hover")
