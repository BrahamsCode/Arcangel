    ...
    <li className="w-full toc-item" data-testid="toc-item">
      <Link
        ...
        className={clsx(
          ...
          item.id !== activeItemId &&
            "text-arcangel-fg-subtle hover:text-arcangel-fg-base text-x-small",
          item.id === activeItemId && "text-arcangel-fg-base text-x-small-plus",
          "truncate"
        )}
        onClick={(e) => {
          e.preventDefault()
      ...
      {(item.children?.length ?? 0) > 0 && (
        <TocList
          items={item.children!}
          activeItemId={activeItemId}
          className="pl-docs_0.75 relative after:content-[''] after:absolute after:left-0 after:top-docs_0.25 after:bottom-docs_0.25 after:w-docs_0.125 after:h-[calc(100%-8px)] after:bg-arcangel-border-strong"
        />
      )}
    </li>
  )
