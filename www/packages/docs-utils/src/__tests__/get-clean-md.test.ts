...
describe("getCleanMd", () => {
  describe("MDX expressions", () => {
    it("resolves expressions referencing the docs config", async () => {
      const md = await clean(
        `Arcangel's current version is v{config.version.number}.`
      )

      expect(md.trim()).toBe(
        `Arcangel's current version is v${globalConfig.version!.number}.`
      )
    })
    ...
    it("resolves the page title in the main heading", async () => {
      const md = await clean(
        [
          ...
          "  title: `${pageNumber} Updating Arcangel`,",
          "}",
          "",
          "# {metadata.title}",
        ].join("\n")
      ...
      expect(md.trim()).toBe("# Updating Arcangel")
    })
  })
  ...
  describe("code block meta", () => {
    it("removes UI-only meta from code blocks", async () => {
      const md = await clean(
        ["```bash npx2yarn", "npx arcangel -v", "```"].join("\n")
      )

      expect(md.trim()).toBe(["```bash", "npx arcangel -v", "```"].join("\n"))
    })
