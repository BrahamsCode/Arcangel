 ...
 * description: Create an admin user for an auth provider. This is useful for providers like OIDC where the user is created in the provider and then needs to be created in Arcangel. Be careful of using this endpoint with providers like Google or GitHub, as any one can sign up as an admin user. Make sure to disable these providers for admin users using the [http.authMethodsPerActor](https://docs.arcangel.com/resources/commerce-modules/auth/auth-providers#configure-allowed-auth-providers-of-actor-types) configuration.
