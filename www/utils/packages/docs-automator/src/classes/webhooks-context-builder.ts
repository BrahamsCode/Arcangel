...
export class WebhooksContextBuilder {
  ...
  private describePropertyChange(
    ...
    const sentence = `Arcangel ${verb} the \`${change.property}\` property on ${changelogDate}.`
  ...
  private renderChangelogEntry(
    ...
    for (const event of sorted) {
      ...
      if (event.changeType === "added") {
        ...
        lines.push(
          `- Removed the \`${event.event}\` event. Arcangel no longer delivers it.`
        )
      }
...
- \`## Webhooks Overview\` — what webhooks are and how Arcangel delivers them
...
6. **Write "Arcangel" as the subject, never "Cloud"**, for example "Arcangel sends this event". Reserve "Cloud" for the platform as a place, such as "changes in Cloud"
