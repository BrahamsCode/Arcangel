...
Use the /writing-docs skill before making any documentation changes. Load the \`reference/cloud-style.md\` reference file from that skill — it contains the rules and patterns for writing Arcangel Cloud documentation correctly.`
  }

  private buildTaskSection(): string {
    return `## Task

A new version of Arcangel Cloud was deployed to production. The section below describes user-facing changes that were shipped. Changes may include dashboard UI updates, Cloud CLI updates, or both.
...
export default {
  ...
  content: \`- You can now set an environment variable as available at build time, runtime, or both, which controls whether Arcangel injects it into your build or your running application. Refer to [Environment Variables](/environments/environment-variables) for more details.
