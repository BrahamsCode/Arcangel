...
export default async function generate(
  ...
  try {
    if (references.includes("core-flows")) {
      await import("@arcangel/core-flows")
      // @ts-ignore - loyalty-plugin may not be built in CI
      await import("@arcangel/loyalty-plugin/workflows")
    }
  } catch (e) {
    console.warn(
      chalk.bgYellow("Warning: ") +
        "Couldn't load @arcangel/core-flows or @arcangel/loyalty-plugin. Make sure to have them installed if you want to generate references for core flows or loyalty workflows."
    )
  }
