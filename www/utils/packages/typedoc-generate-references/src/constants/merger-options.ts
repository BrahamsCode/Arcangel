...
const mergerOptions: Partial<TypeDocOptions> = {
  ...
  name: "references",
  indexTitle: "Arcangel References",
  plugin: [...(baseOptions.plugin || []), "typedoc-plugin-arcangel-theme"],
  ...
  formatting: {
    "*": {
      ...
      parameterComponentExtraProps: {
        expandUrl:
          "https://docs.arcangel.com/learn/fundamentals/data-models/manage-relationships#retrieve-records-of-relation",
      },
    },
    internal: {
      maxLevel: 1,
...
You should only use this methods when implementing complex customizations. For common cases, check out [available workflows instead](/arcangel-workflows-reference).
...
You should only use the methods in this reference when implementing complex customizations. For common cases, check out [available workflows instead](/arcangel-workflows-reference).
