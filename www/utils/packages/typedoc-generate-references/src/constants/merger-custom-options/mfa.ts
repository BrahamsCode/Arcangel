...
An MFA Module Provider is a provider that implements the \`AuthMfaProvider\` interface. It allows you to integrate a new MFA provider into the Arcangel Auth Module, enabling users to enroll and authenticate with MFA factors beyond the built-in TOTP provider.

Arcangel provides a built-in Totp MFA Module Provider that you can use out-of-the-box. You can also
...
If you need to refer to an existing implementation as an example, check the [Totp MFA Module Provider in the Arcangel repository](https://github.com/arcangel/arcangel/blob/develop/packages/modules/auth/src/providers/mfa/totp.ts).`,
      `## 1. Create Module Provider Directory

Start by creating a new directory for your module provider.

If you're creating the module provider in a Arcangel application, create it under the \`src/modules\` directory. For example, \`src/modules/my-mfa\`.
...
Create the file \`src/modules/my-mfa/service.ts\` that holds the module provider's main service. It must implement the \`AuthMfaProvider\` interface imported from \`@arcangel/framework/types\`:

\`\`\`ts title="src/modules/my-mfa/service.ts"
import { AuthMfaProvider } from "@arcangel/framework/types"
...
To use your MFA Module Provider, add it to the \`mfa.providers\` array of the Auth Module in \`arcangel-config.ts\`:
...
Arcangel provides a built-in Recovery Code MFA Module Provider that you can use out-of-the-box. You can also
...
If you need to refer to an existing implementation as an example, check the [Recovery Code MFA Module Provider in the Arcangel repository](https://github.com/arcangel/arcangel/blob/develop/packages/modules/auth/src/providers/mfa/recovery-code.ts).`,
      `## 1. Create Module Provider Directory

Start by creating a new directory for your module provider.

If you're creating the module provider in a Arcangel application, create it under the \`src/modules\` directory. For example, \`src/modules/my-recovery-code\`.
...
Create the file \`src/modules/my-recovery-code/service.ts\` that holds the module provider's main service. It must implement the \`RecoveryCodeAuthMfaProvider\` interface imported from \`@arcangel/framework/types\`:

\`\`\`ts title="src/modules/my-recovery-code/service.ts"
import { RecoveryCodeAuthMfaProvider } from "@arcangel/framework/types"
...
To use your Recovery Code MFA Module Provider, add it to the \`mfa.providers\` array of the Auth Module in \`arcangel-config.ts\`:
