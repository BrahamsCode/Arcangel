...
If you need to refer to an existing implementation as an example, check the [PostHog Analytics Module Provider in the Arcangel repository](https://github.com/arcangel/arcangel/tree/develop/packages/modules/providers/analytics-posthog).`,
      `## Create Module Provider Directory

Start by creating a new directory for your module provider.

If you're creating the module provider in a Arcangel application, create it under the \`src/modules\` directory. For example, \`src/modules/my-analytics\`.
...
Create the file \`src/modules/my-analytics/service.ts\` that holds the implementation of the module provider's main service. It must extend the \`AbstractAnalyticsProviderService\` class imported from \`@arcangel/framework/utils\`:

\`\`\`ts title="src/modules/my-analytics/service.ts"
import { AbstractAnalyticsProviderService } from "@arcangel/framework/utils"
...
To use your Analytics Module Provider, add it to the \`providers\` array of the Analytics Module in \`arcangel-config.ts\`:
...
module.exports = defineConfig({
  ...
  modules: [
    {
      resolve: "@arcangel/arcangel/analytics",
      options: {
        providers: [
          {
...
import { createWorkflow } from "@arcangel/framework/workflows-sdk"
import { createStep } from "@arcangel/framework/workflows-sdk"
import { Modules } from "@arcangel/framework/utils"
import { OrderDTO } from "@arcangel/framework/types"
...
In the step, you resolve the service of the Analytics Module from the [Arcangel container](!docs!/learn/fundamentals/arcangel-container) and use its \`track\` method to track the event. This method will use the underlying provider configured (which is your provider, in this case) to track the event.
...
You'll now track the order creation event whenever an order is placed in your Arcangel application. You can test this out by placing an order and checking in your third-party provider if the event was tracked successfully.
`,
