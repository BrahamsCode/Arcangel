...
If you need to refer to an existing implementation as an example, check the [S3 File Module Provider in the Arcangel repository](https://github.com/arcangel/arcangel/tree/develop/packages/modules/providers/file-s3).`,
      `## Create Module Provider Directory

Start by creating a new directory for your module provider.

If you're creating the module provider in a Arcangel application, create it under the \`src/modules\` directory. For example, \`src/modules/my-file\`.
...
Create the file \`src/modules/my-file/service.ts\` that holds the implementation of the module provider's main service. It must extend the \`AbstractFileProviderService\` class imported from \`@arcangel/framework/utils\`:

\`\`\`ts title="src/modules/my-file/service.ts"
import { AbstractFileProviderService } from "@arcangel/framework/utils"
...
To use your File Module Provider, add it to the \`providers\` array of the File Module in \`arcangel-config.ts\`:
...
To test out your File Module Provider, use the Arcangel Admin or the [Upload API route](https://docs.arcangel.com/api/admin/uploads/upload-files) to upload a file.
`,
    ],
  },
}
