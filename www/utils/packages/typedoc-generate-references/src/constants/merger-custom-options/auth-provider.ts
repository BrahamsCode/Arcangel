...
If you need to refer to an existing implementation as an example, check the [Google Auth Module Provider in the Arcangel repository](https://github.com/arcangel/arcangel/tree/develop/packages/modules/providers/auth-google).`,
      `## 1. Create Module Provider Directory

Start by creating a new directory for your module provider.

If you're creating the module provider in a Arcangel application, create it under the \`src/modules\` directory. For example, \`src/modules/my-auth\`.
...
Create the file \`src/modules/my-auth/service.ts\` that holds the module provider's main service. It must extend the \`AbstractAuthModuleProvider\` class imported from \`@arcangel/framework/utils\`:

\`\`\`ts title="src/modules/my-auth/service.ts"
import { AbstractAuthModuleProvider } from "@arcangel/framework/utils"
...
To use your Auth Module Provider, add it to the \`providers\` array of the Auth Module in \`arcangel-config.ts\`:

\`\`\`ts title="arcangel-config.ts"
import { Modules, ContainerRegistrationKeys } from "@arcangel/framework/utils"
...
To test out your Authentication Module Provider, use any of the [Authentication Routes](https://docs.arcangel.com/resources/commerce-modules/auth/authentication-route), using your provider's ID as a path parameter.
...
You can then use the token to create a customer by sending a request to the [Create Customer API route](https://docs.arcangel.com/api/store/customers/register-customer).
      `,
    ],
  },
}
