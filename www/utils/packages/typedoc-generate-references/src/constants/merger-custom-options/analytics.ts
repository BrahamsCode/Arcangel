...
The Analytics Module is available starting [Arcangel v2.8.3](https://github.com/arcangel/arcangel/releases/tag/v2.8.3).
...
Arcangel provides two Analytics Module Providers: [Local](/infrastructure-modules/analytics/local) and [PostHog](/infrastructure-modules/analytics/posthog) module providers.

To configure the Analytics Module and its provider, add it to the list of modules in your \`arcangel-config.ts\` file. For example:
  ...
  modules: [
    {
      ...
      options: {
        providers: [
          {
            resolve: "@arcangel/arcangel/analytics-local",
            id: "local",
          },
        ],
      },
...
In your workflow's step, you can resolve the Analytics Module's service from the Arcangel container:
