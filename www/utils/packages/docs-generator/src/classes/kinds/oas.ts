...
import { ArcangelEvent } from "types"
...
class OasKindGenerator extends FunctionKindGenerator {
  ...
  readonly RESPONSE_TYPE_NAMES = ["ArcangelResponse"]
  ...
  protected events: ArcangelEvent[] = []
   ...
   * This supports both a direct type reference, such as `ArcangelRequest<T>`, and a union
   * of request types, such as `ArcangelRequest<T> | AuthenticatedArcangelRequest<T>`. In the
  ...
  getAssociatedWorkflow(node: FunctionNode): string | undefined {
    ...
    // retrieve workflows imported from `@arcangel/core-flows`
    ...
    ;(sourceFile.imports as ts.Node[]).forEach((importNode) => {
      if (
        ...
        importNode.getText() !== `"@arcangel/core-flows"`
      ) {
        return
      }
    ...
    // if no imports found from `@arcangel/core-flows`, return
