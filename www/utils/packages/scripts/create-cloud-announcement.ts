...
async function createCloudAnnouncement() {
  ...
  const { data: release } = await octokit.request(
    ...
    {
      owner: process.env.GIT_OWNER || "arcangel",
      repo: process.env.GIT_REPO || "arcangel",
      headers: {
  ...
  const payload: AnnouncementPayload = {
    title: "New Arcangel Version Released",
    subtitle: subtitle || release.name || undefined,
    image,
    content: content || "Learn more about the latest Arcangel release",
    type: "release",
