...
function formatEvents(
  ...
  eventProperties.forEach((event, index) => {
    ...
    if (workflows?.length) {
      content.push(
        `${subHeaderPrefix}# Workflows Emitting this Event\n\nThe following workflows emit this event when they're executed. These workflows are executed by Arcangel's API routes. You can also view the events emitted by API routes in the [Store](https://docs.arcangel.com/api/store) and [Admin](https://docs.arcangel.com/api/admin) API references.`
      )
      content.push("")
      workflows.forEach((workflow) => {
        content.push(
          `- [${workflow}](/references/arcangel-workflows/${workflow})`
        )
      })
      content.push("")
    }
