...
API route files live under `packages/arcangel/src/api/admin/` and `packages/arcangel/src/api/store/`. They export named HTTP handler functions (`GET`, `POST`, `DELETE`, `PATCH`).
...
export const GET = async (
  req: AuthenticatedArcangelRequest<HttpTypes.AdminProductListParams>,
  res: ArcangelResponse<HttpTypes.AdminProductListResponse>
) => {
  // ...
}
```
...
export const POST = async (
  req: AuthenticatedArcangelRequest<AdminCreateRegionType>,
...
export const GET = async (
  req: AuthenticatedArcangelRequest<HttpTypes.AdminOrderListParams>,
