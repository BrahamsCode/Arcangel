...
description: Diagnoses GitHub Dependabot / security alerts in the Arcangel monorepo and finds the least-invasive fix. Use when investigating a Dependabot alert, security advisory (GHSA/CVE), vulnerable dependency, "npm audit" finding, or a security update PR. Traces the vulnerable package to the exact affected workspace package(s) under packages/, assesses real impact, and prefers a fix scoped to the affected package over a root package.json / yarn.lock resolutions override (which is a last resort).
---

# Diagnosing Dependabot Alerts

Investigate a Dependabot/security alert in the Arcangel monorepo, identify the exact affected package(s), assess real-world impact, and pick the least-invasive fix. Default output is a diagnosis; only make changes when asked.
...
- **Assess impact, don't assume:** Determine whether the vulnerable code path is actually reachable with untrusted input in Arcangel before recommending urgency.
...
```bash
gh api repos/arcangel/arcangel/dependabot/alerts/<N> | jq '{
...
### 2. Trace to the exact affected Arcangel package(s)
...
Before recommending urgency, check whether the vulnerable code path is reachable with untrusted input in Arcangel. Example from a past alert: `immutable` prototype pollution reached us only through `@graphql-codegen/typescript`, used to generate types from Arcangel's OWN internal GraphQL schema — no untrusted input, so practical exploitability was negligible. State the impact explicitly; it changes how aggressive the fix needs to be.
...
- [ ] Reporting urgency without checking whether the vulnerable path is reachable in Arcangel
