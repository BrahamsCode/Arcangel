...
- **Does not ship.** `resolutions`/`overrides` are honored only at THIS repo's install root. They are not part of a published package's metadata, so a downstream `npm install @arcangel/<pkg>` re-resolves the transitive freshly and can get the vulnerable version again.
