...
4. Map anything non-trivial to **how Arcangel actually uses the package**, so the
...
breaking change lands in code Arcangel depends on.
...
- **Diff** (`get_pr_diff.sh`): confirm which `package.json` files changed (direct
  deps of published `@arcangel/*` packages) versus lockfile-only changes
...
Prioritise by blast radius: **direct:production** deps that Arcangel calls
...
changes worth testing, not as "safe", when Arcangel feeds the relevant input.

## Step D — Map to Arcangel usage

A change only matters if Arcangel exercises the affected surface. For each hard
breaking or behavior change, find the call sites and read them with `Read`:

- Identify which `@arcangel/*` package(s) declare or import the dependency.
- Read the actual usage: which functions/options Arcangel calls, and whether the
  changed behavior is on that path.
- Where practical, reason about whether Arcangel's specific usage is affected. If
  it clearly is not (e.g. the change is to an env-var parser but Arcangel passes
...
| Dependency | Typical Arcangel call site |
|-----------|--------------------------|
| routing / path matching (e.g. `path-to-regexp`) | `packages/core/framework/src/http/routes-finder.ts` |
| query string (`qs`) | `packages/arcangel/src/loaders/api.ts` (`parse`), `packages/core/js-sdk/src/client.ts` (`stringify`) |
| HTTP logging (`morgan`) | `packages/core/framework/src/http/express-loader.ts` |
| uploads (`multer`) | `packages/arcangel/src/api/admin/uploads/middlewares.ts`, `.../products/middlewares.ts` |
| CSV (`json-2-csv`) | `packages/core/core-flows/src/product/utils/*.ts`, `.../order/steps/export-orders.ts` |
| telemetry (`@opentelemetry/*`) | `packages/arcangel/src/instrumentation/index.ts`, `packages/deps/` |
...
- **Default outcome for a clean bump: `approve`.** Put the value in the review:
  ...
  test"** list derived from the behavior changes mapped to Arcangel usage (e.g.
...
- **`needs-changes` (`requires-more`)** only when a hard breaking change or a
  behavior change genuinely affects a Arcangel call site and the PR does not
