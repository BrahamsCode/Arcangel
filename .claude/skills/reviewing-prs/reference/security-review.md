...
them and Arcangel-specific patterns that are easy to miss.
...
obvious case, but Arcangel uses MikroORM and the remote-query layer, which have
...
- **Query-filter / operator injection:** passing `req.body`, `req.query`, or
  ...
  check into a broad match. Arcangel routes are expected to validate/whitelist
