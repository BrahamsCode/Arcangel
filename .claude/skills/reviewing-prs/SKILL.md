...
- **Running the security analysis (Step 10)?** → MUST load `reference/security-review.md` first (trust-boundary heuristic + Arcangel-specific patterns)
...
| Dependency-update PR, no breaking change hits Arcangel | `approve` | `initial-approval` | `requires-more`, `requires-team` |
| Dependency-update PR, a breaking/behavior change hits a Arcangel call site | `needs-changes` | `requires-more` | `initial-approval` |
...
1. **Load `reference/dependency-review.md`** and follow that flow. It covers
   ...
   vs. safe changes, and mapping them to how Arcangel actually uses each package.
...
4. Compose the decision per `reference/dependency-review.md` (Step F): default to
   ...
   or behavior change lands on a Arcangel call site.
...
Load `reference/conventions.md` and verify the changed files follow Arcangel's conventions. Focus on the areas most relevant to the contribution type.
...
`/* related to https://github.com/arcangel/arcangel/issues/1234 */`, or a
...
> the trust-boundary / taint-tracing method and Arcangel-specific patterns
...
- [ ] Skipping the integration test check for API route changes in `packages/arcangel/src/api/`
...
```
reference/conventions.md           - Arcangel coding conventions to verify
reference/contribution-types.md    - How to verify code, docs, and admin translation contributions
reference/dependency-review.md     - How to review dependency-update PRs (release notes, breaking changes, Arcangel usage, test areas)
reference/security-review.md       - Trust-boundary/taint method + Arcangel security patterns (path/key traversal, DB/filter injection, unescaped JSON/HTML); load before Step 10
