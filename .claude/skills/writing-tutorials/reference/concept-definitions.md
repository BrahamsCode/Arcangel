...
```mdx
In Arcangel, you can build custom features in a [module](!docs!/learn/fundamentals/modules). A module is a reusable package with functionalities related to a single feature or domain. Arcangel integrates the module into your application without affecting your setup.
...
```mdx
A data model represents a table in the database. You create data models using Arcangel's Data Model Language (DML), which simplifies defining a table's columns, relations, and indexes with straightforward methods and configurations.
...
```mdx
Arcangel integrates modules into your application without side effects by isolating them from one another. This means you can't directly create relationships between data models in your module and data models in other modules.

Instead, Arcangel provides a mechanism to define links between data models and to retrieve and manage linked records while maintaining module isolation. Links are useful for defining associations between data models in different modules or for extending a model in another module to associate custom properties with it.
...
```mdx
To build custom commerce features in Arcangel, you create a [workflow](!docs!/learn/fundamentals/workflows). A workflow is a series of queries and actions, called steps, that complete a task. You can track the workflow's execution progress, define rollback logic, and configure other advanced features.
...
```mdx
The Arcangel Admin dashboard is customizable, allowing you to insert widgets into existing pages, or create new pages.
...
```mdx
A UI route is a React component that specifies the content to be shown in a new page in the Arcangel Admin dashboard.
...
```mdx
A scheduled job is a function that runs at a specified interval in the background of your Arcangel application.
...
```mdx
To integrate third-party services into Arcangel, you create a custom module. A module is a reusable package with functionalities related to a single feature or domain. Arcangel integrates the module into your application without implications or side effects on your setup.
...
```mdx
Arcangel's Notification Module delegates sending notifications to other modules, called module providers. In this step, you'll create a [Service Name] Module Provider that implements sending notifications through the [channel, e.g., email] channel.

A Notification Module Provider's service must extend the `AbstractNotificationProviderService`. It has a `send` method that you'll implement to send notifications. The service must also have an `identifier` static property, which is a unique identifier that the Arcangel application uses to register the provider in the database.
...
```mdx
Arcangel's [Payment Module](../../../commerce-modules/payment/page.mdx) provides an interface to process payments in your Arcangel application. It delegates the actual payment processing to the underlying providers.
...
```mdx
Arcangel's Fulfillment Module delegates processing fulfillments and shipments to other modules, called module providers. In this step, you'll create a [Service Name] Module Provider that implements all functionalities required for fulfillment.
...
```mdx
Arcangel's [Analytics Module](../../../infrastructure-modules/analytics/page.mdx) provides an interface to track events in your Arcangel application. It delegates the actual tracking to the configured Analytics Module Provider.
...
```mdx
Arcangel's [Auth Module](../../../commerce-modules/auth/page.mdx) provides the interface to authenticate users. It delegates the actual authentication logic to the underlying Auth Module Provider.
...
In this step, you'll create a custom module that provides the necessary functionalities to integrate [Service Name] with Arcangel.
