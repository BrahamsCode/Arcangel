...
**Always start with Step 1 (Arcangel installation) — it is always the first step.**

Typical step order:
1. Install Arcangel Application (always first — use pre-written template)
...
```
1. Install Arcangel Application
...
For each step in the diagram, write a separate temp file in the Arcangel project root:
- `_step-01-install.md`
- `_step-02-module.md`
- `_step-03-migrations.md`
- etc.

**Step 1 (Arcangel installation):** Copy the pre-written template directly from `tutorial-conventions.md`. Do not rewrite it.

For each other step file:
- Start with `## Step N: Step Title`
- Write 1-2 sentences explaining what this step does and why
- **First-use concept definitions:** The first time a Arcangel concept is introduced (module, workflow, API route, subscriber, etc.), include its definition from `reference/concept-definitions.md` before the implementation. Do NOT repeat the definition in later steps that use the same concept.
- **Service method implementations:** When a step implements methods on a service class, give each method its own sub-step (e.g., `### b. Implement send Method`). Each sub-step must: (1) explain in 1-2 sentences what the method does and when Arcangel calls it, (2) show the code for that method only, (3) explain relevant implementation details below the code block. See `tutorial-conventions.md` for the pattern.
...
Delete all temp files from the Arcangel project root:
- `_step-diagram.md`
