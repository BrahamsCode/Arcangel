...
3. **Example project path** — Where is the target Arcangel example project? (e.g., `~/projects/my-arcangel-app`)
...
1. Load `arcangel-dev:building-with-arcangel` — required for all backend work
   - Also load `arcangel-dev:building-admin-dashboard-customizations` if admin UI is in scope
   - Also load `arcangel-dev:building-storefronts` if storefront is in scope
2. Use `mcp__arcangel__ask_arcangel_question` for questions about Arcangel APIs, modules, and patterns
...
**Useful commands:**
- Generate migration: `/arcangel-dev:db-generate <module-name>`
- Run migrations: `/arcangel-dev:db-migrate`
- Create admin user: `/arcangel-dev:new-user <email> <password>`

Load reference files from `arcangel-dev:building-with-arcangel` for each component as you build it.
...
1. Tell the user to test the feature manually:
   - Start the Arcangel app and test relevant API routes
