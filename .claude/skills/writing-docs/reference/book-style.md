...
The `book` project (`www/apps/book/`) is the Arcangel learning platform covering framework fundamentals, architecture, configurations, and how-to guides for building with Arcangel.
...
| Configuration files (arcangel-config, pnpm, tsconfig) | `app/learn/configurations/` |
...
Always include TypeScript examples. Follow the Arcangel import patterns exactly:
...
import type { ArcangelContainer } from "@arcangel/framework/types"
```
...
- A configuration option is added to `arcangel-config`
