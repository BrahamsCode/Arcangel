...
The `cloud` project (`www/apps/cloud/`) covers how developers and technical users manage their Arcangel applications on Cloud: creating projects, managing environments and deployments, configuring resources, and monitoring usage.
...
The cloud docs are for developers and technical users who deploy and manage Arcangel applications on Cloud. Assume the reader:

- Is comfortable with Git, GitHub, and web development concepts
- Is not necessarily a Arcangel expert — define Cloud-specific terms on first use
...
- **Cloud naming**: Never write "Arcangel Cloud". Use **"Arcangel"** when referring to it as a noun/subject ("Arcangel allows you to..."), and **"Cloud"** when referring to it as a location or service ("Deploy to Cloud", "the Cloud dashboard")
...
export const metadata = {
  title: `<topic> Command - Arcangel Cloud CLI Reference`,
}
...
export default {
  ...
  content: `- You can now set an environment variable as available at build time, runtime, or both. Refer to [Environment Variables](https://docs.arcangel.com/cloud/environments/environment-variables) for more details.
...
## Webhooks Overview      → what webhooks are, how Arcangel delivers them
...
Arcangel sends this event when it creates a deployment for an environment.

<Note title="Changes">

- Arcangel removed the `data.commit_sha` property on August 4, 2026.
...
Arcangel sends this event when a deployment finishes.
````
...
```mdx
- Arcangel removed the `data.commit_sha` property on August 4, 2026.
- Arcangel added the `data.build_id` property on August 4, 2026.
```
...
A custom domain lets you serve your Arcangel application from your own domain name instead of the default Cloud subdomain.
