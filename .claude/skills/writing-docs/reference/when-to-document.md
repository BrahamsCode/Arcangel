...
- Config options accepted by `arcangel-config` but absent from the configurations docs
...
- A new configuration option is added to `arcangel-config` (document in `book` under `configurations/`)
...
- New screens, tabs, or sections added to the Arcangel admin dashboard
...
**`packages/cli/create-arcangel-app`** — the `create-arcangel-app` CLI tool:
- Changes to flags, options, or the default project structure → update `www/apps/resources/app/create-arcangel-app/` and any page across `book` or `resources` that shows a `create-arcangel-app` command
...
**`packages/cli/arcangel-cli`** (the `arcangel` CLI) — commands like `arcangel start`, `arcangel db:migrate`, `arcangel plugin:develop`, etc.:
- New subcommands or new flags on existing commands → update `www/apps/resources/app/arcangel-cli/`
...
```bash
grep -r "create-arcangel-app" www/apps/book/app www/apps/resources/app
grep -r "arcangel db:migrate" www/apps/book/app www/apps/resources/app
```
