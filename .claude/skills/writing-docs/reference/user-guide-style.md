...
The `user-guide` project (`www/apps/user-guide/`) covers how non-technical admin users perform tasks in the Arcangel dashboard (`packages/admin/dashboard`).

## Audience

The user guide is for store operators and merchants, not developers. Assume the reader:
- Has no coding knowledge
- Manages the store through the Arcangel admin UI
...
export const metadata = {
  title: `Manage Products in Arcangel Admin`,
}

# {metadata.title}

In this guide, you'll learn how to manage products in the Arcangel admin.
...
- A new page or screen is added to the Arcangel admin dashboard
...
- Anything not visible in the Arcangel admin UI