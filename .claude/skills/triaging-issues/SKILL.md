...
- Picking a `close-*` template tells the downstream step to **post the
  ...
  Use these sparingly and only when the issue is clearly:
    ...
    - `close-invalid`: clearly not actionable (e.g. nonsense body, asking
      for help with a non-Arcangel product, malformed in a way that no
...
| `support` | User needs help implementing something or understanding how to use Arcangel |
...
reference/doc-links.md          - URL conventions for linking to docs.arcangel.com (load when the summary needs to reference docs)
```
