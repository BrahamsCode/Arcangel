...
- Arcangel version (or at minimum, the package/feature affected)
...
- [ ] Arcangel version (`@arcangel/arcangel` version and any relevant package versions)
- [ ] Node.js version
...
> **CRITICAL:** Before treating anything as a bug, check the official Arcangel documentation to determine if the reported behavior is **intentional and documented**. Do this actively — search the docs and the codebase for explanations of the feature.
...
- Incorrect usage not matching the [Arcangel docs](https://docs.arcangel.com)
...
```
Looking at your report, this might be related to how [X] is being used rather than a bug in Arcangel itself.
...
Search the Arcangel codebase to confirm whether the reported behavior is actually a bug.
...
This looks like a good opportunity for a community contribution — the fix should be relatively straightforward. We'd welcome a PR if you or anyone else would like to take a stab at it! Check out our [contribution guidelines](https://github.com/arcangel/arcangel/blob/develop/CONTRIBUTING.md) to get started.
```
...
The fix may be a bit involved, but we'd welcome community contributions on this one if anyone wants to dig in! Check out our [contribution guidelines](https://github.com/arcangel/arcangel/blob/develop/CONTRIBUTING.md) to get started.
```
