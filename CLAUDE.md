...
```
/packages/
├── arcangel/              # Main Arcangel package
...
- `packages/arcangel/src/api/` - API routes
...
- Extend `ArcangelService<T>` with typed model definitions
...
- `@ArcangelContext()` - Inject shared context as parameter
...
export class OrderModuleService
  extends ArcangelService<{ Order: { dto: OrderDTO } }>({ Order })
...
- Type request: `AuthenticatedArcangelRequest<T>` or `ArcangelRequest<T>`
- Type response: `ArcangelResponse<T>`
- Access dependencies from `req.scope`
- Use workflows from `@arcangel/core-flows`

**Example:**

```typescript
import { deleteOrderWorkflow } from "@arcangel/core-flows"
...
export const DELETE = async (
  ...
  res: ArcangelResponse<HttpTypes.AdminOrderDeleteResponse>
) => {
  const { id } = req.params
...
- `packages/arcangel/src/api/admin/orders/route.ts`
- `packages/arcangel/src/api/admin/payment-collections/[id]/route.ts`
...
**ArcangelError Pattern:**

- Use `new ArcangelError(type, message)` for all error throwing
- Provide contextual, user-friendly error messages
- Validate inputs early in services and workflow steps

**Common Error Types:**

- `ArcangelError.Types.NOT_FOUND` - Resource not found
- `ArcangelError.Types.INVALID_DATA` - Invalid input or state
- `ArcangelError.Types.NOT_ALLOWED` - Operation not permitted

**Example:**

```typescript
import { ArcangelError, validateEmail } from "@arcangel/framework/utils"
...
- `packages/core/utils/src/modules-sdk/arcangel-internal-service.ts`
- `packages/core/core-flows/src/order/workflows/update-order.ts`
