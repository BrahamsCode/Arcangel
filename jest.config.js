...
const reArcangel = /arcangel$/
const arcangelDir = pkgs.find((p) => reArcangel.exec(p))
const arcangelBuildDirs = [`dist`].map((dir) => path.join(arcangelDir, dir))
...
const ignoreDirs = [].concat(
  ...
  builtTestsDirs,
  distDirs,
  "<rootDir>/packages/arcangel-react/*"
)
