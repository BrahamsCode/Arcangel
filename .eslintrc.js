...
const arcangel = require("@arcangel/eslint-plugin")
...
//
// We now also run `@arcangel/eslint-plugin` (Arcangel convention rules) over all
// of the framework's own packages. Those packages must get the Arcangel rules
// WITHOUT suddenly being subjected to the full style/prettier lint.
//
// NOTE: of the Arcangel target packages, only `packages/arcangel` was ever actually
...
// every Arcangel target except `packages/arcangel`, plus data-model files (which the
...
function arcangelOverrides(presetName, dirs) {
  return arcangel.configs[presetName]
...
module.exports = {
  ...
  // style rules come from the scoped overrides below; the Arcangel rules from the
  ...
  plugins: ["prettier", "@arcangel", "@typescript-eslint", "react-hooks"],
  env: {
    es6: true,
    node: true,
    jest: true,
  ...
  overrides: [
    ...
    // --- Base style rules — historical scope only (Arcangel-only pkgs excluded) ---
    {
    ...
    // --- Arcangel convention rules (generated from the plugin presets) ---
    // `recommended` for the app-like packages. `packages/modules` and
    // `packages/core/framework` are intentionally excluded from the Arcangel rules
    // here (see ARCANGEL_EXCLUDED_DIRS) — they're linted for conventions
    // separately — so no `modules`-preset override is registered.
    ...arcangelOverrides("recommended", [
      "packages/arcangel",
      "packages/plugins/*",
      "packages/core/core-flows",
    ]),
